...
- CI-attested: yes [CI]
- evidence basis: the committed category list the gate grades against, which is independent of frisk's output; the gate requires the published membership to equal that list exactly
- reproduce: `cargo test -p disrobe-bench-head-to-head published_planted_ioc_category_bar_is_pinned_by_membership`
- gate source: crates/disrobe-core/tests/frisk_gauntlet.rs test published_planted_ioc_category_bar_is_pinned_by_membership pins the four fixture paths, detects all six expected categories, and checks the published membership, raw counts, and percentage.
