...
pub(crate) fn instruction_length(bytes: &[u8]) -> Result<InstructionLen> {
    ...
    let Some(&op): Option<&u8> = bytes.get(idx) else {
        return Err(CextractError::HotpatchFailed {
            stage: "lde",
    ...
    idx += 1;
    if op == 0x0F {
        let Some(&op2): Option<&u8> = bytes.get(idx) else {
            return Err(CextractError::HotpatchFailed {
                stage: "lde",
        ...
        if has_modrm {
            ...
            return Ok(InstructionLen {
                length: instruction_len.saturating_add(imm),
                uses_rip_relative: rip_rel || rip_branch,
            });
        }
        idx += imm as usize;
        return Ok(InstructionLen {
            length: u8::try_from(idx).map_or(0, |value: u8| value),
