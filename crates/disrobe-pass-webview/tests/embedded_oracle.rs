...
fn clang_static_pie(source: &str, tag: &str, grade: &str) -> Option<Vec<u8>> {
    ...
    let outcome: std::io::Result<Output> = command.output();
    let ok: bool = outcome
        .as_ref()
        .is_ok_and(|output: &Output| output.status.success() && out.exists());
    let bytes: Option<Vec<u8>> = if ok { fs::read(&out).ok() } else { None };
    let _ = fs::remove_dir_all(&workdir);
    if bytes.is_none() {
        let diagnostic: String = match outcome {
            Ok(output) => {
                let stderr: String = String::from_utf8_lossy(&output.stderr).trim().to_owned();
                if stderr.is_empty() {
                    format!("clang exited {} without stderr", output.status)
                } else {
                    format!("clang exited {}: {stderr}", output.status)
                }
            }
        ...
        announce_unmeasured(
            "clang",
            grade,
            &format!("the clang build produced no image to grade: {diagnostic}"),
        );
    }
    bytes
}
