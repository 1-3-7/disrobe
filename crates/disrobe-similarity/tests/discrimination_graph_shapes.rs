...
const REPORT_DEPENDENCIES: [&str; 2] = ["[dependencies]", "serde = { workspace = true }"];
...
fn declared_dependencies(manifest: &str) -> Vec<&str> {
    ...
    manifest
        ...
        .filter(|line: &&str| {
            ...
            in_dependencies && !line.is_empty() && !line.starts_with('#')
        })
        .collect()
}
...
fn the_inputs_here_are_authored_graphs_because_this_crate_carries_no_image_reader() {
    ...
    assert_eq!(
        ...
        REPORT_DEPENDENCIES,
        "{} may depend only on serde for report serialization. Reading a compiled image needs \
         the container parser, the decoder and the span discovery that live in {EXTRACTOR_HOME}, \
         which depends on this crate; grading real images belongs there, and the cases here \
         cover graph shapes a single real binary will not contain.",
        manifest.display()
    );
}

#[test]
fn the_dependency_contract_rejects_image_readers_and_other_dependency_tables() {
    let permitted: String = REPORT_DEPENDENCIES.join("\n");
    assert_eq!(declared_dependencies(&permitted), REPORT_DEPENDENCIES);
    for addition in [
        ...
        let changed: String = format!("{permitted}\n{addition}\n");
        assert_ne!(declared_dependencies(&changed), REPORT_DEPENDENCIES);
    }
}
