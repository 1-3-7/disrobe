...
use crate::structure::{InstructionCategory, InstructionMix, StructuralKey};
...
pub fn match_functions(left: &[FunctionFeatures], right: &[FunctionFeatures]) -> MatchReport {
    ...
    resolve_reference_shapes(&mut report, &left_index, &right_index);
...
type ReferenceShapeIndex<'a> =
    BTreeMap<(&'a BTreeSet<DataReference>, StructuralKey), BTreeSet<FunctionId>>;

fn reference_shapes<'a>(
    entries: &[FunctionVerdict],
    side: &SideIndex<'a>,
) -> ReferenceShapeIndex<'a> {
    let mut shapes: ReferenceShapeIndex<'a> = BTreeMap::new();
    for entry in entries {
        ...
        shapes
            .entry((features.references(), key))
            .or_default()
            .insert(entry.subject);
    }
    shapes
}

fn resolve_reference_shapes(report: &mut MatchReport, left: &SideIndex<'_>, right: &SideIndex<'_>) {
    let own: ReferenceShapeIndex<'_> = reference_shapes(&report.left, left);
    let other: ReferenceShapeIndex<'_> = reference_shapes(&report.right, right);
    for ((references, key), holders) in &own {
        let Some(candidates): Option<&BTreeSet<FunctionId>> = other.get(&(*references, *key))
        else {
            continue;
        };
        if holders.len() != 1 || candidates.len() != 1 {
            continue;
        }
        let (Some(&subject), Some(&counterpart)) = (holders.first(), candidates.first()) else {
            continue;
        };
        let (Some(left_verdict), Some(right_verdict)) = (
            lookup_mut(&mut report.left, subject),
            lookup_mut(&mut report.right, counterpart),
        ) else {
            continue;
        };
        *left_verdict = Verdict::Structural {
            counterpart,
            fingerprint: key.fingerprint,
            instruction_mix: key.instruction_mix,
            shared_references: (*references).clone(),
        };
        *right_verdict = Verdict::Structural {
            counterpart: subject,
            fingerprint: key.fingerprint,
            instruction_mix: key.instruction_mix,
            shared_references: (*references).clone(),
        ...
        |candidates: &BTreeSet<FunctionId>| {
            let independent: bool = anchor_strength(anchor) == AnchorStrength::Distinctive
                ...
                && candidates
                    ...
                    .is_some_and(|features: &&FunctionFeatures| {
                        !features.requires_reference_corroboration()
                    });
            forced_or_ambiguous(anchor, own_side, candidates, independent)
        },
    )
}
...
fn forced_or_ambiguous(
    ...
    if independent && own_side == 1 && other_side == 1 {
...
fn shape_verdict(
    ...
    if own_side == 1
        && other_side == 1
        && key.instruction_mix.count(InstructionCategory::Call) == 0
