...
fn has_unmodeled_effect(function: &NirFunction) -> bool {
    function
        ...
        .any(|instruction: &NirInstr| match &instruction.op {
            ...
            NirOp::CallOther { effect } => effect.unknown_registers || effect.writes_memory,
...
mod tests {
    ...
    use disrobe_nir::{CallOtherEffect, NirModule, SourceLang, SourceRef};
    ...
    #[test]
    fn stateful_callother_refuses_a_summary_while_a_pure_flag_helper_is_accepted() {
        let mut trapped: NirFunction = wide_add_then_truncate(0x1000);
        trapped.instructions.insert(
            1,
            pcode(
                0x1001,
                NirOp::CallOther {
                    effect: CallOtherEffect {
                        name: "x86_unmodeled_ud2_side_effecting_v1".to_owned(),
                        writes_memory: true,
        ...
        assert_eq!(
            symbolic_summary(&trapped),
            Err(SummaryDecline::UnmodeledEffect)
        );

        let mut unknown: NirFunction = wide_add_then_truncate(0x1800);
        unknown.instructions.insert(
            1,
            pcode(
                0x1801,
                NirOp::CallOther {
                    effect: CallOtherEffect {
                        name: "unknown-register-clobber".to_owned(),
        ...
        assert_eq!(
            symbolic_summary(&unknown),
            Err(SummaryDecline::UnmodeledEffect)
        );

        let mut pure: NirFunction = wide_add_then_truncate(0x2000);
        pure.instructions.insert(
            1,
            pcode(
                0x2001,
                NirOp::CallOther {
        ...
        assert!(symbolic_summary(&pure).is_ok());
    }
