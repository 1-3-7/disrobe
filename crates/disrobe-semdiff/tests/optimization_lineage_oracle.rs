...
fn runtime_file(mut command: Command, name: &str) -> PathBuf {
    ...
    assert!(
        path.is_file(),
        "compiler did not resolve {name} to a file: {}",
        path.display()
    );
    path
}

fn gcc_prefix(libgcc: &Path) -> Option<&Path> {
    let gcc: &Path = libgcc.parent()?.parent()?.parent()?;
    let lib: &Path = gcc.parent()?;
    if libgcc.file_name()? != "libgcc.a" || gcc.file_name()? != "gcc" || lib.file_name()? != "lib" {
        return None;
    }
    lib.parent()
}

fn same_runtime_file(gcc: &Path, clang: &Path) -> std::io::Result<bool> {
    Ok(gcc.canonicalize()? == clang.canonicalize()?)
}

#[test]
fn gcc_runtime_prefix_requires_the_declared_installation_layout() {
    assert_eq!(
        gcc_prefix(Path::new(
            "toolchain/lib/gcc/x86_64-w64-mingw32/16.2.0/libgcc.a"
        )),
    ...
    for path in [
        ...
        "toolchain/lib/other/x86_64-w64-mingw32/16.2.0/libgcc.a",
        "toolchain/bin/gcc/x86_64-w64-mingw32/16.2.0/libgcc.a",
    ] {
        assert_eq!(gcc_prefix(Path::new(path)), None, "{path}");
    }
}

#[test]
fn runtime_identity_rejects_a_second_installation_and_accepts_path_aliases() {
    let scratch: ScratchDir = ScratchDir::create("disrobe-semdiff-runtime-identity")
        .expect("create runtime identity scratch directory");
    let first: PathBuf = scratch.path().join("first");
    let second: PathBuf = scratch.path().join("second");
    std::fs::create_dir(&first).expect("create first runtime directory");
    std::fs::create_dir(&second).expect("create second runtime directory");
    for name in ["crt2.o", "libgcc.a"] {
        let gcc: PathBuf = first.join(name);
        let other: PathBuf = second.join(name);
        std::fs::write(&gcc, b"first runtime").expect("write first runtime input");
        std::fs::write(&other, b"second runtime").expect("write second runtime input");
        assert!(
            same_runtime_file(&gcc, &first.join(".").join(name)).expect("resolve path aliases")
        );
        assert!(!same_runtime_file(&gcc, &other).expect("resolve separate runtimes"));
        assert!(same_runtime_file(&gcc, &first.join("absent")).is_err());
    }
}
...
fn require_toolchain() -> Option<Toolchain> {
    ...
    if cfg!(target_os = "windows")
        && toolchain.compilers.contains(&"gcc")
        && toolchain.compilers.contains(&"clang")
    {
        let libgcc: PathBuf = runtime_file(toolchain.command("gcc"), "libgcc.a");
        toolchain.clang_sysroot = Some(gcc_prefix(&libgcc).unwrap_or_else(|| {
            panic!("expected GCC runtime at <prefix>/lib/gcc/<target>/<version>/libgcc.a, got {}", libgcc.display())
        }).to_path_buf());
        for name in ["crt2.o", "libgcc.a"] {
            let gcc: PathBuf = runtime_file(toolchain.command("gcc"), name);
            let clang: PathBuf = runtime_file(toolchain.command("clang"), name);
            assert!(
                same_runtime_file(&gcc, &clang).expect("resolve both compiler runtime paths"),
                "cross-compiler optimization grading requires the same {name}: gcc={} clang={}",
                gcc.display(),
                clang.display()
            );
        }
...
fn lto_refusal(toolchain: &Toolchain, compiler: &str, scratch: &Path) -> Option<String> {
...
fn plan_matrix(toolchain: &Toolchain, scratch: &Path) -> Vec<CompilerPlan> {
    toolchain
        ...
        .map(|&compiler: &&'static str| {
            ...
            let refusal: Option<String> = lto_refusal(toolchain, compiler, scratch);
            if refusal.is_none() {
...
fn prepare_battery() -> Option<Battery> {
    ...
    let planned: usize = plans
        ...
        .sum();
    let builds: Vec<Build> = prepare_builds(&directory, &plans, &toolchain, &source_path);
    assert_eq!(
        builds.len(),
        planned,
...
struct CaseResult {
    ...
    wrong_summary_pairs: Vec<String>,
...
fn grade(
    ...
    let mut wrong_summary_pairs: Vec<String> = Vec::new();
    ...
    for pair in &report.matches {
        ...
        if *expected != pair.other_address {
            if is_summary {
                let actual_name: String = target_named
                    .iter()
                    .find_map(|(candidate, &address): (&String, &u64)| {
                        (address == pair.other_address).then(|| candidate.clone())
                    })
                    .unwrap_or_else(|| format!("<unnamed@{:#x}>", pair.other_address));
                wrong_summary_pairs.push(format!(
                    "{name}@{:#x} -> {actual_name}@{:#x}, expected {:#x}",
                    pair.base_address, pair.other_address, expected
                ));
            }
            continue;
        }
...
fn symbolic_summary_recovers_optimization_variants_without_a_wrong_commit() {
    ...
    for result in &results {
        ...
        assert_eq!(
            result.summary_correct,
            result.summary_predicted,
            "the symbolic summary tier committed to a wrong pair for {}: {}/{} correct; pairs={:?}",
            result.label,
            result.summary_correct,
            result.summary_predicted,
            result.wrong_summary_pairs
        );
        assert!(
