...
fn validate_cli(path: &Path) -> Result<(), String> {
    if !path.is_file() {
        return Err(format!("{} is not a CLI executable file", path.display()));
    }
    let output: CapturedOutput = run_captured(path, &["--version"], Duration::from_secs(10), 4096)
...
const PUBLISHED_O2_HEADING: &str = "Native taint source-to-sink flow (Juliet CWE-78 char/system corpus, gcc 16.2.0 -O2 -fno-builtin, Windows x86-64)";
