...
fn header_and_trailer_present() {
    ...
    assert_eq!(graph.modules.len(), 1, "exactly one library node");
    assert_eq!(
        graph.modules.get("kernel32.dll").map(Vec::as_slice),
        Some(["GetProcAddress".to_owned()].as_slice()),
        "the single library must own exactly its one imported symbol"
    );
    assert!(graph.exports.is_empty(), "no exports were supplied");
    assert_eq!(
        dot.matches("\" -> \"").count(),
        1,
        "one import and no exports must yield exactly one edge"
    );
