...
use disrobe_binfmt::{
    ...
    InputClassification, NativeFile, SectionInfo, StructuralFormat, asar, carve_recursive,
    classify_input, detect_and_extract_with_hint, detect_container, detect_container_with_hint,
...
#[test]
fn a_pe_section_ending_past_the_address_space_is_a_typed_parse_error() {
    const IMAGE_BASE_OFFSET: usize = 0x70;
    const SECTION_VIRTUAL_SIZE_OFFSET: usize = 0x150;
    const OVERFLOWING_IMAGE_BASE: u64 = 0xFFFF_FFFF_F000_0000;
    const OVERFLOWING_VIRTUAL_SIZE: u32 = 0x1000_0000;
    let control: NativeFile = parse_native(&pe64_seed()).expect("the PE64 seed parses");
    assert_eq!(
        control
            .sections
            .iter()
            .map(|section: &SectionInfo| (section.address, section.size))
            .collect::<Vec<(u64, u64)>>(),
        vec![(0x0001_4000_1000, 0x100)]
    );
    let mut bytes: Vec<u8> = pe64_seed();
    write_u64_le(&mut bytes, IMAGE_BASE_OFFSET, OVERFLOWING_IMAGE_BASE);
    write_u32_le(
        &mut bytes,
        SECTION_VIRTUAL_SIZE_OFFSET,
        OVERFLOWING_VIRTUAL_SIZE,
    );
    let outcome: std::thread::Result<Result<NativeFile>> =
        std::panic::catch_unwind(|| parse_native(&bytes));
    assert!(
        matches!(&outcome, Ok(Err(disrobe_binfmt::Error::NativeParse(_)))),
        "an overflowing section range must be a typed native parse error: {outcome:?}"
    );
    if let Ok(Err(disrobe_binfmt::Error::NativeParse(message))) = outcome {
        assert!(
            message.contains("section \".text\"")
                && message.contains("ends past the 64-bit address space"),
            "{message}"
        );
    }
}
