...
fn assert_archive_parity(kind: ContainerKind, bytes: Vec<u8>, partial: bool) {
    ...
    let mut expected_members: Vec<(String, [u8; 32])> = direct
        .entries
        .iter()
        ...
        .collect();
    if kind == ContainerKind::AppImage {
        ...
        for row in manifest.lines().skip(1) {
            ...
            if fields[1] == "directory" {
                assert!(scratch.path().join(fields[0]).is_dir());
                expected_members.push((fields[0].to_owned(), hash(&[])));
            }
        }
        for name in [".disrobe-appimage-layout.json", ".disrobe-iso-layout.json"] {
            let data: Vec<u8> = std::fs::read(scratch.path().join(name))
                .expect("read the direct extraction layout");
            expected_members.push((name.to_owned(), hash(&data)));
        }
        expected_members
            .sort_by(|left: &(String, [u8; 32]), right: &(String, [u8; 32])| left.0.cmp(&right.0));
    }
    assert!(
