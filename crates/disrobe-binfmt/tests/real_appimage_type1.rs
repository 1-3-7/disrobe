...
fn official_type1_members_reach_the_registered_container_pass() {
    ...
    for fields in rows {
        ...
        let child: &disrobe_core::chain::ChildArtifact = children
            .iter()
            .find(|child: &&disrobe_core::chain::ChildArtifact| {
                child.handle.relative_path == fields[0]
            })
            .expect("manifest member reaches the registered pass");
        #[cfg(unix)]
        let unix_mode: Option<u32> =
            Some(u32::from_str_radix(fields[2], 8).expect("manifest mode"));
        #[cfg(not(unix))]
        let unix_mode: Option<u32> = None;
        let expected: ChildMaterialization = match fields[1] {
            "directory" => {
            ...
            "regular" | "hardlink" => {
                assert_eq!(child.bytes.len(), fields[3].parse::<usize>().expect("size"));
        ...
        assert_eq!(child.handle.materialization, expected, "{}", fields[0]);
    }
    for name in [".disrobe-appimage-layout.json", ".disrobe-iso-layout.json"] {
        let child: &disrobe_core::chain::ChildArtifact = children
            .iter()
            .find(|child: &&disrobe_core::chain::ChildArtifact| child.handle.relative_path == name)
            .expect("generated layout reaches the registered pass");
        assert!(matches!(
