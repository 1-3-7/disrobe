...
fn representable_range(kind: &str, name: &str, address: u64, size: u64) -> Result<()> {
    match address.checked_add(size) {
        Some(_) => Ok(()),
        None => Err(Error::NativeParse(format!(
            "{kind} {name:?} at {address:#x} with size {size:#x} ends past the 64-bit address space"
        ))),
    }
}
...
pub fn parse_native(bytes: &[u8]) -> Result<NativeFile> {
    ...
    for sec in file.sections() {
        ...
        let address: u64 = sec.address();
        let size: u64 = sec.size();
        representable_range("section", &name, address, size)?;
    ...
    for seg in file.segments() {
        ...
        let address: u64 = seg.address();
        let size: u64 = seg.size();
        representable_range("segment", name.as_deref().unwrap_or(""), address, size)?;
