...
Haxe is a transpiler: it emits NATIVE bytecode/source for each target, NOT a Haxe-specific container. The
emitted artifact carries only a compiler FINGERPRINT (a banner, a magic, or a marker string), no portable
Haxe-level IR. So the precise ceiling for this pass is FINGERPRINT + ROUTE, never reimplement-decompilation:
we identify the emitted target and hand the bytes to the pass that already owns that format.

Routing matrix (`haxe.rs`, codified in `tests/real_haxe_targets.rs`):

| emitted target | detector marker          | route_pass_id        | owner |
|----------------|--------------------------|----------------------|-------|
| JavaScript     | `// Generated by Haxe`   | `js.deob`            | js deobfuscator |
| SWF / Flash    | FWS/CWS/ZWS + `haxe`     | `as3.classify`       | as3 pass |
| HashLink (.hl) | `HLB` magic              | `scriptlang.classify`| stays here (haxe's own vm, no downstream pass) |
| JVM classfile  | CAFEBABE + major 45..=69 | `jvm.classify`       | jvm pass |
| .NET CIL (PE)  | PE + non-empty CLR dir   | `dotnet.classify`    | dotnet pass |
| hxcpp / C++    | `hx::`/`HX_` + native/.cpp | `disrobe-pass-native` | native pass |

The `HaxeFingerprint` / `HaxeCrossRoute` structs carry target + route_pass_id + version + confirmation flag
ONLY; `symbol_count` is 0 by design because source recovery is the routed-to pass's job. This is why Haxe is
"fingerprint+route only" in the mission: reimplementing a Haxe decompiler would duplicate six target backends
we already have passes for. Real recovered tokens for a Haxe program therefore come out of (e.g.) the jvm or
as3 pass after routing, not out of this crate.
