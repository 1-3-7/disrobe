...
fn normalize_fixture_dump(tree_text: &str) -> Vec<NormalizedSub> {
    let mut subs: Vec<NormalizedSub> = normalize_dump(tree_text);
    for sub in &mut subs {
        for line in &mut sub.lines {
            let Some((op, _)): Option<(&str, &str)> = line.rsplit_once(" ->") else {
                continue;
            };
            let Some((body, flags)): Option<(&str, &str)> = op.rsplit_once(' ') else {
                continue;
            };
            let Some((context, private)): Option<(&str, &str)> = flags.split_once('/') else {
                continue;
            };
            if context.starts_with('s') && private.split(',').any(|flag: &str| flag == "TARGMY") {
                let start: usize = body.len() + 1;
                line.replace_range(start..=start, "v");
            }
        }
    }
    subs
...
fn normalize_op_line(line: &str) -> Option<String> {
    ...
    let flags: String = normalize_seq_arrows(body[boundary..].trim()).replace("/STMT,", "/");
...
fn committed_concise_fixtures_still_match_the_installed_perl() {
    ...
    for (fixture, expected) in FIXTURES.iter().zip(EXPECTED) {
        ...
        let live_subs: Vec<NormalizedSub> = normalize_fixture_dump(&live.tree_text);
        let committed_text: String =
            String::from_utf8_lossy(fixture.committed_concise).into_owned();
        let committed_subs: Vec<NormalizedSub> = normalize_fixture_dump(&committed_text);
...
#[test]
fn statement_context_flag_is_normalized() {
    let baseline: Option<String> = normalize_op_line("8           <|> and(other->9) vK/1 ->c");
    let flagged: Option<String> = normalize_op_line("8           <|> and(other->9) vK/STMT,1 ->c");
    assert_eq!(baseline, flagged);
}

#[test]
fn statement_context_rewrite_changes_a_dump_without_void_private_flags() {
    let dump: &str = "main::greet:\n7  <1> leavesub[1 ref] K/REFC,1 ->(end)\n";
    let rewrites: Vec<(&str, String)> = incidental_rewrites(dump);
    let (_, rewritten): &(&str, String) = rewrites
        .iter()
        .find(|(label, _)| *label == "add a statement-context flag")
        .expect("statement-context rewrite remains in the probe set");
    assert_ne!(rewritten, dump);
    assert_eq!(normalize_dump(rewritten), normalize_dump(dump));
}

#[test]
fn fixture_comparison_accepts_the_perl_target_context_change() {
    let scalar: &str = "main::total:\nq  <2> add[$sum:12,13] sK/TARGMY,2 ->r\n";
    let void: String = scalar.replace(" sK/", " vK/");
    assert_ne!(normalize_dump(scalar), normalize_dump(&void));
    assert_eq!(
        normalize_fixture_dump(scalar),
        normalize_fixture_dump(&void)
    );
}

#[test]
fn fixture_comparison_preserves_other_contexts_and_semantic_changes() {
    let scalar: &str = "main::total:\nq  <2> add[$sum:12,13] sK/TARGMY,2 ->r\n";
    let base: Vec<NormalizedSub> = normalize_fixture_dump(scalar);
    for changed in [
        scalar.replace("add[", "subtract["),
        scalar.replace("$sum", "$other"),
        scalar.replace(" sK/", " lK/"),
        scalar.replace("TARGMY,2", "TARGMY,1"),
        scalar.replace("TARGMY,", ""),
        scalar.replace("<2>", "<1>"),
    ] {
        assert_ne!(normalize_fixture_dump(&changed), base, "{changed}");
    }
    let ordinary: String = scalar.replace("TARGMY,", "");
    let ordinary_void: String = ordinary.replace(" sK/", " vK/");
    assert_ne!(
        normalize_fixture_dump(&ordinary),
        normalize_fixture_dump(&ordinary_void)
    );
}
...
fn incidental_rewrites(dump: &str) -> Vec<(&'static str, String)> {
    let mut rewrites: Vec<(&'static str, String)> = vec![
        ...
        (
            "add a statement-context flag",
            dump.replacen("/REFC", "/STMT,REFC", 1),
        ),
    ];
