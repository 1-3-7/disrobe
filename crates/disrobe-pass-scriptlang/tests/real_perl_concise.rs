...
#[test]
fn real_concise_recovers_multiconcat_literal_oracle() {
    let tree: PerlOpTree = read_concise(HELLO_CONCISE).expect("parse");
    let all_consts: Vec<String> = tree
        .subs
        .iter()
        ...
        .collect();
    assert!(
        all_consts.iter().any(|c: &String| c.contains("Hello, !")),
        "multiconcat literal \"Hello, !\" (present only in the multiconcat op, with an embedded comma) must be recovered: {all_consts:?}"
    );
}
