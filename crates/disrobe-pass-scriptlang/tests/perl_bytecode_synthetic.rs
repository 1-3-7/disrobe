...
//! Validates the Perl `ByteLoader` reader against a spec-faithful synthetic `ByteLoader 0.06` stream.
...
fn hello_bytecode(order: ByteOrder) -> Vec<u8> {
    ...
    for pv in [
        ...
        out.push(OP_PV_CUR);
        put_u32(&mut out, pv.len() as u32, order);
