...
fn read_corpus(rel: &str) -> Vec<u8> {
    ...
    std::fs::read(&path).unwrap_or_else(|e: std::io::Error| {
        panic!("corpus fixture missing {}: {e}", path.display())
    })
}
...
fn real_yarv_greeter_iseq_parses_with_3_4_header() {
    let bytes: Vec<u8> = read_corpus("mri/yarv/greeter.rb.yarvc");
    assert!(
        bytes.len() > 200,
...
fn real_yarv_hello_source_is_real_ruby() {
    let bytes: Vec<u8> = read_corpus("hello.rb");
...
fn real_yarv_greeter_source_has_module_class_def() {
    let bytes: Vec<u8> = read_corpus("greeter.rb");
...
fn real_yarv_megafile_iseq_parses() {
    let bytes: Vec<u8> = read_corpus("mri/yarv/edge_cases.rb.yarvc");
    assert!(
        bytes.len() > 100_000,
...
fn real_yarv_megafile_source_has_broad_feature_surface() {
    let bytes: Vec<u8> = read_corpus("megafile/edge_cases.rb");
    assert!(
        bytes.len() > 30_000,
