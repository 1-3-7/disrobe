...
fn load_corpus(rel: &str) -> Vec<u8> {
    let path: PathBuf = corpus_path(rel);
    std::fs::read(&path).unwrap_or_else(|_| panic!("missing committed fixture corpus/ruby/{rel}"))
}

#[test]
fn real_yarv_hello_iseq_parses_with_3_4_header() {
    let bytes: Vec<u8> = load_corpus("mri/yarv/hello.rb.yarvc");
    assert!(
        bytes.len() > 36,
...
fn real_yarv_greeter_iseq_parses_with_3_4_header() {
    let bytes: Vec<u8> = load_corpus("mri/yarv/greeter.rb.yarvc");
    assert!(
        bytes.len() > 200,
...
fn real_yarv_hello_source_is_real_ruby() {
    let bytes: Vec<u8> = load_corpus("hello.rb");
...
fn real_yarv_greeter_source_has_module_class_def() {
    let bytes: Vec<u8> = load_corpus("greeter.rb");
...
fn real_yarv_megafile_iseq_parses() {
    let bytes: Vec<u8> = load_corpus("mri/yarv/edge_cases.rb.yarvc");
    assert!(
        bytes.len() > 100_000,
...
fn real_yarv_megafile_source_has_broad_feature_surface() {
    let bytes: Vec<u8> = load_corpus("megafile/edge_cases.rb");
    assert!(
        bytes.len() > 30_000,
