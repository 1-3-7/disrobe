//! Clean-room decoder for the flat mruby (RITE) `iseq` byte stream.
//!
//! Implements the `FETCH_*` / `READ_*` operand semantics from `include/mruby/opcode.h`:
//! `B` reads one byte, `S` reads a 16-bit big-endian value (`pc[0]<<8 | pc[1]`), `W` reads a
//! 24-bit big-endian value. The `EXT1`/`EXT2`/`EXT3` prefix opcodes widen the first / second /
//! both leading `B` operands of the *following* opcode to `S`.
...
/// A single decoded mruby instruction with its resolved operand values.
...
/// Decode one IREP record's flat `iseq` into a typed instruction stream.
///
/// Fails fast with [`RubyError::MrubyUnknownOpcode`] on an out-of-table opcode byte and
/// [`RubyError::MrubyIrepTruncated`] if an operand runs past the end of `iseq`.
pub(crate) fn disassemble_iseq(iseq: &[u8]) -> Result<Vec<MrubyInstruction>> {
