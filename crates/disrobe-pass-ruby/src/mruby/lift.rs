...
//!
//! Tracks per-register symbolic values across a single IREP body and reconstructs Ruby surface
//! statements: literal loads, `OP_SEND` method calls (receiver, method name, args, block),
//! arithmetic/comparison operators, `OP_DEF`/`OP_CLASS`/`OP_MODULE`/`OP_METHOD` definitions
//! (recursing into the referenced child IREP), assignments to globals/ivars/cvars/consts, and
//! `OP_RETURN`. Operands index this IREP's own `pool`/`symbols`/child lists, resolved up front.

use core::fmt::Write as _;

use crate::error::Result;
use crate::mruby::disasm::{MrubyInstruction, disassemble_iseq};
use crate::mruby::irep::{IrepRecord, IrepTree, PoolKind};
...
fn lift_record(
    ...
    let Some(rec) = tree.records.get(index as usize) else {
        return Ok(());
    };
...
fn def_params(tree: &IrepTree, child: u32) -> String {
    let Some(rec) = tree.records.get(child as usize) else {
        return String::new();
    };
...
fn pool_value(rec: &IrepRecord, idx: u32) -> RegVal {
    let Some(entry) = rec.pool.get(idx as usize) else {
...
mod tests {
    ...
    #[test]
    fn lifts_method_definition_with_recursion() {
