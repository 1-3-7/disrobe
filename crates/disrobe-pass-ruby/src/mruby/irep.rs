...
pub(crate) fn parse_irep(section_body: &[u8], format_version: [u8; 4]) -> Result<IrepTree> {
    ...
    let total_insn_bytes: u32 = records.iter().map(|r| r.insn_len).sum();
    let total_symbols: u32 =
        u32::try_from(records.iter().map(|r| r.symbols.len()).sum::<usize>()).unwrap_or(u32::MAX);
    let total_pool_entries: u32 =
        u32::try_from(records.iter().map(|r| r.pool.len()).sum::<usize>()).unwrap_or(u32::MAX);
