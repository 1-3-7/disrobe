...
mod opcode_tables_legacy;
mod opcode_tables_v30_v32;
pub mod opcodes;
pub mod reader;
    ...
    for obj in &image.objects {
        match &obj.literal {
            Some(text) => {
                push_fmt_line(
                    &mut out,
                    format_args!("obj[{}] {:?} = {:?}", obj.index, obj.kind, text),
                );
            }
            None => {
                push_fmt_line(&mut out, format_args!("obj[{}] {:?}", obj.index, obj.kind));
            }
        }
    }
    out
...
mod tests {
    ...
    fn structural_ibf_with_string(version: (u32, u32), literal: &str) -> Vec<u8> {
    ...
    #[test]
    fn analyze_parses_minimal_structural_ibf_without_panic() {
        let bytes: Vec<u8> = structural_ibf_with_string((3, 4), "hello world");
    ...
    fn analyze_out_of_range_object_offset_is_safe() {
        let mut bytes: Vec<u8> = structural_ibf_with_string((3, 2), "x");
