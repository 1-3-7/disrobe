...
/// Wraps a top-level `rescue`/`ensure` catch range in `begin`/`rescue`/`ensure`/`end`.
...
/// Decompile a `rescue in ...` handler iseq into one or more `rescue [Class => var]` clauses.
...
/// Parse one rescue clause starting at `i`, returning its class names, `=> var` binding, and body range.
...
/// Render instructions `[lo, hi)` of `body`, structuring forward conditional branches into `if`/`unless`/`else`.
...
/// Find where the `case/in` arm ladder ends and the optional `else`-body range.
...
/// Find the `putspecialobject`/`core#raise` fall-through that marks a `case/in` region.
...
/// The instruction that loads the `case/in` subject.
...
/// Parse the `in <pattern>` arms in `[lo, raise_idx)`.
...
/// Parse an arm guard expression in `[test_end, success_branch)`.
fn parse_arm_guard(
    body: &YarvIseqBody,
    ctx: &DecompileContext<'_>,
    test_end: usize,
...
/// Recognize a `while`/`until` loop at instruction `i`, rendering its condition and body.
#[allow(clippy::too_many_arguments)]
fn try_loop(
    body: &YarvIseqBody,
    ctx: &DecompileContext<'_>,
...
/// Recognize a compound conditional assignment `target ||= value` / `target &&= value`.
...
/// The first instruction index after a rendered conditional region, i.e. its merge point.
...
/// Render a nested body (method/class/module) bracketed by `header` and `end`.
fn render_nested(
    header: String,
...
/// A line that is a single bare literal/identifier with no side effect, safe to drop as an implicit result.
...
/// Cross-iseq decompile context: block-parameter names, iseq bodies, and the object table.
...
impl<'a> DecompileContext<'a> {
    ...
    /// Resolve a constant-path cache array into `A::B::C`.
...
/// Reconstruct a `concatstrings` join as a Ruby interpolation or `+` concatenation.
...
/// The inner text of a Rust-`Debug`-rendered string literal, or `None` when not a plain literal.
...
/// Render an `opt_getconstant_path` operand, resolving the cache array into `A::B::C` when possible.
...
/// Render a block inline (`{ |x| body }`) for a single statement, else as a `do...end` form.
...
/// `invokesuper` -> `super(args)`, or bare `super` for zero args or anonymous-forwarding markers.
...
/// Map an unresolved/sentinel calldata method name to a valid identifier.
