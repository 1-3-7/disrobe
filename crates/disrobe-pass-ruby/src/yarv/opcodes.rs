...
//!
//! The per-version opcode arrays live in [`crate::yarv::opcode_tables`] and are ported from the
//! `insns.def` `DEFINE_INSN` line order (opcode number = line order) for each ruby tag. This
//! module maps a [`YarvVersion`] onto the matching table.
