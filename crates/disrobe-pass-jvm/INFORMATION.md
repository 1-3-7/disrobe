...
- dex2jar BRANCH frontier (NOT shipped this batch): bodies with `if`/`goto`/`switch`/`try` (91/355 = ~26% of code items) stay on the verifiable stub. A correct CFG emitter is theory-achievable but requires synthesizing a `StackMapTable` whose per-target local-type frames EXACTLY match the strict verifier's own merge inference. A first attempt (locals dataflow over the Dalvik CFG + `full_frame` synthesis) was built and validated against the JVM verifier, but the abstract register model drifted from the verifier's inference ("Inconsistent stackmap frames" on 2-3 classes), and shipping a partial StackMapTable would regress the sacred zero-failure verifier gate. This is an engineering-cost/correctness boundary, not a theory limit: the fix is to make the frame builder a faithful abstract interpreter of the exact emitted bytecode (or two-pass emit-then-frame), deferred to avoid regressing the green verifier on this OOM-fragile box.
