...
pub struct Dex2JarResult {
    ...
    /// Count of methods whose real Dalvik body was lowered to a verifiable JVM `Code` attribute.
...
fn parse_type_list(dex_bytes: &[u8], off: usize, type_names: &[String]) -> Vec<String> {
    ...
    let mut out: Vec<String> = Vec::with_capacity(size as usize);
...
pub fn build_class_model(dex: &DexFile, dex_bytes: &[u8]) -> Vec<TranslatedClass> {
    ...
    let mut classes: Vec<TranslatedClass> = Vec::with_capacity(header.class_defs_size as usize);
...
/// A lowered method body: either real recovered bytecode or the verifiable stub.
struct BuiltBody {
    code: Vec<u8>,
    max_stack: u16,
    max_locals: u16,
