...
/// Synthetic reference transform for tests only; not the proprietary `PreEmptive`
/// `DashO` algorithm. Real recovery emulates the embedded decrypt stub via
...
/// decrypt stub via [`crate::protectors::recover_via_embedded_stub`]. The `DashO`
/// string algorithm is proprietary and opaque without that stub, so when no stub
/// is present this returns [`PeelStatus::DetectOnly`]: markers are logged and
/// residual encrypted strings counted, but no string plaintext is claimed.
/// `class_name` is retained for the synthetic self-consistency fixtures only.
...
mod tests {
    ...
    /// NOT real `DashO`-sample recovery.
    #[test]
