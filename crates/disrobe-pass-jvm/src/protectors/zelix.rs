...
/// Synthetic reference cipher used only to exercise the peel pipeline in tests.
///
/// This is NOT the proprietary Zelix string algorithm, which is opaque
/// without the embedded decrypt stub. Real recovery runs through
/// [`crate::protectors::recover_via_embedded_stub`], which emulates the class's own
/// decrypt routine.
#[must_use]
...
///
/// This is an honest detect-only metric: it does not claim recovery, only that N
/// constants remain opaque after the embedded-stub path declined to decrypt them.
#[must_use]
...
/// decrypt stub via [`crate::protectors::recover_via_embedded_stub`]. The Zelix
/// string algorithm is proprietary and opaque without that stub, so when no stub
/// is present this returns [`PeelStatus::DetectOnly`]: it counts residual
/// encrypted strings and characterises the control-flow flattening but makes no
/// claim to have decrypted anything. The `key` parameter is retained for the
/// synthetic self-consistency fixtures only and is not consulted here.
...
mod tests {
    ...
    /// This is NOT real Zelix-sample recovery: it encrypts with our own
    /// stand-in transform and decrypts with its inverse.
    #[test]
