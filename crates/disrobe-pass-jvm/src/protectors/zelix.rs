...
#[must_use]
pub fn strip_strings(cf: &ClassFile, key: ZelixKey) -> (BTreeMap<u16, String>, usize) {
    ...
    for (idx, s) in &strings {
        ...
        let candidate: String = zelix_decrypt_chars(&utf16, key);
        ...
        if candidate_readable && !original_readable {
            recovered.insert(*idx, candidate);
        } else if original_readable {
            recovered.insert(*idx, s.clone());
...
pub fn peel(cf: &ClassFile, key: ZelixKey) -> ProtectorPeelReport {
    let (recovered, residual): (BTreeMap<u16, String>, usize) = strip_strings(cf, key);
    let mut report: ProtectorPeelReport =
        ProtectorPeelReport::new(ProtectorFamily::ZelixKlassMaster);
    report.strings_recovered = recovered;
    ...
    for method in &cf.methods {
        for attr in &method.attributes {
            ...
            let pattern: CffPattern = unflatten_cff(&attr.info);
