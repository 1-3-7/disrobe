...
pub fn peel(cf: &ClassFile, class_name: &str, method_name: &str) -> ProtectorPeelReport {
    let mut report: ProtectorPeelReport = ProtectorPeelReport::new(ProtectorFamily::Stringer);
    let key: StringerKey = StringerKey {
        class_id: class_key(class_name),
        method_id: class_key(method_name),
    };
    let strings: BTreeMap<u16, String> = cf.collect_strings();
    let mut residual: usize = 0;
    for (idx, s) in &strings {
        let candidate: String = stringer_decrypt(s, key);
