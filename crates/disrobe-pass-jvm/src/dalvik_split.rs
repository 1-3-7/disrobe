...
fn plan_split_candidate(
    ...
    for (i, d) in du.iter().enumerate() {
        let (Some(position), Some((reg, _)), Some(&di)) =
            (d.def_position, d.def, def_index.get(&i))
        else {
            continue;
        };
        if !d.use_positions.contains(&position) || !conflicted.contains(&reg) {
            continue;
        }
        let assigned: u16 = *web_reg.get(&uf.find(di))?;
