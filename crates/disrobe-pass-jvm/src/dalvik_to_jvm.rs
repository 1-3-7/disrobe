...
pub(crate) fn emit_branch_method_code(
    ...
    let pc_entry_int_const: BTreeMap<u32, BTreeMap<u16, i32>> = compute_entry_int_constants(
        dex,
        &insns,
        &parsed,
        &states,
...
fn compute_entry_int_constants(
    ...
    let Some(entry_states): Option<Vec<Option<BTreeMap<u16, i32>>>> =
        must_narrow_constant_states(dex, insns, parsed, switch_targets, handler_edges, pc_to_idx)
    else {
        return BTreeMap::new();
    };
...
fn must_narrow_constant_states(
    ...
    while let Some(idx) = work.pop_front() {
        ...
        apply_narrow_constant_transfer(dex, insn, parsed, &mut post, &tracked);
...
fn apply_narrow_constant_transfer(
    ...
    match insn.op {
        ...
        _ if defines_register(insn) => {
            state.remove(&dest);
            let (_, category, _): (Option<u16>, Cat, Vec<(u16, Cat)>) =
                register_effects(dex, insn, parsed);
            if matches!(category, Cat::Two) {
...
impl Emitter<'_> {
    ...
    fn return_value(&mut self, regs: &[u16], parsed: &MethodDescriptor) {
        ...
        if let Some(&src) = regs.first() {
            if matches!(slot, Slot::Ref) {
                if !self.const_zero.contains(&src)
                    && self.entry_int_constant(src) != Some(0)
                    && !matches!(self.reg_slot(src), Slot::Ref)
                {
...
pub(crate) fn narrow_const_float_pcs(
    ...
    for (idx, insn) in insns.iter().enumerate() {
        ...
        match flow {
            ConstFlow::Conflict => {
                record_bail_kind("constant-float-reference-conflict");
                return None;
            }
            ConstFlow::Int | ConstFlow::Reference => continue,
            ConstFlow::Float | ConstFlow::Unknown => {}
        }
        out.insert(insn.pc);
    }
    Some(out)
        ...
        for probe in [Slot::Float, Slot::Double] {
            for (start, insn) in insns.iter().enumerate() {
                ...
                match const_value_int_or_float_use(
                    ...
                    ConstFlow::Conflict => return true,
                    ConstFlow::Reference | ConstFlow::Unknown => {}
                }
            }
        }
...
fn const_value_reaches_float_use(
    ...
    while let Some(idx) = work.pop() {
        ...
        match float_use_kind(dex, insn, parsed, reg, slot) {
            ...
            FloatUse::Reference | FloatUse::Redefined => continue,
...
fn const_value_int_or_float_use(
    ...
    let mut saw_reference: bool = false;
    while let Some(idx) = work.pop() {
        if !visited.insert(idx) {
            continue;
        }
        ...
        match float_use_kind(dex, insn, parsed, reg, slot) {
            FloatUse::Match => saw_float = true,
            FloatUse::Reference => saw_reference = true,
            FloatUse::Redefined => continue,
            FloatUse::Ambiguous | FloatUse::None => {}
        }
        if saw_float && saw_reference {
            return ConstFlow::Conflict;
        }
        if defines_register(insn) && insn.regs.first() == Some(&reg) {
            continue;
        }
