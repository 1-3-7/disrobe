...
pub(crate) struct EmittedCode {
    ...
    /// Serialized `exception_table` entries (each `start_pc(u2) end_pc(u2)
    /// handler_pc(u2) catch_type(u2)`), empty when the method has no try blocks.
    pub(crate) exception_table: Vec<u8>,
    /// Number of entries packed into `exception_table`.
...
/// Branch-mode emission state attached to the [`Emitter`] when lowering a body with control flow.
struct CfgEmit {
    cur_pc: u32,
...
/// One try region: the half-open Dalvik PC range it guards and its handlers.
struct TryRegion {
    start_pc: u32,
    end_pc: u32,
...
/// A `tableswitch`/`lookupswitch` whose target operands are patched to i32 deltas once all PCs are known.
struct SwitchFixup {
    insn_offset: usize,
...
/// Flattened `SwitchFixup` for the patch pass.
...
/// CFG-aware lowering for bodies containing `if`/`goto` control flow.
#[must_use]
...
/// Resolves the JVM internal class name for a Dalvik catch type descriptor; `None` maps to `Throwable`.
...
/// Builds the method's try regions from the Dalvik `try_item`/handler tables.
...
/// True when every `move-exception` in the method is the first instruction of a handler block.
...
/// Maps each instruction PC inside a try range to the handler PCs it can reach by exception.
...
/// Maps each handler PC to the internal class name its `move-exception` types the caught value as.
...
/// Maps each handler PC to the throwable type the JVM places on the operand stack at handler entry.
...
/// Parses every `packed-switch`/`sparse-switch` payload referenced by the method.
...
/// Maps each switch PC to all its successor PCs: case targets plus the fall-through default.
...
/// True when the switch keys form a gap-free ascending run (a dense `tableswitch`).
fn is_contiguous(keys: &[i32]) -> bool {
    keys.windows(2)
        .all(|w: &[i32]| w[1].checked_sub(w[0]) == Some(1))
}

/// Block leaders including switch case targets and switch fall-through defaults.
...
/// Block leaders are the method entry plus every branch target and conditional-branch fallthrough.
...
/// True when every `new-instance vDest` reaches its matching `invoke-direct <init>` within one block.
...
impl Emitter<'_> {
    ...
    /// Lowers a Dalvik `goto`/`if-*` to the matching JVM branch.
    ...
    /// Lowers a Dalvik `packed-switch`/`sparse-switch` to a JVM `tableswitch` or `lookupswitch`.
    ...
    /// Emits the 0-3 padding bytes that align a switch table's operands to a 4-byte boundary.
    ...
    /// Patches every recorded branch and switch operand to the signed delta from its instruction to the target.
    ...
    /// Serializes the JVM `exception_table` from the method's try regions.
    ...
    /// Synthesizes the `StackMapTable` from the analysis's per-block-entry register states.
    ...
    /// Loads `reg` for a context that requires a reference, emitting `aconst_null` for a zero constant.
    ...
    /// Lowers `move-exception vN` at a handler entry, storing the on-stack throwable into the destination local.
    ...
    /// Records a pending `new-instance vDest, Type` without emitting yet, to fuse with its constructor call.
    ...
    /// Emits the fused `new; dup; <args>; invokespecial <init>; astore` for a `new-instance`/`invoke-direct` pair.
    fn emit_constructor(
        &mut self,
        recv: u16,
        owner: &str,
...
/// True for d8/r8 synthetic desugaring classes whose final name segment is purely numeric (`Foo$1`).
...
/// Rejects methods whose Dalvik register categories cannot be proven consistent by a single forward pass.
