...
const V1_SIGNATURE_FILE_COUNT_CAP: usize = 256;
const V1_SIGNATURE_ENTRY_BYTES_CAP: u64 = 16 * 1024 * 1024;
const V1_SIGNATURE_PREALLOC_BYTES_CAP: u64 = 1024 * 1024;
...
pub struct ApkSignatureReport {
    ...
    pub v1_certificates: Vec<CertificateInfo>,
...
impl ApkSignatureReport {
    #[inline]
    #[must_use]
    pub fn certificates(&self) -> Vec<&CertificateInfo> {
        let mut out: Vec<&CertificateInfo> = Vec::new();
        for scheme in &self.schemes {
            for cert in &scheme.certificates {
                out.push(cert);
            }
        }
        for cert in &self.v1_certificates {
            out.push(cert);
        }
        out
    }
...
fn parse_signer_block(signer: &[u8]) -> Option<ParsedSignerDigests> {
    ...
    while cc < certs_seq.len() {
        ...
        certificate_count += 1;
        if let Some(info) = der::parse_certificate(cert) {
    ...
    if let Some(signers_seq) = take_len_prefixed(value, &mut cursor) {
        ...
        while sc < signers_seq.len() {
            ...
            if let Some(parsed) = parse_signer_block(signer) {
                ...
                certificates.extend(parsed.certificates);
...
pub struct V4Report {
    ...
    pub apk_digest_matches_v2_or_v3: bool,
    pub matched_scheme: Option<SignatureScheme>,
}

fn parse_v4_signature(idsig: &[u8]) -> Option<V4Report> {
    let mut cursor: usize = 0;
    let version: u32 = u32::from_le_bytes(idsig.get(0..4)?.try_into().ok()?);
    cursor += 4;
    let hashing_info: &[u8] = take_len_prefixed(idsig, &mut cursor)?;
    let signing_info: &[u8] = take_len_prefixed(idsig, &mut cursor)?;

    let mut hc: usize = 0;
    let hash_algorithm: i32 = i32::from_le_bytes(hashing_info.get(0..4)?.try_into().ok()?);
    hc += 4;
    let log2_block_size: u8 = *hashing_info.get(hc)?;
    hc += 1;
    let salt: &[u8] = take_len_prefixed(hashing_info, &mut hc)?;
    let root_hash: &[u8] = take_len_prefixed(hashing_info, &mut hc)?;

    let mut sc: usize = 0;
    let apk_digest: &[u8] = take_len_prefixed(signing_info, &mut sc)?;
    let cert_der: &[u8] = take_len_prefixed(signing_info, &mut sc)?;
    let signature_algorithm_id: u32 =
        u32::from_le_bytes(signing_info.get(sc..sc + 4)?.try_into().ok()?);
    sc += 4;
    let _signature: &[u8] = take_len_prefixed(signing_info, &mut sc)?;
    ...
    Some(V4Report {
        ...
        signature_algorithm: SignatureAlgorithm::from_id(signature_algorithm_id),
        certificate: der::parse_certificate(cert_der),
        apk_digest_matches_v2_or_v3: false,
        matched_scheme: None,
    })
}

pub fn verify_v4(apk_bytes: &[u8], idsig_bytes: &[u8]) -> Result<V4Report> {
    let mut report: V4Report = parse_v4_signature(idsig_bytes).ok_or_else(|| {
        Error::Zip("malformed v4 .idsig signature (apksig V4Signature)".to_string())
    })?;

    let base: ApkSignatureReport = verify(apk_bytes)?;
    for scheme in [SignatureScheme::V3, SignatureScheme::V2] {
        let Some(sr): Option<&SchemeReport> = base.scheme(scheme) else {
            continue;
        };
        let matched: bool = sr
            .digests
            .iter()
            .any(|d: &SignerDigest| d.computed_digest == report.apk_digest);
        if matched {
            report.apk_digest_matches_v2_or_v3 = true;
            report.matched_scheme = Some(scheme);
            break;
        }
    }
    Ok(report)
...
pub fn verify(bytes: &[u8]) -> Result<ApkSignatureReport> {
    ...
    report.v1_certificates = v1.certificates;
    report.notes.extend(v1.notes);
...
mod der {
    ...
    pub(super) fn parse_certificate(cert_der: &[u8]) -> Option<CertificateInfo> {
        let fingerprint: String = sha256_hex(cert_der);
    ...
    pub(super) fn certificate_from_pkcs7(pkcs7_der: &[u8]) -> Option<CertificateInfo> {
        let cert: &[u8] = find_signed_cert(pkcs7_der, 0)?;
        parse_certificate(cert)
    }

    fn find_signed_cert(buf: &[u8], depth: u32) -> Option<&[u8]> {
        if depth > 8 {
            return None;
        }
        let mut offset: usize = 0;
        while offset < buf.len() {
            let tlv: Tlv<'_> = read_tlv(&buf[offset..])?;
            let full: &[u8] = buf.get(offset..offset + tlv.total_len)?;
            let header: usize = tlv.total_len - tlv.content.len();
            if tlv.tag == (CLASS_CONTEXT | 0x20)
                && let Some(found) = scan_cert_set(tlv.content, full, header)
            {
                return Some(found);
            }
            if (tlv.tag == TAG_SEQUENCE || tlv.tag == (CLASS_CONTEXT | 0x20))
                && let Some(found) = find_signed_cert(tlv.content, depth + 1)
            {
                return Some(found);
            }
    ...
    fn scan_cert_set<'a>(set_content: &'a [u8], full: &'a [u8], header: usize) -> Option<&'a [u8]> {
        let mut offset: usize = 0;
        while offset < set_content.len() {
            let tlv: Tlv<'_> = read_tlv(&set_content[offset..])?;
            if tlv.tag == TAG_SEQUENCE && looks_like_certificate(tlv.content) {
                let start: usize = header + offset;
                return full.get(start..start + tlv.total_len);
            }
            offset += tlv.total_len;
        }
        None
    ...
    fn looks_like_certificate(content: &[u8]) -> bool {
        let Some(tbs): Option<Tlv<'_>> = read_tlv(content) else {
            return false;
        };
        if tbs.tag != TAG_SEQUENCE {
            return false;
        }
        let mut inner: SeqReader<'_> = SeqReader::new(tbs.content);
        let Some(first): Option<Tlv<'_>> = inner.next() else {
            return false;
        };
        first.tag & CLASS_CONTEXT != 0 || first.tag == TAG_INTEGER
    }
    ...
    fn sha256_hex(data: &[u8]) -> String {
        ...
        let mut out: String = String::with_capacity(95);
...
#[inline]
fn v1_signature_entry_prealloc(uncompressed: u64, compressed: u64, limit: u64) -> usize {
    let bound: u64 = uncompressed
        .min(compressed.saturating_mul(2))
        .min(limit)
        .min(V1_SIGNATURE_PREALLOC_BYTES_CAP);
...
fn validate_v1_signature_file_count(count: usize) -> Result<()> {
    if count > V1_SIGNATURE_FILE_COUNT_CAP {
        return Err(Error::Zip(format!(
            "v1 signature entry count {count} exceeds cap {V1_SIGNATURE_FILE_COUNT_CAP}"
        )));
    }
    Ok(())
}

fn read_v1_signature_file(file: &mut zip::read::ZipFile<'_>, limit: u64) -> Result<Vec<u8>> {
    let name: String = file.name().to_string();
    let declared_size: u64 = file.size();
    if declared_size > limit {
        return Err(Error::Zip(format!(
            "v1 signature entry '{name}' declared size {declared_size} exceeds entry cap {limit}"
        )));
    }
    let read_limit: u64 = limit.checked_add(1).ok_or_else(|| {
        Error::Zip(format!(
            "v1 signature entry '{name}' read limit overflowed entry cap {limit}"
        ))
    })?;
    let mut buf: Vec<u8> = Vec::with_capacity(v1_signature_entry_prealloc(
        declared_size,
        file.compressed_size(),
        limit,
    ));
    let mut limited: std::io::Take<&mut zip::read::ZipFile<'_>> =
        std::io::Read::take(file, read_limit);
    let read: usize = std::io::Read::read_to_end(&mut limited, &mut buf)?;
    let read_u64: u64 = u64::try_from(read).map_err(|_| {
        Error::Zip(format!(
            "v1 signature entry '{name}' read size exceeds entry cap {limit}"
        ))
    })?;
    if read_u64 > limit {
        return Err(Error::Zip(format!(
            "v1 signature entry '{name}' read size {read_u64} exceeds entry cap {limit}"
        )));
    }
    Ok(buf)
}
...
fn scan_v1_signing(bytes: &[u8]) -> Result<V1Inventory> {
    ...
    let mut pkcs7_names: Vec<String> = Vec::new();
    ...
    for i in 0..zip.len() {
        ...
        {
            pkcs7_names.push(name.to_string());
        }
    }
    validate_v1_signature_file_count(signature_files.len())?;
    let mut certificates: Vec<CertificateInfo> = Vec::new();
    for name in pkcs7_names {
        let mut file: zip::read::ZipFile<'_> = zip
            .by_name(&name)
            .map_err(|e| Error::Zip(format!("failed to read v1 signature entry '{name}': {e}")))?;
        let buf: Vec<u8> = match read_v1_signature_file(&mut file, V1_SIGNATURE_ENTRY_BYTES_CAP) {
            Ok(buf) => buf,
            Err(Error::Io(err)) => {
                notes.push(format!(
                    "v1 signature entry '{name}' could not be decoded: {err}"
                ));
                continue;
            }
            Err(err) => return Err(err),
        };
        if let Some(info) = der::certificate_from_pkcs7(&buf) {
            certificates.push(info);
        }
    }
    Ok(V1Inventory {
...
mod tests {
    ...
    #[test]
    fn v1_signature_entry_over_limit_errors() {
        ...
        writer
            .start_file("META-INF/CERT.RSA", options)
            .expect("start signature entry");
        writer.write_all(b"cert").expect("write signature entry");
        let bytes: Vec<u8> = writer.finish().expect("finish zip").into_inner();
        let mut archive: zip::ZipArchive<Cursor<Vec<u8>>> =
            zip::ZipArchive::new(Cursor::new(bytes)).expect("open zip");
        let mut file: zip::read::ZipFile<'_> = archive.by_name("META-INF/CERT.RSA").expect("entry");

        let err: Error =
            read_v1_signature_file(&mut file, 3).expect_err("oversize signature entry");

        assert!(matches!(err, Error::Zip(message) if message.contains("exceeds entry cap 3")));
    }
