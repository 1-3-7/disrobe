...
pub fn undo_control_flow(cf: &ClassFile) -> CffUndoStats {
    ...
    for m in &cf.methods {
        for attr in &m.attributes {
            ...
            let code: &[u8] = attr.info.as_slice();
            let goto_count: usize = code.iter().filter(|&&b| b == 0xA7 || b == 0xC8).count();
            let tableswitch_count: usize = code.iter().filter(|&&b| b == 0xAA).count();
            let lookupswitch_count: usize = code.iter().filter(|&&b| b == 0xAB).count();
            if (goto_count + tableswitch_count + lookupswitch_count) > 8 {
