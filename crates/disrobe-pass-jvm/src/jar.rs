...
const ZIP_ENTRY_COUNT_CAP: usize = 65_536;
const ZIP_ENTRY_BYTES_CAP: u64 = 256 * 1024 * 1024;
const ZIP_TOTAL_BYTES_CAP: u64 = 1024 * 1024 * 1024;
...
fn validate_zip_entry_count(count: usize) -> Result<()> {
    if count > ZIP_ENTRY_COUNT_CAP {
        return Err(Error::Zip(format!(
            "zip entry count {count} exceeds cap {ZIP_ENTRY_COUNT_CAP}"
        )));
    }
    Ok(())
}

fn read_zip_file(file: &mut zip::read::ZipFile<'_>, total_read: &mut u64) -> Result<Vec<u8>> {
    let name: String = file.name().to_string();
    let declared_size: u64 = file.size();
    if declared_size > ZIP_ENTRY_BYTES_CAP {
        return Err(Error::Zip(format!(
            "zip entry '{name}' declared size {declared_size} exceeds entry cap {ZIP_ENTRY_BYTES_CAP}"
        )));
    }
    let mut buf: Vec<u8> =
        Vec::with_capacity(entry_prealloc(declared_size, file.compressed_size()));
    let read: usize = std::io::Read::take(file, ZIP_ENTRY_BYTES_CAP + 1).read_to_end(&mut buf)?;
    let read_u64: u64 = u64::try_from(read).map_err(|_| {
        Error::Zip(format!(
            "zip entry '{name}' read size exceeds entry cap {ZIP_ENTRY_BYTES_CAP}"
        ))
    })?;
    if read_u64 > ZIP_ENTRY_BYTES_CAP {
        return Err(Error::Zip(format!(
            "zip entry '{name}' read size {read_u64} exceeds entry cap {ZIP_ENTRY_BYTES_CAP}"
        )));
    }
    let next_total: u64 = total_read.checked_add(read_u64).ok_or_else(|| {
        Error::Zip(format!(
            "zip expanded byte total overflowed cap {ZIP_TOTAL_BYTES_CAP}"
        ))
    })?;
    if next_total > ZIP_TOTAL_BYTES_CAP {
        return Err(Error::Zip(format!(
            "zip expanded byte total {next_total} exceeds total cap {ZIP_TOTAL_BYTES_CAP}"
        )));
    }
...
pub fn extract(bytes: &[u8]) -> Result<JarExtract> {
    ...
    let zip_len: usize = zip.len();
    validate_zip_entry_count(zip_len)?;
    let mut entries: Vec<JarEntry> = Vec::with_capacity(zip_len);
    ...
    for i in 0..zip_len {
        ...
        let buf: Vec<u8> = read_zip_file(&mut file, &mut total_read)?;
...
pub fn extract_apk(bytes: &[u8]) -> Result<ApkExtract> {
    ...
    for entry in &jar.entries {
        ...
        } else if p.starts_with("lib/") && p.ends_with(".so") {
