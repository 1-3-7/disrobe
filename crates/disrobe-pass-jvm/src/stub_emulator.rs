...
pub struct DecryptStub {
    code: Vec<u8>,
    max_locals: u16,
    int_constants: BTreeMap<u16, i32>,
}

fn integer_constant_pool(cf: &ClassFile) -> BTreeMap<u16, i32> {
    let mut out: BTreeMap<u16, i32> = BTreeMap::new();
    for (i, entry) in cf.constant_pool.iter().enumerate() {
        if let ConstantPoolEntry::Integer(v) = entry
        ...
        match op {
            ...
            0x12 | 0x13 => match &insn.operands {
                Operands::ConstPool(cp) => {
                    let v: i32 = *stub.int_constants.get(cp).ok_or(EmulationError::BadShape)?;
...
mod tests {
    ...
    fn build_ldc_xor_decrypt_stub(key: i32, key_const_index: u16) -> DecryptStub {
        let mut code: Vec<u8> = Vec::new();
        code.push(0x2A);
        code.push(0xBE);
        code.push(0xBC);
        ...
        code.push(0x34);
        code.push(0x12);
        code.push(u8::try_from(key_const_index).unwrap());
        code.push(0x82);
        code.push(0x55);
        code.push(0x84);
        code.push(0x02);
        ...
        let mut int_constants: BTreeMap<u16, i32> = BTreeMap::new();
        int_constants.insert(key_const_index, key);
        DecryptStub {
            code,
            max_locals: 4,
            int_constants,
    ...
    #[test]
    fn ldc_loads_xor_key_from_integer_constant_pool() {
        let key: i32 = 0x2C3D;
        let plain: &str = "jdbc:postgres://prod";
        let encrypted: Vec<u16> = plain.encode_utf16().map(|c| c ^ (key as u16)).collect();
        let stub: DecryptStub = build_ldc_xor_decrypt_stub(key, 7);
        let out: Vec<u16> = emulate_char_array(&stub, &encrypted).expect("emulate ldc-keyed stub");
        assert_eq!(String::from_utf16(&out).expect("utf16"), plain);
    }

    #[test]
    fn ldc_without_known_constant_fails_cleanly() {
        let stub: DecryptStub = DecryptStub {
            code: vec![0x12, 0x09, 0xB0],
            max_locals: 1,
            int_constants: BTreeMap::new(),
        };
        let err: EmulationError = emulate_char_array(&stub, &[]).expect_err("missing constant");
        assert!(matches!(err, EmulationError::BadShape));
    }
