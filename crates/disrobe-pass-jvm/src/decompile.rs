...
/// Hoists a `Type varN;` declaration to the top of the body for every non-parameter local written.
...
/// Renders a catch handler, seeding the caught exception onto the operand stack at handler entry.
...
/// Renders a basic block's instructions to source, optionally pre-seeding the operand stack.
...
/// Lifts a pure value-producer block to the single `Expr` it leaves on the stack, else `None`.
...
/// Reconstructs a `cond ? a : b` from an if/then/else whose arms are single-value producers feeding a join.
...
/// True when the head block emits any side-effecting statement before its conditional branch.
...
/// True when the join block's first non-shuffle instruction consumes exactly one operand-stack value.
...
/// Returns the condition expression of a head block without emitting its prefix.
...
/// Folds a javac array-initializer's per-element `array[index] = value` into an `Expr::ArrayInit`.
...
/// Parses a constant integer expression to its `usize` value.
...
/// Reconstructs a `String` concatenation desugared into a `makeConcatWithConstants` invokedynamic.
