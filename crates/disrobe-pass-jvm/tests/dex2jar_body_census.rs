...
fn report_whole_corpus_body_fidelity() {
    ...
    assert!(
        recovered_real >= 210,
        "recovered-real-body count regressed below its floor: {recovered_real} of {comparable}"
    );
    assert!(
        matched >= 107,
        "skeleton-match count regressed below its floor: {matched} of {comparable} comparable \
         (add-int/lit iinc-vs-expression and negative-literal subtraction lowering must hold)"
    );
}
