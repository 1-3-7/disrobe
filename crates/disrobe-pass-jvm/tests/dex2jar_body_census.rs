...
/// Non-circular per-method body fidelity across the WHOLE corpus: for every
/// method present in both the dex2jar translation and the independent `javac`
/// baseline jar, compares the normalized semantic skeletons. The baseline is
/// real `javac` output, never re-emitted through the dex2jar builder. Reports
/// the recovered-and-matching fraction; pure measurement, never asserts a hard
/// number so it tracks progress honestly.
#[test]
