...
/// Measures per-method fidelity of the in-house `.class -> Java` decompiler against the `javac` baseline.
#[test]
...
/// Splits a decompiled class body into one fragment per method by brace-depth scanning.
...
/// Per-method recovery fraction across the corpus: a body counts as recovered with no invalidity marker.
#[test]
...
/// Non-circular recompile gate: emit every decompiled class and bulk-`javac` compile, reporting acceptance.
#[test]
...
/// Line ranges of each method body in a generated `.java`, keyed by a method signature label.
...
/// Non-circular per-method recompile oracle: `javac`-compile the decompiled `EdgeCases` and attribute errors per method.
#[test]
