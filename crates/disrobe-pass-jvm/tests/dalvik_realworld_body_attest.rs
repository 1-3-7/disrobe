...
use common::{
    ...
    read_real_apk, real_apk_inbox, real_apks_absent,
};
...
const SELF_REPORTED_BODIES: usize = 83_662;

const CANDIDATE_BODIES: usize = 83_630;

const SAMPLED_BODIES: usize = 8_419;

const ATTESTED_PRESENTED: usize = 2_998;

const ATTESTED_CLEAN: usize = 2_988;

const ATTESTED_REJECTED: usize = 10;
...
fn attest_apk(verifier: &JvmVerifier, apk: &RealApk) -> BodyAttest {
...
fn realworld_dalvik_body_lowering_is_verifier_attested() {
    ...
    for apk in REAL_APKS {
        let attest: BodyAttest = attest_apk(&verifier, apk);
        eprintln!(
        ...
        check_membership(&[(apk.golden, &attest)]);
        assert_eq!(
            attest.self_reported_bodies,
...
fn check_membership(membership: &[(&str, &BodyAttest)]) {
