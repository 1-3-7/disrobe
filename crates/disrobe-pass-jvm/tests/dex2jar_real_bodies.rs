...
/// Mnemonics that only shuffle Dalvik registers through JVM locals or the operand stack.
...
/// Normalizes a body's semantic skeleton: constant pushes collapse to `const` and consecutive-op runs collapse.
...
/// A translated body structurally matches the baseline when their normalized semantic skeletons are identical.
...
/// Measures real body fidelity by differencing each curated leaf method's skeleton against the `javac` baseline.
#[test]
...
/// Byte-level pin: the recovered `Square.area()` Code attribute must equal the hand-computed JVM opcode bytes.
#[test]
...
/// Loads every translated class under `-Xverify:all`, asserting no recovered body produces a `VerifyError`.
#[test]
