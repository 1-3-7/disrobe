...
use common::{
    ...
    read_real_apk, real_apk_inbox, real_apk_path, real_apks_absent,
};
use disrobe_pass_jvm::dex2jar::{Dex2JarDiagnostic, Dex2JarResult, translate_dex_bytes};
...
struct BodyCensus {
    ...
    method_total: usize,
    diagnostics: Vec<Dex2JarDiagnostic>,
}

fn recovered_bodies(apk: &RealApk) -> BodyCensus {
    let bytes: Vec<u8> = read_real_apk(apk.file);
    ...
    for (name, dex_bytes) in &extract.dex_files {
        ...
        census.diagnostics.extend(result.diagnostics);
    }
    census
}
...
fn realworld_apk_bodies_recovered_match_their_pinned_counts() {
    ...
    for apk in REAL_APKS {
        ...
        if bodies_recovered != apk.self_reported_bodies_pinned {
            for diagnostic in census.diagnostics.iter().take(40) {
                eprintln!(
                    "{}::{}: {}",
                    diagnostic.class,
                    diagnostic.method.as_deref().unwrap_or("<class>"),
                    diagnostic.reason
                );
            }
        }
...
fn realworld_apk_translated_classes_verify() {
    ...
    let bytes: Vec<u8> = read_real_apk(CLASS_SCOPE_APK);
    let extract: ApkExtract = extract_apk(&bytes).expect("extract apk");
