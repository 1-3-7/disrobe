...
/// Validity (not just quantity) of the real-world lifter: assemble the translated classes of the
/// smallest FOSS apk into a jar and run the JVM verifier (`-Xverify:all`) via a synthetic-supertype
/// class loader (missing `android.*`/`kotlin.*` refs become stub interfaces). The honest counterpart
/// to the recovery-quantity gate: `bodies_recovered` counts attempted lifts, this counts how many
/// actually pass the JVM verifier over the linkable subset. The current branch-mode lifter has known
/// soundness bugs at real-app scale (bad-local-type / inconsistent-stackmap families) absent from the
/// narrow `EdgeCases` corpus; `MAX_VERIFY_FAILURES` is a ratcheting ceiling driven toward 0 as they are
/// fixed. Skips when the local apk fixture or a JDK 24+ (java/javac) is absent.
#[test]
...
/// Ratcheting ceiling on the real-app JVM verify FAILURE RATE (transmissionic classes.dex, linkable
