...
fn read_first_classfile_from_jar(jar_path: &PathBuf) -> Vec<u8> {
    let f: fs::File = fs::File::open(jar_path).expect("open jar");
...
fn real_baseline_jar_classfile_parses_for_protector_input() {
    ...
    assert!(jar.exists(), "baseline jar missing: {}", jar.display());
...
#[test]
fn synthetic_zelix_encrypted_strings_round_trip_through_peel() {
    ...
    let matched: usize = accepted
        .iter()
        .filter(|p: &&String| recovered_values.iter().any(|s: &String| s == *p))
        .count();
    assert!(
        matched >= 1,
...
#[test]
fn unrelated_strings_pass_through_zelix_unchanged() {
    let key: ZelixKey = ZelixKey::new(0xCAFE, 0xBABE, 11);
    let mut cp: Vec<ConstantPoolEntry> = vec![ConstantPoolEntry::Placeholder];
    cp.push(ConstantPoolEntry::Utf8("simple ascii".to_string()));
    let cf: ClassFile = ClassFile {
        minor_version: 0,
        major_version: 52,
        constant_pool: cp,
