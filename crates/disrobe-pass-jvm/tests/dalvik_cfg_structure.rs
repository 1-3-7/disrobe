...
fn count_while(region: &Region, acc: &mut usize) {
    match region {
        ...
        Region::TryWithResources { try_body, .. } => count_while(try_body, acc),
...
fn find_try_handlers(region: &Region) -> Option<Vec<Option<String>>> {
    match region {
        ...
        Region::TryWithResources { try_body, .. } => find_try_handlers(try_body),
        Region::Synchronized { body, .. } | Region::LabeledLoop { body, .. } => {
            find_try_handlers(body)
        }
        Region::Block(_)
        | Region::Break { .. }
        | Region::Continue { .. }
        | Region::Irreducible { .. } => None,
    }
}
