...
fn read_real_strings(max: usize) -> Vec<String> {
    let f: fs::File = fs::File::open(baseline_jar_path()).expect("open jar");
...
#[test]
fn stringer_roundtrips_real_jar_string_constants() {
    let strings: Vec<String> = read_real_strings(15);
...
#[test]
fn stringer_peel_recovers_synthetic_protected_class() {
    ...
    assert!(
        matched >= 1,
        "expected >=1 plaintext recovered, got {matched} out of {len}",
        len = plaintexts.len()
    );
}
