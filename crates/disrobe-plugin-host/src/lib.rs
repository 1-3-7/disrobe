...
//!
//! [`PluginHost::run`] executes an untrusted wasm module under four independent
//! containment caps, all backed by wasmtime's own enforcement primitives:
//!
//! * **Fuel** — [`wasmtime::Store::set_fuel`] bounds total executed
//!   instructions. Exhaustion surfaces as [`SandboxError::Fuel`].
//! * **Wall-clock timeout** — [`wasmtime::Config::epoch_interruption`] plus a
//!   watchdog thread that calls [`wasmtime::Engine::increment_epoch`] after a
//!   real-time deadline. Tripping it surfaces as [`SandboxError::Timeout`].
//!   Fuel is per-instruction, not wall-clock; the epoch watchdog is the true
//!   timeout, so both are always armed.
//! * **Memory** — a [`wasmtime::ResourceLimiter`] denies linear-memory and
//!   table growth past a byte cap, surfacing as [`SandboxError::Memory`].
//! * **Ambient authority** — an empty [`wasmtime::Linker`] resolves no host
//!   imports, so any module import (WASI sockets, filesystem, anything) is
//!   rejected as [`SandboxError::DeniedImport`] before guest code runs.
//!   [`wasmtime::Linker::define_unknown_imports_as_traps`] is also installed as
...
/// Per-run containment budget. Construct with [`Limits::default`] or tune each
/// cap explicitly.
...
/// Tracks whether the guest hit the memory cap so a denied-growth trap can be
/// classified as [`SandboxError::Memory`] rather than a generic trap.
#[derive(Debug)]
struct MemoryGate {
    cap_bytes: usize,
    denied: bool,
...
impl PluginHost {
    /// Run `wasm` against `input` under `limits`, returning the bytes the guest
    /// wrote back, or the [`SandboxError`] that aborted it.
    ///
    /// The guest ABI: the host writes `input` into linear memory at offset 0,
    /// calls the exported `run(input_len: i32) -> i32`, and reads back the
    /// returned number of output bytes from offset 0.
...
/// Returns the first import in `module` as `"module::name"`, or `None` if the
/// module is fully self-contained. Any import is a denied capability request
/// under the empty-linker policy.
...
/// Maps a wasmtime call error onto the sandbox taxonomy, distinguishing fuel
/// exhaustion and epoch interruption from generic traps.
...
/// interrupting any guest still executing. The `stopper` flag lets a guest that
/// returns early retire the watchdog without waiting out the deadline.
