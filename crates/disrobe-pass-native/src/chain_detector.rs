...
/// Routes a detected packer to its unpacker per [`Packer::unpacker_status`], else an actionable error.
...
/// Invokes the concrete unpacker for an [`UnpackerStatus::Implemented`] packer into a [`Rung::Raw`] artifact.
