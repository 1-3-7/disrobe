...
//! Native DWARF sourcemap synthesizer for ELF / Mach-O / PE object files.
...
/// Recursion depth cap for the type renderer, guarding against cyclic DWARF.
...
impl TypeReconstruction {
    ...
    /// Fraction of reconstructed types that resolved to a concrete name, in `0.0..=1.0`.
    #[must_use]
...
impl CoverageScore {
    /// Percentage of `.text` bytes that fall inside a line-program sequence, in `0.0..=100.0`.
    #[must_use]
    pub fn pct(&self) -> f64 {
        if self.text_size == 0 {
            return 0.0;
...
pub struct SplitDwarfInfo {
    /// `true` when a skeleton unit referenced a `.dwo` via `DW_AT_dwo_name` / `DW_AT_GNU_dwo_name`.
    pub has_skeleton_units: bool,
    /// Names of the `.dwo` companion files the skeleton units pointed at.
    pub dwo_names: Vec<String>,
    /// `true` when DWARF5 split infrastructure (`.debug_str_offsets` / `.debug_addr`) is present.
...
/// Reconstructs the type graph from an object file's `.debug_info`.
///
/// # Errors
///
/// Returns [`Error::UnknownFormat`] if `bytes` is not a recognized object file,
...
/// Computes line-program coverage of `.text` as the union of address sequences over its size.
...
/// One type DIE's distilled attributes plus render-relevant children.
#[derive(Debug, Clone)]
struct TypeDie {
    tag: gimli::DwTag,
    name: Option<String>,
...
impl TypeDie {
    /// Whether this DIE is a top-level named type worth surfacing in the reconstruction list.
...
/// Walks a unit once, distilling every type-relevant DIE into [`TypeDieIndex`].
fn index_type_dies(
...
/// Attaches a member to the most-recently-inserted composite type.
...
/// Folds a `DW_TAG_subrange_type` bound onto its parent `DW_TAG_array_type`.
fn attach_subrange(
...
/// Renders a renderable-root type DIE into a [`ReconstructedType`].
...
/// Recursively renders a `DW_AT_type` reference (or `None` = `void`) into a type expression.
...
/// Renders a struct/class/union, appending `<T, ...>` for template parameters.
...
/// Renders one template parameter, preferring its resolved type then its own name.
...
/// Collects a CU root's `DW_AT_dwo_name` / `DW_AT_GNU_dwo_name` split-DWARF reference.
fn collect_dwo_name(
    dwarf: &Dwarf<EndianSlice<'_, RunTimeEndian>>,
    unit: &gimli::Unit<EndianSlice<'_, RunTimeEndian>>,
    out: &mut Vec<String>,
