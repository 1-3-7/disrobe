...
/// Guards the recursive type renderer against pathological cycles
/// (`T* -> T -> T*`) and adversarial DWARF; real type graphs are far shallower.
...
/// One reconstructed named type, resolved from a `.debug_info` type DIE.
///
/// `name` is the rendered, human-readable type expression (`int *`,
/// `struct Foo`, `char [16]`, `Vec<u8>`), `kind` records which DWARF type tag
/// produced it, and `byte_size` is the `DW_AT_byte_size` when present.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
/// One struct/class/union member recovered from a `DW_TAG_member` child.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
/// The DWARF type-tag family a [`ReconstructedType`] was built from.
...
/// Type-graph reconstruction recovered from one object file's `.debug_info`.
///
/// `types` holds every named composite/base/derived type DIE rendered into a
/// readable type expression. `coverage` reports how much of `.text` the line
/// program covers (the honest line-recovery metric). `split_dwarf` records
/// whether the object pulled type/string data from companion `.dwo` units.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
impl TypeReconstruction {
    /// Count of fully-named types (no anonymous `<anon>` placeholder).
    #[must_use]
    ...
    /// Fraction of reconstructed types that resolved to a concrete name,
    /// in `0.0..=1.0`. The honest type-reconstruction metric: optimized builds
    /// elide/inline types, so this is bounded below 100% by the compiler, not
...
/// Line-program coverage of the executable section.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
...
impl CoverageScore {
    /// Percentage of `.text` bytes that fall inside a line-program sequence,
    /// in `0.0..=100.0`.
    #[must_use]
    pub fn pct(&self) -> f64 {
        if self.text_size == 0 {
...
/// Whether companion split-DWARF (`.dwo`) data was found and consumed.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct SplitDwarfInfo {
    /// `true` when the skeleton unit referenced a `.dwo` via `DW_AT_dwo_name`
    /// / `DW_AT_GNU_dwo_name` (the unit's full DIEs live in a separate file).
    pub has_skeleton_units: bool,
    /// Names the skeleton units pointed at (`foo.dwo`), for the resolver to
    /// locate companion files; empty for single-file DWARF.
    pub dwo_names: Vec<String>,
    /// `true` when DWARF5 split infrastructure (`.debug_str_offsets` /
    /// `.debug_addr`) is present, which the resolver consults for indexed
...
/// Reconstruct the type graph from an object file's `.debug_info`.
///
/// Walks every compilation unit, indexes its type DIEs by unit offset, then
/// recursively renders each named base/derived/composite type into a readable
/// type expression (`int *`, `struct Foo`, `char [16]`, generic
/// `Vec<u8>`-style template instantiations). Also computes the line-program
/// coverage of `.text` ([`CoverageScore`]) and detects split-DWARF
/// ([`SplitDwarfInfo`]).
///
/// The recovery is non-circular: every name comes from the binary's own
/// `.debug_str` / type DIEs, never from a re-emit. On optimized builds the
/// compiler elides and inlines types, so [`TypeReconstruction::type_reconstruction_ratio`]
/// is honestly bounded below 100% - that is a property of the input, not the
/// reconstructor.
///
/// # Errors
///
/// Returns [`Error::UnknownFormat`] if `bytes` is not a recognized object file,
/// [`Error::SignatureDb`] if it carries no debug sections, and [`Error::Dwarf`]
/// if the debug sections are malformed.
...
/// Compute line-program coverage of the executable section: the union of every
/// `[start, end_sequence)` address range, clamped to `.text`, over `.text` size.
...
/// Total length covered by a set of `[lo, hi)` ranges after merging overlaps.
...
/// A flattened, offset-indexed snapshot of one unit's type DIEs.
///
/// Built in a single pass so the recursive renderer can resolve `DW_AT_type`
/// references by `UnitOffset` lookup without re-walking the tree (which gimli's
/// cursor model makes awkward and quadratic).
struct TypeDieIndex {
    entries: BTreeMap<UnitOffset, TypeDie>,
}

/// One type DIE's distilled attributes, plus the children that matter for
/// rendering (struct/union members, subrange bounds, template parameters).
#[derive(Debug, Clone)]
struct TypeDie {
    tag: gimli::DwTag,
    name: Option<String>,
...
impl TypeDie {
    /// Whether this DIE is a top-level *named* type worth surfacing in the
    /// reconstruction list (vs. an anonymous intermediary like a bare
    /// `pointer_type` that only exists to be referenced by something else).
...
/// Walk a unit once, distilling every type-relevant DIE into [`TypeDieIndex`].
fn index_type_dies(
...
/// Attach a member to the most-recently-inserted composite type (DFS guarantees
/// the parent composite was inserted before its `DW_TAG_member` children).
...
/// Fold a `DW_TAG_subrange_type`'s bound onto its parent `DW_TAG_array_type`
/// so the renderer can print `[N]`.
fn attach_subrange(
...
/// Render a renderable-root type DIE into a [`ReconstructedType`], resolving
/// member/template type references through the index.
...
/// Recursively render a `DW_AT_type` reference (or `None` = `void`) into a
/// readable type expression, following pointer/const/array/typedef chains.
...
/// Render a struct/class/union, appending `<T, ...>` when the DIE carries
/// template parameters (the generic-instantiation case).
...
/// Render one template parameter: prefer its resolved type, falling back to the
/// parameter's own name for value parameters (const generics) that carry no
/// `DW_AT_type`.
...
/// Detect a split-DWARF skeleton: a CU root carrying `DW_AT_dwo_name` (DWARF5)
/// or `DW_AT_GNU_dwo_name` (GNU extension) points at a companion `.dwo` whose
/// full DIE tree lives outside this object.
fn collect_dwo_name(
    dwarf: &Dwarf<EndianSlice<'_, RunTimeEndian>>,
    unit: &gimli::Unit<EndianSlice<'_, RunTimeEndian>>,
    out: &mut Vec<String>,
