...
pub(super) fn fold_constant_conditions(
...
fn decide(
    ...
    let condition: ConditionCode = decoded.get(position)?.as_ref()?.condition_code();
    if condition == ConditionCode::None {
        return None;
    }
    let start: usize = window_start(decoded, transfers, leaders, position);
    let run: &[Option<Instruction>] = decoded.get(start..position)?;
    if run.is_empty() {
        return None;
    }
    ...
    for slot in run {
        apply(slot.as_ref()?, &mut constants, &mut flags, &mut factory);
    }
