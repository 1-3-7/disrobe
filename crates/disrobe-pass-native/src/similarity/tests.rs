...
#[test]
fn stack_memory_data_immediates_remain_references() {
    let index: ImageIndex = ImageIndex::default();
    let expected: DataReference = DataReference::UnusualConstant(0xdead_beef);
    assert_eq!(index.constant(0xdead_beef), Some(expected.clone()));
    for bytes in [
        &[0x81, 0x7D, 0x10, 0xEF, 0xBE, 0xAD, 0xDE][..],
        &[0xC7, 0x45, 0xFC, 0xEF, 0xBE, 0xAD, 0xDE][..],
        &[0x81, 0x7C, 0x24, 0x20, 0xEF, 0xBE, 0xAD, 0xDE][..],
        &[0xC7, 0x44, 0x24, 0x20, 0xEF, 0xBE, 0xAD, 0xDE][..],
        &[0x81, 0x7C, 0xA8, 0x08, 0xEF, 0xBE, 0xAD, 0xDE][..],
    ] {
        let insn: DisasmInstruction = x86_insn(0x1000, "", InsnFlow::Sequential, bytes);
        let decoded: Instruction = decode_x86(64, &insn).expect("whole stack data instruction");
        assert_eq!(
            x86_immediate_reference(&decoded, &index),
            Some(expected.clone()),
            "the memory address and data immediate have separate roles: {decoded:?}"
        );
    }
}

#[test]
fn explicit_frame_register_immediates_remain_excluded() {
    let index: ImageIndex = ImageIndex::default();
    let value: u64 = 0x1357_9bdf;
    assert_eq!(
        index.constant(value),
        Some(DataReference::UnusualConstant(value))
    );
    for bytes in [
        &[0x48, 0x81, 0xEC, 0xDF, 0x9B, 0x57, 0x13][..],
        &[0x48, 0x81, 0xC4, 0xDF, 0x9B, 0x57, 0x13][..],
        &[0x48, 0x81, 0xC5, 0xDF, 0x9B, 0x57, 0x13][..],
        &[0x48, 0x81, 0xFD, 0xDF, 0x9B, 0x57, 0x13][..],
        &[0xBD, 0xDF, 0x9B, 0x57, 0x13][..],
        &[0x48, 0xBC, 0xDF, 0x9B, 0x57, 0x13, 0, 0, 0, 0][..],
    ] {
        let insn: DisasmInstruction = x86_insn(0x1000, "", InsnFlow::Sequential, bytes);
        let decoded: Instruction = decode_x86(64, &insn).expect("whole frame-register instruction");
        assert_eq!(decoded.immediate(1), value);
        assert_eq!(
            x86_immediate_reference(&decoded, &index),
            None,
            "{decoded:?}"
...
#[test]
fn frame_memory_displacements_are_not_data_immediates() {
    let index: ImageIndex = ImageIndex::default();
    let value: u64 = 0x1357_9bdf;
    assert_eq!(
        index.constant(value),
    ...
    for bytes in [
        &[0x8B, 0x85, 0xDF, 0x9B, 0x57, 0x13][..],
        &[0x8B, 0x84, 0x24, 0xDF, 0x9B, 0x57, 0x13][..],
        &[0x48, 0x8D, 0x8C, 0x24, 0xDF, 0x9B, 0x57, 0x13][..],
    ] {
        let insn: DisasmInstruction = x86_insn(0x1000, "", InsnFlow::Sequential, bytes);
        let decoded: Instruction = decode_x86(64, &insn).expect("whole frame memory instruction");
        assert_eq!(decoded.memory_displacement64(), value);
        assert_eq!(
            x86_immediate_reference(&decoded, &index),
            None,
            "{decoded:?}"
...
#[test]
fn decoded_x86_nops_do_not_supply_structural_categories() {
    let baseline: Vec<FunctionFeatures> = features_of(&[0xB8, 0xDF, 0x9B, 0x57, 0x13, 0xC3]);
    for nop in [
        vec![0x90],
        vec![0x66, 0x90],
        vec![0x0F, 0x1F, 0x44, 0x00, 0x00],
        vec![0x66, 0x66, 0x2E, 0x0F, 0x1F, 0x84, 0x00, 0, 0, 0, 0],
    ] {
        let decoded: Instruction =
            decode_x86(64, &x86_insn(0x1000, "nop", InsnFlow::Sequential, &nop))
                .expect("the complete NOP decodes");
        assert_eq!(decoded.mnemonic(), Mnemonic::Nop);
        let mut text: Vec<u8> = nop;
        text.extend_from_slice(&[0xB8, 0xDF, 0x9B, 0x57, 0x13, 0xC3]);
        let features: Vec<FunctionFeatures> = features_of(&text);
        assert_eq!(features.len(), 1);
        assert_eq!(features[0].structure(), baseline[0].structure());
        assert_eq!(features[0].references(), baseline[0].references());
    }
}

#[test]
fn x86_nop_only_blocks_keep_their_branch_targets_and_successors() {
    let features: Vec<FunctionFeatures> =
        features_of(&[0x85, 0xC0, 0x74, 0x02, 0x66, 0x90, 0x90, 0xC3]);
    let graph: &ControlFlowGraph = features[0]
        .structure()
        .expect("the branch into a NOP stays resolved");
    assert_eq!(graph.block_count(), 3);
    assert_eq!(graph.blocks()[0].successors(), &[2, 1]);
    assert!(graph.blocks()[1].categories().is_empty());
    assert_eq!(graph.blocks()[1].successors(), &[2]);
    assert_eq!(
        graph.blocks()[2].categories(),
...
#[test]
fn x86_nop_normalization_retains_effectful_and_invalid_instructions() {
    for (bytes, category) in [
        (vec![0x83, 0xC0, 0x01], InstructionCategory::Arithmetic),
        (vec![0x8B, 0x00], InstructionCategory::Load),
        (vec![0x89, 0x00], InstructionCategory::Store),
        (vec![0xF8], InstructionCategory::Other),
        (vec![0xF3, 0x90], InstructionCategory::System),
        (vec![0x41, 0x90], InstructionCategory::Move),
        (vec![0xCC], InstructionCategory::System),
    ] {
        let mut text: Vec<u8> = bytes.clone();
        text.push(0xC3);
        let features: Vec<FunctionFeatures> = features_of(&text);
        let graph: &ControlFlowGraph = features[0]
            .structure()
            .expect("the effectful instruction still forms a graph");
        assert_eq!(graph.instruction_mix().count(category), 1, "{bytes:02x?}");
    }
    let body: [DisasmInstruction; 2] = [
        x86_insn(0x1000, "nop", InsnFlow::Sequential, &[0xF0, 0x90]),
        x86_insn(0x1002, "ret", InsnFlow::Return, &[0xC3]),
    ];
    assert!(decode_x86(64, &body[0]).is_none());
    let parts: X86FeatureParts =
        x86_feature_parts(&body, 64, &ImageIndex::default(), &BTreeSet::new());
...
#[test]
fn undecodable_x86_data_after_a_closed_body_does_not_supply_features() {
    let entries: BTreeSet<u64> = BTreeSet::from([0x2000]);
    for (terminal, next, calls) in [
        (
            x86_insn(0x1000, "ret", InsnFlow::Return, &[0xC3]),
            0x1001,
            BTreeSet::new(),
        ),
        (
            ...
            0x1005,
            BTreeSet::from([FunctionId(0x2000)]),
        ),
    ] {
        let mut immediate: Vec<u8> = vec![0x48, 0xB8];
        immediate.extend_from_slice(&0xc4ce_b9fe_1a85_ec53_u64.to_le_bytes());
        let body: [DisasmInstruction; 3] = [
            terminal,
            x86_insn(next, "db", InsnFlow::Sequential, &[0xFF]),
            x86_insn(next + 1, "mov", InsnFlow::Sequential, &immediate),
        ];
        assert!(decode_x86(64, &body[1]).is_none());
        let parts: X86FeatureParts = x86_feature_parts(&body, 64, &ImageIndex::default(), &entries);
        let structure: ControlFlowGraph = parts.structure.expect("the body already terminated");
        assert_eq!(structure.block_count(), 1);
        assert_eq!(structure.instruction_mix().total(), 1);
        assert!(parts.references.is_empty());
        assert_eq!(parts.calls, calls);
    }
}

#[test]
fn reachable_undecodable_x86_bytes_refuse_structure() {
    let bad: DisasmInstruction = x86_insn(0x1002, "db", InsnFlow::Sequential, &[0xFF]);
    let ret: DisasmInstruction = x86_insn(0x1003, "ret", InsnFlow::Return, &[0xC3]);
    for body in [
        vec![bad.clone(), ret.clone()],
        vec![
            x86_insn(0x1001, "nop", InsnFlow::Sequential, &[0x90]),
            bad.clone(),
            ret.clone(),
        ],
        vec![
            x86_insn(0x1000, "jmp", InsnFlow::UnconditionalBranch, &[0xEB, 0x00]),
            bad.clone(),
            ret.clone(),
        ],
        vec![
            x86_insn(0x1000, "jz", InsnFlow::ConditionalBranch, &[0x74, 0x01]),
            bad,
            ret,
        ],
    ] {
        let parts: X86FeatureParts =
            x86_feature_parts(&body, 64, &ImageIndex::default(), &BTreeSet::new());
        assert!(
            parts.structure.is_none(),
            "reachable invalid instruction: {body:?}"
        );
    }
}

#[test]
fn only_a_proved_x86_condition_can_skip_undecodable_bytes() {
    for (condition, bytes, resolved) in [("jz", [0x74, 0x01], true), ("jnz", [0x75, 0x01], false)] {
        let body: [DisasmInstruction; 4] = [
            x86_insn(0x1000, "xor", InsnFlow::Sequential, &[0x31, 0xC0]),
            x86_insn(0x1002, condition, InsnFlow::ConditionalBranch, &bytes),
            x86_insn(0x1004, "db", InsnFlow::Sequential, &[0xFF]),
            x86_insn(0x1005, "ret", InsnFlow::Return, &[0xC3]),
        ];
        let parts: X86FeatureParts =
            x86_feature_parts(&body, 64, &ImageIndex::default(), &BTreeSet::new());
        assert_eq!(parts.structure.is_some(), resolved, "{condition}");
        if let Some(structure) = parts.structure {
            assert_eq!(structure.block_count(), 2);
            assert_eq!(structure.instruction_mix().total(), 3);
        }
    }
}

#[test]
fn x86_branch_leaders_still_break_proofs_before_undecodable_bytes() {
    let body: [DisasmInstruction; 7] = [
        x86_insn(0x1000, "test", InsnFlow::Sequential, &[0x85, 0xC9]),
        x86_insn(0x1002, "jnz", InsnFlow::ConditionalBranch, &[0x75, 0x02]),
        x86_insn(0x1004, "xor", InsnFlow::Sequential, &[0x31, 0xC0]),
        x86_insn(0x1006, "test", InsnFlow::Sequential, &[0x85, 0xC0]),
        x86_insn(0x1008, "jz", InsnFlow::ConditionalBranch, &[0x74, 0x01]),
        x86_insn(0x100A, "db", InsnFlow::Sequential, &[0xFF]),
        x86_insn(0x100B, "ret", InsnFlow::Return, &[0xC3]),
    ];
    let parts: X86FeatureParts =
        x86_feature_parts(&body, 64, &ImageIndex::default(), &BTreeSet::new());
    assert!(parts.structure.is_none());
