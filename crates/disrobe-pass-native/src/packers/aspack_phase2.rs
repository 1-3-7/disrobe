...
//! ASPack 2.x phase-2 unpacking driven by the shared x86 stub emulator.
//!
//! The first pass ([`super::aspack_unpack`]) recovers ASPack's structure - PE
//! header, reconstructed object table, carved compressed blocks - but cannot
//! byte-decode the payload: ASPack 2.x compresses each content section with a
//! modified aPLib dialect that a static classic-aPLib decoder scores 0% on.
//!
//! ASPack is an in-place packer: it leaves every original section header intact
//! (`.text`/`.rdata`/`.data`/`.rsrc` keep their original virtual addresses) and
//! stores each section body aPLib-compressed under the same VA, then appends an
//! `.aspack`/`.adata` stub pair whose entry point decompresses every section
//! in-place over the mapped image and finally `popad; jmp OEP`s into the
//! restored `.text`. This module wires that stub through the [`crate::stub_emu`]
//! interpreter:
//!
//! 1. Map the packed PE into emulated memory at `ImageBase`, copying the headers
//!    and every section body (compressed) to its mapped VA.
//! 2. Rewrite the stub's bootstrap IAT slots (the `GetProcAddress` /
//!    `GetModuleHandleA` / `LoadLibraryA` trio ASPack imports from kernel32) to
//!    synthetic unmapped addresses, so the host shim services them.
//! 3. Map a scratch stack and a synthetic TEB (ASPack reads `fs:[0]`).
//! 4. Single-step from the entry point, halting the instant control transfers
//!    out of the `.aspack`/`.adata` stub region into a content section - the OEP
//!    hand-off - so the snapshot captures the image exactly when decompression
//!    completes, before the original program mutates it.
//! 5. Snapshot `[ImageBase, ImageBase + SizeOfImage)`; the content sections now
//!    hold their decompressed bytes.
...
/// Sections the loader rebuilds (relocations applied, IAT thunks resolved to
/// live pointers) rather than restores byte-for-byte from the compressed body.
/// Excluded from the non-circular content-recovery oracle.
const LOADER_REBUILT_SECTIONS: &[&[u8]] = &[b".reloc", b".idata"];

/// Outcome of an ASPack phase-2 emulated unpack.
#[derive(Debug, Clone)]
pub struct AspackPhaseTwoOutput {
    ...
    /// Byte-recovery over content sections (`.text`/`.rdata`/`.data`/`.rsrc`)
    /// versus the independent original's loaded layout, in percent. `None` when
...
///
/// ASPack's stub bootstraps via `GetModuleHandleA("kernel32")` then a sequence
/// of `GetProcAddress` calls to resolve the APIs its in-place decompressor needs
/// (`VirtualAlloc` / `VirtualFree` / `VirtualProtect`). Each `GetProcAddress`
/// returns a fresh synthetic address which, when later `call`ed, must dispatch
/// to the matching Win32 emulation with the correct stdcall stack cleanup. The
/// shim keys that dispatch off the function name observed at resolution time.
#[derive(Debug)]
struct AspackHost {
    heap_brk: u64,
    heap_end: u64,
...
/// Run an ASPack 2.x packed PE through the stub emulator to byte-recover its
/// content sections. When `original` is supplied, compute the non-circular
/// content-recovery percentage against its independent loaded layout.
pub fn unpack_aspack_phase2_emulated(
    ...
    let capacity: u64 = u64::from(img.size_of_image).max(last_section_end_va(&img));
    ...
    let mut host: AspackHost = AspackHost::new();
...
/// Rewrite ASPack's bootstrap import thunks (GetProcAddress / GetModuleHandleA /
/// LoadLibraryA, and any VirtualAlloc/Free/Protect it uses) to synthetic
/// unmapped addresses so `call dword ptr [iat]` dispatches into the host shim.
...
/// Single-step until control transfers out of the `.aspack` stub region into a
/// content section (the OEP hand-off), the host halts, or the budget is spent.
fn run_until_oep(
    cpu: &mut Cpu,
    host: &mut AspackHost,
    image_base: u64,
