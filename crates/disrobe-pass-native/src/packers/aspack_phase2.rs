...
/// Sections rebuilt by the loader, excluded from the content-recovery oracle.
...
pub struct AspackPhaseTwoOutput {
    ...
    /// Content-section byte-recovery versus the original's loaded layout, or `None` without a baseline.
    pub content_recovery_pct: Option<f64>,
    pub whole_image_recovery_pct: Option<f64>,
    /// Per-section recovery breakdown, or `None` when no original baseline was supplied.
...
/// Runs an ASPack 2.x packed PE through the stub emulator to byte-recover its content sections.
...
/// Rewrites ASPack's bootstrap import thunks to synthetic unmapped addresses for host-shim dispatch.
...
/// Single-steps until control leaves the `.aspack` stub for a content section, host halt, or budget end.
fn run_until_oep(
    cpu: &mut Cpu,
    host: &mut AspackHost,
    image_base: u64,
