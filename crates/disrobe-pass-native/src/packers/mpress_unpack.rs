...
/// Absolute ceiling on a reconstructed `MPRESS` image, guarding against a crafted
/// `SizeOfImage` optional-header field that would otherwise request a multi-GiB
/// heap allocation from a tiny packed input.
const MAX_IMAGE_BYTES: usize = 256 * 1024 * 1024;
/// Maximum plausible expansion of the packed input into the reconstructed image;
/// `SizeOfImage` may not exceed the packed file length scaled by this factor.
...
pub fn unpack_mpress(packed_bytes: &[u8]) -> Result<MpressUnpackOutput> {
    ...
    let payload_decompressed_size: usize = (info.mpress_page_count as usize) << MPRESS_PAGE_SHIFT;
...
fn synthesize_structural_pe(
    ...
    let hdr_aligned: u32 = align_up(info.size_of_headers, file_align);
    let payload_aligned: u32 = align_up(lzmat_payload.len() as u32, file_align);
    let mpress2_size: u32 = info.mpress2_raw_size;
    let mpress2_aligned: u32 = align_up(mpress2_size, file_align);
    let total_size: u32 = hdr_aligned + payload_aligned + mpress2_aligned;
...
mod tests {
    use super::{MpressRecoveryStatus, MpressUnpackOutput, Result, align_up, unpack_mpress};
