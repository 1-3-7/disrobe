...
fn elf_block_at(
    ...
    let output_limit: u64 =
        expansion_credit.checked_add(u64::from(info.c_len).checked_mul(MAX_VERIFY_EXPANSION)?)?;
    if u_len == 0
        || u_len > remaining
        || c_len == 0
        || c_len > u_len
        || u64::from(info.u_len) > output_limit
        || info.extra != 0
    {
        return None;
    }
    ...
    Some(ElfBlock {
        bytes,
        next: data_end,
        expansion_credit: output_limit - u64::from(info.u_len),
    })
}
    ...
    while image.len() < target {
        ...
        if block.is_none() {
            ...
            while probe + B_INFO_LEN <= packed.len() && scanned < MAX_RESYNC_OFFSETS {
                if let Some(candidate) = elf_block_at(
                    packed,
                    probe,
                    remaining,
                    expansion_credit,
...
mod tests {
    ...
    #[test]
    fn elf_extents_use_per_block_bounds_when_tail_c_len_is_smaller() {
        ...
        assert_eq!(
            decode_elf_extents(&packed, &header),
            Some((vec![b'a'; 65], 1)),
            "ELF recovery must use the bounded BInfo extent rather than treating the tail c_len \
             as the whole compressed image"
        );
    }

    #[test]
    fn overexpanded_elf_block_is_rejected_before_output_allocation() {
        let header: UpxPackHeader = header_with_lengths(65, 1, 1);
        let mut packed: Vec<u8> = vec![0u8; 20 + B_INFO_LEN + 1];
        packed[..UPX_MAGIC.len()].copy_from_slice(UPX_MAGIC);
        packed[12..16].copy_from_slice(&header.u_len.to_le_bytes());
        packed[20..24].copy_from_slice(&65u32.to_le_bytes());
        packed[24..28].copy_from_slice(&1u32.to_le_bytes());
        packed[28] = header.method.id();
        packed[32] = 0xff;
        let mut budget: DecompressionBudget =
            DecompressionBudget::with_quotas(DecodeQuota::new(1, 65), DecodeQuota::new(0, 0));
        assert!(elf_first_block_offset(&packed, header.u_len).is_some());
        assert!(
            decode_elf_extents_with_budget(&packed, &header, &mut budget).is_none(),
            "the tail-header exception must not admit an overexpanded BInfo block"
        );
        assert_eq!(budget.attempts(DecodeRoute::ElfExtents), 0);
    }

    #[test]
    fn elf_padding_uses_only_expansion_credit_from_decoded_blocks() {
        let compressed: [u8; 14] = [
            0x90, 0x88, 0x2a, 0x92, 0x00, 0x92, 0x24, 0x49, 0x92, 0x00, 0x00, 0x00, 0xa0, 0xff,
        ];
        let mut packed: Vec<u8> = vec![0; B_INFO_LEN + 64 + B_INFO_LEN];
        packed[..4].copy_from_slice(&64u32.to_le_bytes());
        packed[4..8].copy_from_slice(&64u32.to_le_bytes());
        packed[B_INFO_LEN..B_INFO_LEN + 64].fill(0xaa);
        let padding: usize = B_INFO_LEN + 64;
        packed[padding..padding + 4].copy_from_slice(&3567u32.to_le_bytes());
        packed[padding + 4..padding + 8].copy_from_slice(&14u32.to_le_bytes());
        packed[padding + 8] = UpxMethod::Nrv2e.id();
        packed.extend_from_slice(&compressed);
        let mut budget: DecompressionBudget = DecompressionBudget::new();
        assert!(elf_block_at(&packed, padding, 3567, 0, UpxMethod::Nrv2e, &mut budget).is_none());
        assert_eq!(budget.attempts(DecodeRoute::ElfExtents), 0);
        let prefix: ElfBlock = elf_block_at(&packed, 0, 3631, 0, UpxMethod::Nrv2e, &mut budget)
            .expect("the raw block supplies actual compressed-byte credit");
        assert_eq!(prefix.bytes, vec![0xaa; 64]);
        let gap: ElfBlock = elf_block_at(
            &packed,
            padding,
            3567,
            prefix.expansion_credit,
            UpxMethod::Nrv2e,
            &mut budget,
        )
        .expect("a compact zero gap fits the cumulative 64x limit");
        assert_eq!(gap.bytes, vec![0; 3567]);
        assert_eq!(
            gap.expansion_credit,
            (64 + 14) * MAX_VERIFY_EXPANSION - 3631
        );
        assert!(
            elf_block_at(
                &packed,
        ...
        assert_eq!(budget.attempts(DecodeRoute::ElfExtents), 2);
    }
