...
pub fn read_u16(b: &[u8], off: usize) -> Result<u16> {
    let end: usize = off.checked_add(2).ok_or(Error::Truncated {
        needed: usize::MAX,
        had: b.len(),
    })?;
    if end > b.len() {
...
pub fn read_u32(b: &[u8], off: usize) -> Result<u32> {
    let end: usize = off.checked_add(4).ok_or(Error::Truncated {
        needed: usize::MAX,
        had: b.len(),
    })?;
    if end > b.len() {
...
pub fn read_u64(b: &[u8], off: usize) -> Result<u64> {
    let end: usize = off.checked_add(8).ok_or(Error::Truncated {
        needed: usize::MAX,
        had: b.len(),
    })?;
    if end > b.len() {
