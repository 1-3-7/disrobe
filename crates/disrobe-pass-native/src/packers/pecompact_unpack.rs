...
codec that does not decode as the classic PEC/LZMA stream (classic decode 0%). This phase-1 path \
is structural only (PEC2/PECompact2 markers + SEH stub + carved executable section); full \
byte-exact recovery is delivered by the shipped stub_emu second pass \
