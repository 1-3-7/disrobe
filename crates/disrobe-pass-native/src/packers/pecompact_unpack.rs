...
pub struct CarvedCode {
    ...
    /// `PECompact` 2.x drives an in-stub codec variant; classic decode is 0 by
    /// construction. Recorded honestly, never rounded up.
    pub classic_decode_pct: f64,
}

/// Structurally recover a `PECompact` 2.x packed PE.
///
/// `PECompact` 2.x collapses the host into a single executable section plus
/// `.rsrc`, installs an `SEH`-based decompressor stub at the entry point, and
/// stamps `PEC2`/`PECompact2` markers. This function performs honest structural
/// recovery: it locates the markers, confirms the `SEH` stub, and carves the
/// compressed code section. It does NOT byte-decode the payload - `PECompact`'s
/// in-stub codec is not the classic `PEC`/`LZMA` stream, so classic decode is 0%,
/// reported as `StructuralCarve` pending a `stub_emu` second pass (future work).
///
/// # Errors
///
