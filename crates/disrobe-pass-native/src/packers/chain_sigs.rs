//! Multi-layer packer-chain fingerprinting over the single-signature detect matrix.

use serde::{Deserialize, Serialize};

use super::{Confidence, Detection, Packer, detect};

/// A curated multi-layer packer chain, with `layers` ordered inner-to-outer.
...
/// A detected packer chain for a concrete input buffer, with per-layer witnesses.
...
impl ChainDetection {
    /// Joint confidence score for the whole chain, the product of per-stage probabilities.
    #[must_use]
...
/// Maps a single-signature witness [`Confidence`] to a per-stage probability.
...
/// Curated ledger of multi-layer packer/protector chains, each ordered inner-to-outer.
...
/// Detects curated multi-layer packer chains in `bytes`, sorted most-layers-first then by confidence.
#[must_use]
