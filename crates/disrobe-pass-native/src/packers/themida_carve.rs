...
use crate::packers::vmprotect_carve::{
    CarvedVmpSection, MAX_CARVED_PROTECTED_BYTES, MAX_CARVED_PROTECTED_SECTIONS,
    carve_bounded_section,
};
...
pub fn carve_themida(packed: &[u8]) -> Result<ThemidaCarve> {
    ...
    let mut protected_sections: Vec<CarvedVmpSection> = Vec::new();
    for section in img
        .sections
        .iter()
        .filter(|s: &&PeSection| {
        ...
        .take(MAX_CARVED_PROTECTED_SECTIONS)
    {
        ...
        protected_sections.push(carved);
    }
...
x86 is lifted into one of Themida's randomized VMs (CISC/RISC/FISH/TIGER); the generic native-VM \
devirtualizer (vm_devirt / `disrobe native devirt`) is the engine that lifts such a bytecode VM to \
a re-executable IR, validated end-to-end on real Tigress-shape VM binaries, with the Themida \
front-end extended per published RE write-ups. This carve step is the detect-and-extract half."
            .to_owned(),
    })
}
