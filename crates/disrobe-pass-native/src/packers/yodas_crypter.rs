...
/// Recover a Yoda's Crypter 1.x/2.0 packed PE against its independent original.
///
/// Yoda's Crypter preserves the host PE section layout verbatim: `.rsrc` and
/// `.reloc` are stored byte-for-byte (the in-stub decryptor never touches them),
/// while `.text`/`.rdata`/`.data` are XOR/ADD-stream encrypted by the `yC` stub
/// and are recoverable only by emulating that stub (future work). This function
/// performs the honest split: byte-identical recovery of the verbatim sections,
/// and an [`SectionRecovery::EncryptedCarve`] for the rest where the reported
/// plaintext fraction is the measured raw overlap with the original (typically
/// 0.4-0.8% on `.text`), never rounded up.
///
/// # Errors
///
...
pub struct YodasCrypterCarve {
    pub stub_section_present: bool,
    pub has_yc2_marker: bool,
    /// Verbatim-recovered sections (`.rsrc`/`.reloc`) carved byte-for-byte from
    /// the packed file. These are byte-identical to the original by construction
    /// because Yoda's Crypter never touches them.
    pub verbatim_sections: Vec<(Vec<u8>, Vec<u8>)>,
    /// The recovered partial image: a copy of the packed file with the host
    /// section bodies left in place. Verbatim sections are already correct; the
    /// encrypted `.text`/`.rdata`/`.data` remain carved-but-encrypted pending
    /// `yC` stub emulation.
    pub recovered_image: Vec<u8>,
}

/// Packed-only recovery for the chain dispatch (no original oracle available).
///
/// Carves the verbatim (`.rsrc`/`.reloc`) sections directly out of the packed
/// image - these are byte-identical to the original because Yoda's Crypter
/// stores them untouched - and returns the partial recovered image. The
/// encrypted code/data are left in place ([`SectionRecovery::EncryptedCarve`]) pending stub
/// emulation. This needs no oracle, so it is wired into the chain pass.
///
/// # Errors
///
/// Returns [`Error::UnknownFormat`] if `packed` is not a PE, or
