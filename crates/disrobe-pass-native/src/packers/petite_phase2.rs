...
//! Petite 2.x phase-2 unpacking driven by the shared x86 stub emulator.
//!
//! The static phase-1 LZ77 decoder in [`super::petite_unpack`] can recover the
//! first ~24 bytes of the phase-1 unpacker stub byte-exact, but every byte
//! beyond that depends on reading backreferences that the live stub satisfies
//! from a freshly `VirtualAlloc`'d zero buffer below the destination. The real
//! Petite EP wrapper performs that allocation, decodes phase-1, then transfers
//! to phase-1 which performs the full program decompression in-place over
//! `ImageBase`.
//!
//! This module wires the wrapper through the [`crate::stub_emu`] interpreter:
//!
//! 1. Map the packed PE into emulated memory at its `ImageBase`.
//! 2. Replace every IAT thunk that holds a hint/name RVA with a synthetic
//!    address `0xFE00_xxxx`; the host shim dispatches on `xxxx` to one of
//!    the Win32 functions Petite uses (`VirtualAlloc`, `VirtualFree`,
//!    `VirtualProtect`, `LoadLibraryA`, `GetProcAddress`, `ExitProcess`).
//! 3. Map a scratch stack and an emu heap region from which VirtualAlloc
//!    serves new allocations.
//! 4. Run the emulator from the entry point until any of:
//!    - `ExitProcess` is invoked (clean OEP halt - Petite calls this if the
//!      original program completes via the unpacker itself),
//!    - the emulator branches into an unmapped page (= OEP transfer to a
//!      VirtualAlloc'd region or to original code in `ImageBase`),
//!    - a step or page-write budget is hit.
//! 5. Snapshot the contents of every page in `[ImageBase, ImageBase +
//!    SizeOfImage)` and return them. The pages that the unpacker wrote into
//!    are the deobfuscated bytes the caller needs.
...
/// Absolute ceiling on a reconstructed on-disk PE image, guarding against
/// crafted section `RawSize`/`PointerToRawData` fields that would otherwise
/// request a multi-GiB heap allocation from a tiny in-memory image.
const MAX_FILE_IMAGE_BYTES: usize = 256 * 1024 * 1024;
/// Maximum plausible expansion of the in-memory image into the on-disk image;
/// the summed raw layout may not exceed the source length scaled by this factor.
...
const EMU_TEB_SIZE: u64 = 0x0000_2000;
const EMU_PEB_BASE: u64 = 0x7EFD_D000;
const EMU_PEB_SIZE: u64 = 0x0000_1000;
const TEB_SELF_OFFSET: u64 = 0x18;
const TEB_PEB_OFFSET: u64 = 0x30;

/// Bounded lazy-commit ceiling (4 KiB pages) for the Petite wrapper's scratch
/// writes outside the mapped image/stack/heap. 16384 pages = 64 MiB hard cap,
/// well above any legitimate Petite scratch footprint yet a firm DoS guard.
const EMU_LAZY_PAGE_BUDGET: u32 = 16_384;

/// Outcome of a phase-2 emulated unpack attempt.
#[derive(Debug, Clone)]
...
/// Host shim that intercepts Petite's Win32 IAT calls.
#[derive(Debug)]
struct PetiteHost {
    heap_brk: u64,
    heap_end: u64,
...
impl PetiteHost {
    fn new(heap_base: u64, heap_size: u64, image_base: u64, image_capacity: u64) -> Self {
        Self {
            heap_brk: heap_base,
...
impl HostCall for PetiteHost {
    fn dispatch(&mut self, target: u64, regs: &mut Regs, mem: &mut Memory) -> Result<bool> {
        let symbol: &'static str = match self.iat_map.get(&target).copied() {
            Some(s) => s,
            None => {
        ...
        match symbol {
            "VirtualAlloc" => {
                let _lp_address: u32 = read_arg(1)?;
                let size: u32 = read_arg(2)?;
                let _alloc_type: u32 = read_arg(3)?;
                let _protect: u32 = read_arg(4)?;
                ...
                let pad_hi_lo: u64 = next_brk;
                mem.map(pad_hi_lo, EMU_HEAP_ZERO_PAD, Perm::RW);
                regs.write_sized(Reg::Rax, alloc_at, 32);
            ...
            "GetProcAddress" => {
                if self.snapshot_taken_at.is_none() && self.snapshot_on_first_iat_resolution {
                    self.snapshot_taken_at = Some(regs.rip);
                    self.image_snapshot =
                        Some(mem.read_lossy(self.image_base, self.image_capacity as usize));
                }
                regs.write_sized(Reg::Rax, SYNTH_IAT_BASE + 0xFFF0, 32);
...
/// Attempt phase-2 emulated unpacking on a packed Petite 2.x PE32.
///
/// On success, returns the snapshot of `[ImageBase, ImageBase + SizeOfImage)`
/// after the emulator either reaches the OEP transfer (jump out of mapped
/// range) or invokes a halting Win32 stub.
pub fn unpack_petite_phase2_emulated(packed: &[u8]) -> Result<PhaseTwoEmulatedOutput> {
    ...
    let mut host: PetiteHost =
        PetiteHost::new(EMU_HEAP_BASE, EMU_HEAP_SIZE, image_base, image_capacity);
    cpu.mem.map(SYNTH_IAT_BASE, 0x0001_0000, Perm::RX);

    rewrite_iat(packed, &pe_layout, &mut cpu, &mut host)?;
    ...
    let recovered_size: usize = pe_layout.size_of_image.max(pe_layout.last_section_end_va) as usize;
...
/// Map a synthetic Win32 Thread Environment Block and point FS at it.
///
/// Petite's outer wrapper (and the DirCmp megafile variant in particular)
/// installs a structured-exception frame during bootstrap: `mov eax, fs:[0]`
/// reads the current SEH chain head, `push`es a new record, then writes it back
/// to `fs:[0]`. Without a TEB the FS-relative read lands on linear address 0,
/// which is unmapped, and the emulator faults out before the decompressor runs.
/// We map a zeroed TEB with the SEH chain head set to the Win32 end-of-list
/// sentinel `0xFFFF_FFFF`, wire `TEB.Self` and `TEB.ProcessEnvironmentBlock`,
/// and set the FS base so segment-prefixed accesses resolve here.
fn map_synthetic_teb(cpu: &mut Cpu) -> Result<()> {
    cpu.mem.map(EMU_TEB_BASE, EMU_TEB_SIZE, Perm::RW);
    cpu.mem.map(EMU_PEB_BASE, EMU_PEB_SIZE, Perm::RW);
    cpu.mem.write_u32(EMU_TEB_BASE, 0xFFFF_FFFF)?;
    cpu.mem
        .write_u32(EMU_TEB_BASE + TEB_SELF_OFFSET, EMU_TEB_BASE as u32)?;
    cpu.mem
        .write_u32(EMU_TEB_BASE + TEB_PEB_OFFSET, EMU_PEB_BASE as u32)?;
    cpu.set_fs_base(EMU_TEB_BASE);
    Ok(())
}

/// Convert an in-memory PE image (sections aligned by SectionAlignment) into a
/// canonical on-disk PE file image (sections aligned by FileAlignment with
/// `PointerToRawData`-driven layout).
...
/// After phase-2 emulation, the IAT slots in the in-memory image hold our
/// synthetic Win32 stub addresses (because `GetProcAddress` returned a
/// sentinel). The on-disk PE file format expects those slots to hold either
/// hint/name RVAs (for by-name imports) or ordinal markers. Walk the import
/// directory and restore each IAT slot from its parallel OFT (Original First
/// Thunk) table so the resulting bytes match what a freshly-linked PE on disk
/// would look like.
...
fn rewrite_iat(packed: &[u8], pe: &PeLayout, cpu: &mut Cpu, host: &mut PetiteHost) -> Result<()> {
