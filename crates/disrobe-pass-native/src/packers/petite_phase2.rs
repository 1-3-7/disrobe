...
/// Absolute ceiling on a reconstructed on-disk PE image.
const MAX_FILE_IMAGE_BYTES: usize = 256 * 1024 * 1024;
/// Maximum on-disk-image expansion factor over the in-memory source length.
...
/// Lazy-commit ceiling (4 KiB pages) for the Petite wrapper's scratch writes.
...
/// Attempts phase-2 emulated unpacking on a packed Petite 2.x PE32.
...
/// Maps a synthetic Win32 Thread Environment Block and points FS at it.
...
/// Converts an in-memory PE image into a canonical on-disk PE file image.
...
/// Restores each post-emulation IAT slot from its parallel OFT table to on-disk hint/name RVAs.
