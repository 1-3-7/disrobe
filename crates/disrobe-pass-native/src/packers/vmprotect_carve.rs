...
//! VMProtect detect-and-carve.
//!
//! Detection of VMProtect ships in [`super::detect`] via the `.vmp0`/`.vmp1`
//! section signatures and the grey-zone status
//! [`super::UnpackerStatus::GreyZoneDetectAndCarve`]. This module adds the carve
//! half: it extracts the `.vmpN` section blobs, reconstructs a normalized view
//! of each section header (RVA / virtual size / raw size / characteristics →
//! decoded permission flags), and attempts a synthetic IAT reconstruction by
//! walking the Import Directory the protector leaves in the PE.
//!
//! FULL DEVIRTUALIZATION IS DEFERRED. VMProtect lifts protected functions into a
//! polymorphic stack-machine bytecode whose handler table is per-build mutated;
//! recovering the original x86 requires a dedicated VM-devirt pass that is out
//! of scope here. This carve therefore never claims to recover original code —
//! it surfaces the protected-section geometry and any rebuildable import table
//! so downstream tools (and a future devirt pass) have a structured starting
//! point. The honest ceiling is documented on [`VmProtectCarve::limitation`].
...
/// protected blob extracted verbatim from the on-disk image.
...
/// Detect-and-carve a VMProtect-protected PE.
///
/// Extracts every `.vmpN` section blob, reconstructs the section header geometry
/// and decoded permissions, and rebuilds whatever import table the protector
/// left in the PE Import Directory. Does NOT devirtualize: the protected code
/// remains VMProtect bytecode and is surfaced verbatim for a future devirt pass.
///
/// # Errors
///
...
/// Walk the surviving Import Directory and reconstruct the (dll, thunk-RVA)
/// tuples. VMProtect commonly keeps a valid `IMAGE_IMPORT_DESCRIPTOR` array (it
/// needs the loader to bind a handful of bootstrap APIs) even when it nulls the
/// inner thunk arrays, so the descriptor names and thunk RVAs are recoverable.
