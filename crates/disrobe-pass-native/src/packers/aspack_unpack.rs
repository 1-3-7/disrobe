...
pub enum AspackRecovery {
    /// PE structure and object table parsed; compressed content carved but not byte-decoded.
    StructuralCarve,
}
...
pub struct CarvedBlock {
    ...
    pub bytes: Vec<u8>,
    /// Classic-`aPLib` decode match against the original, in percent.
...
/// Structurally recovers an `ASPack` 2.x packed PE, carving content blocks without byte-decoding.
///
/// # Errors
///
