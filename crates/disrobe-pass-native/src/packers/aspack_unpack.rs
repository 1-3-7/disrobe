...
classic-aPLib decode yields 0%. This phase-1 path is structural only (PE header + reconstructed \
object table + carved compressed blocks); full byte-exact recovery is delivered by the shipped \
stub_emu second pass (unpack_aspack_phase2_emulated)."
            .to_owned(),
    })
}
