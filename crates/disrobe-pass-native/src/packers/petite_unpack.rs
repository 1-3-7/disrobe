...
/// Maximum reconstructed-image expansion factor over the packed input length.
...
/// Structural facts the Petite unpacker recovers from a packed PE.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Decodes a Petite 2.x packed Windows PE32 into a reconstructed image.
///
/// # Errors
///
...
/// Decodes a Petite 2.x packed PE32, returning the recovered bytes plus a structural report.
///
/// # Errors
///
...
/// Parameters extracted from the Petite 2.x phase-one wrapper stub at the entry point.
#[derive(Debug, Clone, Copy)]
struct PhaseOneParams {
    /// File offset of the first compressed-stream byte.
    compressed_file_off: usize,
    /// Output length the phase-one wrapper passes to the decoder.
    phase1_output_bytes: u32,
}

/// Parses the Petite 2.x phase-one wrapper at the EP, returning `None` on shape mismatch.
...
/// Petite 2.x bit-stream decoder state, sharing a byte cursor with the literal reader.
...
/// Decodes the Petite 2.x stream from `compressed_start` for up to `output_size` bytes.
#[allow(clippy::too_many_lines)]
...
/// Minimum trailing-zero run that marks a section boundary in the emulated image.
...
/// Reconstructs the on-disk PE by slicing the phase-2 emulated memory image along section boundaries.
///
/// Returns `None` when the memory image exposes no usable header or trailing `petite` boundary.
#[allow(clippy::too_many_lines)]
