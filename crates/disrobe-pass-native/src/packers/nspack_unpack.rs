...
/// Absolute ceiling on a single decompressed `NSPack` image, guarding against a
/// crafted `dsize` header field that would otherwise request a multi-GiB heap
/// allocation from a tiny packed input.
const MAX_DECOMPRESSED_BYTES: usize = 256 * 1024 * 1024;
/// Maximum plausible decompression ratio for the `NSPack` `LZMA`-family stream;
/// `dsize` may not exceed the compressed body length scaled by this factor.
...
pub struct NspackEmulatedReport {
    ...
    /// Recovery over the compressed content sections only (`.text`/`.rdata`/
    /// `.data`); excludes `.rsrc`/`.reloc`, which `NSPack` does not carry in the
    /// `nsp1` LZMA stream. This is a diagnostic sub-metric, not the headline.
    pub content_recovery_pct: Option<f64>,
    /// Honest whole-image recovery: byte-match percentage over the ENTIRE
    /// decompressed region `[nsp0_base, nsp0_base + dsize)` against the original
    /// mapped image, counting `.rsrc`/`.reloc` (which the decoder leaves zeroed
    /// because `NSPack` stores them outside the `nsp1` stream in a form this
    /// decoder does not yet reconstruct). This is the number tests assert and
    /// callers report; it is materially lower than `content_recovery_pct` on
    /// resource-heavy binaries and that gap is the truth, not a defect to hide.
