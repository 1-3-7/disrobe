...
pub fn unpack_mew(packed_bytes: &[u8]) -> Result<MewUnpackOutput> {
    ...
    let (raw_image, stream_decoded): (Vec<u8>, bool) = attempt_aplib_decode(packed_bytes, &layout);
    let decoded_byte_count: u32 = u32::try_from(raw_image.len()).unwrap_or(u32::MAX);
    Ok(MewUnpackOutput {
        raw_image,
...
/// Byte-identical recovery via static structure parsing of the MEW SE 1.2 container.
///
...
/// consuming any tag bits.
///
/// Returns the decompressed image bytes plus structural metadata.
///
/// # Errors
///
