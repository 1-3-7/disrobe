...
/// Unpacks a MEW 11 SE 1.2 PE32 binary into structural metadata and, when possible, the original image.
...
/// One pre-LZMA aplib chunk preceding the LZMA payload in the MEW stream.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Recovers the original MEW SE 1.2 image by decoding its leading aplib chunks then the LZMA1 rebuilder stream.
///
/// # Errors
///
...
/// Decodes the MEW SE 1.2 aPLib-chunks-then-LZMA1 payload into the original image.
///
/// # Errors
///
...
/// Decodes the aPLib-compressed payload region from a parsed MEW image.
///
/// # Errors
///
...
/// offset escapes the already-decoded buffer.
...
/// Decodes a byte-tagged aPLib stream, returning the buffer plus an [`AplibTrace`].
///
/// # Errors
///
...
/// Lossy diagnostic decoder returning the partial buffer plus any error.
#[must_use]
