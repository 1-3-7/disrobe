//! Section-granule recovery comparison for lossless packers against the independent original.
...
pub enum SectionRole {
    /// Restored verbatim from the compressed body; counted toward the content oracle.
    Content,
    /// Rebuilt by the loader (relocations, IAT thunks); excluded from the content oracle.
    LoaderRebuilt,
    /// The packer's own stub/metadata section; excluded from the content oracle.
    Stub,
}
...
pub struct GranuleRecovery {
    ...
    /// Bytes compared in this section.
    pub compared: usize,
    /// Bytes that matched the original's loaded baseline.
    pub matching: usize,
    /// Section-relative offset of the first mismatching byte, or `None` when byte-identical.
    pub first_mismatch_rel: Option<u32>,
    /// Count of contiguous mismatch runs.
...
impl GranuleRecovery {
    ...
    /// True when the section is byte-identical to the original over its compared span.
    #[must_use]
...
/// Whole-image section breakdown of a recovered image against the original's loaded layout.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl SectionRecoveryReport {
    /// Content-only recovery percentage, excluding loader-rebuilt and stub sections.
    #[must_use]
    ...
    /// The content sections that are not byte-identical, worst recovery first.
    #[must_use]
    ...
    /// Renders a compact one-line-per-section table for diagnostics.
    #[must_use]
...
/// Sections rebuilt by the Windows loader, excluded from the content oracle.
const LOADER_REBUILT: &[&[u8]] = &[b".reloc", b".idata"];

/// Standard PE content section names counted toward the content oracle.
const KNOWN_CONTENT: &[&[u8]] = &[
    b".text", b".rdata", b".data", b".rsrc", b".didat", b".pdata", b".CRT", b".tls", b".xdata",
    b".edata", b".gfids", b".00cfg",
];

/// Classifies a section by name given the caller's packer-specific stub names.
...
/// Builds a PE's loaded memory image by mapping headers and each section to its virtual address.
///
/// # Errors
///
...
/// Compares a recovered VA-indexed image against the independent original at section granularity.
///
/// # Errors
///
/// Propagates [`crate::error::Error`] from PE parsing of `original`.
...
/// Compares `recovered[lo..hi]` against `reference[lo..hi]` into a [`SpanStats`].
...
/// Section-granule recovery for a file-offset-indexed recovered blob, aligned by `file_align`.
///
/// # Errors
///
/// Propagates [`crate::error::Error`] from PE parsing of `original`.
...
/// Compares `recovered[dec_lo..dec_hi]` against `original[orig_lo..]` byte-for-byte.
