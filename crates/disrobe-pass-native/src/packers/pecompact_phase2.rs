...
//! PECompact 2.x phase-2 unpacking driven by the shared x86 stub emulator.
//!
//! The first pass ([`super::pecompact_unpack`]) recovers PECompact's structure -
//! `PEC2`/`PECompact2` markers, the SEH stub, the carved executable section -
//! but cannot byte-decode the payload: PECompact 2.x drives an in-stub codec
//! that scores 0% as a classic PEC/LZMA stream.
//!
//! PECompact 2.x transfers control by *raising an exception*: its entry stub
//! `push`es the address of its decompressor, installs an `SEH` frame at
//! `fs:[0]`, then deliberately writes to address 0 to trigger an access
//! violation. The kernel dispatches the AV to the registered handler (the
//! decompressor), which decompresses the payload over the mapped image and
//! transfers to the original entry point. This module wires that flow through
//! the [`crate::stub_emu`] interpreter with SEH dispatch enabled:
//!
//! 1. Map the packed PE at `ImageBase`, copying headers and the compressed
//!    `.text`/`.rsrc` bodies to their mapped VAs.
//! 2. Rewrite the stub's bootstrap IAT slots (`LoadLibraryA` / `GetProcAddress`
//!    / `VirtualAlloc` / `VirtualFree`) to synthetic addresses the host shim
//!    services.
//! 3. Enable [`Cpu::enable_seh_dispatch`] so the deliberate null write enters the
//!    decompressor rather than faulting out.
//! 4. Single-step until control reaches the recovered OEP (a transfer to the
//!    low-RVA region of `.text` after the stub has run) or the budget is spent.
//! 5. Snapshot `[ImageBase, ImageBase + SizeOfImage)`; the content section now
//!    holds its decompressed bytes.
...
/// Sections the loader rebuilds rather than restoring byte-for-byte; excluded
/// from the non-circular content-recovery oracle.
const LOADER_REBUILT_SECTIONS: &[&[u8]] = &[b".reloc", b".idata"];

/// Outcome of a PECompact phase-2 emulated unpack.
#[derive(Debug, Clone)]
pub struct PecompactPhaseTwoOutput {
    ...
    /// Byte-recovery over content sections (`.text`/`.rdata`/`.data`) versus the
    /// independent original's loaded layout, in percent. `None` when no baseline
...
/// Run a PECompact 2.x packed PE through the SEH-aware stub emulator to
/// byte-recover its content sections. When `original` is supplied, compute the
/// non-circular content-recovery percentage against its independent layout.
pub fn unpack_pecompact_phase2_emulated(
    ...
    let mut host: PecompactHost = PecompactHost::new();
...
/// Single-step until control reaches the recovered OEP. PECompact decompresses
/// in-place into `.text` then transfers to the original entry, which lies in the
/// low-RVA region of the first content section. We halt the moment execution -
/// after the SEH-driven decompressor has run and left the heap-resident stub -
/// re-enters `[ImageBase + page, ImageBase + ep_packed)`: the original program
/// body that PECompact restored, never the packed entry stub itself.
fn run_until_oep(
    cpu: &mut Cpu,
    host: &mut PecompactHost,
    image_base: u64,
