...
//! kkrunchy classic (0.23a) phase-2 unpacking driven by the shared x86 stub emulator.
...
/// Base for synthetic per-import return addresses inside the mapped synthetic-IAT page.
...
/// Attempts phase-2 emulated unpacking on a packed kkrunchy classic PE32.
...
/// Drives the emulator one instruction at a time, halting on entry to `[oep_low, oep_high)` or host halt.
fn run_until_oep(
    cpu: &mut Cpu,
...
/// Reconstructs a canonical on-disk PE32 for the recovered classic-kkrunchy OEP image.
...
/// Rebuilds the OEP import table the kkrunchy packer stripped from `.text`.
...
/// Locates the import and IAT data directories by scanning `.text` for a valid descriptor table.
