...
const SLOT_PRODUCT: u8 = 4;
const SLOT_SHIFTED_QUOTIENT: u8 = 5;
...
const SIGNED_FUSED_QUOTIENT_SIGN: &[Step] = &[
    step(
        StepKind::MulMagic,
        SLOT_DIVIDEND,
        SLOT_DIVIDEND,
    ...
    step(StepKind::Add, SLOT_SHIFTED_QUOTIENT, SLOT_SIGN, SLOT_ACC),
];
...
const RULES: &[Rule] = &[
    ...
    Rule {
        id: "sdiv-magic-fused-quotient-sign",
        steps: SIGNED_FUSED_QUOTIENT_SIGN,
        signed: true,
        implicit_high_bit: false,
        shift_terms: &[ShiftTerm::Captured],
        quotient: SLOT_ACC,
...
fn step_matches(
    ...
    match (step.kind, observed) {
        ...
        (StepKind::Shift, Observed::Shift { dest, op, amount }) => {
            if op != step.op
                || !matcher.holds_slot(dest.reg, step.lhs)
                || (step.amount == Amount::DividendBitsLessOne
                    && dest.width.bits() != dividend_bits)
                || (step.lhs == SLOT_PRODUCT
                    && matcher
                        .held
                        .get(&dest.reg)
                        .is_some_and(|(_, width)| dest.width.bits() < width.bits()))
            {
                return None;
            }
        ...
        (StepKind::Add, Observed::Add { dest, lhs, rhs }) => {
            if step.lhs == SLOT_SHIFTED_QUOTIENT && dest.width.bits() != dividend_bits {
                return None;
            }
            let direct: bool =
...
fn try_rule(items: &[Item], anchor: usize, rule: &'static Rule) -> Option<IdiomMatch> {
    ...
    let (multiplicand, widening, product_bits): (Reg, Option<Extension>, u32) =
        match classify(anchor_stmt, &constants) {
            Observed::MulConst {
                dest,
                src,
                widening,
                ..
            } => (src.reg, widening, dest.width.bits()),
            Observed::WideMul { dest, lhs, rhs, .. } => {
                let multiplicand: Reg = match constants.get(&lhs) {
                    Some((Held::Constant(_), _)) => rhs,
                    _ => lhs,
                };
                (multiplicand, None, dest.width.bits() * 2)
            }
            _ => return None,
        };
        ...
        match observed {
            Observed::Copy {
                ...
                match carried {
                    ...
                    Some((Held::Slot(SLOT_PRODUCT), width))
                        if src.width.bits().min(dest.width.bits()) < width.bits() =>
                    {
                        matcher.held.remove(&dest.reg);
                    }
                    Some((Held::Slot(SLOT_SHIFTED_QUOTIENT), _))
                        if src.width.bits().min(dest.width.bits()) < dividend_bits =>
                    {
                        matcher.held.remove(&dest.reg);
                    }
                    Some((held, _)) => {
