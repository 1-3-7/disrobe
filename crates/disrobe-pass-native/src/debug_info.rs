...
pub enum PdbSymbolKind {
    /// `S_PUB32` - a published (exported) symbol.
    Public,
    /// `S_GPROC32` - a global procedure.
    GlobalProcedure,
    /// `S_LPROC32` - a local (static) procedure.
    LocalProcedure,
    /// `S_LDATA32` / `S_GDATA32` - module-local / global data.
    Data,
}

/// One symbol recovered from a PDB's symbol streams, with name, kind, RVA and type.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Outcome of cross-checking a PDB's age against a binary's debug-directory age.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
/// Full debug-info recovery from a PDB: classified symbols, TPI type map, and age/guid.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl PdbRecovery {
    ...
    /// Cross-checks the PDB's age against a binary's CodeView debug-directory age.
    #[must_use]
...
/// Recovers classified symbols, the TPI type map, and the age/guid from a PDB.
///
/// # Errors
///
...
/// Walks the global symbol stream, classifying each record into a [`PdbSymbolInfo`].
...
/// Maps a parsed CodeView symbol to a classified [`PdbSymbolInfo`], or `None` for other kinds.
fn classify_symbol(
...
/// Maps a parsed CodeView type leaf to a classified [`PdbTypeInfo`], skipping forward refs.
