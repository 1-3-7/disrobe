...
pub fn recover_structure_codescan_only(bytes: &[u8], bitness: Bitness) -> Option<VmStructure> {
    ...
    chained_fixups::apply(&obj, bytes, &mut segments)?;
...
fn append_zero_filled_entry_segment(
    ...
    for section in obj.sections() {
        let zero_filled: bool = section.kind() == object::SectionKind::UninitializedData
            || matches!(
                section.flags(),
                object::SectionFlags::MachO { flags }
                    if flags & object::macho::SECTION_TYPE == object::macho::S_ZEROFILL
            );
        if !zero_filled || !section.data().is_ok_and(|data: &[u8]| data.is_empty()) {
            continue;
        }
        let Some(segment) =
...
mod tests {
    ...
    #[test]
    fn macho_common_entry_uses_zero_fill_metadata_instead_of_its_section_name() {
        for name in [b"__common".as_slice(), b"__renamed".as_slice()] {
            for flags in [
                ...
                let section_id: SectionId = writer.add_section(
                    b"__DATA".to_vec(),
                    name.to_vec(),
                    object::SectionKind::Common,
                );
                let _: u64 = writer.append_section_bss(section_id, 8, 4);
                writer.section_mut(section_id).flags = object::SectionFlags::MachO { flags };
                let bytes: Vec<u8> = writer.write().expect("write Mach-O zero-fill fixture");
                let file: object::File<'_> =
                    object::File::parse(bytes.as_slice()).expect("parse fixture");
                let section: object::Section<'_, '_> = file.sections().next().expect("section");
                assert_ne!(section.kind(), object::SectionKind::UninitializedData);
                assert!(section.data().expect("zero-fill data").is_empty());

                let mut segments: Vec<Segment> = Vec::new();
                append_zero_filled_entry_segment(&file, 4, &mut segments);
                if flags == object::macho::S_ZEROFILL {
                    assert_eq!(segments.len(), 1);
                    assert_eq!(segments[0].va, 4);
                    assert_eq!(segments[0].bytes, vec![0; 4]);
                } else {
                    assert!(
                        segments.is_empty(),
                        "thread-local storage is not an entry word"
                    );
                }
            }
        }
