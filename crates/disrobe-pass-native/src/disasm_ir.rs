...
use iced_x86::{
    ...
    Mnemonic, OpAccess, Register, RflagsBits,
};
use object::{
...
use crate::desync::{
    ...
    NoreturnInferenceOutcome, NoreturnInferenceTermination, ReadOnlyWindow,
    discover_aarch64_functions, discover_functions_with_direct_call_sweep_status,
    discover_functions_with_status, is_noreturn_import_name,
};
...
pub(crate) enum FunctionUniverse {
    Complete,
    Aarch64Fallback {
        discovered_starts: BTreeSet<u64>,
    },
    X86Inferred {
        starts_requiring_reference_corroboration: BTreeSet<u64>,
    },
}
...
impl FunctionUniverse {
    pub(crate) fn reference_anchors_trusted(&self, address: u64) -> bool {
        match self {
            Self::Complete | Self::X86Inferred { .. } => true,
            Self::Aarch64Fallback { discovered_starts } => !discovered_starts.contains(&address),
        }
    }

    pub(crate) fn requires_reference_corroboration(&self, address: u64) -> bool {
        match self {
            Self::X86Inferred {
                starts_requiring_reference_corroboration,
            } => starts_requiring_reference_corroboration.contains(&address),
            Self::Complete | Self::Aarch64Fallback { .. } => false,
        }
    }
}
...
pub(crate) fn build_disasm_payload_with_discovery(bytes: &[u8]) -> Result<DisasmBuild> {
    ...
    } else if matches!(arch, DisasmArch::X86 | DisasmArch::X86_64) && !discovered_starts.is_empty()
    {
        FunctionUniverse::X86Inferred {
            starts_requiring_reference_corroboration: discovered_starts,
        }
    } else {
...
fn inject_discovered_functions(
    ...
    let (discovered, inference_incomplete): (DiscoveredFunctions, bool) =
        if matches!(arch, DisasmArch::Aarch64) {
            if !matches!(
                native.format,
            ...
            let linked_x86_elf: bool = matches!(arch, DisasmArch::X86_64)
                && matches!(native.format, NativeFormat::Elf64)
                && object::File::parse(bytes).is_ok_and(|file| {
                    matches!(
                        file.kind(),
                        object::ObjectKind::Executable | object::ObjectKind::Dynamic
                    )
                });
            let (mut discovered, incomplete): (DiscoveredFunctions, bool) = match outcome
                .into_parts()
            {
                (discovered, NoreturnInferenceTermination::Complete) => (discovered, false),
                (discovered, NoreturnInferenceTermination::IterationLimit) if linked_x86_elf => {
                    (discovered, true)
                }
                (_, _) => return BTreeSet::new(),
            };
            if !incomplete
                && matches!(arch, DisasmArch::X86_64)
                && matches!(native.format, NativeFormat::Elf64 | NativeFormat::Pe64)
            {
                let import_tails: Option<BTreeSet<u64>> =
                    matches!(native.format, NativeFormat::Pe64)
                        .then(|| pe_import_tail_sites(bytes, instructions));
                let mut boundary_seeds: Vec<u64> = x86_boundary_seeds(
                    instructions,
                    &discovered.starts,
                    MAX_DISCOVERY_FUNCTIONS.saturating_sub(discovered.starts.len()),
                    import_tails.as_ref(),
                );
                if matches!(native.format, NativeFormat::Elf64)
                    && object::File::parse(bytes).is_ok_and(|file| {
                ...
                {
                    boundary_seeds.extend(x86_prefix_boundary_seeds(
                        instructions,
                        &input.code,
                        &discovered.starts,
                        MAX_DISCOVERY_FUNCTIONS
                            .saturating_sub(discovered.starts.len())
                            .saturating_sub(boundary_seeds.len()),
                    ));
                }
                if !boundary_seeds.is_empty() {
                    input.seeds.extend(boundary_seeds);
                    input.seeds.sort_unstable();
                    input.seeds.dedup();
                    let expanded: NoreturnInferenceOutcome<DiscoveredFunctions> =
                        if matches!(native.format, NativeFormat::Elf64) {
                            discover_functions_with_direct_call_sweep_status(&input)
                        } else {
                            discover_functions_with_status(&input)
                    ...
                    discovered = expanded.into_value();
                }
            }
    ...
    let authoritative_x86_starts: BTreeSet<u64> =
        if !inference_incomplete && matches!(arch, DisasmArch::X86 | DisasmArch::X86_64) {
            x86_authoritative_starts(native, bytes, decoded_code, instructions, arch)
        } else {
            BTreeSet::new()
        };
    ...
    for start in discovered_starts {
        ...
        if retain_provenance
            || matches!(arch, DisasmArch::X86 | DisasmArch::X86_64)
                && !authoritative_x86_starts.contains(&start)
        {
...
fn x86_authoritative_starts(
    ...
    let Some(windows): Option<Vec<(u64, u64)>> = x86_executable_windows(code) else {
        return BTreeSet::new();
    };
    let mut starts: Vec<u64> = authoritative_entry_export_starts(native, bytes);
    starts.extend(x86_unwind_starts(bytes, code));
    starts.extend(validated_pdata_function_starts(
        bytes,
        &windows,
        instructions,
    ));
    normalize_x86_endbr_starts(arch, instructions, starts)
        .into_iter()
        .filter(|start: &u64| is_x86_code_start(*start, &windows, instructions))
        .collect()
}

fn validated_pdata_function_starts(
    bytes: &[u8],
    windows: &[(u64, u64)],
    instructions: &[DisasmInstruction],
) -> Vec<u64> {
    ...
    if !matches!(file.format(), object::BinaryFormat::Pe)
        || !matches!(file.architecture(), object::Architecture::X86_64)
        || !file.is_64()
        || !file.is_little_endian()
    {
        return Vec::new();
    ...
    validated_pdata_records(data, file.relative_address_base(), windows, instructions)
}

fn validated_pdata_records(
    data: &[u8],
    image_base: u64,
    windows: &[(u64, u64)],
    instructions: &[DisasmInstruction],
) -> Vec<u64> {
    if !data.len().is_multiple_of(12) || data.len() / 12 > MAX_DISCOVERY_FUNCTIONS {
        return Vec::new();
    }
    let mut starts: BTreeSet<u64> = BTreeSet::new();
    let mut duplicated: BTreeSet<u64> = BTreeSet::new();
    for entry in data.chunks_exact(12) {
        let begin_rva: u32 = u32::from_le_bytes([entry[0], entry[1], entry[2], entry[3]]);
        let end_rva: u32 = u32::from_le_bytes([entry[4], entry[5], entry[6], entry[7]]);
        let Some(start): Option<u64> = image_base.checked_add(u64::from(begin_rva)) else {
            continue;
        };
        let Some(end): Option<u64> = image_base.checked_add(u64::from(end_rva)) else {
            continue;
        };
        if begin_rva == 0 || start >= end || !is_x86_code_range(start, end, windows, instructions) {
            continue;
        }
        if !starts.insert(start) {
            duplicated.insert(start);
        }
    }
    for start in duplicated {
        starts.remove(&start);
    }
    starts.into_iter().collect()
}

fn is_x86_code_start(
    address: u64,
    windows: &[(u64, u64)],
    instructions: &[DisasmInstruction],
) -> bool {
    instructions
        ...
        .is_ok()
        && x86_window_containing(address, windows).is_some()
}

fn is_x86_code_range(
    start: u64,
    end: u64,
    windows: &[(u64, u64)],
    instructions: &[DisasmInstruction],
) -> bool {
    is_x86_code_start(start, windows, instructions)
        && x86_window_containing(start, windows)
            .is_some_and(|(_, window_end): &(u64, u64)| end <= *window_end)
}

fn x86_executable_windows(code: &[CodeWindow<'_>]) -> Option<Vec<(u64, u64)>> {
    if code.len() > MAX_DISCOVERY_FUNCTIONS {
        return None;
    }
    let mut windows: Vec<(u64, u64)> = code
        .iter()
        .map(|window: &CodeWindow<'_>| {
            u64::try_from(window.bytes.len())
                .ok()
                .and_then(|length: u64| window.address.checked_add(length))
                .map(|end: u64| (window.address, end))
        })
        .collect::<Option<Vec<(u64, u64)>>>()?;
    windows.sort_unstable_by_key(|(start, _): &(u64, u64)| *start);
    windows
        .windows(2)
        .all(|pair: &[(u64, u64)]| pair[0].1 <= pair[1].0)
        .then_some(windows)
}

fn x86_window_containing(address: u64, windows: &[(u64, u64)]) -> Option<&(u64, u64)> {
    let index: usize = windows.partition_point(|(start, _): &(u64, u64)| *start <= address);
    index
        .checked_sub(1)
        .and_then(|index: usize| windows.get(index))
        .filter(|(start, end): &&(u64, u64)| (*start..*end).contains(&address))
}

fn entry_export_seeds(native: &NativeFile, bytes: &[u8]) -> Vec<u64> {
    if native.exports.len() > MAX_DISCOVERY_FUNCTIONS {
        return Vec::new();
    }
...
fn authoritative_entry_export_starts(native: &NativeFile, bytes: &[u8]) -> Vec<u64> {
    let Ok(file): core::result::Result<object::File<'_>, object::Error> =
        object::File::parse(bytes)
    else {
        return Vec::new();
    ...
    entry_export_seeds(native, bytes)
}
...
fn internal_boundary(
    ...
    import_tails: Option<&BTreeSet<u64>>,
) -> Option<u64> {
    let mut furthest_branch: u64 = span_start;
    for (position, insn) in body.iter().enumerate() {
        let role: BoundaryRole = if import_tails.is_some_and(|sites| sites.contains(&insn.offset)) {
...
fn pe_import_tail_sites(bytes: &[u8], instructions: &[DisasmInstruction]) -> BTreeSet<u64> {
    let Ok(file): core::result::Result<object::File<'_>, object::Error> =
        object::File::parse(bytes)
    else {
        return BTreeSet::new();
    ...
    for section in file.sections() {
        ...
        let Some(rva): Option<u64> = start.checked_sub(image_base) else {
            return BTreeSet::new();
        };
        let Ok(data): core::result::Result<&[u8], object::Error> = section.data() else {
            return BTreeSet::new();
        };
        let size: u64 = section.size().min(data.len() as u64);
        ...
        if rva > u64::from(u32::MAX)
            || end
                ...
                .is_none_or(|end_rva| end_rva > 1_u64 << 32)
        {
            return BTreeSet::new();
        }
        if size != 0 {
    ...
    let imports: Vec<ImportStub> = resolve_pe_iat_imports(bytes);
    if imports.len() > MAX_DISCOVERY_FUNCTIONS {
        return BTreeSet::new();
    }
    let slots: BTreeSet<u64> = imports
        .into_iter()
        .filter_map(|import: ImportStub| {
            let slot: u64 = import.slot_address;
            let rva: u64 = slot.checked_sub(image_base)?;
            let end: u64 = slot.checked_add(8)?;
            if rva > u64::from(u32::MAX) {
                return None;
            }
            let position: usize = ranges.partition_point(|range| range.0 <= slot);
            let &(_, range_end): &(u64, u64) = ranges.get(position.checked_sub(1)?)?;
            (end <= range_end).then_some(slot)
        })
        .collect();
    x86_import_tail_sites(instructions, &slots)
}

fn x86_import_tail_sites(
    instructions: &[DisasmInstruction],
    slots: &BTreeSet<u64>,
) -> BTreeSet<u64> {
    if instructions.len() > MAX_PAYLOAD_INSTRUCTIONS || slots.len() > MAX_DISCOVERY_FUNCTIONS {
        return BTreeSet::new();
    }
    instructions
        .iter()
        .filter_map(|insn: &DisasmInstruction| {
            let decoded: Instruction = decode_one_x86(64, insn.offset, &insn.bytes)?;
            if decoded.len() != insn.bytes.len()
                || decoded.code() != iced_x86::Code::Jmp_rm64
                || decoded.memory_base() != Register::RIP
                || decoded.segment_prefix() != Register::None
            {
                return None;
            }
            let slot: u64 = indirect_jmp_slot(&decoded)?;
            let end: u64 = insn
                .offset
                .checked_add(u64::try_from(decoded.len()).ok()?)?;
            let displacement: i128 = i128::from(slot) - i128::from(end);
            if i32::try_from(displacement).is_err() || slot.checked_add(8).is_none() {
                return None;
            }
            slots.contains(&slot).then_some(insn.offset)
        })
        .collect()
}

fn x86_prefix_boundary_seeds(
    instructions: &[DisasmInstruction],
    code: &[CodeWindow<'_>],
    established: &[u64],
    limit: usize,
    ...
    if instructions.len() > MAX_PAYLOAD_INSTRUCTIONS
        || established.len() >= MAX_DISCOVERY_FUNCTIONS
        || limit == 0
    {
        return Vec::new();
    }
    let Some(&anchor): Option<&u64> = established.iter().min() else {
        return Vec::new();
    };
    let mut containing: Option<&CodeWindow<'_>> = None;
    for window in code {
        let Some(end): Option<u64> = u64::try_from(window.bytes.len())
            .ok()
            .and_then(|length| window.address.checked_add(length))
        else {
            return Vec::new();
        };
        if (window.address..end).contains(&anchor) && containing.replace(window).is_some() {
            return Vec::new();
        }
    }
    let Some(window): Option<&CodeWindow<'_>> = containing else {
        return Vec::new();
    };
    if window.address >= anchor {
        return Vec::new();
    }
    let low: usize =
        instructions.partition_point(|insn: &DisasmInstruction| insn.offset < window.address);
    let high: usize = instructions.partition_point(|insn: &DisasmInstruction| insn.offset < anchor);
    let Some(prefix): Option<&[DisasmInstruction]> = instructions.get(low..high) else {
        return Vec::new();
    };
    if !prefix.iter().all(|insn: &DisasmInstruction| {
        let Some(start): Option<usize> = insn
            .offset
            .checked_sub(window.address)
            .and_then(|offset| usize::try_from(offset).ok())
        else {
            return false;
        };
        let Some(end): Option<usize> = start.checked_add(insn.bytes.len()) else {
            return false;
        };
        window.bytes.get(start..end) == Some(insn.bytes.as_slice())
    }) {
        return Vec::new();
    }
    x86_boundary_seeds(
        prefix,
        &[window.address, anchor],
        limit.min(MAX_DISCOVERY_FUNCTIONS.saturating_sub(established.len())),
        None,
    )
}

fn x86_boundary_seeds(
    instructions: &[DisasmInstruction],
    established: &[u64],
    limit: usize,
    import_tails: Option<&BTreeSet<u64>>,
    ...
    if instructions.is_empty()
        ...
        || established.len() >= MAX_DISCOVERY_FUNCTIONS
        || limit == 0
        || import_tails.is_some_and(BTreeSet::is_empty)
    {
        return Vec::new();
    }
    let Ok(model): Result<FlowModel> = FlowModel::for_arch(DisasmArch::X86_64) else {
        return Vec::new();
    };
    let isa: BoundaryIsa = BoundaryIsa::X86 { bits: 64 };
    let mut ordered: Vec<u64> = established.to_vec();
    ordered.sort_unstable();
    ordered.dedup();
    let budget: usize = limit.min(MAX_DISCOVERY_FUNCTIONS.saturating_sub(ordered.len()));
    let mut seeds: Vec<u64> = Vec::new();
    for gap in ordered.windows(2) {
        if seeds.len() >= budget {
            break;
        }
        let Some((&start, &end)): Option<(&u64, &u64)> = gap.first().zip(gap.get(1)) else {
            continue;
        };
        let low: usize =
            instructions.partition_point(|insn: &DisasmInstruction| insn.offset < start);
        let high: usize =
            instructions.partition_point(|insn: &DisasmInstruction| insn.offset < end);
        let Some(body): Option<&[DisasmInstruction]> = instructions.get(low..high) else {
            continue;
        };
        let mut cursor: u64 = start;
        let complete: bool = body.iter().all(|insn: &DisasmInstruction| {
            let Some(length): Option<u64> = u64::try_from(insn.bytes.len()).ok() else {
                return false;
            };
            let Some(next): Option<u64> = insn.offset.checked_add(length) else {
                return false;
            };
            if insn.offset != cursor || next > end || !decodes_cleanly(&model, insn) {
                return false;
            }
            cursor = next;
            true
        ...
        let mut remaining: &[&DisasmInstruction] = &refs;
        let mut span_start: u64 = start;
        while seeds.len() < budget {
            let Some(boundary): Option<u64> =
                internal_boundary(isa, &model, span_start, end, remaining, import_tails)
            else {
                break;
            };
            let position: usize =
                remaining.partition_point(|insn: &&DisasmInstruction| insn.offset < boundary);
            let Some(next): Option<&[&DisasmInstruction]> = remaining.get(position..) else {
                break;
            };
            if position == 0 || next.is_empty() {
                break;
            }
            if import_tails.is_none_or(|sites| sites.contains(&boundary)) {
                seeds.push(boundary);
            }
            span_start = boundary;
            remaining = next;
...
fn trim_to_internal_boundaries(
    ...
    for span in spans {
        ...
        if let Some(boundary) = internal_boundary(isa, &model, span.address, span.end, body, None) {
            span.end = boundary;
        }
    }
}
...
fn build_symbol_table(bytes: &[u8], native: &NativeFile) -> Vec<DisasmSymbol> {
    ...
    if matches!(native.arch, BinArch::X86 | BinArch::X86_64)
        && matches!(native.format, NativeFormat::Elf32 | NativeFormat::Elf64)
    {
        for import in resolve_elf_plt_imports(bytes) {
...
mod tests {
    ...
    fn boundary_of(arch: DisasmArch, isa: BoundaryIsa, bytes: &[u8]) -> Option<u64> {
        ...
        let model: FlowModel = FlowModel::for_arch(arch).expect("fixture arch has a flow model");
        internal_boundary(isa, &model, X86_BOUNDARY_BASE, end, &refs, None)
    }
    ...
    #[test]
    fn x86_bracketed_prologue_free_bodies_become_seeds_within_the_budget() {
        let mut bytes: Vec<u8> = padded(&[0x31, 0xC0, 0xC3], 13, &[0x31, 0xC0, 0xC3]);
        bytes.extend(std::iter::repeat_n(0x90_u8, 13));
        bytes.extend_from_slice(&[0x31, 0xC0, 0xC3]);
        bytes.extend(std::iter::repeat_n(0x90_u8, 13));
        let body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
        assert_eq!(
            x86_boundary_seeds(&body, &[0x1000, 0x1030], 2, None),
            vec![0x1010, 0x1020]
        );
        assert_eq!(
            x86_boundary_seeds(&body, &[0x1000, 0x1030], 1, None),
            vec![0x1010]
        );
        assert!(x86_boundary_seeds(&body, &[0x1000, 0x1030], 0, None).is_empty());
        assert!(x86_boundary_seeds(&body, &[0x1000], 2, None).is_empty());
        assert!(
            x86_boundary_seeds(&body, &vec![0x1000; MAX_DISCOVERY_FUNCTIONS], 2, None).is_empty()
        );
    }

    #[test]
    fn x86_prefix_boundaries_keep_the_head_outside_the_function_budget() {
        let mut bytes: Vec<u8> = padded(&[0x31, 0xC0, 0xC3], 13, &[0x31, 0xC0, 0xC3]);
        bytes.extend(std::iter::repeat_n(0x90_u8, 13));
        bytes.extend_from_slice(&[0x31, 0xC0, 0xC3]);
        bytes.extend(std::iter::repeat_n(0x90_u8, 13));
        bytes.push(0xC3);
        let body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
        let code: [CodeWindow<'_>; 1] = [CodeWindow {
            address: 0x1000,
            bytes: &bytes,
        }];
        assert_eq!(
            x86_prefix_boundary_seeds(&body, &code, &[0x1030], 2),
            vec![0x1010, 0x1020]
        );
        assert_eq!(
            x86_prefix_boundary_seeds(&body, &code, &[0x1030], 1),
            vec![0x1010]
        );
        assert_eq!(
            x86_prefix_boundary_seeds(&body, &code, &vec![0x1030; MAX_DISCOVERY_FUNCTIONS - 1], 2),
            vec![0x1010]
        );
        assert!(x86_prefix_boundary_seeds(&body, &code, &[0x1030], 0).is_empty());
        assert!(
            x86_prefix_boundary_seeds(&body, &code, &vec![0x1030; MAX_DISCOVERY_FUNCTIONS], 2)
                .is_empty()
        );
    }

    #[test]
    fn x86_prefix_boundaries_require_an_anchor_inside_one_checked_code_window() {
        let mut bytes: Vec<u8> = padded(&[0x31, 0xC0, 0xC3], 13, &[0x31, 0xC0, 0xC3]);
        bytes.push(0xC3);
        let body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
        ...
        let code: &[CodeWindow<'_>] = core::slice::from_ref(&window);
        assert!(x86_prefix_boundary_seeds(&body, code, &[], 2).is_empty());
        assert!(x86_prefix_boundary_seeds(&body, code, &[0x1000], 2).is_empty());
        assert!(x86_prefix_boundary_seeds(&body, code, &[0x2000], 2).is_empty());
        assert!(x86_prefix_boundary_seeds(&body, &[], &[0x1013], 2).is_empty());
        assert!(
            x86_prefix_boundary_seeds(&body, &[window.clone(), window], &[0x1013], 2).is_empty()
        );
        let truncated: CodeWindow<'_> = CodeWindow {
            address: 0x1000,
            bytes: &bytes[..0x13],
        };
        assert!(x86_prefix_boundary_seeds(&body, &[truncated], &[0x1013], 2).is_empty());
        let overflow: CodeWindow<'_> = CodeWindow {
            address: u64::MAX - 1,
            bytes: &bytes,
        };
        assert!(x86_prefix_boundary_seeds(&body, &[overflow], &[u64::MAX], 2).is_empty());
    }

    #[test]
    fn x86_prefix_boundaries_require_complete_bytes_from_the_code_window() {
        let mut bytes: Vec<u8> = padded(&[0x31, 0xC0, 0xC3], 13, &[0x31, 0xC0, 0xC3]);
        bytes.push(0xC3);
        let code: [CodeWindow<'_>; 1] = [CodeWindow {
            address: 0x1000,
        ...
        let body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
        let mut missing: Vec<DisasmInstruction> = body.clone();
        missing.remove(1);
        assert!(x86_prefix_boundary_seeds(&missing, &code, &[0x1013], 2).is_empty());
        let mut changed: Vec<DisasmInstruction> = body.clone();
        changed.get_mut(2).expect("padding instruction").bytes = vec![0xCC];
        assert!(x86_prefix_boundary_seeds(&changed, &code, &[0x1013], 2).is_empty());
        let mut truncated: Vec<DisasmInstruction> = body;
        truncated
            .iter_mut()
            .find(|insn: &&mut DisasmInstruction| insn.offset == 0x1010)
            .expect("interior entry")
            .bytes = vec![0x0F];
        assert!(x86_prefix_boundary_seeds(&truncated, &code, &[0x1013], 2).is_empty());
        let mut invalid_bytes: Vec<u8> = bytes.clone();
        invalid_bytes[0x10..0x12].copy_from_slice(&[0x0F, 0x04]);
        let invalid_code: [CodeWindow<'_>; 1] = [CodeWindow {
            address: 0x1000,
            bytes: &invalid_bytes,
        }];
        truncated
            ...
            .expect("interior entry")
            .bytes = vec![0x0F, 0x04];
        assert!(x86_prefix_boundary_seeds(&truncated, &invalid_code, &[0x1013], 2).is_empty());
    }

    #[test]
    fn x86_prefix_boundaries_refuse_reachable_or_unproved_padding() {
        let cases: Vec<Vec<u8>> = vec![
            padded(&[0x31, 0xC0], 14, &[0x31, 0xC0, 0xC3]),
            padded(&[0x85, 0xC0, 0x75, 0x0C, 0xC3], 11, &[0x31, 0xC0, 0xC3]),
            padded(&[0x85, 0xC0, 0x75, 0x06, 0xC3], 11, &[0x31, 0xC0, 0xC3]),
            padded(&[0xEB, 0x0E, 0xC3], 13, &[0x31, 0xC0, 0xC3]),
            padded(&[0xFF, 0xE0, 0xC3], 13, &[0x31, 0xC0, 0xC3]),
            padded(&[0xC3], 3, &[0x31, 0xC0, 0xC3]),
            padded(&[0xC3], 0, &[0x31, 0xC0, 0xC3]),
        ];
        for mut bytes in cases {
            let anchor: u64 = 0x1000 + u64::try_from(bytes.len()).expect("fixture length fits");
            bytes.push(0xC3);
            let body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
            let code: [CodeWindow<'_>; 1] = [CodeWindow {
                address: 0x1000,
            ...
            assert!(
                x86_prefix_boundary_seeds(&body, &code, &[anchor], 2).is_empty(),
                "the prefix contains no proved interior function boundary: {bytes:02x?}"
            );
        }
    }

    #[test]
    fn x86_boundary_seeds_refuse_reachable_or_unproven_padding_successors() {
        let cases: Vec<Vec<u8>> = vec![
            padded(&[0x85, 0xC0, 0x75, 0x0C, 0xC3], 11, &[0x31, 0xC0, 0xC3]),
            padded(&[0x85, 0xC0, 0x75, 0x06, 0xC3], 11, &[0x31, 0xC0, 0xC3]),
            padded(&[0xEB, 0x0E, 0xC3], 13, &[0x31, 0xC0, 0xC3]),
        ...
        for bytes in cases {
            let body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
            let end: u64 = 0x1000 + u64::try_from(bytes.len()).expect("fixture length fits");
            assert!(
                x86_boundary_seeds(&body, &[0x1000, end], 4, None).is_empty(),
                "the candidate inside {bytes:02x?} has no closed preceding function"
            );
        }
    }

    #[test]
    fn x86_boundary_seeds_require_contiguous_complete_instruction_bytes() {
        let bytes: Vec<u8> = padded(&[0x31, 0xC0, 0xC3], 13, &[0x31, 0xC0, 0xC3]);
        let mut body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
        let candidate: &mut DisasmInstruction = body
            .iter_mut()
            .find(|insn: &&mut DisasmInstruction| insn.offset == 0x1010)
            .expect("candidate instruction");
        candidate.bytes = vec![0x0F];
        assert!(x86_boundary_seeds(&body, &[0x1000, 0x1013], 4, None).is_empty());
        let mut body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
        body.get_mut(1).expect("second instruction").offset += 1;
        assert!(x86_boundary_seeds(&body, &[0x1000, 0x1013], 4, None).is_empty());
        let overflow: Vec<DisasmInstruction> = vec![DisasmInstruction {
            offset: u64::MAX - 1,
            bytes: vec![0x0F, 0x1F, 0x00],
            mnemonic: "nop".to_owned(),
            ..DisasmInstruction::default()
        }];
        assert!(x86_boundary_seeds(&overflow, &[u64::MAX - 1, u64::MAX], 4, None).is_empty());
    }

    #[test]
    fn isolated_import_tails_seed_only_mapped_thunks_within_the_budget() {
        let mut bytes: Vec<u8> = padded(&[0xC3], 15, &[0xFF, 0x25, 0xEA, 0x0F, 0, 0]);
        bytes.extend_from_slice(&[0x90, 0x90, 0xFF, 0x25, 0xE2, 0x0F, 0, 0, 0x90, 0x90]);
        let body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
        let sites: BTreeSet<u64> = x86_import_tail_sites(&body, &BTreeSet::from([0x2000]));
        assert_eq!(sites, BTreeSet::from([0x1010, 0x1018]));
        assert_eq!(
            x86_boundary_seeds(&body, &[0x1000, 0x1020], 2, Some(&sites)),
            vec![0x1010, 0x1018]
        );
        assert_eq!(
            x86_boundary_seeds(&body, &[0x1000, 0x1020], 1, Some(&sites)),
            vec![0x1010]
        );
        assert!(x86_boundary_seeds(&body, &[0x1000, 0x1020], 0, Some(&sites)).is_empty());
        let unknown: BTreeSet<u64> = x86_import_tail_sites(&body, &BTreeSet::from([0x2100]));
        assert!(unknown.is_empty());
        assert!(x86_boundary_seeds(&body, &[0x1000, 0x1020], 2, Some(&unknown)).is_empty());
    }

    #[test]
    fn import_thunk_candidates_refuse_fallthrough_branches_and_embedded_bytes() {
        let tail: [u8; 6] = [0xFF, 0x25, 0xEA, 0x0F, 0, 0];
        let cases: Vec<Vec<u8>> = vec![
            padded(&[0x31, 0xC0], 14, &tail),
            padded(&[0x75, 0x0E, 0xC3], 13, &tail),
            padded(&[0x75, 0x06, 0xC3], 13, &tail),
            padded(&[0xFF, 0xE0], 14, &tail),
            padded(
                &[0xC3],
                15,
                &[0x48, 0xB8, 0xFF, 0x25, 0xE8, 0x0F, 0, 0, 0, 0],
            ),
        ];
        for bytes in cases {
            let body: Vec<DisasmInstruction> = decoded_body(DisasmArch::X86_64, 0x1000, &bytes);
            ...
            let end: u64 = 0x1000 + u64::try_from(bytes.len()).expect("fixture length fits");
            assert!(
                x86_boundary_seeds(&body, &[0x1000, end], 2, Some(&sites)).is_empty(),
                "reachable or embedded candidate {bytes:02x?} is not a function"
            );
        }
    }

    #[test]
    fn import_tail_sites_require_whole_rip_jumps_and_checked_addresses() {
        let cases: [(u64, &[u8], u64); 10] = [
            (0x1000, &[0xFF, 0x25, 0], 0x1006),
            (0x1000, &[0xFF, 0x25, 0, 0, 0, 0, 0x90], 0x1006),
            (0x1000, &[0xFF, 0x15, 0, 0, 0, 0], 0x1006),
            (0x1000, &[0xFF, 0x2D, 0, 0, 0, 0], 0x1006),
            (0x1000, &[0x66, 0xFF, 0x2D, 0, 0, 0, 0], 0x1007),
            (0x1000, &[0x48, 0xFF, 0x2D, 0, 0, 0, 0], 0x1007),
            (0x1000, &[0x64, 0xFF, 0x25, 0, 0, 0, 0], 0x1007),
            (u64::MAX - 3, &[0xFF, 0x25, 0, 0, 0, 0], 2),
            (u64::MAX - 15, &[0xFF, 0x25, 0x20, 0, 0, 0], 22),
            (u64::MAX - 15, &[0xFF, 0x25, 8, 0, 0, 0], u64::MAX - 1),
        ];
        for (offset, bytes, slot) in cases {
            let body: [DisasmInstruction; 1] = [DisasmInstruction {
                offset,
                bytes: bytes.to_vec(),
                ..DisasmInstruction::default()
            }];
            assert!(x86_import_tail_sites(&body, &BTreeSet::from([slot])).is_empty());
        }
    }

    fn pe_with_import_tail() -> (Vec<u8>, Vec<DisasmInstruction>) {
        let mut text: Vec<u8> = vec![0; 0x200];
        let code: Vec<u8> = padded(&[0xC3], 15, &[0xFF, 0x25, 0x4A, 1, 0, 0]);
        text[..code.len()].copy_from_slice(&code);
        text[0x100..0x104].copy_from_slice(&0x1140_u32.to_le_bytes());
        text[0x10C..0x110].copy_from_slice(&0x1180_u32.to_le_bytes());
        text[0x110..0x114].copy_from_slice(&0x1160_u32.to_le_bytes());
        text[0x140..0x148].copy_from_slice(&0x1190_u64.to_le_bytes());
        text[0x160..0x168].copy_from_slice(&0x1190_u64.to_le_bytes());
        text[0x180..0x18C].copy_from_slice(b"fixture.dll\0");
        text[0x192..0x199].copy_from_slice(b"shared\0");
        let mut image: Vec<u8> = crate::test_support::pe64_with_text(&text, 0x1000);
        image[0x104..0x108].copy_from_slice(&16_u32.to_le_bytes());
        image[0x110..0x114].copy_from_slice(&0x1100_u32.to_le_bytes());
        image[0x114..0x118].copy_from_slice(&40_u32.to_le_bytes());
        let body: Vec<DisasmInstruction> = decoded_body(
            DisasmArch::X86_64,
            crate::test_support::pe64_text_base() + 0x1000,
            &code,
        );
        (image, body)
    }
    ...
    #[test]
    fn pe_import_tail_evidence_requires_a_complete_unwrapped_mapped_iat_slot() {
        let (image, body): (Vec<u8>, Vec<DisasmInstruction>) = pe_with_import_tail();
        let base: u64 = crate::test_support::pe64_text_base();
        assert_eq!(
            pe_import_tail_sites(&image, &body),
            BTreeSet::from([base + 0x1010])
        );
        let mut truncated_slot: Vec<u8> = image.clone();
        truncated_slot[0x190..0x194].copy_from_slice(&0x164_u32.to_le_bytes());
        assert!(pe_import_tail_sites(&truncated_slot, &body).is_empty());
        let mut unknown_slot: Vec<DisasmInstruction> = body.clone();
        unknown_slot.last_mut().expect("fixture thunk").bytes[2] = 0x52;
        assert!(pe_import_tail_sites(&image, &unknown_slot).is_empty());
        let mut unmapped_slot: Vec<u8> = image.clone();
        unmapped_slot[0x310..0x314].copy_from_slice(&0x3000_u32.to_le_bytes());
        assert!(pe_import_tail_sites(&unmapped_slot, &body).is_empty());
        let mut overlapping: Vec<u8> = image.clone();
        overlapping.copy_within(0x188..0x1B0, 0x1B0);
        overlapping[0x86..0x88].copy_from_slice(&2_u16.to_le_bytes());
        assert!(pe_import_tail_sites(&overlapping, &body).is_empty());
        let mut wrapped: Vec<u8> = image;
        wrapped[0xB0..0xB8].copy_from_slice(&(u64::MAX - 0x800).to_le_bytes());
        assert!(pe_import_tail_sites(&wrapped, &body).is_empty());
    }
    ...
    fn a_body_whose_addresses_overflow_yields_no_boundary() {
        ...
        assert_eq!(
            internal_boundary(X86_ISA, &model, 0, u64::MAX, &refs, None),
            None
        );
    }
    ...
    fn x86_32_prologue_elf() -> Vec<u8> {
        let mut obj: WriteObject<'_> =
            WriteObject::new(BinaryFormat::Elf, Architecture::I386, Endianness::Little);
        let text: object::write::SectionId = obj.section_id(StandardSection::Text);
        let _: u64 = obj.append_section_data(text, &[0x55, 0x89, 0xE5, 0xC3], 16);
        obj.write().expect("i386 elf write")
    }

    fn pdata_record(begin_rva: u32, end_rva: u32) -> [u8; 12] {
        let mut record: [u8; 12] = [0; 12];
        record[..4].copy_from_slice(&begin_rva.to_le_bytes());
        record[4..8].copy_from_slice(&end_rva.to_le_bytes());
        record
    }
    ...
    fn complete_noreturn_inference_keeps_discovered_function_symbols() {
        ...
        assert!(starts.iter().all(|start: &u64| {
            build
                .function_universe
                .requires_reference_corroboration(*start)
        }));
    }

    #[test]
    fn inferred_x86_32_prologue_requires_reference_corroboration() {
        let elf: Vec<u8> = x86_32_prologue_elf();
        let build: DisasmBuild =
            build_disasm_payload_with_discovery(&elf).expect("build i386 payload");
        assert_eq!(discovered_symbol_starts(&build.payload), vec![0]);
        assert!(build.function_universe.requires_reference_corroboration(0));
    }

    #[test]
    fn pdata_authority_requires_a_unique_executable_range() {
        const IMAGE_BASE: u64 = 0x0001_4000_0000;
        const TEXT_RVA: u32 = 0x1000;
        let bytes: [u8; 4] = [0x55, 0x89, 0xE5, 0xC3];
        let text: CodeWindow<'_> = CodeWindow {
            address: IMAGE_BASE + u64::from(TEXT_RVA),
            bytes: &bytes,
        };
        let text_address: u64 = text.address;
        let code: [CodeWindow<'_>; 1] = [text];
        let windows: Vec<(u64, u64)> =
            x86_executable_windows(&code).expect("one non-overlapping executable window");
        let instructions: Vec<DisasmInstruction> =
            decoded_body(DisasmArch::X86_64, text_address, &bytes);
        let valid: [u8; 12] = pdata_record(TEXT_RVA, TEXT_RVA + 4);
        assert_eq!(
            validated_pdata_records(&valid, IMAGE_BASE, &windows, &instructions),
            vec![text_address]
        );

        let mut malformed: Vec<u8> = Vec::new();
        malformed.extend_from_slice(&pdata_record(TEXT_RVA, TEXT_RVA + 4));
        malformed.extend_from_slice(&pdata_record(TEXT_RVA, TEXT_RVA + 4));
        malformed.extend_from_slice(&pdata_record(TEXT_RVA + 8, TEXT_RVA + 12));
        malformed.extend_from_slice(&pdata_record(TEXT_RVA + 4, TEXT_RVA));
        assert!(
            validated_pdata_records(&malformed, IMAGE_BASE, &windows, &instructions,).is_empty(),
            "duplicate, out-of-range, and reversed runtime-function records cannot establish authority"
        );
    }
    ...
    #[test]
    fn incomplete_noreturn_inference_keeps_linked_elf_functions_corroborated() {
        let mut elf: Vec<u8> = bounded_noreturn_chain_elf(65);
        elf.get_mut(16..18)
            .expect("ELF type field")
            .copy_from_slice(&2u16.to_le_bytes());
        let parsed: object::File<'_> = object::File::parse(&*elf).expect("parse linked ELF");
        ...
        assert_eq!(
            starts.len(),
            65,
            "linked ELF retains bounded discovery starts"
        );
        assert!(starts.contains(&0x10));
        assert!(starts.contains(&(0x10 + 64 * 6)));
        assert!(
            build
                .function_universe
                .requires_reference_corroboration(0x10)
        );
        assert!(
            build
                .function_universe
