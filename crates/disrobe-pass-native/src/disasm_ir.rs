...
pub(crate) fn build_disasm_payload_with_discovery(bytes: &[u8]) -> Result<DisasmBuild> {
    ...
    } else if matches!(arch, DisasmArch::X86_64) && !discovered_starts.is_empty() {
...
fn inject_discovered_functions(
    ...
    let mut boundary_starts: BTreeSet<u64> = BTreeSet::new();
    let mut incomplete_noreturn: bool = false;
    let discovered: DiscoveredFunctions = if matches!(arch, DisasmArch::Aarch64) {
        ...
        let (mut discovered, incomplete): (DiscoveredFunctions, bool) = match outcome.into_parts() {
            ...
            && matches!(native.format, NativeFormat::Elf64 | NativeFormat::Pe64)
        {
            ...
            if !boundary_seeds.is_empty() {
                boundary_starts.extend(boundary_seeds.iter().copied());
    ...
    for start in discovered.starts {
        ...
        if retain_provenance || boundary_starts.contains(&start) {
