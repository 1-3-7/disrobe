...
pub fn extract_function_features(bytes: &[u8]) -> Result<Vec<FunctionFeatures>> {
    ...
    for body in bodies {
        ...
        let carried: FunctionFeatures =
            if function_universe.requires_reference_corroboration(body.address) {
                carried.requiring_reference_corroboration()
            } else {
                carried
            };
...
fn x86_blocks(
    ...
    let mut transfers: Vec<Transfer> = decoded
        .iter()
        .map(|slot: &Option<Instruction>| {
            slot.as_ref()
                .map_or(Transfer::Unresolved, |insn: &Instruction| {
                    x86_transfer(insn, tail)
                })
        })
        .collect();
    fold_constant_conditions(decoded, &mut transfers);
...
fn x86_structure(
    ...
    let mut out: Vec<StructureBlock> = Vec::with_capacity(blocks.len());
    for block in blocks {
        let span: &[Option<Instruction>] = decoded.get(block.insns.clone())?;
        let mut categories: Vec<InstructionCategory> = Vec::with_capacity(span.len());
        for slot in span {
            let instruction: &Instruction = slot.as_ref()?;
            if instruction.mnemonic() != Mnemonic::Nop {
                categories.push(x86_category(instruction, &mut factory));
            }
        }
        out.push(StructureBlock::new(
            block.successors.iter().copied(),
...
fn x86_immediate_reference(decoded: &Instruction, index: &ImageIndex) -> Option<DataReference> {
    if has_frame_register_operand(decoded) {
        return None;
    }
...
fn has_frame_register_operand(decoded: &Instruction) -> bool {
    (0..decoded.op_count()).any(|operand: u32| {
        decoded.op_kind(operand) == OpKind::Register
            && is_frame_register(decoded.op_register(operand))
    })
}
