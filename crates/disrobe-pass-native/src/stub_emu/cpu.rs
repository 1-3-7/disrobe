...
use crate::stub_emu::mem::{Memory, PAGE_SIZE, Perm};
...
impl Cpu {
    ...
    pub fn enable_seh_dispatch(&mut self) {
        ...
        let _ = self
            .mem
            .map(Self::SEH_CONTINUE_SENTINEL, PAGE_SIZE as u64, Perm::RX);
    }

    const NULL_GUARD_LIMIT: u64 = PAGE_SIZE as u64;
    ...
    const CTX_ESI: u64 = 0xA0;
    const CTX_EBX: u64 = 0xA4;
    const CTX_EDX: u64 = 0xA8;
    const CTX_ECX: u64 = 0xAC;
    const CTX_EAX: u64 = 0xB0;
    const CTX_EBP: u64 = 0xB4;
    const CTX_EIP: u64 = 0xB8;
    const CTX_EFLAGS: u64 = 0xC0;
    const CTX_ESP: u64 = 0xC4;
    const STATUS_BREAKPOINT: u32 = 0x8000_0003;
    const STATUS_ACCESS_VIOLATION: u32 = 0xC000_0005;

    const REP_ITER_CAP: u64 = 16_000_000;
    ...
    pub fn run<H: HostCall>(&mut self, host: &mut H, step_cap: u64) -> Result<ExitReason> {
        ...
        let result: ExitReason = loop {
            ...
            if self.seh_dispatch && ip == Self::SEH_CONTINUE_SENTINEL {
                match self.seh_continue_from_sentinel() {
                    Some(()) => continue,
                    None => {
                        break ExitReason::GuestFault("seh: continue without context".to_owned());
                    }
                }
            }
            ...
            self.mem.read_lossy_into(ip, 16, &mut fetch_buf);
            let mut decoder: Decoder<'_> =
                Decoder::with_ip(self.mode.bits(), &fetch_buf, ip, DecoderOptions::NONE);
            ...
            match self.execute_one(&insn, host) {
                ...
                Ok(None) => {}
                Err(e) => {
                    if self.seh_dispatch && self.dispatch_seh(ip, Self::STATUS_ACCESS_VIOLATION) {
                        continue;
                    }
    ...
    fn dispatch_seh(&mut self, resume_ip: u64, exception_code: u32) -> bool {
    ...
    fn enter_structured_handler(
        ...
        if self
            .write_exception_record(exc_rec, exception_code, resume_ip)
            .is_err()
        {
            return false;
        }
        ...
        let ok: bool = push(self, &mut sp, dispatcher)
            ...
            && push(self, &mut sp, Self::SEH_CONTINUE_SENTINEL);
        if !ok {
            return false;
        }
    ...
    fn write_context_record(&mut self, ctx: u64, resume_ip: u64, saved_esp: u64) -> Result<()> {
        for off in (0..Self::CONTEXT_SIZE).step_by(4) {
            self.mem.write_u32(ctx + off, 0)?;
        }
        self.mem.write_u32(ctx, 0x0001_0007)?;
        self.mem
            .write_u32(ctx + Self::CTX_EDI, self.regs.get(Reg::Rdi) as u32)?;
        self.mem
            .write_u32(ctx + Self::CTX_ESI, self.regs.get(Reg::Rsi) as u32)?;
        self.mem
            .write_u32(ctx + Self::CTX_EBX, self.regs.get(Reg::Rbx) as u32)?;
        self.mem
            .write_u32(ctx + Self::CTX_EDX, self.regs.get(Reg::Rdx) as u32)?;
        self.mem
            .write_u32(ctx + Self::CTX_ECX, self.regs.get(Reg::Rcx) as u32)?;
        self.mem
            .write_u32(ctx + Self::CTX_EAX, self.regs.get(Reg::Rax) as u32)?;
        self.mem
            .write_u32(ctx + Self::CTX_EBP, self.regs.get(Reg::Rbp) as u32)?;
        self.mem.write_u32(ctx + Self::CTX_EIP, resume_ip as u32)?;
        self.mem
            .write_u32(ctx + Self::CTX_EFLAGS, self.encode_flags() as u32)?;
        self.mem.write_u32(ctx + Self::CTX_ESP, saved_esp as u32)?;
        Ok(())
    }
    ...
    fn write_exception_record(
        ...
        self.mem.write_u32(exc_rec, exception_code)?;
        self.mem.write_u32(exc_rec + 4, 0)?;
        self.mem.write_u32(exc_rec + 8, 0)?;
        self.mem.write_u32(exc_rec + 12, resume_ip as u32)?;
        self.mem.write_u32(exc_rec + 16, 0)?;
        Ok(())
    }
    ...
    fn load_context_record(&mut self, ctx: u64) {
        let read =
            |cpu: &Self, off: u64| -> u64 { u64::from(cpu.mem.read_u32(ctx + off).unwrap_or(0)) };
        self.regs
            .write_sized(Reg::Rdi, read(self, Self::CTX_EDI), 32);
        self.regs
            .write_sized(Reg::Rsi, read(self, Self::CTX_ESI), 32);
        self.regs
            .write_sized(Reg::Rbx, read(self, Self::CTX_EBX), 32);
        self.regs
            .write_sized(Reg::Rdx, read(self, Self::CTX_EDX), 32);
        self.regs
            .write_sized(Reg::Rcx, read(self, Self::CTX_ECX), 32);
        self.regs
            .write_sized(Reg::Rax, read(self, Self::CTX_EAX), 32);
        self.regs
            .write_sized(Reg::Rbp, read(self, Self::CTX_EBP), 32);
        self.regs
            .write_sized(Reg::Rsp, read(self, Self::CTX_ESP), 32);
        self.decode_flags(read(self, Self::CTX_EFLAGS));
        self.regs.rip = read(self, Self::CTX_EIP);
    }

    fn raise_software_exception(&mut self, insn: &Instruction) -> bool {
        let resume_ip: u64 = match Self::software_exception_resume(insn) {
            Some(ip) => ip,
            None => return false,
        };
        self.dispatch_seh(resume_ip, Self::STATUS_BREAKPOINT)
    }

    fn software_exception_resume(insn: &Instruction) -> Option<u64> {
        match insn.code() {
            Code::Int3 | Code::Int_imm8 | Code::Int1 => Some(insn.next_ip()),
            Code::Ud0_r16_rm16
            | Code::Ud0_r32_rm32
            | Code::Ud0_r64_rm64
            | Code::Ud1_r16_rm16
            | Code::Ud1_r32_rm32
            | Code::Ud1_r64_rm64
            | Code::Ud2 => Some(insn.ip()),
            _ => None,
        }
    }

    fn is_executable_target(&self, addr: u64) -> bool {
    ...
    fn execute_one<H: HostCall>(
        ...
        match insn.flow_control() {
            ...
            FlowControl::Return => {
                let ret: u64 = self.pop()?;
                if !self.is_executable_target(ret) {
                    return Ok(Some(ExitReason::JumpedOutOfRange {
                        from: insn.ip(),
                        to: ret,
                    }));
            ...
            FlowControl::Interrupt | FlowControl::XbeginXabortXend | FlowControl::Exception => {
                if self.seh_dispatch && self.raise_software_exception(insn) {
                    return Ok(None);
                }
        ...
        if let Some(truncated) = self.try_string_op(insn)? {
            if truncated {
                return Ok(Some(ExitReason::RepLimit(Self::REP_ITER_CAP)));
            }
            return Ok(None);
        }
    ...
    fn try_string_op(&mut self, insn: &Instruction) -> Result<Option<bool>> {
        ...
        match mnem {
            M::Movsb | M::Movsw | M::Movsd | M::Movsq => {
                ...
                let step: u64 = (dir * stride) as u64;
                let do_one = |cpu: &mut Self| -> Result<()> {
                    ...
                    cpu.regs.write_sized(Reg::Rsi, si.wrapping_add(step), bits);
                    cpu.regs.write_sized(Reg::Rdi, di.wrapping_add(step), bits);
                    Ok(())
                };
                let truncated: bool = self.run_string(has_rep, count_ptr_bits, do_one, None)?;
                Ok(Some(truncated))
            }
...
mod tests {
    ...
    #[test]
    fn int3_under_seh_dispatch_transfers_to_handler() {
        const TEB: u64 = 0x7EFD_E000;
        const HANDLER: u64 = 0x0050_0000;
        let mut cpu: Cpu = Cpu::new(CpuMode::Bits32);
        cpu.enable_seh_dispatch();
        ...
        cpu.mem.write_u32(frame + 4, HANDLER as u32).unwrap();
        cpu.mem.write_u32(TEB, frame as u32).unwrap();
        cpu.mem.write_unchecked(0x4000, &[0xCC]);
        cpu.mem.write_unchecked(HANDLER, &[0xEB, 0xFE]);
        cpu.regs.rip = 0x4000;
        cpu.regs.set(Reg::Rsp, 0x2_3F00);
        let mut host: NoopHost = NoopHost;
        ...
        assert!(
            matches!(exit, ExitReason::StepCap(_)),
            "INT3 under SEH dispatch must route to the registered handler, not halt; got {exit:?}",
        );
        assert_eq!(
            cpu.regs.rip, HANDLER,
    ...
    #[test]
    fn int3_without_seh_dispatch_halts_as_unsupported() {
        let mut cpu: Cpu = Cpu::new(CpuMode::Bits32);
        cpu.mem
            .map(0x4000, 0x1000, Perm::RWX)
            .expect("test map within ceiling");
        cpu.mem.write_unchecked(0x4000, &[0xCC]);
        cpu.regs.rip = 0x4000;
        cpu.regs.set(Reg::Rsp, 0x4F00);
        let mut host: NoopHost = NoopHost;
        let exit: ExitReason = cpu.run(&mut host, 10).unwrap();
        assert!(
            matches!(exit, ExitReason::UnsupportedInstr { .. }),
            "without SEH dispatch an INT3 must surface for the caller, never be silently swallowed; got {exit:?}",
        );
    }
    ...
    #[test]
    fn rep_stosb_with_huge_rcx_halts_at_iteration_cap() {
        const BUF: u64 = 0x10_0000;
        let buf_len: u64 = Cpu::REP_ITER_CAP + 0x1000;
        let prog: [u8; 3] = [0xF3, 0xAA, 0xCC];
        let mut cpu: Cpu = Cpu::new(CpuMode::Bits32);
        cpu.mem
            .map(0x100, 0x1000, Perm::RWX)
            .expect("test map within ceiling");
        cpu.mem.write_unchecked(0x100, &prog);
        cpu.mem
            .map(BUF, buf_len, Perm::RW)
            .expect("test map within ceiling");
        cpu.regs.rip = 0x100;
        cpu.regs.set(Reg::Rdi, BUF);
        cpu.regs.write_sized(Reg::Rax, 0xAB, 8);
        cpu.regs.write_sized(Reg::Rcx, u64::from(u32::MAX), 32);
        let mut host: NoopHost = NoopHost;
        let exit: ExitReason = cpu.run(&mut host, u64::MAX).unwrap();
        assert!(
            matches!(exit, ExitReason::RepLimit(Cpu::REP_ITER_CAP)),
            "an attacker-controlled REP STOSB with RCX = u32::MAX must abandon the op at the \
             per-instruction iteration ceiling rather than running ~4.3 billion inner \
             iterations; got {exit:?}",
        );
        let remaining: u64 = cpu.regs.read_sized(Reg::Rcx, 32);
        let consumed: u64 = u64::from(u32::MAX) - remaining;
        assert_eq!(
            consumed,
            Cpu::REP_ITER_CAP,
            "exactly the iteration ceiling worth of bytes must have been written before the \
             emulator gave up, never the full attacker-supplied count",
        );
        assert!(
            remaining > 0,
            "the REP must have been truncated mid-loop with RCX still nonzero",
        );
    }

    #[test]
    fn rep_stosb_with_small_rcx_completes_fully() {
        const BUF: u64 = 0x10_0000;
        let prog: [u8; 3] = [0xF3, 0xAA, 0xCC];
        let mut cpu: Cpu = Cpu::new(CpuMode::Bits32);
        cpu.mem
        ...
        cpu.mem.write_unchecked(0x100, &prog);
        cpu.mem
            .map(BUF, 0x1000, Perm::RW)
            .expect("test map within ceiling");
        cpu.regs.rip = 0x100;
        cpu.regs.set(Reg::Rdi, BUF);
        cpu.regs.write_sized(Reg::Rax, 0xAB, 8);
        cpu.regs.write_sized(Reg::Rcx, 8, 32);
        let mut host: NoopHost = NoopHost;
        let _exit: ExitReason = cpu.run(&mut host, 50_000).unwrap();
        assert_eq!(
            cpu.regs.read_sized(Reg::Rcx, 32),
            0,
            "a legit short REP STOSB well within budget must run to completion (RCX = 0)",
        );
        assert_eq!(
    ...
    fn btr_clears_bit_and_sets_carry() {
        ...
        assert_eq!(
            cpu.regs.read_sized(Reg::Rax, 32),
            0x0D,
            "btr eax,1 must clear bit 1 of 0x0F â†’ 0x0D"
        );
        assert!(
            cpu.regs.flags.cf,
    ...
    fn fs_segment_read_resolves_to_teb_base() {
        ...
        let mut cpu: Cpu = build_cpu_with(&prog, 0x100);
        cpu.mem
            .map(0x7EFD_E000, 0x1000, Perm::RW)
            .expect("test map within ceiling");
