...
pub enum ExitReason {
    ...
    StepCap(u64),
    /// A REP-prefixed string instruction exceeded [`Cpu::REP_ITER_CAP`] inner iterations.
    RepLimit(u64),
...
/// Callback the emulator invokes for calls into a host-managed Win32 import shim.
pub trait HostCall {
    /// Dispatches a call to an unmapped target; returns `Ok(true)` to continue or `Ok(false)` to halt.
...
impl Cpu {
    ...
    /// Enables structured-exception-handling dispatch on guest memory faults.
    ...
    /// Inner-iteration ceiling for a single REP-prefixed string instruction.
    const REP_ITER_CAP: u64 = 16_000_000;

    /// Sets the linear base address added by an `fs:`-prefixed memory operand.
    pub fn set_fs_base(&mut self, base: u64) {
        self.fs_base = base;
    }

    /// Sets the linear base address added by a `gs:`-prefixed memory operand.
    ...
    /// Dispatches a guest access violation to the current Win32 SEH handler.
    ...
    /// Dispatches the `MOVS`/`STOS`/`LODS`/`CMPS`/`SCAS` family, returning `Some(truncated)` for string ops.
    ...
    /// Executes a REP-able string store/move/load op, returning `Ok(true)` on hitting [`Self::REP_ITER_CAP`].
    ...
    /// Executes a REP-able string compare/scan op, honouring `REPE`/`REPNE` zero-flag termination.
...
mod tests {
    ...
    fn rep_stosb_with_huge_rcx_halts_at_iteration_cap() {
        ...
        let mut cpu: Cpu = Cpu::new(CpuMode::Bits32);
        cpu.mem.map(0x100, 0x1000, Perm::RWX);
        cpu.mem.write_unchecked(0x100, &prog);
        cpu.mem.map(BUF, buf_len, Perm::RW);
        cpu.regs.rip = 0x100;
    ...
    fn rep_stosb_with_small_rcx_completes_fully() {
        ...
        cpu.mem.map(0x100, 0x1000, Perm::RWX);
        cpu.mem.write_unchecked(0x100, &prog);
        cpu.mem.map(BUF, 0x1000, Perm::RW);
        cpu.regs.rip = 0x100;
        cpu.regs.set(Reg::Rdi, BUF);
        cpu.regs.write_sized(Reg::Rax, 0xAB, 8);
