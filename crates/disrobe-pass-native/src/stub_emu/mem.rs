...
/// Single-page null guard never lazily committed, so a write to address 0 faults.
const NULL_RESERVED_REGION: u64 = PAGE_SIZE as u64;

/// Per-page read/write/execute permission bits.
...
impl Memory {
    ...
    /// Enables bounded lazy commit of zero-filled `RW` pages on demand, capped at `max_pages`.
    pub fn enable_lazy_commit(&mut self, max_pages: u32) {
        self.lazy_budget = Some(max_pages);
    }

    /// Refuses to lazily commit the null page so a deliberate write to address 0 faults.
    ...
    /// Reads a contiguous range across pages, returning zero for any unmapped or unreadable byte.
    #[must_use]
