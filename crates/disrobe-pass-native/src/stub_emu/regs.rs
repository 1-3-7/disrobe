...
/// Logical GPR identifier keyed by the canonical 64-bit name.
...
impl Regs {
    ...
    /// Writes a value into a logical GPR honouring the operand size, zero-extending 32-bit writes.
...
/// Translates an iced-x86 `Register` into a logical (`Reg`, size_bits, is_high_byte) tuple.
#[must_use]
...
/// Translates an iced-x86 `Register` into the MMX register file index `0..8`, else `None`.
#[must_use]
