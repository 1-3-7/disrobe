//! Register file + EFLAGS / RFLAGS model for the stub emulator.

use iced_x86::Register;

/// Native pointer width of the emulated guest.
...
/// Logical GPR identifier. We unify x86-32 (8 GPRs) and x86-64 (16 GPRs) into
/// one enum keyed by the canonical 64-bit name; 32-bit code only ever indexes
/// the first eight entries.
...
impl Regs {
    ...
    /// Write a value into a logical GPR honouring the operand size: writing
    /// 32-bit zero-extends to 64 bits (per AMD64 ABI); writing 8 or 16 bits
    /// preserves the upper bytes.
    ...
    /// Read a sized slice of a logical GPR.
    #[must_use]
    ...
    /// Read a high-byte GPR slice (AH/BH/CH/DH).
    #[must_use]
    pub fn read_high8(&self, r: Reg) -> u64 {
        (self.gpr[r as usize] >> 8) & 0xFF
    }

    /// Write a high-byte GPR slice (AH/BH/CH/DH).
...
/// Translate an iced-x86 `Register` into our logical (`Reg`, size_bits)
/// pair, plus a flag indicating it's a high-byte slice (AH/BH/CH/DH).
#[must_use]
