...
/// Extracts the trait name from a trait-impl symbol in either mangling encoding.
...
/// Index of the `>` that closes the caller's already-consumed `<`, honouring nesting.
...
/// The generic-origin stem of a monomorphized symbol, or `None` without generic arguments.
