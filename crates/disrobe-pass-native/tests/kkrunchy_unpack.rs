...
const K7_MEASURED_FLOOR_BP: u32 = 10_000;
...
fn kkrunchy_packer_status_is_implemented_classic_floor_verified() {
    ...
    assert_eq!(
        ...
        "both the classic 0.23a and the k7 0.23a2 depackers reconstruct the OEP image byte-exact vs the \
         independent pre-packed hello.exe original (100.00% measured by classic_cca_recovers_real_fixture_payload \
         and test_kkrunchy_hello_byte_recovery), by replaying the real depacker stub through the in-house \
         stub_emu interpreter; neither is a fake unpack.",
    );
    ...
    assert_eq!(
        ...
        "k7 ceiling is the verified 100.00% floor: the stub_emu whole-stub replay reaches the OEP and \
         reconstructs the original byte-exact",
    );
    assert_eq!(
...
fn detect_and_parse_real_kkrunchy_k7_fixture() {
    ...
    assert!(
        out.note.contains("DisFilter") || out.note.contains("dis_unfilter"),
        "note must document the DisFilter inverse path",
    );
    assert!(
...
#[test]
fn unknown_kkrunchy_prologue_still_tries_classic_stream_decoder() {
    let mut packed: Vec<u8> = HELLO_PACKED_CLASSIC.to_vec();
    let original_header: KkrunchyHeaderInfo =
        parse_kkrunchy_header(&packed).expect("classic header parse");
    let stub_off: usize = original_header.section_raw_offset as usize;
    packed[stub_off] = 0x90;

    let mutated_header: KkrunchyHeaderInfo =
        parse_kkrunchy_header(&packed).expect("mutated header parse");
    assert_eq!(mutated_header.variant, KkrunchyVariant::UnknownVersion);

    let out: KkrunchyUnpackOutput = unpack_kkrunchy(&packed).expect("unknown variant recovery");
    assert_eq!(out.header.variant, KkrunchyVariant::UnknownVersion);
    assert!(
        out.note.contains("CCA range-coder stream"),
        "unknown prologue must still try the structural classic stream decoder (got: {})",
        out.note,
    );

    let marker: &[u8] = b"kernel32.dll";
    assert!(
        out.packed_payload
            .windows(marker.len())
            .any(|w: &[u8]| w == marker),
        "decoded payload must recover the real import marker",
    );
    assert!(
        !packed.windows(marker.len()).any(|w: &[u8]| w == marker),
        "the packed fixture must not contain the import marker verbatim",
    );
}
...
fn test_kkrunchy_hello_byte_recovery() {
    ...
    let out: KkrunchyUnpackOutput = unpack_kkrunchy(packed).expect("k7 stub-emu unpack");
    let report: KkrunchyByteRecoveryReport = compute_byte_recovery(original, &out.packed_payload);
    eprintln!(
        "kkrunchy k7 byte recovery vs 1024 B original: {} / {} matching ({:.2}%) [recovered_len={}] \
         -- the recovered payload is the on-disk OEP image reconstructed by replaying the real k7 \
         depacker stub through the in-house stub_emu interpreter (MMX context-mixing range decoder + \
         LZ + DisFilter inverse) to the original entry point and rebuilding the import table from the \
         descriptor the depacker decodes into its staging area.",
        report.matching_bytes,
        report.original_len,
        report.pct(),
        report.recovered_len,
    ...
    assert!(
        report.recovery_pct_basis_points >= K7_MEASURED_FLOOR_BP,
        "k7 byte recovery regressed below the measured floor: {:.2}% < {:.2}% \
         (the real k7 depacker stub is driven through stub_emu to the OEP and the OEP image is rebuilt \
         byte-exact; a regression here means the interpreter, the OEP detection, the import rebuild, or \
         the PE reconstruction broke)",
        report.pct(),
    ...
    assert!(
        out.note.contains("stub_emu")
            && out.note.contains("OEP")
            && out.note.contains("context-mixing"),
        "k7 note must state the image is reconstructed by replaying the real depacker stub through \
         stub_emu to the OEP, never claim a fabricated image (got: {})",
        out.note,
    );
    ...
    for marker in real_import_markers {
        ...
        assert!(
            !in_packed,
            "the import name {:?} must NOT appear verbatim in the packed input: its presence in the \
             recovered image can only come from decoding the compressed range stream, never a copy",
            std::str::from_utf8(marker).unwrap_or("<bin>"),
        );
    }
}
