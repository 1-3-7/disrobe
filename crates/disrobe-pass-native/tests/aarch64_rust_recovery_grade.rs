...
#[test]
fn recorded_extended_register_cases_have_shared_rust_driver_descriptors() {
    let rows: Vec<(&str, &str, &[u8])> = CASES
        .iter()
        .copied()
        .filter(|(opt, _, _): &(&str, &str, &[u8])| *opt == "NAT001")
        .collect();
    assert_eq!(rows.len(), 5);
    for (opt, name, bytes) in rows {
        assert_eq!(expected_arity(name), Some(2), "{opt}/{name}");
        let recovery: LeafRecovery = recover_aarch64_function(bytes, 0)
            .unwrap_or_else(|error| panic!("{opt}/{name} rejected: {error}"));
        let rust_source: &str = recovery.rust_source.as_deref().expect("Rust rendering");
        let signature: Signature = parse_rust_signature(rust_source).expect("Rust signature");
        assert_eq!(signature.params.len(), 2, "{opt}/{name}");
        let symbol: String = format!("rec_{opt}_{name}");
        let block: String = compare_block(opt, name, &symbol, case_seed(opt, name))
            .unwrap_or_else(|| panic!("{opt}/{name} has no shared driver block"));
        assert!(block.contains(&format!("{symbol}(lhs, rhs)")), "{block}");
    }
}
