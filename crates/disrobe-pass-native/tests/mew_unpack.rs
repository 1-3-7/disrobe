...
/// Best byte-match percentage of `recovered` against `original` over candidate leading offsets.
...
/// Asserts the MEW byte-recovery against the original on a real fixture meets `floor`.
...
/// Asserts the MEW-recovered `.text` is byte-identical to the original file image.
