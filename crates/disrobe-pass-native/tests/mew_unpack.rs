...
fn run_va_content_recovery_test(name: &str, content_floor: f64, rdata_floor: f64) {
    use disrobe_pass_native::packers::section_recovery::{
        GranuleRecovery, SectionRecoveryReport, section_recovery_report,
    };
    const MEW_OUTPUT_RVA: usize = 0x1000;
    let packed: &[u8] = fixture(name);
    let original_name: String = name.replace(".packed.mew.exe", ".original.exe");
    let original: &[u8] = fixture(&original_name);
    let out: MewUnpackOutput = unpack_mew(packed).expect("MEW unpack must succeed");
    assert!(out.stream_decoded, "{name}: stream must decode");
    let mut va_image: Vec<u8> = vec![0u8; MEW_OUTPUT_RVA + out.raw_image.len()];
    let header_len: usize = MEW_OUTPUT_RVA.min(packed.len());
    va_image[..header_len].copy_from_slice(&packed[..header_len]);
    va_image[MEW_OUTPUT_RVA..MEW_OUTPUT_RVA + out.raw_image.len()].copy_from_slice(&out.raw_image);
    let report: SectionRecoveryReport =
        section_recovery_report(original, &va_image, &[]).expect("VA report");
    let text: &GranuleRecovery = report
        .sections
        .iter()
        .find(|s: &&GranuleRecovery| s.name == ".text")
        .expect(".text row");
    let rdata: Option<&GranuleRecovery> = report
        .sections
        .iter()
        .find(|s: &&GranuleRecovery| s.name == ".rdata");
    println!(
        "{name}: VA-aligned .text {:.2}% .rdata {:.2}% CONTENT {:.2}%",
        text.recovery_pct(),
        rdata.map_or(0.0, GranuleRecovery::recovery_pct),
        report.content_recovery_pct(),
    );
    assert!(
        text.is_byte_identical(),
        "{name}: .text must be byte-identical in the VA image; got {:.2}%",
        text.recovery_pct(),
    );
    assert!(
    ...
    if let Some(rd) = rdata {
        assert!(
            rd.recovery_pct() >= rdata_floor,
            "{name}: .rdata recovery {:.2}% below floor {rdata_floor:.1}%; the residual is the \
             runtime-bound import directory (IAT/ILT thunks the MEW loader rebuilds), absent from \
             the static stream",
            rd.recovery_pct(),
        );
    }
...
#[test]
fn test_mew_accessenum_va_content_recovery() {
    run_va_content_recovery_test("AccessEnum.packed.mew.exe", 93.0, 55.0);
}

#[test]
fn test_mew_autologon_va_content_recovery() {
    run_va_content_recovery_test("Autologon.packed.mew.exe", 96.0, 92.0);
}

#[test]
fn test_mew_clockres_va_content_recovery() {
    run_va_content_recovery_test("Clockres.packed.mew.exe", 97.0, 94.0);
}
