...
fn test_petite_hello32_round_trip() {
    let packed: Vec<u8> = read_fixture("hello.exe");
    let baseline: Vec<u8> = read_fixture("hello.original.exe");
    assert!(
...
fn test_petite_dircmp_byte_recovery() {
    let packed: Vec<u8> = read_fixture("megafile_DirCmp.exe");
    let result: PetiteUnpackResult =
...
fn test_petite_hello32_phase2_byte_recovery_beats_static() {
    ...
    let recovered: &Vec<u8> = &result.recovered_image;
    let smaller: usize = baseline.len().min(recovered.len());
    let matching: usize = baseline
        .iter()
        .zip(recovered.iter())
    ...
    let pct_x100: u64 = (matching as u64).saturating_mul(10_000) / baseline.len() as u64;
    println!(
        "petite phase-2 byte-recovery hello32: matching={matching} ({:.2}%)",
        pct_x100 as f64 / 100.0
    );
    assert!(
        pct_x100 >= 5000,
        "phase-2 emulated unpack must achieve >=50% byte recovery on hello32 \
         (v0.8 W4B static baseline was 16.91%); got {:.2}%",
        pct_x100 as f64 / 100.0
    );
}
...
fn test_petite_dircmp_phase2_emulated_smoke() {
    ...
    let result: PetitePhase2EmulatedOutput =
        unpack_petite_phase2_emulated(&packed).expect("phase-2 emu must produce output");
    println!(
        "petite phase-2 emu DirCmp: exit={} oep={:?} host_calls={} image_size={} recovered_len={}",
        result.exit_reason,
        result.oep_estimate,
        result.host_calls.len(),
        result.size_of_image,
    ...
    assert!(
        !result.recovered_image.is_empty(),
        "DirCmp phase-2 emu must produce non-empty image (even on early fault — DirCmp uses a Petite variant with FS:0 SEH setup that the v0.9 A2 emulator does not model; tracked for v0.9 A3)"
    );
    assert!(
        result.recovered_image.starts_with(b"MZ"),
