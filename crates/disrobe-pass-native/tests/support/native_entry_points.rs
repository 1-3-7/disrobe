...
const fn naming_subject<'a>(ctx: &Ctx<'a>) -> [sig_engine::FunctionNameSubject<'a>; 1] {
    [sig_engine::FunctionNameSubject {
        name: "sub_1000",
        address: 0x1000,
        code: ctx.bytes,
    }]
...
fn program_function(ctx: &Ctx<'_>) -> [pseudo_c::ProgramFunction; 1] {
    [pseudo_c::ProgramFunction {
        name: "probe".to_owned(),
        address: 0x1000,
...
pub(crate) const ENTRY_POINTS: &[Entry] = &[
    ...
    Entry {
        path: "cxx_recovery::recover_itanium_exception_regions",
        cheap: false,
        drive: |ctx: &Ctx<'_>| match cxx_recovery::recover_itanium_exception_regions(ctx.bytes) {
            Ok(regions) => bounded_len(regions.len(), ctx),
            Err(error) => Verdict::Failed(error.to_string()),
        },
    },
    Entry {
    ...
    Entry {
        path: "sig_engine::assert_fail_function_names",
        cheap: false,
        drive: |ctx: &Ctx<'_>| {
            bounded_len(
                sig_engine::assert_fail_function_names(ctx.bytes, &naming_subject(ctx)).len(),
                ctx,
            )
        },
    },
    Entry {
        path: "sig_engine::exported_function_names",
        cheap: false,
        drive: |ctx: &Ctx<'_>| {
            bounded_len(
                sig_engine::exported_function_names(ctx.bytes, &naming_subject(ctx)).len(),
                ctx,
            )
        },
    },
    Entry {
        path: "sig_engine::resolved_import_thunk_names",
        cheap: false,
        drive: |ctx: &Ctx<'_>| {
            bounded_len(
                sig_engine::resolved_import_thunk_names(ctx.bytes, &naming_subject(ctx)).len(),
                ctx,
            )
        },
    },
    ...
    Entry {
        ...
        cheap: false,
        drive: |ctx: &Ctx<'_>| {
            let functions: [pseudo_c::ProgramFunction; 1] = program_function(ctx);
    ...
    Entry {
        path: "pseudo_c::recover_program_with_naming",
        cheap: false,
        drive: |ctx: &Ctx<'_>| {
            let recovered: pseudo_c::NamedRecoveredProgram = pseudo_c::recover_program_with_naming(
                ctx.bytes,
                &program_function(ctx),
                Abi::MsX64,
            );
            bounded_len(
                recovered.program.recovered.len()
                    + recovered.program.unrecovered.len()
                    + recovered.names.len(),
                ctx,
            )
        },
    ...
    Entry {
        path: "pseudo_c::recover_program_with_sibling_extern_policy",
        cheap: false,
        drive: |ctx: &Ctx<'_>| {
            for policy in [
                pseudo_c::SiblingExternPolicy::DropResolved,
                pseudo_c::SiblingExternPolicy::Retain,
            ] {
                let recovered: pseudo_c::RecoveredProgram =
                    pseudo_c::recover_program_with_sibling_extern_policy(
                        ctx.bytes,
                        &program_function(ctx),
                        Abi::MsX64,
                        policy,
                    );
                let _ = bounded_len(recovered.recovered.len() + recovered.unrecovered.len(), ctx);
            }
            Verdict::NotFallible
        },
    },
