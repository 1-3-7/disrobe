...
fn a_two_optimization_level_pair_matches_functions_across_stripped_images() {
    ...
    let low_stripped: Option<Vec<u8>> =
        stripped_copy(&low_path, &scratch.path().join("probe_o0.strip"));
    let high_stripped: Option<Vec<u8>> =
        stripped_copy(&high_path, &scratch.path().join("probe_o2.strip"));
    assert!(
        (low_stripped.is_some() && high_stripped.is_some()) || !cfg!(target_os = "linux"),
        "Linux similarity extraction requires llvm-strip or strip"
    );
