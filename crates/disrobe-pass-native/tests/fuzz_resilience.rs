...
#[test]
fn chained_macho_vm_recovers_every_compiled_handler_before_hostile_mutation() {
    use disrobe_pass_native::vm_devirt::detect::{
        Bitness, DispatchKind, VmDetection, VmStructure, detect_vm, recover_structure,
        recover_structure_codescan_only,
    };
    use object::{Object, ObjectSection, ObjectSymbol};

    const IMAGE: &[u8] = include_bytes!("fixtures/vm_oracle_x86_64_chained.macho");
    const HANDLERS: [&str; 18] = [
        "_h_push_imm",
        "_h_push_reg",
        "_h_pop_reg",
        "_h_add",
    ...
    let file: object::File<'_> = object::File::parse(IMAGE).expect("parse compiled Mach-O VM");
    assert_eq!(file.architecture(), object::Architecture::X86_64);
    let detection: VmDetection = detect_vm(IMAGE, Bitness::Bits64)
        .expect("the compiled chained-fixup VM must satisfy the recovery precondition");
    let exported: VmStructure = recover_structure(IMAGE, Bitness::Bits64, &detection)
        .expect("recover the exported VM structure");
    let scanned: VmStructure = recover_structure_codescan_only(IMAGE, Bitness::Bits64)
        .expect("recover the same compiled handler table through code scanning");
    let expected_bytecode: Vec<u8> = (0..=u8::MAX).collect();
    assert_eq!(exported.bytecode, expected_bytecode);
    assert_eq!(
        scanned.bytecode.get(..256),
        Some(expected_bytecode.as_slice())
    );
    assert_eq!(scanned.bytecode_va, exported.bytecode_va);
    for structure in [exported, scanned] {
        assert_eq!(structure.dispatch_kind, DispatchKind::CallThreaded);
        assert_eq!(structure.entry_vip, 0);
        assert_eq!(structure.handlers.len(), HANDLERS.len());
        for (handler, name) in structure.handlers.iter().zip(HANDLERS) {
            let symbol: object::Symbol<'_, '_> = file
                .symbols()
                .find(|symbol: &object::Symbol<'_, '_>| symbol.name().ok() == Some(name))
                .expect("the independent compiler symbol table must name every handler");
            assert_eq!(handler.va, symbol.address(), "{name}");
            assert!(!handler.code.is_empty(), "{name}");
            let section: object::Section<'_, '_> = file
                .section_by_index(symbol.section_index().expect("handler has a code section"))
                .expect("read handler section");
            assert_eq!(
                section
                    .data_range(
                        handler.va,
                        u64::try_from(handler.code.len()).expect("code length fits")
                    )
                    .expect("read compiled handler bytes"),
                Some(handler.code.as_slice()),
                "{name}",
            );
        }
    }
