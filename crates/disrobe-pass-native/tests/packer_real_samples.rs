...
#[test]
fn yodas_crypter_code_data_is_encrypted_carve_with_honest_low_plaintext() {
    let report: YodasCrypterReport =
    ...
    let text: &RecoveredSection = report
        ...
        .expect(".text section");
    assert_eq!(text.recovery, SectionRecovery::EncryptedCarve);
    let pct: f64 = text.plaintext_pct();
    assert!(
        pct < 2.0,
        "AccessEnum .text is stream-encrypted; plaintext fraction must be honestly tiny, got {pct:.4}%",
    );
    assert!(
        !text.is_byte_identical(),
        ".text must never be reported byte-identical",
    );
    println!("yodas_crypter AccessEnum: .text EncryptedCarve plaintext={pct:.4}%");
}
