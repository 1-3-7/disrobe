...
fn host_o3_nested_loop_recovers_after_vector_lane_reduction() {
    ...
    let (code, base): (Vec<u8>, u64) =
        function_code(&object, "wp_nested_loop_h").expect("nested-loop helper code");
    let instructions: Vec<DisasmInsn> =
        disassemble(Arch::X86_64, base, &code).expect("nested-loop helper instructions");
    let duplicate_moves: Vec<&DisasmInsn> = instructions
        .iter()
        .filter(|instruction: &&DisasmInsn| instruction.mnemonic == "movddup")
        .collect();
    assert!(
        !duplicate_moves.is_empty(),
        "host fixture must exercise movddup"
    );
    let recovered_lines: Vec<&str> = recovered.tu.lines().map(str::trim).collect();
    for instruction in duplicate_moves {
        let (dest, src): (&str, &str) = instruction
            .operands
            .split_once(',')
            .expect("movddup operand pair");
        let parse_xmm = |operand: &str| -> usize {
            let register: usize = operand
                .trim()
                .strip_prefix("xmm")
                .expect("register-source movddup")
                .parse()
                .expect("XMM register number");
            assert!(register < 16, "legacy movddup XMM register range");
            register
        };
        let dest: usize = parse_xmm(dest);
        let src: usize = parse_xmm(src);
        for register in [dest, src] {
            assert!(
                recovered.tu.contains(&format!("uint64_t v{register}_lo")),
                "movddup register must be declared: {}",
                instruction.operands
            );
        }
        assert!(
            recovered.tu.contains(&format!("uint64_t v{dest}_hi")),
            "movddup high output lane must be declared"
        );
        let low: String = format!("v{dest}_lo = v{src}_lo;");
        let high: String = format!("v{dest}_hi = v{src}_lo;");
        assert!(
            recovered_lines
                .windows(2)
                .any(|lines: &[&str]| { lines == [low.as_str(), high.as_str()] }),
            "movddup must copy its actual low source lane into both outputs: {}",
            instruction.operands
        );
    }
    assert!(recovered.tu.matches("while (").count() >= 2);
    let driver: String = build_program_driver(program, &recovered);
