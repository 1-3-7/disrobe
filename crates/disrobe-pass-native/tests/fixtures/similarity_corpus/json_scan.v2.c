...
u32 json_count_separators(const char *text, u32 length) {
    u32 commas = 0;
    u32 colons = 0;
...
u64 corpus_main(u64 seed) {
    ...
    u32 separators = json_count_separators(sample, length);
    const char *diagnosis = json_diagnosis(outcome);

    u64 total = (u64)(outcome + 3) * 7919u + (u64)(truncated + 3) * 104729u + separators;
