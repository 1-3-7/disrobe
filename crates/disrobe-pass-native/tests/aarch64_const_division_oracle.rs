...
#[test]
fn aarch64_signed_fused_quotient_sign_recovers_division_and_remainder() {
    let cases: [(&[u32], char, u64); 4] = [
        (
            &[
                0x528c_cce8,
                0x72ac_ccc8,
    ...
    for (words, operator, divisor) in cases {
        let code: Vec<u8> = words
            .iter()
            .flat_map(|word: &u32| word.to_le_bytes())
            .collect();
        let recovery: LeafRecovery = recover_aarch64_function(&code, 0x1000)
            .expect("signed fused quotient-sign instructions must lift");
        assert_eq!(
            recovered_constant_operator(&recovery.source),
            Some((operator, divisor)),
            "the quotient sign must round the signed operation toward zero: {}",
            recovery.source
        );
    }
}
...
#[test]
fn aarch64_signed_fused_quotient_sign_rejects_near_misses() {
    for (multiplier, shift, correction) in [
        (0x528c_ccc8u32, 0x9362_fd08u32, 0x0b48_7d00u32),
        (0x528c_cce8, 0xd362_fd08, 0x0b48_7d00),
        (0x528c_cce8, 0x9362_fd08, 0x0b48_7900),
        (0x528c_cce8, 0x9362_fd08, 0x8b48_7d00),
    ] {
        let words: [u32; 6] = [
            multiplier,
            0x72ac_ccc8,
        ...
        let code: Vec<u8> = words
            ...
            .collect();
        let recovery: LeafRecovery = recover_aarch64_function(&code, 0x1000)
            .expect("near-miss instructions must lift before their idiom is rejected");
        assert!(
            recovered_constant_operator(&recovery.source).is_none(),
            "an altered multiplier, signed shift, sign bit, or correction width is not division: {}",
            recovery.source
        );
    }
}
...
#[test]
fn aarch64_low_half_multiply_cannot_use_a_full_width_division_witness() {
    let words: [u32; 5] = [
        0xd280_0068,
        0x9b08_7c08,
        0x937f_fd08,
    ...
    let code: Vec<u8> = words
        ...
        .collect();
    let recovery: LeafRecovery = recover_aarch64_function(&code, 0x1000)
        .expect("the low-half multiply and sign corrections must lift");
    assert!(
        recovered_constant_operator(&recovery.source).is_none(),
        "a wrapping 64-bit product cannot satisfy a full-width division witness: {}",
        recovery.source
    );
}

#[test]
fn aarch64_truncated_product_cannot_use_a_full_width_division_witness() {
    let words: [u32; 8] = [
        0x528c_cce8,
        0x72ac_ccc8,
        0x9b28_7c08,
    ...
    let code: Vec<u8> = words
        ...
        .collect();
    let recovery: LeafRecovery = recover_aarch64_function(&code, 0x1000)
        .expect("the narrowing copies and sign corrections must lift");
    assert!(
        recovered_constant_operator(&recovery.source).is_none(),
        "discarding the product's upper half invalidates its division witness: {}",
        recovery.source
    );
}
