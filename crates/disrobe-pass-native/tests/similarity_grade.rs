...
use truth::{Address, ImageSymbols, SizeBand, TruthTable, names_code};
...
use disrobe_similarity::{
    ...
    FunctionVerdict, InstructionCategory, MatchReport, Verdict, body_of, match_functions,
};
    ...
    let has_defined_function_symbol: bool =
        file.symbols()
            ...
            .any(|symbol: object::Symbol<'_, '_>| {
                !symbol.is_undefined()
                    && (names_code(&file, &symbol) || symbol.kind() == ObjectSymbolKind::Label)
...
struct PairOutcome {
    ...
    dropped_left: usize,
    dropped_right: usize,
    local_names_the_right_linker_dropped: usize,
    folded_left: usize,
    folded_right: usize,
...
fn run_pair(
    ...
    assert!(
        table.local_names_the_right_linker_dropped == 0
            || (matches!(recipe.flavor, Flavor::Hosted)
                && matches!(recipe.right.compiler, Compiler::Clang)),
        "{program}: only a hosted clang right image may name no local functions, yet {} left addresses were excluded against a {} {} build",
        table.local_names_the_right_linker_dropped,
        recipe.flavor.label(),
    ...
    Some(PairOutcome {
        ...
        local_names_the_right_linker_dropped: table.local_names_the_right_linker_dropped,
...
#[test]
fn an_authoritative_single_import_call_remains_a_weak_exact_match() {
    ...
    assert!(matches!(
        ...
        Some(Verdict::Exact {
            strength: AnchorStrength::SingleImportedCall,
            ..
        })
    ));
}
...
#[test]
fn an_inferred_single_import_call_without_structure_cannot_anchor_a_match() {
    let references: [DataReference; 1] = [DataReference::imported_call("shared_service")];
    let left: [FunctionFeatures; 1] = [FunctionFeatures::new(FunctionId(10), references.clone())
        .requiring_reference_corroboration()];
    let right: [FunctionFeatures; 1] =
        [FunctionFeatures::new(FunctionId(20), references).requiring_reference_corroboration()];
    let report: MatchReport = match_functions(&left, &right);
    assert_eq!(report.exact_count(), 0);
    assert_eq!(report.matched_count(), 0);
    assert_eq!(
...
#[test]
fn a_callback_loop_reports_an_authoritative_single_import_as_weak_exact_evidence() {
    let shape: ControlFlowGraph = callback_loop_shape(InstructionCategory::Call);
    let references: [DataReference; 1] = [DataReference::imported_call("shared_service")];
    let left: [FunctionFeatures; 1] = [FunctionFeatures::with_structure(
        FunctionId(10),
    ...
    assert!(matches!(
        report.left_verdict(FunctionId(10)),
        Some(Verdict::Exact { shared_references, strength: AnchorStrength::SingleImportedCall, .. })
...
#[test]
fn an_explicit_corroboration_flag_keeps_distinctive_references_ambiguous() {
...
#[test]
fn a_stripped_optimized_elf_marks_its_prologue_free_string_function_as_inferred() {
    ...
    for (image, addresses) in [
        ...
        for address in addresses {
            ...
            assert!(
                ...
                "the PE runtime-function table independently establishes digest_label: {features:?}"
            );
        }
    }
    let report: MatchReport = match_functions(&left.features, &right.features);
...
fn the_corpus_grades_the_matcher_against_compiler_produced_ground_truth() {
    ...
    println!(
        "verdicts: recovered is a correspondence the matcher paired with the right counterpart, wrong is one it paired with something else, refused is one where it returned Ambiguous or Unmatched, missed is one whose left address never entered its subject list, unbacked is a pair it emitted for a function that exists only in the left image, unjudged is a pair whose left address carries no name in the unstripped build, or whose left names are all local symbols against a PE right image that names no local functions"
    );
    ...
    let mut local_names_the_right_linker_dropped: usize = 0;
    ...
    for outcome in &outcomes {
        ...
        local_names_the_right_linker_dropped += outcome.local_names_the_right_linker_dropped;
    ...
    println!(
        "excluded: {dropped_left} named functions present only in the left image and {dropped_right} present only in the right image, which is what inlining and a source edit produce; {local_names_the_right_linker_dropped} left addresses are named only by local symbols against a PE right image that names no local functions, so pairs on them are unjudged"
    );
    println!(
