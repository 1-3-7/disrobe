...
fn header(feature_flags: u16) -> Vec<u8> {
    ...
    buf.extend_from_slice(&0u16.to_le_bytes());
    buf.extend_from_slice(&feature_flags.to_le_bytes());
    buf.extend_from_slice(&0u16.to_le_bytes());
    buf.extend_from_slice(&0u16.to_le_bytes());
...
fn parses_minimal_real_sig() {
    ...
    assert_eq!(sig.header.version, 6);
...
#[test]
fn compressed_body_inflates_to_real_tree() {
    use std::io::Write;
    ...
    body.push(0x02);
    body.push(0x00);
    body.extend_from_slice(&[0x55, 0x8B]);
    body.push(0x00);
    body.push(0x04);
    ...
    enc.write_all(&body).expect("zlib encode");
    let compressed: Vec<u8> = enc.finish().expect("zlib finish");

    let mut buf: Vec<u8> = header_versioned(7, 0x10);
    buf.extend_from_slice(&compressed);
    let sig: FlirtSig = parse_flirt(&buf).expect("compressed body must inflate and parse");
    assert_eq!(sig.modules.len(), 1);
    assert_eq!(sig.modules[0].public_names[0].name, "main");
    assert_eq!(sig.modules[0].crc16, crc);
}

#[test]
fn corrupt_compressed_body_errors() {
    let mut buf: Vec<u8> = header_versioned(7, 0x10);
    buf.extend_from_slice(&[0x00, 0x01, 0x02, 0x03]);
    assert!(matches!(parse_flirt(&buf), Err(Error::SignatureDb(_))));
}

fn header_versioned(version: u8, feature_flags: u16) -> Vec<u8> {
    let mut buf: Vec<u8> = Vec::new();
    buf.extend_from_slice(b"IDASGN");
    buf.push(version);
    buf.push(0x00);
