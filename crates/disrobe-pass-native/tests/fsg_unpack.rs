...
fn data_dir(orig: &[u8], index: usize) -> (u32, u32) {
    let pe_off: usize =
        u32::from_le_bytes([orig[0x3C], orig[0x3D], orig[0x3E], orig[0x3F]]) as usize;
    let opt: usize = pe_off + 0x18;
    let base: usize = opt + 0x60 + index * 8;
    let rva: u32 = u32::from_le_bytes([orig[base], orig[base + 1], orig[base + 2], orig[base + 3]]);
...
fn test_fsg_ftp_residual_diff_is_confined_to_bound_iat() {
    ...
    let out: FsgUnpackOutput = unpack_fsg(&packed).expect("FSG unpack must succeed");
    let dest_rva: u32 = out.unpack_dest_va.saturating_sub(out.image_base);
    let (import_rva, import_sz): (u32, u32) = data_dir(&orig, 1);
    let (iat_rva, iat_sz): (u32, u32) = data_dir(&orig, 12);

    let (_ib, sections): (u32, Vec<OriginalSection>) =
        parse_original_sections(&orig).expect("parse original PE");
    let mut total_diffs: u64 = 0;
    let mut diffs_outside_iat: u64 = 0;
    for sec in &sections {
        if sec.bytes.is_empty() || sec.rva < dest_rva {
            continue;
        }
        let off: usize = (sec.rva - dest_rva) as usize;
        if off >= out.raw_image.len() {
        ...
        let take: usize = sec.bytes.len().min(out.raw_image.len() - off);
        for j in 0..take {
            if out.raw_image[off + j] != sec.bytes[j] {
                total_diffs += 1;
                let rva: u32 = sec.rva + j as u32;
                let in_iat: bool = iat_sz > 0 && rva >= iat_rva && rva < iat_rva + iat_sz;
                let in_import: bool =
                    import_sz > 0 && rva >= import_rva && rva < import_rva + import_sz;
                if !in_iat && !in_import {
                    diffs_outside_iat += 1;
                }
            }
    ...
    println!(
        "ftp residual: {total_diffs} diffs, {diffs_outside_iat} outside the bound IAT (0x{iat_rva:X}+{iat_sz}) / import dir (0x{import_rva:X}+{import_sz})"
    );
    assert!(
        total_diffs > 0,
        "ftp is expected to carry a bound-IAT residual"
    );
    assert_eq!(
        diffs_outside_iat, 0,
        "the FSG honest ceiling is the bound IAT: every residual byte-diff must fall inside the \
         original's IAT or import-directory ranges (machine-resolved addresses absent from the \
         packed stream, not a depacker defect)"
    );
}
