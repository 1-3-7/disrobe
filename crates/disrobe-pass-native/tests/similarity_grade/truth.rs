...
pub(crate) struct ImageSymbols {
    ...
    local_only_names: BTreeSet<String>,
    format: &'static str,
}
...
impl ImageSymbols {
    pub(crate) fn read(bytes: &[u8]) -> Option<Self> {
        ...
        carried.local_only_names = carried
            .address_of
            .keys()
            .filter(|name: &&String| !global_names.contains(*name))
            .cloned()
            .collect();
...
pub(crate) fn names_code(file: &object::File<'_>, symbol: &object::Symbol<'_, '_>) -> bool {
    match symbol.kind() {
        SymbolKind::Text => true,
        SymbolKind::Data => untyped_coff_symbol_starts_code(file, symbol),
        _ => false,
    }
}

fn untyped_coff_symbol_starts_code(
    file: &object::File<'_>,
    ...
    let Some(section): Option<object::Section<'_, '_>> = symbol
        .section_index()
        .and_then(|index: SectionIndex| file.section_by_index(index).ok())
    else {
        return false;
    };
    ...
    let Some(offset): Option<usize> = symbol
        .address()
        .checked_sub(section.address())
        .and_then(|delta: u64| usize::try_from(delta).ok())
    else {
        return false;
    };
    let Ok(body): Result<&[u8], object::Error> = section.data() else {
    ...
    let Some(tail): Option<&[u8]> = body.get(offset..) else {
        return false;
    };
    let window: &[u8] = &tail[..tail.len().min(MAX_X86_INSTRUCTION_BYTES)];
    let mut decoder: Decoder<'_> =
        Decoder::with_ip(bitness, window, symbol.address(), DecoderOptions::NONE);
    decoder.can_decode() && !decoder.decode().is_invalid()
}
...
pub(crate) struct TruthTable {
    ...
    pub(crate) local_names_the_right_linker_dropped: usize,
    ...
    pub(crate) fn derive(left: &ImageSymbols, right: &ImageSymbols) -> Self {
        ...
        for (address, names) in &left.names_at {
            ...
            if accepted.is_empty() {
                let right_coff_names_no_local_functions: bool =
                    matches!(right.format(), "pe" | "coff") && right.local_only_names.is_empty();
                let named_only_locally: bool = names
                    .iter()
                    .all(|name: &String| left.local_only_names.contains(name));
                if right_coff_names_no_local_functions && named_only_locally {
                    table.local_names_the_right_linker_dropped += 1;
                } else {
