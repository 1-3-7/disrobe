...
/// Assert the HONEST whole-image recovery floor AND the content-section floor.
///
/// `whole_floor` is the real measured whole-image byte-recovery (entire
/// decompressed region including the `.rsrc`/`.reloc` zone `NSPack` stores
/// outside the `nsp1` stream and that this decoder leaves zeroed).
/// `content_floor` is the `.text`/`.rdata`/`.data` recovery, which is materially
/// higher; the gap is the truth about unreconstructed resources, not a hidden
/// defect.
