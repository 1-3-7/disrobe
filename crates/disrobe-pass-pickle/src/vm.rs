...
fn charged_total_after_clone(v: &PickleValue, already: u64) -> Result<u64> {
    charged_total_after_nodes(node_count_capped(v, NODE_BUDGET.saturating_add(1)), already)
}

fn charged_total_after_nodes(nodes: u64, already: u64) -> Result<u64> {
...
impl Machine {
    ...
    fn push(&mut self, v: PickleValue) {
        ...
        let nodes: u64 = node_count_capped(&v, NODE_BUDGET.saturating_add(1));
        self.push_sized(v, None, depth, nodes);
    }

    fn push_sized(&mut self, v: PickleValue, memo_id: Option<u64>, depth: u32, nodes: u64) {
    ...
    fn push_from_memo(&mut self, v: PickleValue, memo_id: u64) {
        let depth: u32 = value_depth(&v, MAX_VALUE_DEPTH + 1);
        let nodes: u64 = node_count_capped(&v, NODE_BUDGET.saturating_add(1));
        self.push_sized(v, Some(memo_id), depth, nodes);
    }
    ...
    fn push_clone_of_top(&mut self, op: &'static str, offset: usize) -> Result<()> {
        ...
        let (top, memo_id, depth, nodes): (&PickleValue, Option<u64>, u32, u64) =
            match self.stack.last() {
                Some(Slot::Value {
                    ...
                    depth,
                    nodes,
                }) => (value, *memo_id, *depth, *nodes),
                Some(Slot::Mark) | None => return Err(Error::StackUnderflow { op, offset }),
            };
        let total: u64 = charged_total_after_nodes(nodes, already)?;
...
impl Session {
    ...
    pub fn run(&mut self, dis: &Disassembly) -> Result<PickleValue> {
        ...
        let cleaned_memo: BTreeMap<u64, PickleValue> =
            clean_referenced_memos(&self.machine.memo, &self.machine.memo_used);
...
fn push_tuple(m: &mut Machine, placed: Vec<Placed>) -> Result<()> {
    ...
    let (items, deepest_child, child_nodes): (Vec<PickleValue>, u32, u64) = resolve_all(placed);
    m.charge(1)?;
    m.push_sized(
        PickleValue::Tuple(items),
        None,
        deepest_child.saturating_add(1),
        child_nodes.saturating_add(1),
    );
    m.enforce_top_depth()
}

fn resolve_shared(placed: Placed) -> (PickleValue, u32, u64) {
    match placed.memo_id {
        Some(k) if is_container(&placed.value) => (PickleValue::MemoRef { key: k }, 1, 1),
        _ => (placed.value, placed.depth, placed.nodes),
    }
}

fn resolve_all(placed: Vec<Placed>) -> (Vec<PickleValue>, u32, u64) {
    let mut items: Vec<PickleValue> = Vec::with_capacity(placed.len());
    let mut deepest_child: u32 = 0;
    let mut child_nodes: u64 = 0;
    for p in placed {
        let (value, depth, nodes): (PickleValue, u32, u64) = resolve_shared(p);
        deepest_child = deepest_child.max(depth);
        child_nodes = child_nodes.saturating_add(nodes);
        items.push(value);
    }
    (items, deepest_child, child_nodes)
}
...
fn clean_referenced_memos(
    memo: &BTreeMap<u64, PickleValue>,
    memo_used: &BTreeMap<u64, bool>,
) -> BTreeMap<u64, PickleValue> {
    memo.iter()
        .map(|(&key, value): (&u64, &PickleValue)| {
            let cleaned: PickleValue = if memo_used.get(&key).copied().unwrap_or(false) {
                inline_unused_refs(value, memo, memo_used, 0)
            } else {
                value.clone()
            };
            (key, cleaned)
...
fn append_into(m: &mut Machine, placed: Vec<Placed>, off: usize) -> Result<()> {
    m.flush_placed_memos(&placed);
    let (mut items, deepest_child, child_nodes): (Vec<PickleValue>, u32, u64) = resolve_all(placed);
...
fn set_items(m: &mut Machine, placed: Vec<Placed>, off: usize) -> Result<()> {
    m.flush_placed_memos(&placed);
    let (values, deepest_child, child_nodes): (Vec<PickleValue>, u32, u64) = resolve_all(placed);
...
mod tests {
    ...
    #[test]
    fn shared_overwritten_memo_inlines_an_unused_nested_tuple_before_reconstruction() {
        let dis: Disassembly = disassemble(b"\x80\x04K\x08\x85q\x07\x85q\x02h\x02\x86.")
            .expect("disassemble shared overwritten memo");
        let (trace, memo): (VmTrace, BTreeMap<u64, PickleValue>) =
            execute_full(&dis).expect("execute shared overwritten memo");
        let reconstruction: crate::reconstruct::Reconstruction =
            crate::reconstruct::reconstruct(&trace.result, &memo, trace.root_memo_key);
        assert!(reconstruction.reexecutable);
        assert!(
            reconstruction.program.contains("((8,),)"),
            "the shared memo entry must inline its unused nested tuple: {}",
            reconstruction.program
        );
        assert!(
            !reconstruction.program.contains("_m[0]"),
            "the unused nested tuple must not become a forward memo reference: {}",
            reconstruction.program
        );
        assert!(
            reconstruction.program.matches("_m[1]").count() >= 3,
            "both result positions must preserve the shared parent alias: {}",
            reconstruction.program
        );
    }
