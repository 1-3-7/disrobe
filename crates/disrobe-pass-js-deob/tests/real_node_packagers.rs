...
/// REAL-PARSE assertion: the parser is invoked, magic is the real `0x0143DA20`,
/// `code_path` is a non-empty `.js` file. Replaces the prior tautological
/// "head bytes are mostly nonzero" check that never called the SEA parser.
#[test]
...
/// REAL-PARSE assertion: the bytenode header parser is invoked on the real
/// Node-18 `.jsc` and MUST exactly match the Node-18 magic + V11 header layout.
/// Replaces the prior `len > 16` byte-count check that never called the parser.
#[test]
