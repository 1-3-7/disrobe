...
fn evaluates_basic_payload_returns_string() {
    ...
    assert_eq!(
        recovered, "undefinednltre",
        "basic fixture must decode to the exact plaintext a real JS engine yields; got {recovered:?}",
    );
}
...
fn atoms_payload_decodes() {
    ...
    assert!(decode.detection.matched);
    let Some(recovered): Option<String> = decode.recovered else {
        panic!("expected boa to evaluate the atoms payload");
    };
    assert_eq!(
        recovered, "1falsetrueundefined0undefined3falsefalse",
        "atoms fixture must decode to its exact plaintext; got {recovered:?}",
    );
}
