...
#[test]
fn detector_emits_pace_verdict_with_detect_only_metadata() {
...
#[test]
fn pace_runner_emits_detect_only_json_artifact() {
    ...
    let body: &str =
        std::str::from_utf8(out.envelope.as_slice()).expect("pace artifact must be utf-8 json");
    assert!(body.contains("DR-JS-PACE-DetectOnly"), "body={}", body);
    assert!(body.contains("docs/legal/pace-js-stance.md"));
    assert!(body.contains("\"reversed\": 0"));
}
