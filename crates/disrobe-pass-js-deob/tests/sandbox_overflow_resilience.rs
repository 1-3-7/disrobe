...
//! Resilience regression: the embedded `boa` sandbox must never abort the
//! process via a non-unwindable native stack overflow, even on adversarial or
//! megabyte-scale esoteric payloads. Prior to the `sandbox_guard` pre-eval
//! depth/operator-chain bound, the 145 KB `jjencode` corpus megafile (a single
//! ~950-operand `+` concatenation) overflowed the worker stack and SIGABRT'd
//! the whole test binary. These tests pin the bounded-refusal behaviour.
