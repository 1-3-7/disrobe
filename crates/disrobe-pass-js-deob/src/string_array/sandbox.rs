...
fn run_rotation_search(
    ...
    for k in 0..max_k {
        let sample_results: Vec<Option<String>> =
            eval_batch(&mut context, &batch_script, sample_exprs.len());
    ...
    let full: Vec<Option<String>> = decode_all(&mut fresh, expressions);
...
pub(super) fn probe_rotation_to_match(
    ...
    let provider_owned: String = provider_name.to_owned();
    let exprs_owned: Vec<String> = expressions.to_vec();
    let reference_owned: Vec<Option<String>> = reference.to_vec();
    let (tx, rx): (mpsc::SyncSender<Option<u32>>, mpsc::Receiver<Option<u32>>) =
        mpsc::sync_channel::<Option<u32>>(1);
    let Ok(handle): std::io::Result<thread::JoinHandle<()>> = thread::Builder::new()
        .name("disrobe-boa-rotmatch".to_owned())
        .stack_size(WORKER_STACK_BYTES)
        .spawn(move || {
            let result: Option<u32> = run_rotation_to_match(
                &prelude_owned,
                &provider_owned,
                &exprs_owned,
                &reference_owned,
...
fn run_rotation_to_match(
    ...
    for k in 0..max_k {
        let results: Vec<Option<String>> =
            eval_batch(&mut context, &batch_script, sample_exprs.len());
        let all_match: bool = results.len() == expected.len()
            && results
                .iter()
                .zip(expected.iter())
...
fn build_batch_decode_script(exprs: &[&String]) -> String {
    let mut script: String = String::from("JSON.stringify([");
    for (i, expr) in exprs.iter().enumerate() {
        if i > 0 {
            script.push(',');
        }
        script.push_str("function(){return String(");
        script.push_str(expr);
        script.push_str(");}");
    }
    script.push_str("].map(function(f){try{return f();}catch(e){return null;}}))");
    script
}

fn eval_batch(context: &mut Context, batch_script: &str, expected: usize) -> Vec<Option<String>> {
    let rendered: Option<String> = context
        .eval(Source::from_bytes(batch_script.as_bytes()))
        .ok()
        .and_then(|v: boa_engine::JsValue| {
            v.as_string()
                .map(boa_engine::JsString::to_std_string_escaped)
        });
    let Some(json): Option<String> = rendered else {
        return vec![None; expected];
    };
    match serde_json::from_str::<Vec<Option<String>>>(&json) {
        Ok(parsed) if parsed.len() == expected => parsed,
        _ => vec![None; expected],
    }
}
...
fn decode_all(context: &mut Context, expressions: &[String]) -> Vec<Option<String>> {
    ...
    for chunk in expressions.chunks(DECODE_BATCH_CHUNK) {
        ...
        let decoded: Vec<Option<String>> = eval_batch(context, &script, refs.len());
        out.extend(decoded);
    }
    out
}
