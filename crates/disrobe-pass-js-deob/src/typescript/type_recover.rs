...
fn annotate_var_decl(
    ...
    for d in &decl.declarations {
        let BindingPatternKind::BindingIdentifier(id): &BindingPatternKind<'_> = &d.id.kind else {
            continue;
        };
...
fn annotate_function(
    ...
    let Some(id): &Option<oxc_ast::ast::BindingIdentifier<'_>> = &func.id else {
        return;
    };
    let name: String = id.name.as_str().to_owned();
    if let Some(sym) = corpus.lookup_global(&name)
...
fn emit_typescript(source: &str, annotations: &BTreeMap<String, String>) -> String {
    ...
    for (name, ty) in annotations {
        push_format(&mut header, format_args!("declare const {name}: {ty};\n"));
    }
    header.push('\n');
