    ...
    if !opts.i_have_authorization {
        ...
        return TransformOutput {
            ...
            stats: TransformStats {
                matched: warned,
                skipped: warned,
                errors: vec!["authorization required to strip selfDefending".to_owned()],
...
const fn pattern() -> &'static str {
    r"\(\s*function\s*\(\s*\)\s*\{\s*var\s+\w+\s*=\s*function\s*\(\s*\)\s*\{[^}]*toString[^}]*search[^}]*\}[^}]*\}\s*\(\s*\)\s*\)\s*;?"
}
...
mod tests {
    ...
    #[test]
    fn detects_self_defending_iife() {
        let src: &str =
    ...
    fn strips_when_authorized() {
        let src: &str = "var x = 1; (function(){var t = function(){return ('xy').toString().search('z');}; t();}());";
