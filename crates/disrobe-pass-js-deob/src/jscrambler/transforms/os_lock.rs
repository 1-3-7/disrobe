    ...
    if !opts.i_have_authorization {
        ...
        return TransformOutput {
            ...
            stats: TransformStats {
                matched: warned,
                skipped: warned,
                errors: vec!["authorization required to bypass osLock".to_owned()],
...
const fn patterns() -> [&'static str; 3] {
    [
        r"navigator\s*\.\s*platform\s*[!=]==?\s*['\x22][^'\x22]+['\x22]",
        r"navigator\s*\.\s*oscpu\s*[!=]==?\s*['\x22][^'\x22]+['\x22]",
        r"/\s*Win|Mac|Linux\s*/\s*\.\s*test\s*\(\s*navigator\s*\.\s*platform\s*\)",
    ]
}
