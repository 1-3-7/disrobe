    ...
    if !opts.i_have_authorization {
        ...
        return TransformOutput {
            ...
            stats: TransformStats {
                matched: warned,
                skipped: warned,
                errors: vec!["authorization required to bypass domainLock".to_owned()],
...
const fn patterns() -> [&'static str; 3] {
    [
        r"location\s*\.\s*hostname\s*[!=]==?\s*['\x22][^'\x22]+['\x22]",
        r"window\s*\.\s*location\s*\.\s*hostname\s*[!=]==?\s*['\x22][^'\x22]+['\x22]",
        r"document\s*\.\s*domain\s*[!=]==?\s*['\x22][^'\x22]+['\x22]",
    ]
}
