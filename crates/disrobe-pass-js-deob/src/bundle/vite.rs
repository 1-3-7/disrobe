...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    ...
    extract_import_statements(source, modules);
}
...
fn extract_import_statements(source: &str, modules: &mut ModuleCollector) {
    let Ok(re): Result<Regex, regex::Error> =
    ...
    for caps in re.captures_iter(source) {
        ...
        modules.push(&format!("import:{path}"), None, full.as_str());
    }
}
