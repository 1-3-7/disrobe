...
use super::{BundlerDetection, BundlerKind};
...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    extract_export_const(source, modules);
    extract_export_functions(source, modules);
    extract_export_classes(source, modules);
}
...
fn extract_export_const(source: &str, modules: &mut ModuleCollector) {
    ...
    for caps in re.captures_iter(source) {
        ...
        modules.push(name, None, snippet.trim());
    }
}
...
fn extract_export_functions(source: &str, modules: &mut ModuleCollector) {
    ...
    for caps in re.captures_iter(source) {
        ...
        modules.push(name, None, source[full.start()..=body_close].trim());
    }
}

fn extract_export_classes(source: &str, modules: &mut ModuleCollector) {
    let Ok(re): Result<Regex, regex::Error> =
