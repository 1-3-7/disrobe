...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    extract_system_register(source, modules);
    extract_amd_define(source, modules);
}
...
fn extract_system_register(source: &str, modules: &mut ModuleCollector) {
...
fn extract_amd_define(source: &str, modules: &mut ModuleCollector) {
    let bytes: &[u8] = source.as_bytes();
    ...
    for mat in re.find_iter(source) {
        ...
        modules.push(&id, Some("amd"), body_text.trim());
    }
}
