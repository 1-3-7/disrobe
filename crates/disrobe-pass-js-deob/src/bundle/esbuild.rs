...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    extract_commonjs_modules(source, modules);
    extract_es_exports(source, modules);
}
...
fn extract_commonjs_modules(source: &str, modules: &mut ModuleCollector) {
    ...
    for caps in re.captures_iter(source) {
        ...
        modules.push(path, Some(var_name), body_text.trim());
    }
}

fn extract_es_exports(source: &str, modules: &mut ModuleCollector) {
