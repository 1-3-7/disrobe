...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    extract_iife_array(source, modules);
    extract_jsonp_chunks(source, modules);
}
...
fn extract_iife_array(source: &str, modules: &mut ModuleCollector) {
    ...
    for mat in re.find_iter(source) {
        ...
        for (idx, (start, end)) in entries.iter().enumerate() {
            ...
            modules.push(&format!("{idx}"), Some("main"), body_text.trim());
        }
        if !modules.is_empty() {
            return;
        }
...
fn extract_jsonp_chunks(source: &str, modules: &mut ModuleCollector) {
