...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    let calls: Vec<DefineCall<'_>> = parse_define_calls(source);
    for (idx, call) in calls.into_iter().enumerate() {
        ...
        modules.push(&id, Some(&chunk_id), call.factory);
    }
}
...
fn parse_define_calls(source: &str) -> Vec<DefineCall<'_>> {
    let bytes: &[u8] = source.as_bytes();
    let Ok(re): Result<Regex, regex::Error> = Regex::new(r"(?:^|[^.\w$])define\s*\(") else {
        return Vec::new();
    };
    let mut out: Vec<DefineCall<'_>> = Vec::new();
...
fn parse_define_args(source: &str, start: usize, end: usize) -> Option<DefineCall<'_>> {
    let bytes: &[u8] = source.as_bytes();
