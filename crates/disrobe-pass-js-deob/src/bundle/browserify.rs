...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    extract_browser_pack(source, modules);
}
...
fn extract_browser_pack(source: &str, modules: &mut ModuleCollector) {
...
fn collect_object_form(source: &str, object_open: usize, modules: &mut ModuleCollector) {
    let bytes: &[u8] = source.as_bytes();
    ...
    while i < object_close {
        ...
        let Some((module_text, deps_text)): Option<(&str, &str)> =
        ...
        modules.push(&key, Some(&chunk_id), module_text.trim());
        i = array_close + 1;
        i = skip_ws_and_commas(bytes, i, object_close);
    }
}

fn collect_array_form(source: &str, array_open: usize, modules: &mut ModuleCollector) {
    let bytes: &[u8] = source.as_bytes();
    let Some(array_close): Option<usize> = find_bracket_close(bytes, array_open + 1) else {
        return;
    };
    ...
    while i < array_close {
        ...
        let chunk_id: String = format!("deps:{}", deps_text.trim());
        modules.push(&idx.to_string(), Some(&chunk_id), module_text.trim());
        idx += 1;
        i = inner_close + 1;
...
fn split_pack_pair(source: &str, start: usize, hard_end: usize) -> Option<(&str, &str)> {
    let bytes: &[u8] = source.as_bytes();
