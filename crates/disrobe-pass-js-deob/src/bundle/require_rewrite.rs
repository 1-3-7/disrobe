...
use super::scan::{find_brace_close, find_paren_close, skip_trivia};
use crate::error::Error;

const REQUIRE_PATTERN: &str =
    r#"(__webpack_require__|require)\s*\(\s*(?:(\d+)|["']([^"']+)["'])\s*\)"#;
...
fn unwrap_module_wrapper(source: &str) -> Option<String> {
    ...
    let paren_open: usize = if let Some(header) = re.find(rest) {
        func_start + header.end() - 1
    } else if open_paren.is_some() {
        open_paren = None;
        trimmed_start
    } else {
        return None;
    ...
    let mut tail: usize = skip_trivia(bytes, body_close + 1, bytes.len());
    if open_paren.is_some() {
        if bytes.get(tail) != Some(&b')') {
            return None;
        }
        tail = skip_trivia(bytes, tail + 1, bytes.len());
    }
    if bytes.get(tail) == Some(&b';') {
        tail = skip_trivia(bytes, tail + 1, bytes.len());
    }
...
fn dedent_block(inner: &str) -> String {
    ...
    let min_indent: usize = trimmed
        ...
        .map(|l: &str| l.chars().take_while(|ch: &char| ch.is_whitespace()).count())
        .min()
        .unwrap_or(0);
    ...
    for (i, line) in trimmed.lines().enumerate() {
        ...
        let remainder: &str = line
            .char_indices()
            .nth(min_indent)
            .map_or("", |(offset, _): (usize, char)| &line[offset..]);
...
pub fn rewrite_requires(source: &str, map: &BTreeMap<String, String>) -> String {
    let Ok(re): Result<Regex, regex::Error> = Regex::new(REQUIRE_PATTERN) else {
...
pub(super) fn rewrite_modules_with_limit(
    ...
    let mut total: usize =
        modules
            .iter()
            .try_fold(0usize, |total: usize, module: &ExtractedModule| {
                [
                    module.id.len(),
                    module.chunk_id.as_ref().map_or(0, String::len),
                    module.source.len(),
                ]
                .into_iter()
                .try_fold(total, |size: usize, part: usize| {
                    checked_size(size, part, maximum)
                })
            })?;
    let map: BTreeMap<String, String> = build_id_to_path_map(modules);
    let re: Regex = Regex::new(REQUIRE_PATTERN)?;
    for module in modules {
        let previous: usize = module.source.len();
        if let Some(unwrapped) = unwrap_module_wrapper(&module.source) {
            module.source = unwrapped;
        }
        let remaining: usize = maximum - (total - previous);
        let mut size: usize = 0;
        let mut cursor: usize = 0;
        for captures in re.captures_iter(&module.source) {
            let Some(whole) = captures.get(0) else {
                continue;
            };
            let name: &str = captures
                .get(1)
                .map_or("require", |value: regex::Match<'_>| value.as_str());
            let id: &str = captures
                .get(2)
                .or_else(|| captures.get(3))
                .map_or("", |value: regex::Match<'_>| value.as_str());
            let target: &str = map.get(id).map_or(id, String::as_str);
            for part in [whole.start() - cursor, name.len(), target.len(), 4] {
                size = checked_size(size, part, remaining)?;
            }
            cursor = whole.end();
        }
        size = checked_size(size, module.source.len() - cursor, remaining)?;
        module.source = rewrite_requires(&module.source, &map);
        total = total - previous + size;
    }
    Ok(())
}

fn checked_size(size: usize, part: usize, maximum: usize) -> crate::error::Result<usize> {
    let size: usize = size.checked_add(part).ok_or(Error::SyntaxLimit {
        kind: "bundle output bytes",
        observed: usize::MAX,
        maximum,
    })?;
    check_limit("bundle output bytes", size, maximum)?;
    Ok(size)
}
