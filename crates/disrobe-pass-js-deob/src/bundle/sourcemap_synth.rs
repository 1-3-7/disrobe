...
pub fn decode_vlq(segment: &str) -> Option<Vec<i64>> {
    ...
    for &c in bytes {
        ...
        if chunk > u32::MAX.checked_shr(shift)? {
            return None;
        }
...
#[must_use]
pub fn encode_mappings(decoded: &DecodedMappings) -> String {
    ...
    for (line_idx, line) in decoded.lines.iter().enumerate() {
        ...
        for (seg_idx, segment) in line.iter().enumerate() {
            ...
            encode_vlq(segment.generated_column - prev_generated_column, &mut out);
            prev_generated_column = segment.generated_column;
            let (Some(source_index), Some(source_line), Some(source_column)) = (
            ...
            encode_vlq(source_index - prev_source_index, &mut out);
            prev_source_index = source_index;
            encode_vlq(source_line - prev_source_line, &mut out);
            prev_source_line = source_line;
            encode_vlq(source_column - prev_source_column, &mut out);
            prev_source_column = source_column;
            if let Some(name_index) = segment.name_index {
                encode_vlq(name_index - prev_name_index, &mut out);
