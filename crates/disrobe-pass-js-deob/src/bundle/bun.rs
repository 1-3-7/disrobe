...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    extract_bun_register_table(source, modules);
...
fn extract_bun_register_table(source: &str, modules: &mut ModuleCollector) {
    ...
    for mat in re.find_iter(source) {
        ...
        for entry in entries {
            ...
            modules.push(&entry.key, Some("bun"), value_text.trim());
        }
        if !modules.is_empty() {
            return;
        }
