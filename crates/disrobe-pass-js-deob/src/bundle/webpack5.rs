use super::limits::ModuleCollector;
...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    extract_module_table(source, modules);
    extract_chunk_push(source, modules);
    if modules.is_empty() {
        extract_concatenated_modules(source, modules);
    }
    extract_inlined_entry(source, modules);
}

#[cfg(test)]
pub(super) fn extract(source: &str) -> Vec<ExtractedModule> {
    super::limits::collect_for_test(source, collect)
}

const ENTRY_MODULE_ID: &str = "__webpack_entry__";

fn extract_inlined_entry(source: &str, modules: &mut ModuleCollector) {
    ...
    modules.push(ENTRY_MODULE_ID, Some("main"), body);
}
...
pub(super) fn extract_concatenated_modules(source: &str, modules: &mut ModuleCollector) {
...
fn extract_module_table(source: &str, modules: &mut ModuleCollector) {
    let Ok(re): Result<Regex, regex::Error> =
    ...
    for entry in entries {
        ...
        modules.push(&entry.key, Some("main"), value_text.trim());
    }
}

fn extract_chunk_push(source: &str, modules: &mut ModuleCollector) {
    let Ok(re): Result<Regex, regex::Error> = Regex::new(
    ...
    for mat in re.find_iter(source) {
        ...
        for entry in entries {
            let value_text: &str = &source[entry.value_span.0..entry.value_span.1];
            modules.push(&entry.key, Some(&chunk_id), value_text.trim());
        }
    }
}
...
mod tests {
    ...
    fn extracts_inlined_entry_program_after_runtime() {
        ...
        let mut mods: ModuleCollector =
            ModuleCollector::new(super::super::UnbundleLimits::UNLIMITED);
