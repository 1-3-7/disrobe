...
use limits::{ModuleCollector, check_limit};

pub use limits::UnbundleLimits;
...
pub use sourcemap_recover::{
    ...
    SourceMapLimits, SourceMapLocation, SourceTreeRecovery, decode_data_url_json, merge_reports,
    parse as parse_source_map_v3, parse_with_limits as parse_source_map_with_limits,
    recover as recover_source_map, recover_from_inline_data_url as recover_source_map_inline,
...
) -> Result<Vec<ExtractedModule>> {
    let mut modules: ModuleCollector = ModuleCollector::new(limits);
    match kind {
        BundlerKind::Amd => amd::collect(source, &mut modules),
        BundlerKind::Webpack4 => webpack4::collect(source, &mut modules),
        BundlerKind::Webpack5 => webpack5::collect(source, &mut modules),
        BundlerKind::Vite => vite::collect(source, &mut modules),
        BundlerKind::Rollup => rollup::collect(source, &mut modules),
        BundlerKind::Rolldown => rolldown::collect(source, &mut modules),
        BundlerKind::Esbuild => esbuild::collect(source, &mut modules),
        BundlerKind::Turbopack => turbopack::collect(source, &mut modules),
        BundlerKind::Bun => bun::collect(source, &mut modules),
        BundlerKind::Browserify => browserify::collect(source, &mut modules),
        BundlerKind::Parcel => parcel::collect(source, &mut modules),
        BundlerKind::SystemJs => systemjs::collect(source, &mut modules),
    }
    modules.finish()
}
...
pub fn unbundle(kind: BundlerKind, source: &str) -> Result<UnbundleResult> {
    unbundle_with_limits(kind, source, UnbundleLimits::UNLIMITED)
}

pub fn unbundle_with_limits(
    kind: BundlerKind,
    source: &str,
    limits: UnbundleLimits,
) -> Result<UnbundleResult> {
    check_limit("bundle input bytes", source.len(), limits.input_bytes)?;
    let detection: BundlerDetection = detect_kind(kind, source);
    let mut modules: Vec<ExtractedModule> = extract_kind(kind, source, limits)?;
    if matches!(
        kind,
        ...
        require_rewrite::rewrite_modules_with_limit(&mut modules, limits.output_bytes)?;
    }
...
pub fn auto_unbundle(source: &str) -> Result<UnbundleResult> {
    auto_unbundle_with_limits(source, UnbundleLimits::UNLIMITED)
}

pub fn auto_unbundle_with_limits(source: &str, limits: UnbundleLimits) -> Result<UnbundleResult> {
    check_limit("bundle input bytes", source.len(), limits.input_bytes)?;
    let detection: BundlerDetection = detect_bundle(source).ok_or(Error::NoFamilyMatched)?;
    unbundle_with_limits(detection.kind, source, limits)
}

pub fn detect_bundle(source: &str) -> Option<BundlerDetection> {
    let mut best: Option<BundlerDetection> = None;
    for &kind in BundlerKind::ALL {
        let det: BundlerDetection = detect_kind(kind, source);
        if det.matched {
            let take: bool = best
                .as_ref()
                .is_none_or(|prev| det.confidence > prev.confidence);
            if take {
                best = Some(det);
            }
        }
