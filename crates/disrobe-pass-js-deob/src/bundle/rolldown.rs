...
pub(super) fn collect(source: &str, modules: &mut ModuleCollector) {
    ...
    if modules.is_empty() {
        extract_named_export_functions(source, modules);
    }
}
...
fn extract_module_table(source: &str, modules: &mut ModuleCollector) {
    ...
    for mat in re.find_iter(source) {
        ...
        for entry in entries {
            ...
            modules.push(&entry.key, Some("rolldown"), value_text.trim());
        }
        if !modules.is_empty() {
            return;
        }
...
fn extract_named_export_functions(source: &str, modules: &mut ModuleCollector) {
    let Ok(re): Result<Regex, regex::Error> =
