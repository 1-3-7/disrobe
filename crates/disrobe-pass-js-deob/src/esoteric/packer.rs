...
pub fn unpack(source: &str) -> PackerDecode {
    ...
    let Some(args): Option<PackerArgs> = extract_args(source) else {
        return PackerDecode {
            detection: PackerDetection::default(),
            recovered: None,
        };
    };
    let detection: PackerDetection = PackerDetection {
        matched: true,
        base: args.base,
        word_count: args.words.len(),
    };
    let recovered: Option<String> = unpack_with(&args.payload, args.base, &args.words);
    PackerDecode {
        detection,
        recovered,
    }
