...
/// Legacy alias for [`V8_MAGIC_NODE_24`].
#[deprecated(
    since = "0.3.0",
...
/// V8 `SerializedCodeData` header size for Node 18 / 20 (V8 ≤ 11.x).
pub const V8_HEADER_SIZE_V11: usize = 24usize;

/// V8 `SerializedCodeData` header size for Node 22 / 24 (V8 ≥ 12.x).
pub const V8_HEADER_SIZE_V12: usize = 32usize;

/// Maximum plausible payload size (256 MiB).
...
impl NodeVersion {
    ...
    /// Maps a raw `SerializedCodeData::kMagicNumber` to its Node major version.
    #[must_use]
    ...
    /// Header layout this V8 version uses (`Unknown` defaults to [`HeaderLayout::V12`]).
    #[must_use]
...
pub struct BytenodeCacheBody {
    ...
    /// The serialized V8 snapshot payload.
    pub payload: Vec<u8>,
}

/// What the pipeline can recover from a parsed `SerializedCodeData` payload per V8 version.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case", tag = "kind")]
pub enum SnapshotDeserializeStatus {
    /// Recognized V8 version, but full snapshot deserialization is not implemented.
    SnapshotDeserializeWall {
        node_version: NodeVersion,
        v8_version_label: &'static str,
        reason: &'static str,
    },
    /// V8 magic high bits matched `0xC0DE` but the low marker is unrecognized.
...
/// Parses a V8 `SerializedCodeData` header from the start of `bytes`.
...
/// Parses the full `.jsc` blob: header plus the snapshot payload that follows.
...
/// Recovery status for the snapshot payload of a parsed `.jsc` header.
#[must_use]
