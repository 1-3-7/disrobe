...
pub fn detect_pkg_payload(bytes: &[u8]) -> Option<PkgLocation> {
    ...
    let footer_start: u64 = total_len.checked_sub(16)?;
    let payload_offset: u64 = if payload_offset_raw <= footer_start {
        let payload_end: u64 = payload_offset_raw.checked_add(payload_size)?;
        if payload_end > footer_start {
            return None;
        }
        payload_offset_raw
    } else {
        footer_start.checked_sub(payload_size)?
    };
    Some(PkgLocation {
        payload_offset,
        payload_size,
...
mod tests {
    ...
    #[test]
    fn impossible_payload_size_returns_none() {
        let mut bytes: Vec<u8> = vec![0u8; 128];
        bytes[16..16 + PKG_PRELUDE_MARKER.len()].copy_from_slice(PKG_PRELUDE_MARKER);
        bytes.extend_from_slice(&u64::MAX.to_le_bytes());
        bytes.extend_from_slice(&u64::MAX.to_le_bytes());
        assert!(detect_pkg_payload(&bytes).is_none());
    }
}
