...
impl DecodedInstruction {
    ...
    pub fn render(&self) -> String {
        ...
        for (i, operand) in self.operands.iter().enumerate() {
            ...
            match operand.kind {
                OperandKind::Imm => {
                    let _ = write!(out, "#{}", operand.signed_value);
                }
                OperandKind::UImm
                | OperandKind::Flag8
                | OperandKind::Flag16
                | OperandKind::RegCount => {
                    let _ = write!(out, "#{}", operand.unsigned_value);
                }
                OperandKind::Reg
                | OperandKind::RegOut
                ...
                | OperandKind::RegPair
                | OperandKind::RegList => {
                    let _ = write!(out, "r{}", operand.signed_value);
                }
                OperandKind::Idx => {
                    let _ = write!(out, "[{}]", operand.unsigned_value);
                }
                OperandKind::RuntimeId | OperandKind::IntrinsicId => {
                    let _ = write!(out, "rt#{}", operand.unsigned_value);
                }
                OperandKind::NativeContextIndex => {
                    let _ = write!(out, "nc[{}]", operand.unsigned_value);
                }
                OperandKind::None => {}
            }
        }
...
impl Disassembly {
    ...
    pub fn render_text(&self) -> String {
        ...
        for ins in &self.instructions {
            let _ = writeln!(out, "{:>5}: {}", ins.offset, ins.render());
        }
        out
    }
