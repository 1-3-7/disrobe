//! Pre-evaluation safety guard bounding syntactic nesting before the `boa` JS sandbox parses.

/// Maximum bracket / paren / brace nesting depth accepted before a script is rejected.
pub(crate) const MAX_SYNTACTIC_NESTING_DEPTH: usize = 600;

/// Maximum length of a single left-associative binary-operator chain (`a + b + c + …`).
...
/// Returns the deepest `(`/`[`/`{` nesting reached in `script`, ignoring literals and comments.
#[must_use]
...
/// Returns the longest run of consecutive binary operators at a single grouping level.
#[must_use]
...
/// `true` when `script` is safe to hand to the `boa` parser without risking a stack overflow.
#[must_use]
