//! Shared string-/comment-aware balanced-delimiter scanning over raw JS bytes.

/// Index of the `)` that closes the group opened just before `start`, or `None` if unbalanced.
#[must_use]
...
/// Index one past the closing quote of the string literal opening at `start`.
#[must_use]
