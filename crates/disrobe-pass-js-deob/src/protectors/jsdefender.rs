...
use crate::jscrambler::{
    JscramblerTransform, TransformOpts as JscramblerOpts,
...
pub fn deobfuscate(source: &str, opts: &ProtectorOptions) -> Result<ProtectorOutput> {
    ...
    let cff_out = jscrambler_reverse(
    ...
    let dead_out = jscrambler_reverse(JscramblerTransform::DeadCodeInjection, &current, &js_opts)?;
    ...
    let enc_out = jscrambler_reverse(JscramblerTransform::StringEncoding, &current, &js_opts)?;
