...
use oxc_ast::ast::{
    CallExpression, Expression, MemberExpression, ObjectPropertyKind, Program, PropertyKey,
...
fn walk_expression(
    ...
    if let Expression::CallExpression(call) = expr
        && is_create_element(call)
        && let Some(jsx) = render_element(call, source, stats)
    {
        edits.push(Edit {
...
fn is_create_element(call: &CallExpression<'_>) -> bool {
    let Some(member): Option<&MemberExpression<'_>> = call.callee.as_member_expression() else {
        return false;
    };
    let MemberExpression::StaticMemberExpression(sm) = member else {
        return false;
    };
    sm.property.name.as_str() == "createElement"
}
...
fn render_element(
    ...
    let tag_arg: &Expression<'_> = call.arguments[0].as_expression()?;
    let tag: Tag = classify_tag(tag_arg, source)?;

    let props_src: Option<String> = if call.arguments.len() >= 2 {
        let props_arg: &Expression<'_> = call.arguments[1].as_expression()?;
        match render_props(props_arg, source) {
            PropsResult::Skip => return None,
            PropsResult::None => None,
            PropsResult::Attrs(text) => Some(text),
        }
    } else {
        None
    };
    ...
    for child in call.arguments.iter().skip(2) {
        let child_expr: &Expression<'_> = child.as_expression()?;
        children.push(render_child(child_expr, source, stats)?);
    }

    let rendered: String = match tag {
        Tag::Fragment => {
            if props_src.is_some() {
                return None;
            }
            stats.fragments_restored += 1;
            render_fragment(&children)
        }
        Tag::Named(name) => {
            stats.elements_restored += 1;
            render_named(&name, props_src.as_deref(), &children)
        }
    };
    Some(rendered)
}
...
fn render_props(expr: &Expression<'_>, source: &str) -> PropsResult {
    match expr {
        Expression::NullLiteral(_) => PropsResult::None,
        Expression::Identifier(id) if id.name.as_str() == "undefined" => PropsResult::None,
            ...
            for prop in &obj.properties {
                match prop {
                    ObjectPropertyKind::ObjectProperty(p) => {
                        ...
                        parts.push(render_attr(key, &p.value, source));
                    }
...
fn render_child(
    ...
    if let Expression::CallExpression(call) = expr
        && is_create_element(call)
    {
        return render_element(call, source, stats);
    }
...
fn render_named(name: &str, props: Option<&str>, children: &[String]) -> String {
    let open_attrs: String = props.map_or_else(String::new, |p| format!(" {p}"));
    if children.is_empty() {
        return format!("<{name}{open_attrs} />");
    }
    let inner: String = children.concat();
    format!("<{name}{open_attrs}>{inner}</{name}>")
}

fn render_fragment(children: &[String]) -> String {
    let inner: String = children.concat();
    format!("<>{inner}</>")
}
