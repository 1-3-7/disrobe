//! Clean-room reimplementation of CFF switch-dispatch unflattening, ref
//! webcrack (MIT) `control-flow-switch.ts`. Recognizes the obfuscator.io
//! `const seq = "2|0|1".split("|"); let i = 0; while(true){ switch(seq[i++])
//! { case "N": ...; continue; } break; }` shape and splices the case bodies
//! back into the linear order encoded by the sequence string.
