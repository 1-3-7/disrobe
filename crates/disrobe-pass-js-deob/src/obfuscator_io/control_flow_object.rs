//! Clean-room reimplementation of the control-flow-object / proxy-function
//! merge, ref webcrack (MIT) `control-flow-object.ts`. Algorithm studied and
//! reimplemented from understanding over disrobe's byte-scanned IR: collect a
//! readonly dispatch object (inline props, later `obj.KEY = value` assignments,
//! and `const alias = obj`), then inline each `obj.KEY` / `obj.KEY(args)`
//! reference as the underlying string literal, binary/logical op, or forwarded
//! call. Self-defending / debug-protection IIFE spans are guarded out so the
//! merge never corrupts integrity wrappers.
...
fn collect_objects(source: &str) -> Vec<CfObject> {
    let Ok(re): Result<Regex, regex::Error> =
        Regex::new(r"(?:var|let|const)\s+([A-Za-z_$][\w$]*)\s*=\s*\{")
    else {
        return Vec::new();
    };
    let bytes: &[u8] = source.as_bytes();
    ...
    for caps in re.captures_iter(source) {
        ...
        let keyword: &str = caps
            .get(0)
            .map(|m: regex::Match<'_>| m.as_str())
            .and_then(|s: &str| s.split_whitespace().next())
            .unwrap_or("var");
        let next_nonws: usize = skip_ws(bytes, alias_end);
        let (range_end, decl_replacement): (usize, String) = if bytes.get(next_nonws) == Some(&b',')
        {
            (next_nonws + 1, format!("{keyword} "))
        } else {
            (alias_end, String::new())
        };
        consumed_until = range_end;
        out.push(CfObject {
            var: var.to_owned(),
            aliases,
            decl_range: whole.start()..range_end,
            decl_replacement,
            props,
        });
    }
...
fn render_ref(value: &PropValue, r: &MemberRef) -> Option<(Range<usize>, String)> {
    match value {
        ...
        | PropValue::LogicalOp {
            ...
            let l: &String = positional(left, args)?;
            let rg: &String = positional(right, args)?;
        ...
        PropValue::CallForward { arity } => {
            ...
            let callee: &String = &call_args[0];
            let forwarded: Vec<String> = call_args[1..].to_vec();
            Some((
