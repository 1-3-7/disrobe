//! Scope-aware control-flow-object (proxy) merge built on the oxc semantic
//! binding/reference graph. Unlike the regex/byte merge in
//! [`super::control_flow_object`], this pass resolves every reference to a
//! candidate proxy object exactly, so it is safe to inline references that live
//! inside self-defending / debug-protection IIFEs (the regex merge guards those
//! out to avoid corrupting the wrapper). A binding qualifies only when it is a
//! `const`/`var` object literal whose every reference is a read-only
//! `obj.KEY` / `obj['KEY']` member access and whose properties are all simple
//! string literals or proxy functions (`(a,b) => a OP b`, call-forwards).
...
pub(super) fn merge_scope_proxies(source: &str) -> ScopeProxyResult {
    ...
    for symbol_id in symbols.symbol_ids() {
        let Some((props, decl_span)) = candidate_object(source, symbols, nodes, symbol_id) else {
            continue;
        };
...
fn find_declarator<'a>(
    ...
    nodes.iter().find_map(|node: &oxc_semantic::AstNode<'a>| {
        ...
        let oxc_ast::ast::BindingPatternKind::BindingIdentifier(ident) = &declarator.id.kind else {
            return None;
        };
...
fn classify_value(source: &str, value: &Expression<'_>) -> Option<PropValue> {
    ...
    let Expression::FunctionExpression(func) = value else {
        return None;
    };
