...
fn inspect_declarator_for_rgf(decl: &VariableDeclarator<'_>, source: &str) -> Option<RgfShape> {
    ...
    let Some(init): &Option<Expression<'_>> = &decl.init else {
        return None;
    };
    let Expression::ArrayExpression(array_expr): &Expression<'_> = init else {
        return None;
    };
...
fn is_new_function(expr: &NewExpression<'_>) -> bool {
    let Expression::Identifier(ident): &Expression<'_> = &expr.callee else {
        return false;
    };
    ident.name == "Function"
}
...
fn call_matches_rgf_apply(callee: &Expression<'_>, array_id: &str) -> bool {
    let Expression::StaticMemberExpression(member): &Expression<'_> = callee else {
        return false;
    };
    if member.property.name != "apply" {
        return false;
    }
    let Expression::ComputedMemberExpression(inner): &Expression<'_> = &member.object else {
        return false;
    };
    let Expression::Identifier(id): &Expression<'_> = &inner.object else {
        return false;
    };
    id.name == array_id
}
...
fn identify_object_create_null_decl(stmt: &Statement<'_>) -> Option<String> {
    ...
    for declarator in &decl.declarations {
        let Some(name): Option<&str> = declarator.id.get_identifier_name() else {
            continue;
        };
        let Some(init): &Option<Expression<'_>> = &declarator.init else {
            continue;
        };
...
fn call_is_object_create_null(call: &oxc_ast::ast::CallExpression<'_>) -> bool {
    let Expression::StaticMemberExpression(member): &Expression<'_> = &call.callee else {
        return false;
    };
    if member.property.name != "create" {
        return false;
    }
    let Expression::Identifier(id): &Expression<'_> = &member.object else {
        return false;
    };
    if id.name != "Object" {
        return false;
...
fn assignment_targets_table_index(target: &AssignmentTarget<'_>, table_id: &str) -> bool {
    let AssignmentTarget::ComputedMemberExpression(member): &AssignmentTarget<'_> = target else {
        return false;
    };
    let Expression::Identifier(id): &Expression<'_> = &member.object else {
        return false;
...
pub fn detect_calculator_shapes(source: &str) -> Vec<CalculatorShape> {
    ...
    for stmt in &parsed.program.body {
        let candidate: Option<(String, usize)> = match stmt {
            Statement::FunctionDeclaration(fd) => {
                let Some(name): Option<String> = fd.id.as_ref().map(|i| i.name.to_string()) else {
                    continue;
                };
                fd.body
                    .as_ref()
            ...
            Statement::VariableDeclaration(decl) => decl.declarations.iter().find_map(|d| {
                ...
                let Expression::FunctionExpression(func): &Expression<'_> = init else {
                    return None;
                };
                let body: &oxc_allocator::Box<'_, oxc_ast::ast::FunctionBody<'_>> =
                    func.body.as_ref()?;
