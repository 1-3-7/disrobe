...
pub fn reverse_flatten(source: &str) -> FlattenReversalResult {
    ...
    for caps in re.captures_iter(source) {
        ...
        let Some(sequence) = unroll_sequence(&cases, state_var, initial_state) else {
            continue;
        };
        let replacement: String = sequence.join("\n");
        let Some(decl_range) = locate_state_decl(source, state_var, whole.start()) else {
            continue;
        };
...
fn parse_switch_cases(body: &str) -> BTreeMap<i64, String> {
    ...
    for (idx, mat) in matches.iter().enumerate() {
        let Some(key_match) = re.captures(&body[mat.start()..mat.end()]) else {
            continue;
        };
...
fn unroll_sequence(
    ...
    while !visited.contains(&current) {
        ...
        let Some(case_body) = cases.get(&current) else {
            return Some(out);
        };
