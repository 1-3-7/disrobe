...
fn match_string_eq(s: &str) -> Option<PredicateValue> {
    ...
    let rhs: &str = cap.get(3).or_else(|| cap.get(4))?.as_str();
    Some(pick(lhs == rhs))
}
...
fn collect_if_edits(source: &str, edits: &mut Vec<(Range<usize>, Option<String>)>) {
    ...
    for mat in re.find_iter(source) {
        ...
        let replacement: String = match value {
            PredicateValue::AlwaysTrue => source[body_start + 1..body_close].trim().to_owned(),
            PredicateValue::AlwaysFalse => String::new(),
        };
        edits.push((mat.start()..body_close + 1, Some(replacement)));
    }
}
