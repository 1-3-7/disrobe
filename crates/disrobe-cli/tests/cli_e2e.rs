...
fn detect_machine_formats_preserve_real_packer_findings() {
    ...
    .expect("write benign UPX fixture");
    let path: &str = input.to_str().expect("fixture path");
    let json: Run = run_disrobe(&["detect", path, "--json"]);
    assert_eq!(json.code, 0, "{}", json.stderr);
    let report: serde_json::Value = serde_json::from_str(&json.stdout).expect("JSON report");
    let hits: &[serde_json::Value] = report["detections"].as_array().expect("detections");
    assert!(hits.iter().any(|hit: &serde_json::Value| {
        hit["pass_id"] == "native.packer-unpack"
            && hit["confidence"]
                .as_f64()
                .is_some_and(|value: f64| value >= 0.8)
            && hit["markers"]
                .as_array()
                .is_some_and(|values: &Vec<serde_json::Value>| !values.is_empty())
    }));
    let ndjson: Run = run_disrobe(&["detect", path, "--ndjson"]);
    assert_eq!(ndjson.code, 0, "{}", ndjson.stderr);
    assert_eq!(ndjson.stdout.lines().count(), 1);
    assert_eq!(
        serde_json::from_str::<serde_json::Value>(&ndjson.stdout).expect("NDJSON report"),
        report
    );
    let sarif: Run = run_disrobe(&["detect", path, "--sarif"]);
    assert_eq!(sarif.code, 0, "{}", sarif.stderr);
    let document: serde_json::Value = serde_json::from_str(&sarif.stdout).expect("SARIF document");
    assert_eq!(document["version"], "2.1.0");
    assert_eq!(document["runs"][0]["tool"]["driver"]["name"], "disrobe");
    let results: &[serde_json::Value] = document["runs"][0]["results"]
        .as_array()
        .expect("SARIF results");
    assert_eq!(results.len(), hits.len());
    for (result, hit) in results.iter().zip(hits) {
        assert_eq!(result["ruleId"], hit["pass_id"]);
        let sarif_confidence: f32 =
            serde_json::from_value(result["properties"]["confidence"].clone())
                .expect("SARIF confidence");
        let json_confidence: f32 =
            serde_json::from_value(hit["confidence"].clone()).expect("JSON confidence");
        assert_eq!(sarif_confidence.to_bits(), json_confidence.to_bits());
        assert_eq!(result["properties"]["markers"], hit["markers"]);
        assert_eq!(result["level"], "note");
        assert!(
            result["message"]["text"]
                .as_str()
                .expect("message")
                .contains(hit["display_name"].as_str().expect("name"))
        );
        let uri: &str = result["locations"][0]["physicalLocation"]["artifactLocation"]["uri"]
            .as_str()
            .expect("artifact URI");
        assert!(uri.contains("%20") && uri.contains("%C3%BC"), "{uri}");
    }
    drop(scratch);
}
...
fn detect_machine_formats_report_no_findings_for_plain_text() {
    ...
    std::fs::write(&input, b"ordinary documentation text\n").expect("write plain text");
    for format in ["--json", "--ndjson", "--sarif"] {
        let result: Run = run_disrobe(&["detect", input.to_str().expect("path"), format]);
        assert_eq!(result.code, 0, "{}", result.stderr);
        let document: serde_json::Value =
            serde_json::from_str(&result.stdout).expect("structured output");
        let findings: &serde_json::Value = if format == "--sarif" {
            &document["runs"][0]["results"]
        } else {
            &document["detections"]
        };
        assert_eq!(findings.as_array().expect("findings array").len(), 0);
    }
}
