...
fn load_corpus(rel: &[&str]) -> Vec<u8> {
    ...
    std::fs::read(&path)
        .unwrap_or_else(|e: std::io::Error| panic!("read corpus fixture {path:?}: {e}"))
}
...
fn assert_golden_at(path: &PathBuf, name: &str, doc: &ChainDocument) {
    ...
    let expected: String = std::fs::read_to_string(path)
...
fn golden_jvm_classfile() {
    let spec: ChainSpec = ChainSpec::Auto { cap: 8 };
    let seed: Vec<u8> = load_corpus(&["jvm", "proguard", "Hello-baseline.class"]);
    let doc: ChainDocument = run_to_document(
        seed,
        &spec,
...
fn golden_wasm_module() {
    let spec: ChainSpec = ChainSpec::Auto { cap: 8 };
    let seed: Vec<u8> = load_corpus(&["wasm", "wat", "function_refs.wasm"]);
    let doc: ChainDocument = run_to_document(
        seed,
        &spec,
...
fn golden_beam_module() {
    let spec: ChainSpec = ChainSpec::Auto { cap: 8 };
    let seed: Vec<u8> = load_corpus(&["beam", "erlang", "hello.beam"]);
    let doc: ChainDocument = run_to_document(
        seed,
        &spec,
...
fn golden_beam_edge_cases_module() {
    let spec: ChainSpec = ChainSpec::Auto { cap: 8 };
    let seed: Vec<u8> = load_corpus(&["beam", "megafile", "edge_cases.beam"]);
    let doc: ChainDocument = run_to_document(
        seed,
        &spec,
