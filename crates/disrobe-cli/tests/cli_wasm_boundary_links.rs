...
use disrobe_pass_wasm_deob::{BoundaryLinks, MAX_BOUNDARY_LINKS, extract_signatures};
...
fn wasm_deob_writes_a_canonical_boundary_links_sidecar() {
    ...
    let report: serde_json::Value = serde_json::from_slice(
        &std::fs::read(output.with_extension("recovery.json")).expect("recovery report"),
    )
    .expect("parse recovery report");
    assert_eq!(
        report["boundary_link_collection_status"]["status"],
        "complete"
    );
    assert!(
        report.get("mba_expressions_folded").is_some(),
        "existing recovery fields remain at the root"
    );
...
#[test]
fn wasm_deob_reports_truncated_boundary_links_without_losing_exports() {
    let count: usize = MAX_BOUNDARY_LINKS + 1;
    let mut source: String = "(module".to_owned();
    for index in 0..count {
        source.push_str("(func (export \"call_");
    ...
    let scratch: tempfile::TempDir = tempfile::tempdir().expect("scratch directory");
    let input: PathBuf = scratch.path().join("many-exports.wasm");
    let output: PathBuf = scratch.path().join("many-exports.deob.wat");
    std::fs::write(&input, &module).expect("write module");
    let result: std::process::Output = Command::new(cli_binary())
        .args(["wasm", "deob"])
        .arg(&input)
        .arg("--out")
        .arg(&output)
    ...
    .expect("parse recovery report");
    assert_eq!(
        report["boundary_link_collection_status"],
        serde_json::json!({"status": "truncated", "link_count": count, "retained_link_count": MAX_BOUNDARY_LINKS}),
    );
    let links: BoundaryLinks = BoundaryLinks::from_json(
        &std::fs::read(output.with_extension("boundary-links.json")).expect("boundary links"),
    )
    .expect("strict boundary-links schema remains valid");
    assert_eq!(links.links().len(), MAX_BOUNDARY_LINKS);
    let recovered: Vec<u8> = wat::parse_file(&output).expect("recovered WAT assembles");
    let summary: disrobe_pass_wasm_deob::ModuleSummary =
        disrobe_pass_wasm_deob::analyze_module(&recovered).expect("recovered summary");
    assert_eq!(summary.exports.len(), count);
}
