...
const SOURCE: &str =
    include_str!("../../disrobe-pass-js-deob/tests/fixtures/rollup_iife_param/fixture.min.js");
    ...
    for entry in entries.filter_map(Result::ok) {
        ...
        if let Ok(source) = std::fs::read_to_string(&path) {
            ...
            if compact.contains("function(MathUtils,TextFormat)")
                && compact.contains("TextFormat(MathUtils.sum(20,22))")
            {
                return Some(source);
            }
        }
...
#[test]
fn auto_recovers_rollup_iife_global_parameter_names() {
    assert_eq!(SOURCE.len(), 224);
    let input_scratch: disrobe_core::scratch::ScratchDir = scratch("auto-js-iife-input");
    let input: PathBuf = input_scratch.path().join("bundle.js");
    std::fs::write(&input, SOURCE).expect("write Rollup IIFE fixture");

    let output_scratch: disrobe_core::scratch::ScratchDir = scratch("auto-js-iife-output");
    ...
    assert!(
        ...
        "auto must route the Rollup IIFE through js.deob: {passes:?}"
    );
    let recovered: String = find_recovered_source(output_scratch.path())
        .expect("captured js.deob output must contain both recovered global names");
    assert!(!recovered.contains("function(s,u)"));
}
