...
//! End-to-end proof that `disrobe as3 disasm --emit source` decompiles a real
//! SWF (FWS container + DoABC tag wrapping a spec-accurate hand-assembled ABC)
//! into readable AS3 with lifted method bodies. Real-corpus SWF fixtures are
//! gitignored and absent in CI, so this synthesises a minimal-but-valid SWF;
//! the lifter walks the same production code path a real DoABC block would.
...
/// Hand-assemble a one-class ABC modelling:
/// `public class Greeter extends Object { public function greet(): * { trace("hi"); } }`.
///
/// Pool indices: strings 1=Greeter 2=Object 3=trace 4=greet 5=hi; namespace 1
/// = public package; multinames 1=Greeter 2=Object 3=trace 4=greet.
fn build_abc() -> Vec<u8> {
