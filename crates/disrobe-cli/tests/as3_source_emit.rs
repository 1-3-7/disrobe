...
//! End-to-end proof that `disrobe as3 disasm --emit source` decompiles a synthesised SWF into readable AS3 with lifted method bodies.
...
/// Hand-assemble a one-class ABC modelling a `Greeter` class with a `greet` method that traces `"hi"`.
fn build_abc() -> Vec<u8> {
