...
fn action_out_and_path_pairing_matches_the_built_cli() {
    ...
    let sarif_validation: usize = script
        .find("if [ ! -s \"${DR_SARIF}\" ]")
        .expect("missing SARIF rejection");
    assert!(
        rejection < invocation
            && invocation < runtime_rejection
            && runtime_rejection < sarif_validation,
        "argument rejection must precede invocation, and the runtime rejection guard must precede SARIF validation"
    );
}
...
#[test]
fn capability_map_covers_the_compiled_commands_and_reachable_references() {
    let root: PathBuf = workspace_root();
    let map: serde_json::Value =
        serde_json::from_str(&read(&root.join("evidence/capabilities.json")))
            .expect("capability map JSON");
    let cli: &serde_json::Value = &map["cli"];
    let declared: BTreeSet<String> = capability_strings(cli, "root_commands");
    let hidden: BTreeSet<String> = capability_strings(cli, "hidden_commands");
    let compiled: BTreeSet<String> = top_level_commands();
    assert!(
        compiled.is_subset(&declared),
        "unmapped commands: {:?}",
        compiled.difference(&declared)
    );
    if cfg!(feature = "full") {
        assert_eq!(
            compiled, declared,
            "full-build commands differ from the map"
        );
    }
    assert_eq!(
        cli["root_command_count"].as_u64(),
        Some(declared.len() as u64)
    );
    assert!(hidden.is_subset(&declared));

    let readme: String = read(&root.join("README.md"));
    for command in declared.difference(&hidden) {
        assert!(
            readme.contains(&format!("`{command}`")),
            "README does not expose the {command} command"
        );
    }

    let mut grouped: BTreeSet<String> = BTreeSet::new();
    let mut identities: BTreeSet<String> = BTreeSet::new();
    for category in ["feature_groups", "public_integrations"] {
        let surfaces: &[serde_json::Value] =
            map[category].as_array().expect("capability surfaces array");
        assert!(!surfaces.is_empty(), "{category} is empty");
        for surface in surfaces {
            let id: &str = surface["id"].as_str().expect("surface id");
            assert!(identities.insert(id.to_owned()), "duplicate surface {id}");
            if category == "feature_groups" {
                for command in capability_strings(surface, "commands") {
                    assert!(
                        grouped.insert(command.clone()),
                        "{command} is grouped twice"
                    );
                }
            }
            let entry: &str = surface["entry_point"].as_str().expect("entry point");
            assert!(
                root.join(entry).is_file(),
                "{id}: missing entry point {entry}"
            );
            for field in ["source", "tests", "docs", "demos"] {
                let paths: BTreeSet<String> = capability_strings(surface, field);
                if field != "demos" {
                    assert!(!paths.is_empty(), "{id}: {field} is empty");
                }
                for path in paths {
                    assert!(
                        Path::new(&path).components().all(
                            |part: std::path::Component<'_>| matches!(
                                part,
                                std::path::Component::Normal(_)
                            )
                        ),
                        "{id}: reference must stay inside the repository: {path}"
                    );
                    ...
                    assert!(
                        ...
                        "{id}: missing {field} reference {path}"
                    );
                }
            }
            if let Some(status) = surface.get("status") {
                let status: &str = status.as_str().expect("capability status");
                assert!(
                    map["status_definitions"].get(status).is_some(),
                    "{id}: undefined status {status}"
                );
                if status == "working" {
                    let result: &str = surface["accepted_result"]
                        .as_str()
                        .expect("working support requires an accepted result reference");
                    assert!(
                        root.join(result).is_file(),
                        "{id}: missing accepted result {result}"
                    );
                }
            }
        }
    }
    let expected: BTreeSet<String> = declared.difference(&hidden).cloned().collect();
    assert_eq!(
        grouped, expected,
        "every visible command needs exactly one feature group"
    );
}

fn capability_strings(value: &serde_json::Value, key: &str) -> BTreeSet<String> {
    let values: &[serde_json::Value] = value[key].as_array().expect("string array");
    let strings: BTreeSet<String> = values
        .iter()
        .map(|entry: &serde_json::Value| entry.as_str().expect("string entry").to_owned())
        .collect();
    assert_eq!(strings.len(), values.len(), "{key} has duplicate entries");
    strings
}
