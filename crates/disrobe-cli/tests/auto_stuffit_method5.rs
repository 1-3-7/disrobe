...
const FIXTURE: &[u8] =
    include_bytes!("../../disrobe-binfmt/tests/fixtures/stuffit/stuffit-method5.sit");
const MANIFEST: &str =
    include_str!("../../disrobe-binfmt/tests/fixtures/stuffit/MANIFEST_METHOD5.tsv");
...
#[test]
fn auto_recovers_stuffit_method5_identically_at_jobs_one_and_four() {
    let input: disrobe_core::scratch::ScratchDir =
        disrobe_core::scratch::ScratchDir::create("auto-stuffit-method5-input")
            .expect("create method 5 input directory");
    std::fs::write(input.path().join("fixture.sit"), FIXTURE)
        .expect("stage StuffIt method 5 fixture");
    let output: disrobe_core::scratch::ScratchDir =
        disrobe_core::scratch::ScratchDir::create("auto-stuffit-method5-output")
            .expect("create method 5 output directory");
    ...
    assert_eq!(
        expected_hashes.len(),
        3,
        "the pinned method 5 archive carries three distinct forks"
    );
    ...
    assert_eq!(
        serial.len(),
        expected_hashes.len(),
        "disrobe auto must reach every method 5 fork the reference recovered"
    );
}
