...
/// Spawns `disrobe serve` bound to an ephemeral port with a hard wall-clock
/// deadline of [`SERVE_TEST_DEADLINE`]; a watchdog thread aborts the process
/// (and panics the test thread) if the deadline elapses.
