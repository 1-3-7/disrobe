...
const FIXTURE: &[u8] =
    include_bytes!("../../disrobe-binfmt/tests/fixtures/stuffit/stuffit5-651.sit");
const MANIFEST: &str =
    include_str!("../../disrobe-binfmt/tests/fixtures/stuffit/MANIFEST_SIT5.tsv");
...
#[test]
fn auto_recovers_stuffit5_identically_at_jobs_one_and_four() {
    let input: disrobe_core::scratch::ScratchDir =
        disrobe_core::scratch::ScratchDir::create("auto-stuffit5-input")
            .expect("create stuffit 5 input directory");
    std::fs::write(input.path().join("fixture.sit"), FIXTURE).expect("stage StuffIt 5 fixture");
    let output: disrobe_core::scratch::ScratchDir =
        disrobe_core::scratch::ScratchDir::create("auto-stuffit5-output")
            .expect("create stuffit 5 output directory");

    let expected_hashes: BTreeSet<&str> = MANIFEST
        .lines()
        .skip(1)
        .filter(|row: &&str| !row.trim().is_empty())
        .map(|row: &str| row.split('\t').nth(5).expect("manifest SHA-256"))
        .collect();
    assert_eq!(
        expected_hashes.len(),
        9,
        "the pinned StuffIt 5 archive carries nine forks"
    );
    ...
    assert_eq!(
        serial, parallel,
        "one worker and four workers must produce identical bytes"
    );
    assert_eq!(
        serial.len(),
        expected_hashes.len(),
        "disrobe auto must reach every StuffIt 5 fork the reference recovered"
    );
}
