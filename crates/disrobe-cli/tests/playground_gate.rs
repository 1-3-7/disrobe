...
const TRACKED_DETECTION_PINS: [&str; 2] = ["dotnet:HelloApp", "dotnet:EdgeCases_baseline"];

const TRACKED_DIFFERENTIAL_PINS: [&str; 2] =
    ["playground:berserker_greeting", "berserker:edge_async_fn"];
...
fn build_sample() -> Vec<ResolvedFixture> {
    ...
    for id in TRACKED_DETECTION_PINS {
        push_named(&resolved, &mut sample, id);
    }
    for id in TRACKED_DIFFERENTIAL_PINS {
        push_named(&resolved, &mut sample, id);
    }
...
fn byte_identical_oracle_has_at_least_one_real_unpacker_route() {
    ...
    assert!(
        routed,
        "byte-identical oracle must route at least one upx/fsg/mew fixture to a real in-house unpacker",
    );
}

#[test]
fn pinned_fixtures_are_git_tracked_so_ci_matches_local() {
    let index: ManifestIndex = ManifestIndex::build(&corpus_root());
    let resolved: Vec<ResolvedFixture> = index.resolve(&corpus_root());
    let runner: Runner = Runner::new(RunnerConfig::default());
    let pins: [(&str, OracleKind, OracleVerdict); 4] = [
        (
            TRACKED_DETECTION_PINS[0],
            OracleKind::DetectionDeterministic,
            OracleVerdict::DetectCorrect,
        ),
        (
            TRACKED_DETECTION_PINS[1],
            OracleKind::DetectionDeterministic,
            OracleVerdict::DetectCorrect,
        ),
        (
            TRACKED_DIFFERENTIAL_PINS[0],
    ...
    for (id, kind, expected) in pins {
        let id: &str = id;
        let kind: OracleKind = kind;
        let expected: OracleVerdict = expected;
        let fx: &ResolvedFixture = resolved
            .iter()
            .find(|f: &&ResolvedFixture| f.fixture_id == id)
            .unwrap_or_else(|| panic!("pinned fixture {id} not resolved from any MANIFEST.toml"));
        assert_eq!(fx.oracle, kind, "pinned fixture {id} wrong oracle kind");
        assert!(
            fx.input_path.exists(),
            "pinned fixture {id} must be a GIT-TRACKED file present in a clean checkout (input {}); a gitignored pin reintroduces the oracles-no-op-in-clean-clone bug",
            fx.input_rel,
        );
        let verdict: OracleVerdict = runner.evaluate(fx);
        assert_eq!(
            verdict, expected,
            "pinned fixture {id} verdict regressed; the 100%-of-evaluated floor requires every pin to pass",
        );
    }
}
