...
fn serve_spawn_lock() -> MutexGuard<'static, ()> {
    static LOCK: OnceLock<Mutex<()>> = OnceLock::new();
    let mtx: &Mutex<()> = LOCK.get_or_init(|| Mutex::new(()));
    mtx.lock()
        .unwrap_or_else(|e: PoisonError<MutexGuard<'_, ()>>| e.into_inner())
}
...
/// Spawns `disrobe serve --grpc` and arms a watchdog that hard-kills the child
/// after [`GRPC_TEST_DEADLINE`] so a hung server cannot stall the test suite.
fn spawn_grpc_serve() -> Option<ServeHandle> {
    let bin: PathBuf = cli_binary();
    if !bin.exists() {
        return None;
    }
    let guard: MutexGuard<'static, ()> = serve_spawn_lock();
