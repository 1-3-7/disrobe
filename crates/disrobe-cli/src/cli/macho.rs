...
pub(crate) enum MachoCmd {
    #[command(
        about = "dump Mach-O / Fat-Mach-O / .ipa / .framework header, segments, sections, and encryption-info"
    )]
    Dump {
    ...
    #[command(
        about = "walk a Fat-Mach-O envelope and report each slice's CPU type, size, and offset"
    )]
    Fat {
