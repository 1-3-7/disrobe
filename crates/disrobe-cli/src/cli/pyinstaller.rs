...
pub(crate) enum PyinstallerCmd {
    ...
    #[command(
        about = "detect a PyInstaller build & report its cookie, Python version, & TOC offsets without extracting"
    )]
    Detect {
...
fn extract(input: PathBuf, out: Option<PathBuf>) -> miette::Result<()> {
    ...
    let manifest: serde_json::Value = serde_json::json!({
        ...
        "entries": result.entries.iter().map(|e| serde_json::json!({
            ...
            "pyc_compression": e.pyc_compression.map(disrobe_pass_pyinstaller::ZipperCompression::label),
        })).collect::<Vec<_>>(),
    });
    let manifest_bytes: Vec<u8> = serde_json::to_vec_pretty(&manifest)
        .map_err(|e| miette::miette!("DR-CLI-0016: serialize manifest: {e}"))?;
    std::fs::write(out_dir.join("manifest.json"), manifest_bytes)
        .map_err(|e| miette::miette!("DR-CLI-0014: cannot write manifest: {e}"))?;

    for entry in &result.entries {
        let Some(rel): Option<PathBuf> = safe_entry_path(&entry.toc.name) else {
            bar.tick();
            continue;
        };
        let suffix: &'static str = if entry.toc.entry_type.is_pyc_carrier()
            && !entry.toc.entry_type.carries_full_filename()
        {
            ".pyc"
        } else {
            ""
        };
        let path: PathBuf = if suffix.is_empty() {
            out_dir.join(&rel)
        } else {
            out_dir.join(format!("{}{suffix}", rel.display()))
        };
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| miette::miette!("DR-CLI-0017: cannot create dir: {e}"))?;
        }
...
fn safe_entry_path(name: &str) -> Option<PathBuf> {
    ...
    for raw in name.split(['/', '\\']) {
        let part: &str = raw.trim_start_matches('.');
        if part.is_empty() || raw == ".." || raw.contains(':') {
            if raw == ".." || raw.contains(':') {
                return None;
            }
            continue;
