...
mod tests {
    ...
    #[test]
    fn base64_rejects_invalid_quantum_length() {
        let bad: Result<Vec<u8>, String> = decode_base64("A");
        assert!(bad.is_err());
    }

    #[test]
    fn base64_rejects_padding_before_tail() {
        let bad: Result<Vec<u8>, String> = decode_base64("AA=A");
        assert!(bad.is_err());
    }
