...
#[cfg(feature = "nir-lift")]
fn aarch64_recover_image_source(
    code: &[u8],
    address: u64,
    emitted_name: &str,
    obj: &object::File<'_>,
    nir_recovery: Option<&PcodeRecovery>,
) -> Result<Option<String>, String> {
    use disrobe_pass_native::{LeafRecovery, recover_aarch64_function_with_image};
    use std::cell::Cell;

    let image_requested: Cell<bool> = Cell::new(false);
    let recovered: Result<LeafRecovery, disrobe_pass_native::Error> =
        recover_aarch64_function_with_image(
            code,
            address,
            &|image_address: u64| {
                image_requested.set(true);
                aarch64_image_slice(obj, image_address)
            },
            &|_: u64| None,
        );
    if !image_requested.get()
        && (recovered.is_err()
            || nir_recovery.is_some_and(|recovery: &PcodeRecovery| {
                recovery.structured && recovery.modelled == recovery.decoded
            }))
    {
        return Ok(None);
    }
    let recovered: LeafRecovery =
        recovered.map_err(|error: disrobe_pass_native::Error| error.to_string())?;
    Ok(Some(rename_emitted_function(
        &recovered.source,
        "recovered",
        emitted_name,
    )))
...
fn decompile_native_pcode<'data>(
    ...
    let arm_mapping: ArmMappingSymbols = match arch {
        DecompileArch::Arm32 => ArmMappingSymbols::of(obj),
        DecompileArch::AArch64 | DecompileArch::Mips32Be | DecompileArch::Mips32Le => {
    ...
    for f in module.functions() {
        ...
        let lifted: Result<PcodeRecovery, String> =
            pcode_recover_source(function_arch, &code, entry, &emitted_name, effective_devirt);
        ...
        let mut flagship: Option<(String, &'static str)> = if fold_wins {
            None
        } else {
            whole_program
                .get(&f.address)
                .map(|function: &&RecoveredFunction| {
                    (
                        rename_emitted_function(&function.source, &function.name, &emitted_name),
                        "whole-program",
                    )
                })
        };
        if flagship.is_none() && !fold_wins && arch.has_whole_program_engine() {
            match aarch64_recover_image_source(
                &code,
                f.address,
                &emitted_name,
                obj,
                ...
                Ok(Some(source)) => flagship = Some((source, "image-leaf")),
                Ok(None) => {}
                Err(reason) => {
                    unrecovered.push(serde_json::json!({
                        "name": f.name,
                        "address": f.address,
                        "reason": format!("image-backed recovery rejected the function: {reason}"),
                    }));
                    continue;
                }
            }
    ...
    let devirt_summary: serde_json::Value = if devirt_active {
        serde_json::json!({
            "enabled": true,
    ...
    } else if cfg!(feature = "devirt") {
        serde_json::json!({ "enabled": false })
    } else {
        serde_json::json!({
            "enabled": false,
            "available": false,
            "reason": "devirt feature is not built",
        })
    };
...
mod tests {
    ...
    #[cfg(feature = "nir-lift")]
    #[test]
    fn reading_thumb_code_in_a32_mode_invents_instructions_the_image_does_not_hold() {
        use disrobe_nir_lift::PcodeArch;

        let correct: PcodeRecovery =
            pcode_recover_source(PcodeArch::Arm32Thumb, ARM32_THUMB_O2, 0x0, "t", false)
                .expect("thumb code recovers in thumb mode");
        assert_eq!(
            correct.decoded, 22,
            "the committed thumb corpus holds 22 instructions"
        );
        assert_eq!(
            correct.modelled, correct.decoded,
            "every thumb instruction of the corpus is modelled"
        );

        let misread: PcodeRecovery =
            pcode_recover_source(PcodeArch::Arm32A32, ARM32_THUMB_O2, 0x0, "t", false)
                .expect("the a32 reading still produces a body");
        assert!(
            misread.modelled < correct.modelled,
            "reading thumb bytes as a32 recovered {} of {} instructions, so the mode argument is \
             being ignored",
            misread.modelled,
            correct.modelled
        );
        assert_ne!(
            misread.source, correct.source,
            "the two decode modes produced identical source from the same bytes"
        );
    }
    ...
    #[cfg(feature = "nir-lift")]
    #[test]
    fn aarch64_leaf_preserves_recovery_when_nir_cannot_model_the_function() {
        let bytes: [u8; 8] = [0x20, 0x84, 0xa0, 0x4e, 0xc0, 0x03, 0x5f, 0xd6];
        let nir: Result<PcodeRecovery, String> = pcode_recover_source(
            disrobe_nir_lift::PcodeArch::AArch64,
            &bytes,
            0x1000,
            "vector_add",
        ...
        assert!(
            !nir.as_ref().is_ok_and(|recovery: &PcodeRecovery| {
                recovery.structured && recovery.modelled == recovery.decoded
            }),
            "the control must exercise a function outside complete structured NIR recovery"
        );
        let image: Vec<u8> = object::write::Object::new(
            object::BinaryFormat::Elf,
            object::Architecture::Aarch64,
            object::Endianness::Little,
        )
        .write()
        .expect("write empty ELF image");
        let obj: object::File<'_> = object::File::parse(image.as_slice()).expect("parse ELF image");
        let source: String =
            aarch64_recover_image_source(&bytes, 0x1000, "vector_add", &obj, nir.as_ref().ok())
                .expect("leaf recovery succeeds without mapped image data")
                .expect("successful leaf recovery must remain available when NIR cannot model it");
        assert!(source.contains("vector_add("), "{source}");
        assert!(source.contains("v0 = v1 + v0"), "{source}");
    }
