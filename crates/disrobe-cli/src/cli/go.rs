...
pub(crate) enum GoCmd {
    #[command(
        about = "recover symbols, pclntab, moduledata, garble obfuscation report, and embed.FS contents from a Go PE / ELF / Mach-O"
    )]
    Recover {
    ...
    #[command(about = "report Go build version, pclntab version, and stripped/garble fingerprint")]
    Info {
        #[arg(help = "input Go binary")]
        input: PathBuf,
    },
...
fn recover(input: PathBuf, out: Option<PathBuf>) -> miette::Result<()> {
    ...
    println!("  embed files:  {}", analysis.embed.files.len());
