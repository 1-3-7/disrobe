...
pub(crate) fn run(action: PhpCmd) -> miette::Result<()> {
    match action {
        PhpCmd::Decode {
            ...
            list,
        } => decode(input, out, encoder, i_have_authorization, list),
...
fn decode(
    ...
    if list {
        super::emit::print_obfuscator_catalog(
            ...
            "disrobe php decode <input.php> --out <output-dir>",
        );
        return Ok(());
    }
    let Some(input): Option<PathBuf> = input else {
        return Err(miette::miette!(
            "DR-CLI-0550b: php decode needs an input file (or `--list` to show supported obfuscators)"
        ));
    };
    ...
    let manifest_bytes: Vec<u8> = serde_json::to_vec_pretty(&manifest)
        .map_err(|e| miette::miette!("DR-CLI-0558: serialize manifest: {e}"))?;
...
fn deobfuscate(
    ...
    let manifest_bytes: Vec<u8> = serde_json::to_vec_pretty(&manifest)
        .map_err(|e| miette::miette!("DR-CLI-0565: serialize manifest: {e}"))?;
    std::fs::write(&manifest_path, manifest_bytes)
...
fn extract(input: PathBuf, out: Option<PathBuf>) -> miette::Result<()> {
    ...
    let manifest_bytes: Vec<u8> = serde_json::to_vec_pretty(&manifest)
        .map_err(|e| miette::miette!("DR-CLI-0577: serialize manifest: {e}"))?;
    std::fs::write(&manifest_path, manifest_bytes)
...
fn decode_encoder(
    ...
    let surface: Option<disrobe_pass_php::ContainerSurface> = match family {
        EncoderFamily::IonCube | EncoderFamily::SourceGuardian => None,
        EncoderFamily::ZendGuard => disrobe_pass_php::surface_zend_guard(bytes).ok(),
    };
    let surface_json: serde_json::Value = surface.as_ref().map_or(serde_json::Value::Null, |s| {
        serde_json::json!({
            "container_parsed": s.container_parsed,
            "header_fields": s.header_fields.iter()
                .map(|(k, v)| (k.clone(), *v))
                .collect::<std::collections::BTreeMap<_, _>>(),
            "static_layers_stripped": s.static_layers_stripped.iter()
                .map(|l| l.label())
                .collect::<Vec<_>>(),
            "opcode_stream_len": s.opcode_stream_len,
            "source_reconstructed": s.source_reconstructed,
            "wall_note": s.wall_note,
        })
    });
    ...
    Ok(serde_json::json!({
        ...
        "container_surface": surface_json,
