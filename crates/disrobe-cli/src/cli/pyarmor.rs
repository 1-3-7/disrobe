...
pub(crate) enum PyarmorCmd {
    ...
    Unpack {
        ...
        #[arg(
            long,
            help = "list the PyArmor versions disrobe can detect and unpack, then exit"
        )]
        list: bool,
...
fn unpack(
    ...
    if list {
        crate::cli::emit::print_obfuscator_catalog(
            ...
            "disrobe pyarmor unpack <wrapper.py> --out <out-dir>",
        );
        return Ok(());
    }
    let Some(input): Option<PathBuf> = input else {
        return Err(miette::miette!(
            "DR-CLI-0047: pyarmor unpack needs an input file (or `--list` to show supported versions)"
        ));
    };
...
fn compute_limitations(result: &disrobe_pass_pyarmor::UnpackOutput) -> Vec<String> {
    ...
    {
        limits.push(
            "v6/v7 static key extraction did not resolve for this image (super-mode); used dynamic-hook fallback."
                .to_owned(),
        );
    }
    if let Some(ref dh) = result.dynamic_hook {
    ...
    if matches!(
        ...
        limits.push(
            "PyArmor v3/v4/v5 detected; static decryption is an information-theoretic wall: the code-object AES-128-CTR key is RSA-wrapped in the capsule and absent from the distributed artifact. Recovery needs the original capsule private key or a runtime dump under the matching Python build."
                .to_owned(),
        );
    }
    if matches!(
