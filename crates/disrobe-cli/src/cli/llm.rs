...
//!
//! Builds a [`disrobe_llm_metadata::MetadataSelection`] from the clap-parsed
//! flag bundle and writes a full schema-conforming bundle to disk via
//! [`disrobe_llm_metadata::BundleBuilder`]. CLI errors in this module use the
//! `DR-CLI-04xx` code band.
...
impl LlmFlags {
    /// Resolve clap-parsed flags to a `MetadataSelection`.
    ///
    /// Returns `Ok(None)` when no flag was passed - callers must skip LLM bundle
    /// generation entirely in that case.
...
/// Compute the blake3 hex of a byte slice using `disrobe_ir::compute_root_hash`
/// where available; falls back to the `blake3` workspace crate here so the CLI
/// does not need to round-trip through IR for sidecar emission.
#[must_use]
...
/// Build the LLM bundle from one or more per-pass envelope maps & write it.
///
/// When `flags.llm_briefs` is set, also renders deterministic `AGENTS.md` &
/// `SKILL.md` reconstruction briefs into the output directory (the parent of
/// `primary_output`).
