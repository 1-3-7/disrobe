...
fn style_block() -> String {
    let mut theme: String = SHARED_THEME_TOKENS.to_owned();
    for (path, bytes, license) in [
        (
            "../../assets/fonts/Manrope.ttf",
            include_bytes!("../../../../docs/src/assets/fonts/Manrope.ttf").as_slice(),
            include_str!("../../../../docs/src/assets/fonts/Manrope-OFL.txt"),
        ),
        (
            "../../assets/fonts/JetBrainsMono-Regular.ttf",
            include_bytes!("../../../../docs/src/assets/fonts/JetBrainsMono-Regular.ttf")
                .as_slice(),
            include_str!("../../../../docs/src/assets/fonts/JetBrainsMono-OFL.txt"),
        ),
    ] {
        let encoded: String = base64::engine::general_purpose::STANDARD.encode(bytes);
        theme = theme.replace(path, &format!("data:font/ttf;base64,{encoded}"));
        let _ = write!(theme, "\n/* {license} */\n");
    }
    format!(
        "{theme}\
...
.brand{{display:flex;align-items:center;gap:8px;font-family:{FONT_SANS};\
font-size:24px;font-weight:200;letter-spacing:-.02em;color:{COLOR_TEXT}}}\
.brand .mark{{flex:none}}\
.brand .dim{{font-family:{FONT_MONO};font-size:13px;color:{COLOR_MUTED};\
...
.panel:focus-visible{{outline:2px solid {COLOR_TEXT};outline-offset:3px}}\
.panel pre{{margin:0;padding:16px;font-family:{FONT_MONO};white-space:pre-wrap;overflow-wrap:anywhere}}\
...
table:has(th:nth-child(3)){{min-width:56rem}}\
...
.empty{{padding:16px;color:{COLOR_MUTED};font-size:13px;overflow-wrap:anywhere}}\
...
:root[data-theme=light] .brand .mark,:root[data-theme=light] footer img{{filter:invert(1)}}\
@media (prefers-reduced-motion:no-preference){{\
a,.chip,.stage{{transition:color .15s ease,opacity .15s ease}}}}"
    )
}

fn brand_icon_uri() -> String {
    format!(
        "data:image/svg+xml;base64,{}",
        base64::engine::general_purpose::STANDARD.encode(include_bytes!(
            "../../../../docs/assets/brand/section-mark-dark.svg"
        ))
    )
}
...
fn doc_close(tool_version: &str, out: &mut String) {
    let _: Result<(), std::fmt::Error> = write!(
        out,
        "<footer><img src=\"{mark}\" width=\"18\" height=\"18\" alt=\"\"><span class=\"mono\">disrobe {ver}</span>\
...
fn brand_mark() -> String {
    format!(
        "<span class=\"brand\"><img class=\"mark\" src=\"{ico}\" width=\"24\" height=\"24\" alt=\"\">\
...
fn render_artifacts(report: &SingleReport, out: &mut String) {
    ...
    if report.artifacts.is_empty() {
        out.push_str(
            "<div class=\"panel\"><div class=\"empty\">no artifact paths recorded</div></div></section>",
        );
        return;
    }
    out.push_str(
...
fn render_capabilities(report: &SingleReport, out: &mut String) {
    ...
    out.push_str(
        "<div class=\"panel\" tabindex=\"0\" role=\"region\" aria-label=\"Capabilities table\"><table><thead><tr><th class=\"r\">address</th><th>rule</th>\
...
fn render_indicators(section: &IndicatorSection, out: &mut String) {
    ...
    out.push_str(
        "<div class=\"panel\" tabindex=\"0\" role=\"region\" aria-label=\"Indicators table\"><table><colgroup><col style=\"width:14ch\">\
...
fn render_behavior(report: &BehaviorReport, out: &mut String) {
    ...
    out.push_str(
        "<div class=\"panel\" tabindex=\"0\" role=\"region\" aria-label=\"Behavior table\">",
    );
...
fn render_walls(report: &SingleReport, out: &mut String) {
    ...
    out.push_str(
        "<div class=\"panel\" tabindex=\"0\" role=\"region\" aria-label=\"Failures table\"><table><thead><tr><th>kind</th><th class=\"r\">node</th>\
...
fn render_evidence(report: &SingleReport, out: &mut String) {
    ...
    out.push_str(
        "<div class=\"panel\" tabindex=\"0\" role=\"region\" aria-label=\"Evidence table\"><table><thead><tr><th>role</th><th>artifact</th>\
...
<span class=\"schema\">schema {schema}</span></div>",
    ...
    out.push_str(
        "<div class=\"panel\" tabindex=\"0\" role=\"region\" aria-label=\"Batch recovery table\"><table><colgroup><col><col style=\"width:14ch\">\
...
mod tests {
    ...
    fn assert_self_contained_html(html: &str) {
        ...
        let mut resources: String = html.to_owned();
        for license in [
            include_str!("../../../../docs/src/assets/fonts/Manrope-OFL.txt"),
            include_str!("../../../../docs/src/assets/fonts/JetBrainsMono-OFL.txt"),
        ] {
            assert!(!license.contains("*/"));
            let comment: String = format!("/* {license} */");
            assert_eq!(resources.matches(&comment).count(), 1);
            resources = resources.replace(&comment, "");
        }
        assert!(!resources.to_lowercase().contains("http://"));
        assert!(
            !resources.to_lowercase().contains("https://"),
            "no external/CDN URLs may appear outside the embedded font licenses"
        );
        assert!(!resources.contains("cdn"));
    }

    #[test]
    fn offline_check_rejects_external_resources_beside_font_licenses() {
        ...
        for resource in [
            ...
            "<style>@font-face{src:url(http://example.com/font.ttf)}</style>",
        ] {
            let external: String = html.replacen("</head>", &format!("{resource}</head>"), 1);
            assert!(
                std::panic::catch_unwind(|| assert_self_contained_html(&external)).is_err(),
                "the offline check must reject an external resource"
            );
        }
    }
    ...
    #[test]
    fn design_tokens_and_embedded_fonts_present() {
        let report: SingleReport = sample_single();
        let html: String = render_single_html(&report, &Enrichment::default());
        for line in SHARED_THEME_TOKENS
            .lines()
            .filter(|line| !line.starts_with("@font-face"))
        {
            assert!(html.contains(line), "missing shared theme rule: {line}");
        }
        assert!(!html.contains("../../assets/fonts/"));
        let icon: &str = html
            .split_once("data:image/svg+xml;base64,")
            .expect("embedded report icon")
            .1
            .split('"')
            .next()
            .expect("icon URI");
        assert_eq!(
            ...
            include_bytes!("../../../../docs/assets/brand/section-mark-dark.svg")
        );
        let fonts: Vec<Vec<u8>> = html
            .split("data:font/ttf;base64,")
            .skip(1)
            .map(|tail| {
                base64::engine::general_purpose::STANDARD
                    .decode(tail.split('"').next().expect("font URI"))
        ...
        assert_eq!(
            fonts[0],
            include_bytes!("../../../../docs/src/assets/fonts/Manrope.ttf")
        );
        assert_eq!(
            fonts[1],
            include_bytes!("../../../../docs/src/assets/fonts/JetBrainsMono-Regular.ttf")
        ...
        assert!(
            html.contains("\"Manrope\", sans-serif"),
            "Manrope must lead the canonical sans stack"
        );
        assert!(
