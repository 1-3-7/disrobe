...
fn decompile(
    ...
    if list {
        super::emit::print_obfuscator_catalog(
            ...
            "disrobe lua decompile <input.luac> --out <output.lua>",
        );
        return Ok(());
    }
    let Some(input): Option<PathBuf> = input else {
        return Err(miette::miette!(
            "DR-CLI-0520b: lua decompile needs an input file (or `--list` to show supported obfuscators)"
        ));
    };
...
fn deobfuscate(
    ...
    let result: PeelResult = match family {
        ...
        LuaFamilyChoice::Slua => slua::peel(&bytes, &opts),
...
fn detect_family(bytes: &[u8], family: LuaFamilyChoice) -> Option<ObfuscatorDetection> {
    match family {
        ...
        LuaFamilyChoice::Slua => slua::detect(bytes),
    }
}
...
fn auto_peel(bytes: &[u8], opts: &DeobfOptions) -> disrobe_pass_lua::Result<PeelResult> {
    ...
    if slua::detect(bytes).is_some() {
        return slua::peel(bytes, opts);
    }
