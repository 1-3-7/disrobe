...
fn navigation_call_edges(module: &NirModule) -> miette::Result<Vec<CallEdge>> {
    ...
    let targets: BTreeSet<u64> = calls
        .iter()
        .filter_map(|call: &NavigationCall| call.outcome.target_address())
        .collect();
    let imports: BTreeMap<u64, &str> = module
        .symbols
        .iter()
        .filter(|symbol| symbol.kind.is_external() && targets.contains(&symbol.address))
        .map(|symbol| (symbol.address, symbol.name.as_str()))
        .collect();
    calls
        .iter()
        .filter(|call: &&NavigationCall| !extern_sites.contains(&call.call_site))
        .map(|call: &NavigationCall| {
            if let Some(target) = call.outcome.target_address()
                && let Some(symbol) = imports.get(&target)
            {
                return Ok(CallEdge::definite(
                    call.call_site,
                    target,
            ...
            call_edge_from_navigation(call)
        })
        .collect()
}
...
mod tests {
    use disrobe_nir::{NirFunction, NirInstr, NirSymbol, SourceLang, SourceRef, SymbolKind};
    use disrobe_taint::CallEdgeLabel;

    use super::*;

    fn imported_flow_module() -> NirModule {
        let instructions: Vec<NirInstr> = [
            ...
            (0x1005, NirOp::Nop, "mov", vec!["rdi", "rax"]),
            (
                0x1008,
                NirOp::Call {
                    target: Some(0x2010),
            ...
            (0x100d, NirOp::Return, "ret", vec![]),
        ]
        .into_iter()
        .map(
            |(address, op, mnemonic, operands): (u64, NirOp, &str, Vec<&str>)| NirInstr {
                address,
                op,
        ...
        NirModule {
            ...
            symbols: [(0x2000, "fgets"), (0x2010, "system")]
                .into_iter()
                .map(|(address, name): (u64, &str)| NirSymbol {
                    address,
                    name: name.to_owned(),
    ...
    #[test]
    fn imported_calls_inside_a_coarse_function_range_keep_their_exact_targets() {
        let module: NirModule = imported_flow_module();
        let config: TaintConfig = TaintConfig::from_lists(["fgets"], ["system"]);
        let direct: TaintReport = disrobe_taint::analyze(&module, &config);
        assert_eq!(direct.findings().len(), 1, "{direct:?}");
        let calls: Vec<NavigationCall> = Module::from_nir(&module)
            .navigation_calls(NavigationLimits {
                functions: 1,
                instructions: 4,
                calls: 2,
        ...
        let edges: Vec<CallEdge> = navigation_call_edges(&module).expect("taint call edges");
        assert_eq!(edges.len(), 2);
        assert_eq!(edges[0].label, CallEdgeLabel::Definite { target: 0x2000 });
        assert_eq!(edges[1].label, CallEdgeLabel::Definite { target: 0x2010 });
        let guided: TaintReport = disrobe_taint::analyze_with_call_edges(&module, &config, &edges);
        assert_eq!(guided.findings().len(), 1, "{guided:?}");
        assert_eq!(
            guided.findings()[0].source_site,
            direct.findings()[0].source_site
        );
        assert_eq!(
            guided.findings()[0].sink_site,
            direct.findings()[0].sink_site
        );
    }

    #[test]
    fn internal_symbol_targets_keep_navigation_function_resolution() {
        let mut module: NirModule = imported_flow_module();
        ...
        assert!(
            edges
                .iter()
                .all(|edge| edge.label == CallEdgeLabel::Definite { target: 0x1000 })
        );
        assert!(
            edges
                .iter()
                .all(|edge| edge.evidence().iter().all(|evidence| matches!(
                    evidence,
                    CallEdgeEvidence::NavigationFunctionInterior { .. }
                )))
        );
