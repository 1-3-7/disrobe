...
fn gcc_o2_switch_resolves_against_objdump_ground_truth() {
    ...
    let read_only_section: &str = if cfg!(target_os = "windows") {
        ".rdata"
    } else {
        ".rodata"
    };
    let (Some(disasm_text), Some(rdata_text)): (Option<String>, Option<String>) = (
        objdump(&exe, &["-d"]),
        objdump(&exe, &["-s", "-j", read_only_section]),
    ) else {
        panic!(
    ...
    let Some(rdata): Option<SectionBytes> = parse_section_bytes(&rdata_text) else {
        panic!(
            "objdump reported {read_only_section} for this image, so failing to parse it is a defect in this \
