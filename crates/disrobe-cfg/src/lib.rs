...
pub fn loop_forest(cfg: &Cfg) -> LoopForest {
    ...
    for header in headers {
        ...
        let body: BTreeSet<NodeId> = dom.natural_loop_body(header, &latches);
...
mod tests {
    ...
    #[test]
    fn loop_forest_excludes_unreachable_predecessors_from_natural_bodies() {
        let cfg: Cfg =
            Cfg::new(0, vec![goto(1), br(0, 2, 3), br(1, 1, 3), ret(), goto(2)]).unwrap();
        let forest: LoopForest = loop_forest(&cfg);
        let dominance: FlowGraph<NodeId> = flow_of(&cfg).unwrap();
        assert!(!forest.irreducible);
        assert_eq!(forest.loops.len(), 1);
        assert_eq!(forest.loops[0].body, BTreeSet::from([1, 2]));
        assert_eq!(forest.loops[0].body, dominance.natural_loop_body(1, &[2]));

        let live: Cfg =
            Cfg::new(0, vec![goto(1), br(0, 2, 4), br(1, 1, 3), ret(), goto(2)]).unwrap();
        let forest: LoopForest = loop_forest(&live);
        assert!(!forest.irreducible);
        assert_eq!(forest.loops.len(), 1);
        assert_eq!(forest.loops[0].body, BTreeSet::from([1, 2, 4]));
    }
