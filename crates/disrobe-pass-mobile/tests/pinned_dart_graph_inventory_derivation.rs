...
use disrobe_pass_mobile::{
    DartClusterBodyKind, DartGraphClusterSummary, DartGraphDeclaredObjects,
    DartGraphRecoveryReport, DartGraphSnapshotSummary, DartPinnedLibraryInventory,
...
fn recover(build: &RecoveryBuild) -> DartGraphRecoveryReport {
    let bytes: Vec<u8> = read_tracked(&build.artifact);
    let options: disrobe_pass_mobile::DartGraphRecoveryOptions =
        disrobe_pass_mobile::DartGraphRecoveryOptions {
            obfuscation_hint: build.names.hint(),
            ..disrobe_pass_mobile::DartGraphRecoveryOptions::default()
        };
    recover_dart_pinned_elf(&bytes, &options).unwrap_or_else(|error: disrobe_pass_mobile::Error| {
        panic!(
...
fn required_summary<'report>(
    summary: Option<&'report DartGraphSnapshotSummary>,
    role: &str,
    artifact: &str,
) -> &'report DartGraphSnapshotSummary {
...
fn cluster_objects(summary: &DartGraphSnapshotSummary, kind: DartClusterBodyKind) -> usize {
    summary
        .clusters
        .iter()
        .filter(|cluster: &&DartGraphClusterSummary| cluster.kind == kind)
        .fold(
            0_usize,
            |total: usize, cluster: &DartGraphClusterSummary| {
                total.saturating_add(cluster.object_count)
            },
        )
}

fn measured_declaration(summary: &DartGraphSnapshotSummary) -> DeclaredSnapshot {
    DeclaredSnapshot {
        objects: summary.total_objects,
        base_objects: summary.base_objects,
        libraries: cluster_objects(summary, DartClusterBodyKind::Library),
        classes: cluster_objects(summary, DartClusterBodyKind::Class),
        patch_classes: cluster_objects(summary, DartClusterBodyKind::PatchClass),
        functions: cluster_objects(summary, DartClusterBodyKind::Function),
        fields: cluster_objects(summary, DartClusterBodyKind::Field),
    }
}

const fn declared_across_snapshots(build: &RecoveryBuild) -> DartGraphDeclaredObjects {
...
fn cluster_headers_declare_the_pinned_object_totals() {
    ...
    for build in &oracle.builds {
        let report: DartGraphRecoveryReport = recover(build);
        for (role, summary, pinned) in [
            ...
            let summary: &DartGraphSnapshotSummary =
                required_summary(summary, role, &build.artifact);
            assert!(
            ...
            let clustered: usize = summary.clusters.iter().fold(
                0_usize,
                |total: usize, cluster: &DartGraphClusterSummary| {
                    total.saturating_add(cluster.object_count)
                },
...
fn inventory_walk_reaches_every_declaration_the_snapshot_declares() {
    ...
    for build in &oracle.builds {
        let report: DartGraphRecoveryReport = recover(build);
        let declared: DartGraphDeclaredObjects = report.inventory.declared;
        assert_eq!(
            declared,
        ...
        let residue: disrobe_pass_mobile::DartGraphAttributionResidue = report.inventory.residue;
        assert_eq!(
            report
                .inventory
                .counts
...
fn library_entries_without_a_url_are_exactly_the_synthesized_ones() {
    ...
    for build in &oracle.builds {
        ...
        let backed: usize = report
            ...
            .libraries
            .iter()
            .filter(|library: &&DartPinnedLibraryInventory| library.url.is_some())
            .count();
        let synthesized: usize = report
            .inventory
            .libraries
            .iter()
            .filter(|library: &&DartPinnedLibraryInventory| library.url.is_none())
            .count();
        assert_eq!(
            backed,
