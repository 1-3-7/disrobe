...
use disrobe_pass_mobile::{
    ...
    DART_VM_INSTR_SYMBOL, DartSnapshotHeader, DartSnapshotKind, DartSnapshotStructure,
    DartStaticRecovery, LibAppLayout, SnapshotSection, decompile_libapp_so,
    decompile_libapp_so_structured, parse_dart_snapshot, parse_libapp_so,
};
...
fn rustdesk_structured_recovery_vs_independent_uri_oracle() {
    let bytes: Vec<u8> = match load_fixture(&fixture_path()) {
        ...
        None => {
            eprintln!(
                "CORPUS GAP (flagged honestly): no real Dart-AOT libapp.so committed under corpus/mobile/flutter/rustdesk/. \
                 Cluster framing + ARM64 traversal + structure recovery are unit-tested against hand-built byte fixtures only. \
                 Drop a real libapp.so here to activate the independent strings-based oracle below."
            );
            return;
        }
    };
