...
use disrobe_pass_mobile::DartGraphAttributionResidue;
use serde::Deserialize;

pub(crate) const FIXTURE_SET: &str = "pinned_graph_fixture";

const REBUILD_HINT: &str = "restore it with `git checkout -- corpus/mobile/flutter/pinned_graph_fixture`, \
                            or rebuild the set with corpus/mobile/flutter/pinned_graph_fixture/rebuild.ps1 \
                            on the Flutter revision that oracle.json records";
...
impl PinnedNames {
    pub(crate) const fn hint(self) -> disrobe_pass_mobile::DartGraphObfuscationHint {
        match self {
            Self::Source => disrobe_pass_mobile::DartGraphObfuscationHint::SourceNames,
            Self::Opaque => disrobe_pass_mobile::DartGraphObfuscationHint::OpaqueNames,
        }
    }
}
...
pub(crate) struct RecoveryBuild {
    ...
    pub(crate) attribution_residue: DartGraphAttributionResidue,
    #[serde(default)]
...
pub(crate) fn relative(name: &str) -> String {
    format!("corpus/mobile/flutter/{FIXTURE_SET}/{name}")
}
