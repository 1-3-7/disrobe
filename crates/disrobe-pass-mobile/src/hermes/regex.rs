...
/// Size of the `RegexBytecodeHeader`: markedCount(u16) + loopCount(u16) +
/// syntaxFlags(u8) + constraints(u8).
const REGEX_HEADER_SIZE: usize = 6;

/// Upper bound on regex opcodes decoded per pattern, guarding against a forged
/// table entry whose `length` points at a region that decodes as an unbounded
/// instruction run.
...
impl RecoveredRegExp {
    /// Renders the recovered regex as a JavaScript regex literal. When the
    /// pattern is fully literal the result round-trips to equivalent source;
    /// otherwise the skeleton carries `(?:...)` placeholders for structure that
    /// HBC discards, so it renders honestly as a literal with those markers.
    #[must_use]
...
/// Decodes the JS flag string from a `syntaxFlags` byte in ES2022 order: d g i
/// m s u y.
#[must_use]
...
///
/// Flags and the capture-group count are recovered exactly; the source pattern
/// is rebuilt as a best-effort skeleton because HBC stores only compiled
/// bytecode, not source.
#[must_use]
