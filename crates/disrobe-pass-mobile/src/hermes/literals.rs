...
/// A buffer length is bounded by `objKeyBufferSize`/`objValueBufferSize` (u32),
/// but a forged length operand could request decoding far past the slice; the
/// decoder is already slice-bounded, and this caps the result `Vec` so an
/// adversarial `numLiterals` cannot drive an unbounded allocation.
...
/// Decodes `count` serialized literals starting at `start` within `buffer`,
/// following the Hermes `SerializedLiteralParser::parseImpl` tag encoding.
///
/// Returns the decoded literals; decoding stops early (rather than panicking)
/// if the slice is exhausted, so a truncated or forged buffer yields a partial,
/// honest result instead of an out-of-bounds read.
#[must_use]
...
/// Renders a decoded value-buffer literal as a JavaScript value, resolving
/// string ids through `resolve_string`. Used for array-literal element text.
#[must_use]
...
///
/// String ids resolve to identifiers; non-identifier keys (numbers) are
/// bracket-quoted by the caller as needed. Private-name keys render as `#name`
/// placeholders.
#[must_use]
