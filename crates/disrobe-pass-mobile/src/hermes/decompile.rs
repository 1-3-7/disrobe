...
/// Upper bound on decoded instructions per function.
...
/// Upper bound on the byte length of a function's rendered pseudo-JS body.
const MAX_RENDER_BYTES: usize = 1 << 20;

/// Upper bound on dense `SwitchImm` case entries reconstructed.
...
impl<'a> LiftCtx<'a> {
    ...
    /// Records that register `reg` holds an environment `level` scopes up the lexical chain.
...
/// Renders an object-literal key, bracket-quoting keys that are not valid JS identifiers.
#[must_use]
...
/// Synthesizes a stable, positional name for a captured variable at lexical `(level, slot)`.
#[must_use]
...
/// Whether register `d` written by `inst` is read by a later instruction before being overwritten.
#[must_use]
...
/// Whether an opcode writes its first operand as a destination register.
#[must_use]
...
mod tests {
    ...
    fn buffer_literal_capacity_ignores_oversized_raw_count() {
        ...
        let ctx: LiftCtx<'_> = LiftCtx::new(&module, &code);
        ...
        let empty_ctx: LiftCtx<'_> = LiftCtx::new(&empty, &code);
    ...
    /// Every opcode with an explicit `lift_instruction` match arm.
