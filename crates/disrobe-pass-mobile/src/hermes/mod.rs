...
impl HermesModule {
    /// Resolves a function's name from its global string-table index.
    #[must_use]
...
/// Parses the bigint `(offset,length)` table then its digit storage, advancing the cursor.
#[must_use]
...
/// Parses the regexp `(offset,length)` table then its bytecode storage, advancing the cursor.
#[must_use]
...
/// Rejects a header whose declared table/storage counts cannot fit in the input.
