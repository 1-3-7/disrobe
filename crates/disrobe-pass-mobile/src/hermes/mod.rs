...
impl HermesModule {
    ...
    pub fn function_code(&self, index: usize) -> &[u8] {
        ...
        let start: usize = f.offset as usize;
        let end: usize = start.saturating_add(f.bytecode_size_bytes as usize);
        if start <= self.raw_image.len() && end <= self.raw_image.len() {
            &self.raw_image[start..end]
        } else {
            &[]
        }
    }
...
/// Rejects a header whose declared table/storage counts cannot physically fit in
/// the input. Each count maps to a fixed minimum on-disk footprint, so a forged
/// `function_count`/`identifier_count`/`string_count` (e.g. `u32::MAX`) is caught
/// here with a typed error before any count-derived `Vec::with_capacity` runs,
/// turning a multi-gigabyte allocation into an honest `Err`.
fn reject_oversized_header_counts(header: &HermesHeader, available: usize) -> Result<()> {
    let declared: usize = (header.function_count as usize)
        .saturating_mul(SMALL_FUNCTION_HEADER_SIZE)
        .saturating_add((header.string_kind_count as usize).saturating_mul(STRING_KIND_ENTRY_SIZE))
        .saturating_add(
            (header.identifier_count as usize).saturating_mul(IDENTIFIER_HASH_ENTRY_SIZE),
        )
        .saturating_add((header.string_count as usize).saturating_mul(SMALL_STRING_ENTRY_SIZE))
        .saturating_add(
            (header.overflow_string_count as usize).saturating_mul(OVERFLOW_STRING_ENTRY_SIZE),
        )
        .saturating_add(header.string_storage_size as usize);
