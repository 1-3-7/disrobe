...
/// Image header size from Dart VM `Image::kHeaderSize`, 64 bytes on every supported target.
const IMAGE_HEADER_SIZE: usize = 64;

/// ARM64 `stp x29, x30, [x15, #-16]!` — Dart-SP `PushPair(FP, LR)` opening every AOT frame.
const ARM64_PUSH_FP_LR_DART_SP: u32 = 0xA9BF_79FD;
/// ARM64 `stp x29, x30, [sp, #-16]!` — the same push using the architectural `sp`.
const ARM64_PUSH_FP_LR_SYS_SP: u32 = 0xA9BF_7BFD;
/// ARM64 `mov x29, x15` — sets FP to the Dart SP.
const ARM64_MOV_FP_DART_SP: u32 = 0xAA0F_03FD;
/// ARM64 `mov x29, sp` — sets FP to the architectural SP.
const ARM64_MOV_FP_SYS_SP: u32 = 0x9100_03FD;

/// Bare-instructions payload alignment from Dart VM `Instructions::kBarePayloadAlignment`.
const BARE_PAYLOAD_ALIGNMENT: usize = 4;

/// Upper bound on function boundaries reported.
...
/// Recovers function boundaries, signatures, and names from a Dart AOT isolate snapshot.
#[must_use]
...
/// Scans an instructions image for ARM64 Dart frame prologues, one boundary each.
#[must_use]
...
/// Infers how many ARM64 argument registers (x0..x7) a function reads in its prologue.
#[must_use]
