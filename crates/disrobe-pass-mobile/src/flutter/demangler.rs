...
/// following the `Object::kInternalName` -> `kScrubbedName` conventions:
///
/// - `get:foo@6be832b` -> `foo` (getter)
/// - `set:foo@6be832b` -> `foo` (setter)
/// - `_MyClass@6b3832b.` -> `_MyClass` (unnamed constructor)
/// - `_MyClass@6b3832b.named` -> `_MyClass.named` (named constructor)
/// - `_privateMethod@hash` -> `_privateMethod` (private member)
#[must_use]
...
/// Removes every `@<hash>` library-privacy suffix from a name, leaving the rest
/// (including any `.named` constructor suffix) intact. A hash runs from the `@`
...
/// Demangles a fully qualified `Library_Class_method` AOT symbol into dotted
/// form.
///
/// Underscore-joined AOT symbols are split on underscores that separate
/// identifier segments while preserving leading underscores that denote privacy.
#[must_use]
