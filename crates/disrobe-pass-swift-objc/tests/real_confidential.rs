...
/// The key the corpus generator XOR'd these fixtures with, which the pass must independently recover.
...
/// Builds the ciphertext a `SwiftConfidential` build would embed for one literal: bytes XOR'd with `TRUE_KEY`, NUL-terminated.
...
/// Short isolated literals yield too few ciphertext bytes to disambiguate the key, so recovery flags `unambiguous == false`.
#[test]
