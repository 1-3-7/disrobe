...
#[test]
fn nested_arrays_and_tuples_under_the_bound_still_recover() {
    for (mangled, expected) in [
        ("SaySaySaySiGGG", "[[[Swift.Int]]]"),
        ("SaySi_SStG", "[(Swift.Int, Swift.String)]"),
        ("SaySiG_SSt", "([Swift.Int], Swift.String)"),
    ] {
        assert_eq!(demangle::demangle_type(mangled).as_deref(), Some(expected));
    }
}

#[test]
fn supported_array_nesting_near_the_depth_bound_still_recovers() {
    for (inner, expected_inner) in [
        ("Si", "Swift.Int"),
        ("SS", "Swift.String"),
        ("4Demo1tV", "Demo.t"),
    ] {
        let levels: usize = 255;
        let mangled: String = format!("{}{inner}{}", "Say".repeat(levels), "G".repeat(levels));
        ...
        assert_eq!(
            demangle::demangle_type(&mangled).as_deref(),
