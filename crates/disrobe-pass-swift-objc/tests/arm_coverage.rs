...
fn required_tool(swiftc: &Path, stem: &str) -> PathBuf {
    ...
    swiftc
        ...
        .or_else(|| swift_toolchain::find_on_path(stem))
        .unwrap_or_else(|| {
            panic!(
                "{stem} is neither beside {} nor on PATH; object symbol inspection requires LLVM \
                 and is never a skip",
                swiftc.display()
            )
        })
...
fn compiled_symbol_table(swiftc: &Path, fixture: CompiledFixture) -> String {
    ...
    let llvm_nm: PathBuf = required_tool(swiftc, "llvm-nm");
