...
fn read_fixture_or_skip(name: &str) -> Option<Vec<u8>> {
    ...
    if bytes.is_none() {
        eprintln!(
            "FIXTURE PENDING: {} missing; regenerate via scripts/regen-macho-edgecases.sh",
            path.display()
        );
    }
    bytes
...
#[test]
#[ignore = "FIXTURE PENDING: EdgeCases.arm64 is a thin Mach-O slice not redistributed in git; regenerate via scripts/regen-macho-edgecases.sh on a macOS host"]
...
#[test]
#[ignore = "FIXTURE PENDING: EdgeCases.x86_64 is a thin Mach-O slice not redistributed in git; regenerate via scripts/regen-macho-edgecases.sh on a macOS host"]
