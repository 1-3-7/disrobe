...
fn emit_message(inner: &str, classes: &BTreeSet<String>, out: &mut Vec<SourceSend>) {
        ...
        .and_then(|s: &str| s.strip_suffix(']'))
    {
        ...
        emit_message(nested_inner, classes, &mut nested);
        if selector == "init"
            && nested.len() == 1
...
fn extract_sends(body: &str, classes: &BTreeSet<String>) -> Vec<SourceSend> {
    ...
    while let Some(open) = body[cursor..].find('[') {
        ...
        emit_message(&body[start + 1..close], classes, &mut out);
        cursor = close + 1;
    }
    out
}
...
fn method_selector(header: &str) -> String {
    ...
    for ch in declaration.chars() {
        match ch {
            ...
            ':' if depth == 0 => {
                selector.push_str(keyword.trim());
                selector.push(':');
                keyword.clear();
            }
...
fn parse_source(text: &str) -> Vec<SourceFunction> {
    ...
    for raw in text.lines() {
            ...
            *depth = depth.saturating_sub(line.matches('}').count());
            if *depth == 0 {
                out.push(SourceFunction {
                    ...
                    sends: extract_sends(body, &classes),
                });
                pending = None;
            } else {
                body.push_str(line);
        ...
        if in_interface || !line.ends_with('{') {
            continue;
        }
...
const fn dynamic_receiver_token(arch: DispatchArch) -> &'static str {
    match arch {
        DispatchArch::Arm64 => ARM64_DYNAMIC_RECEIVER,
        DispatchArch::X86_64 => X86_64_DYNAMIC_RECEIVER,
    }
}

const fn argument_registers(arch: DispatchArch) -> &'static [&'static str] {
    match arch {
        DispatchArch::Arm64 => &ARM64_ARGUMENT_REGISTERS,
        DispatchArch::X86_64 => &X86_64_ARGUMENT_REGISTERS,
    }
}
...
fn expected_receiver_token(send: &SourceSend, arch: DispatchArch) -> String {
    match &send.receiver {
        ...
        SourceReceiver::Dynamic => dynamic_receiver_token(arch).to_owned(),
    }
}
...
fn expected_rendering(send: &SourceSend, arch: DispatchArch) -> String {
    ...
    let registers: &[&str] = argument_registers(arch);
...
fn source_model_describes_every_adversarial_case() {
    ...
    let total: usize = functions
        ...
        .sum();
    assert_eq!(total, 14, "the source declares 14 message sends");
    ...
    assert_eq!(
        all.iter()
            ...
            .count(),
        6,
        "six sends name a class receiver in the source"
    );
    assert_eq!(
        all.iter()
...
fn super_sends_are_distinguishable_from_unresolved_receivers() {
    ...
    let super_functions: Vec<&SourceFunction> = functions
        ...
        .collect();
    assert_eq!(super_functions.len(), 1, "one method dispatches to super");
    ...
    for (name, arch, label) in FIXTURES {
        ...
        assert!(
            ...
            dynamic_receiver_token(arch)
        );
        assert!(
            !send.send.rendered.contains(dynamic_receiver_token(arch)),
...
fn dispatch_maps_resolve_every_selector_and_class_the_source_names() {
    ...
    assert_eq!(
        selectors.len(),
        6,
        "the source names six distinct selectors"
    );
    assert_eq!(classes.len(), 2, "the source names two receiver classes");
