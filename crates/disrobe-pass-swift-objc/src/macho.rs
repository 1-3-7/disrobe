...
pub fn walk_fat(bytes: &[u8]) -> Result<Vec<FatArchEntry>> {
    ...
    let max_entries: usize = bytes.len() / entry_size;
    let mut out: Vec<FatArchEntry> = Vec::with_capacity((nfat as usize).min(max_entries));
...
impl<'a> SliceView<'a> {
    ...
    pub fn read_u32_at(&self, off: usize) -> Option<u32> {
        let raw: &[u8] = self.bytes.get(off..off + 4)?;
        Some(u32::from_le_bytes([raw[0], raw[1], raw[2], raw[3]]))
    }

    #[must_use]
    pub fn read_u64_at(&self, off: usize) -> Option<u64> {
        let raw: &[u8] = self.bytes.get(off..off + 8)?;
        let mut arr: [u8; 8] = [0u8; 8];
