...
pub fn demangle(symbol: &str) -> Result<String> {
    ...
    let module: String = parser
        .parse_identifier()
        .map_err(|()| Error::Demangle(symbol.to_owned()))?;
    let mut path: Vec<String> = vec![module];
    while let Ok(next) = parser.parse_identifier() {
        path.push(next);
    }
    let kind_suffix: &str = parser.kind_suffix();
    Ok(format!("{}{}", path.join("."), kind_suffix))
}
...
impl<'a> Parser<'a> {
    ...
    fn parse_identifier(&mut self) -> core::result::Result<String, ()> {
        ...
        while self.pos < self.src.len() && self.src[self.pos].is_ascii_digit() {
            let digit: usize = (self.src[self.pos] - b'0') as usize;
            len = len.saturating_mul(10).saturating_add(digit);
            self.pos += 1;
        }
        if len == 0 || self.pos + len > self.src.len() {
            return Err(());
        }
        let raw: &[u8] = &self.src[self.pos..self.pos + len];
