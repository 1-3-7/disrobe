...
use crate::swift_typedump::{self, SwiftTypeDump};
...
pub fn class_dump(slice: &[u8], parsed: &ParsedSlice) -> SwiftClassDump {
    ...
    let type_dump: SwiftTypeDump = swift_typedump::parse_type_dump(slice, parsed, demangle_fn);
...
fn collect_mangled_swift_strings(reflstr: Option<&SwiftReflectionStrings>) -> Vec<String> {
    let mut out: Vec<String> = Vec::new();
    let Some(r): Option<&SwiftReflectionStrings> = reflstr else {
        return out;
    };
    for s in &r.strings {
