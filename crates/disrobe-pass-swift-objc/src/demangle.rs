...
impl<'a> Demangler<'a> {
    ...
    fn increase_depth(&mut self) -> Option<()> {
        self.depth = self.depth.checked_add(1)?;
        if self.depth > MAX_DEPTH {
            self.depth_limit_hits = self.depth_limit_hits.checked_add(1)?;
            return None;
        }
        Some(())
    }
            ...
            match self.peek() {
                Some(b'y') => {
                    if let Some(func) = self.try_demangle_empty_param_function(node.clone()) {
                        ...
                        let cp: Checkpoint = self.checkpoint();
                        self.pos += 1;
                        let depth_limit_hits: usize = self.depth_limit_hits;
                        if let Some(bound) = self.demangle_bound_generic_args(node.clone()) {
                            node = bound;
                        } else {
                            self.restore(cp);
                            if self.depth_limit_hits != depth_limit_hits {
                                return None;
                            }
                            break;
                        }
...
mod tests {
    ...
    #[test]
    fn a_depth_refused_generic_list_is_not_returned_as_a_bare_type() {
        let input: String = format!("{}Si{}", "Say".repeat(1_100), "G".repeat(1_100));
        let mut dem: Demangler<'_> = Demangler::new(&input);
        assert!(
            dem.demangle_type().is_none(),
            "a depth-refused argument list must not recover the unbound base and retry its arguments"
        );
    }

    #[test]
    fn the_last_available_node_can_complete_a_shallow_type() {
        let mut dem: Demangler<'_> = Demangler::new("Si");
        dem.suppress_tuple = true;
        dem.node_budget = 1;
        let node: NodeRef = dem
            .demangle_type()
            .expect("the final node remains available");
        assert_eq!(dem.pos, dem.src.len());
        assert_eq!(print_node(&node, Mode::Type), "Swift.Int");
    }
