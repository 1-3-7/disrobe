...
use crate::macho::{self, MangledName, ParsedSlice, Section, SliceView};
use crate::swift_symbolic;
...
#[must_use]
pub fn read_field_list(
    view: &SliceView<'_>,
    fieldmd_off: usize,
    demangle: &dyn Fn(&str) -> Option<String>,
) -> Vec<SwiftField> {
    let Some(kind_raw): Option<u32> = view.read_u32_at(fieldmd_off + 8) else {
        return Vec::new();
    };
    ...
    let Some(num_fields): Option<u32> = view.read_u32_at(fieldmd_off + 12) else {
        return Vec::new();
    };
    let num_fields_usize: usize = (num_fields as usize).min(MAX_FIELDS_PER_TYPE);
    let Some(records_bytes): Option<usize> = num_fields_usize.checked_mul(elem_size) else {
        return Vec::new();
    };
    let Some(end): Option<usize> = fieldmd_off
        .checked_add(FIELD_DESCRIPTOR_HEADER)
        .and_then(|h: usize| h.checked_add(records_bytes))
    else {
        return Vec::new();
    };
    ...
    for i in 0..num_fields_usize {
        let rec_off: usize = fieldmd_off + FIELD_DESCRIPTOR_HEADER + i * elem_size;
...
fn read_field_record(
    ...
    let (mangled_type, demangled_type): (Option<String>, Option<String>) =
        read_mangled_type(view, rec_off + 4, demangle);
...
fn read_mangled_type(
    ...
    let Some(name): Option<MangledName> = view.mangled_name_at_offset(target, MAX_CSTR) else {
        return (None, None);
    };
    if name.raw.is_empty() {
        return (None, None);
    }
    if !name.has_symbolic_ref() {
        let Some(plain): Option<String> = name.as_plain_string() else {
            return (None, None);
        };
        let demangled: Option<String> = demangle(&plain).filter(|d: &String| d != &plain);
        return (Some(plain), demangled);
    }
    let Some(resolved): Option<String> = swift_symbolic::resolve_to_plain_mangling(view, &name)
    else {
        return (None, None);
    };
    let demangled: Option<String> = demangle(&resolved).filter(|d: &String| d != &resolved);
    (Some(resolved), demangled)
}
...
pub fn parse_field_descriptors(
    ...
    while cursor + FIELD_DESCRIPTOR_HEADER <= sect_end && guard < MAX_DESCRIPTORS {
        ...
        let Some(descriptor_end): Option<usize> = cursor
            .checked_add(FIELD_DESCRIPTOR_HEADER)
            .and_then(|v: usize| v.checked_add(records_bytes))
        else {
            break;
        };
        ...
        let fields: Vec<SwiftField> = read_field_list(&view, cursor, demangle);
