...
/// `PyObfuscator` (`Mauricelambert`) wraps the program in `exec(gzip.decompress(b'\x1f\x8b...'))`. We
/// recover the gzip layer statically (an honest `Quality::Partial`: the inner stage is the tool's
/// `chr()`-arithmetic / byte-xor name-mangling, which needs a Python AST evaluator to fold to
/// clean source). The test fails if gzip recovery regresses (would error or drop below Partial).
#[test]
