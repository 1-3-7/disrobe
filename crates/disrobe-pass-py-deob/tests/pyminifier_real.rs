...
/// (slot, substring that MUST appear in the de-minified / canonicalized recovered source).
const SLOTS: &[(&str, &str)] = &[
...
/// (slot, substring that must appear in the recovered source) for the rename/compression/prepend variants.
