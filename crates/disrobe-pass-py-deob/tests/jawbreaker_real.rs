...
/// Jawbreaker's real artifact is `exec(b64(b32(b16("HEX"))))` whose inner shell is a
/// `urllib.request.urlopen` loader that fetches the user's source from a remote (expiring)
/// Hastebin paste. No user source is embedded, so peel is an HONEST `DetectOnly`: we decode the
/// triple-encode to prove the remote-loader shape, but cannot recover source without the live
/// fetch. This test fails if the pass ever pretends to recover source (no-op-as-success).
#[test]
