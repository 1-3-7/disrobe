...
/// Shared synthetic edge-case sources fed to a pass's own `bake()`, not real third-party output.
...
/// Bakes each `EDGE_CASES` source with the pass's own `bake()` and asserts `peel()` accepts it.
