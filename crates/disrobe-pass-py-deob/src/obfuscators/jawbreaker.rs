...
/// Jawbreaker (billythegoat356) emits `exec(b64decode(b32decode(b16decode("HEX"))))`. The HEX
/// shell decodes statically to a `urllib.request.urlopen` loader that fetches the user's source
/// from a remote Hastebin paste assembled from runtime fragments. We peel the triple-encode to
/// prove the loader shape, but the user source is NOT present in the artifact - it lives on the
/// (expiring) remote paste. This is a genuine network wall, so the outcome is honest `DetectOnly`.
