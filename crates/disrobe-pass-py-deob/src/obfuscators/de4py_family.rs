//! Clean-room static decoders for the de4py Specter / Kramer / Berserker family (billythegoat356).
...
/// True when the string contains a control char other than tab/newline/carriage-return.
fn has_disqualifying_control(s: &str) -> bool {
    s.chars()
        .any(|c: char| c.is_control() && c != '\n' && c != '\t' && c != '\r')
}

/// Source-likeness in `[0, 1]`: density of characters that occur in Python source.
...
/// Stage-1 token codepoints fed into the shift stage, before SHIFT subtraction.
...
/// Candidate SHIFT values derived from the first few stage-1 codepoints.
...
/// True when the candidate decode parses as a Python module with zero syntax errors.
...
/// Best decode: a clean Python parse beats a non-parse, ties break on source-likeness score.
