//! Clean-room static decoders for the de4py "Specter / Kramer / Berserker" Python obfuscator
//! family (billythegoat356). Each member emits a self-decrypting `class <Name>()` whose
//! `_sparkle` argument carries the payload as `<delim>`-joined integer or hex tokens. The
//! decode is a pure, deterministic two-stage transform with two per-build constants:
//!   1. token stage: each token `t` -> `chr(int(t) - N)` (decimal, N = token count) or
//!      `chr(int.from_bytes(unhex(t)))` (hex); a lone non-numeric sentinel token -> U+03B6 (zeta).
//!   2. shift stage: `chr(ord(c) - SHIFT)` for every char (zeta -> '\n'), then a forward
//!      rotation across the lowercase-alnum ring `abcdefghijklmnopqrstuvwxyz0123456789`.
//!
//! SHIFT is randomized per build; we enumerate candidate shifts anchored on a printable leading
//! byte and pick the one whose decode is valid Python (zero parse errors), breaking ties on a
//! source-likeness score. No runtime key, no network: fully static.
...
/// A control char (other than tab/newline/carriage-return) never appears in Python source; its
/// presence is a hard signal that the recovered shift is wrong, so it carries a heavy penalty.
fn has_disqualifying_control(s: &str) -> bool {
    s.chars()
        .any(|c: char| c.is_control() && c != '\n' && c != '\t' && c != '\r')
}

/// Source-likeness in `[0, 1]`: density of characters that legitimately occur in Python source
/// (alphanumerics, whitespace, and the standard operator/punctuation set). A correct de4py shift
/// maximizes this; an off-by-one shift drags spaces/quotes/parens into rarer glyphs and scores
/// lower, which disambiguates otherwise all-"printable" candidates.
...
/// Stage-1 token codepoints: the values fed into the shift stage, before SHIFT subtraction.
/// `None` entries are the newline sentinel (already resolved to zeta).
...
/// Candidate SHIFT values derived from the first few stage-1 codepoints under the assumption
/// that the corresponding source byte (pre-rotation) is printable ASCII.
fn candidate_shifts(codepoints: &[u32]) -> Vec<u32> {
    let mut seen: std::collections::BTreeSet<u32> = std::collections::BTreeSet::new();
    let mut out: Vec<u32> = Vec::new();
    for &cp in codepoints.iter().filter(|&&c: &&u32| c != ZETA).take(6) {
...
/// True when the candidate decode parses as a Python module with zero syntax errors. This is the
/// principled disambiguator between an off-by-one shift (which yields a `SyntaxError`) and the
/// real shift (which yields valid source).
...
/// Rank: a clean Python parse always beats a non-parse; ties break on source-likeness score. This
/// recovers the exact per-build shift even on short payloads where multiple shifts stay printable.
...
/// Infer the per-build delimiter as the most frequent non-alphanumeric char in the blob.
...
/// Decode a Berserker-family payload (decimal tokens). Delimiter auto-inferred.
...
/// Decode a Kramer-family payload (hex tokens joined by `/`).
...
/// Extract the triple-quoted `_sparkle='''...'''` payload from a de4py-family wrapper.
...
/// Extract the longest `/`-bearing run of hex chars embedded in a compiled `.pyc` blob.
