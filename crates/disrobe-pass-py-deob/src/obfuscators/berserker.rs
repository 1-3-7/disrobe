...
use crate::obfuscators::de4py_family::{FamilyDecode, decode_decimal_sparkle, extract_sparkle};
...
impl ObfuscatorPass for BerserkerPass {
    ...
    fn peel(&self, source: &[u8]) -> Result<PeelOutcome> {
        ...
        if is_real_berserker(text) {
            return peel_real_sparkle(self.id(), text);
        }
        let literal: &str =
        ...
        Ok(PeelOutcome {
            ...
            lossy_notes: vec![
                "synthetic base64+xor+lzma+zlib self-test layer; real upstream uses the dash-token _sparkle decode path"
                    .to_owned(),
            ],
            diagnostics,
        })
...
fn is_real_berserker(text: &str) -> bool {
    text.contains("class Berserker():") && text.contains("_sparkle=")
}

fn peel_real_sparkle(id: Obfuscator, text: &str) -> Result<PeelOutcome> {
    let blob: &str = extract_sparkle(text).ok_or(Error::LiteralNotFound)?;
    let decode: FamilyDecode = decode_decimal_sparkle(blob).ok_or_else(|| {
        Error::AstCleanup(
            "Berserker _sparkle dash-token decode failed to converge on printable source"
                .to_owned(),
        )
    })?;
    let mut diagnostics: BTreeMap<String, String> = BTreeMap::new();
    ...
    diagnostics.insert("token_delimiter".to_owned(), decode.delimiter.to_string());
    ...
    Ok(PeelOutcome {
        obfuscator: id,
        stages_applied: vec![
            "real-class-detect".to_owned(),
            "sparkle-extract".to_owned(),
            "decimal-token-decode".to_owned(),
            "ord-shift".to_owned(),
        ...
        quality: Quality::Full,
        lossy_notes: vec![
            "de4py Berserker: per-build delimiter and ord-shift recovered statically; no runtime key or network needed"
                .to_owned(),
        ],
        diagnostics,
    })
