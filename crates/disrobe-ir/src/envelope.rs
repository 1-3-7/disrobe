...
pub(crate) const MAX_DECODED_ENVELOPE_BYTES: usize = 1 << 30;
...
pub(crate) struct PayloadRanges {
    pub(crate) hot: (usize, usize),
    pub(crate) cold: (usize, usize),
}
...
impl Envelope {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        let Ok(header): std::result::Result<&[u8; HEADER_SIZE], _> = header_slice.try_into() else {
            return Err(EnvelopeError::Truncated {
                expected: HEADER_SIZE,
                got: bytes.len(),
            });
        ...
        let _: usize = capped_declared_envelope_len(hot_len, cold_len)?;
        let ranges: PayloadRanges = payload_ranges(bytes.len(), hot_len, cold_len)?;
        let hot: Vec<u8> = bytes[ranges.hot.0..ranges.hot.1].to_vec();
        let cold: Vec<u8> = bytes[ranges.cold.0..ranges.cold.1].to_vec();
...
pub(crate) fn payload_ranges(
    bytes_len: usize,
    hot_len: usize,
    cold_len: usize,
) -> Result<PayloadRanges> {
    let hot_start: usize = HEADER_SIZE;
    let hot_end: usize = hot_start
        .checked_add(hot_len)
    ...
    let cold_end: usize = cold_start
        .checked_add(cold_len)
        .ok_or(EnvelopeError::Truncated {
            expected: usize::MAX,
            got: bytes_len,
        })?;
    if bytes_len < cold_end {
        return Err(EnvelopeError::Truncated {
            expected: cold_end,
            got: bytes_len,
        });
...
pub(crate) fn capped_declared_envelope_len(hot_len: usize, cold_len: usize) -> Result<usize> {
    let total: usize = HEADER_SIZE
        .checked_add(hot_len)
        .and_then(|hot_end: usize| hot_end.checked_add(cold_len))
        .ok_or(EnvelopeError::Truncated {
            expected: usize::MAX,
            got: HEADER_SIZE,
        })?;
    if total > MAX_DECODED_ENVELOPE_BYTES {
...
mod tests {
    ...
    #[test]
    fn payload_ranges_rejects_arithmetic_overflow() {
        let hot_err: EnvelopeError =
            payload_ranges(HEADER_SIZE, usize::MAX, 0).expect_err("hot length overflow");
        assert!(matches!(
            hot_err,
        ...
        let cold_err: EnvelopeError = payload_ranges(usize::MAX, usize::MAX - HEADER_SIZE, 1)
            .expect_err("cold length overflow");
        assert!(matches!(
            cold_err,
            EnvelopeError::Truncated {
                expected: usize::MAX,
    ...
    #[test]
    fn decode_rejects_declared_payload_length_overflow() {
        let env: Envelope = Envelope::new(Rung::Raw, vec![], vec![]);
        let mut bytes: Vec<u8> = env.encode().expect("encode");
        bytes[HOT_LEN_OFFSET..HOT_LEN_OFFSET + 4].copy_from_slice(&u32::MAX.to_le_bytes());
        bytes[COLD_LEN_OFFSET..COLD_LEN_OFFSET + 4].copy_from_slice(&u32::MAX.to_le_bytes());
        let err: EnvelopeError = Envelope::decode(&bytes).expect_err("should fail");
        assert!(matches!(
            err,
            EnvelopeError::EnvelopeTooLarge {
                actual,
                max: MAX_DECODED_ENVELOPE_BYTES
            } if actual > MAX_DECODED_ENVELOPE_BYTES
        ));
    }
