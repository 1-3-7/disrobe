//! `LlmMetadataEmitter` impl for the Python decompile pass.
//!
//! Wraps the rich AST + per-function metrics already produced by the
//! `emit_llm_json` sidecar into the canonical `disrobe.metadata.llm.v1`
//! per-pass envelope shape for every category the pass can produce.
...
/// Pass-output struct used by the CLI to feed `LlmMetadataEmitter`. The CLI
/// already knows the decompiled source, Python version, original `.pyc` bytes,
/// and the per-instruction disassembly list - this type ties them together so
/// the trait can pull each category lazily.
#[derive(Debug, Clone)]
