...
/// Raw opcode numbers for the wordcode-era (`3.11`+) structurer. `CPython` renumbers the entire opcode
/// space on every minor release from `3.13` onward, and `3.14` drops `BEFORE_ASYNC_WITH`/`BEFORE_WITH`
/// (replaced by `LOAD_SPECIAL`), so a single hard-coded table is wrong. Each field is `None` when the
/// opcode does not exist in that version. Numbers verified against `opcode.opmap` per interpreter.
#[derive(Debug, Clone, Copy)]
#[allow(dead_code)]
struct Post311Ops {
    push_exc_info: u8,
...
fn nest_frames(mut frames: Vec<Frame>) -> Vec<Frame> {
    ...
    for frame in frames {
        attach_into(&mut roots, frame);
    }
    roots
}

fn attach_into(siblings: &mut Vec<Frame>, frame: Frame) {
    for existing in siblings.iter_mut() {
        if existing.range.start <= frame.range.start && existing.range.end >= frame.range.end {
            attach_into(&mut existing.children, frame);
