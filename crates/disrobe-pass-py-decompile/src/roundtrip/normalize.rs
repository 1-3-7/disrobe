...
pub fn normalize_sequence(code: &CodeObject, version: PyVersion) -> NormalizedSequence {
    ...
    let stripped: BTreeSet<usize> = firstlineno_strip_set(code);
    let normalized: Vec<NormalizedOp> = if stripped.is_empty() {
        normalized
    } else {
        drop_firstlineno_loads(normalized, &stripped)
    };
...
#[must_use]
fn firstlineno_strip_set(code: &CodeObject) -> BTreeSet<usize> {
    let mut out: BTreeSet<usize> = BTreeSet::new();
    let has_firstlineno_in_names: bool = code.names.iter().any(|n| match n {
        Object::String { value, .. } | Object::ShortAscii { value, .. } => {
            value.as_str() == FIRSTLINENO_NAME
        }
        _ => false,
    });
    if !has_firstlineno_in_names {
        return out;
    }
    for (idx, c) in code.consts.iter().enumerate() {
        if let Object::Int(v) = c
            && *v == code.firstlineno
        {
            let used_elsewhere: bool = code.consts.iter().enumerate().any(|(j, o)| {
                if j == idx {
                    return false;
                }
                matches!(o, Object::Int(w) if *w == *v)
            });
            if !used_elsewhere {
                out.insert(idx);
            }
...
#[must_use]
fn drop_firstlineno_loads(seq: Vec<NormalizedOp>, stripped: &BTreeSet<usize>) -> Vec<NormalizedOp> {
    seq.into_iter()
        .filter(|op| {
            let is_load_const: bool = op_name_matches(op, LOAD_CONST_NAME);
            if !is_load_const {
                return true;
            }
            let Some(arg) = op.raw_arg else {
                return true;
            };
            !stripped.contains(&(arg as usize))
        })
        .collect()
}
