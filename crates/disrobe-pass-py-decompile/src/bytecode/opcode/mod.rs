...
pub enum CanonicalOp {
    ...
    LoadSuperAttr { name: NameIndex, two_arg: bool },
    ...
    LoadFromDictOrGlobals(NameIndex),
...
#[must_use]
pub fn shared_load_global_null_order(
    version: &PyVersion,
    raw: u8,
    arg: u32,
) -> LoadGlobalNullOrder {
    let (maj, min): (u8, u8) = (version.major(), version.minor());
    if (maj, min) < (3, 11) || (arg & 1) == 0 || shared_opname(version, raw) != "LOAD_GLOBAL" {
        LoadGlobalNullOrder::Absent
    } else if (maj, min) < (3, 13) {
        LoadGlobalNullOrder::BeforeValue
    } else {
        LoadGlobalNullOrder::AfterValue
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LoadGlobalNullOrder {
    Absent,
    BeforeValue,
    AfterValue,
}
...
fn decode_by_name(
    ...
    match name {
        ...
        "DUP_TOP" => CanonicalOp::Dup,
        "DUP_TOP_TWO" => CanonicalOp::DupTwo,
        "DUP_TOPX" if arg_lo == 2 => CanonicalOp::DupTwo,
        "DUP_TOPX" => CanonicalOp::Dup,
        ...
        "IMPORT_NAME" => CanonicalOp::ImportName(import_index),
...
#[cfg(test)]
mod tests {
    use super::{CanonicalOp, PyVersion, decode_by_name, shared_decode};

    const IMPORT_NAME_314: u8 = 73;
    const IMPORT_NAME_315: u8 = 72;

    #[test]
    fn import_name_index_unshifted_pre_315() {
        let op: CanonicalOp = shared_decode(&PyVersion::V3_14, IMPORT_NAME_314, 8);
        assert_eq!(op, CanonicalOp::ImportName(8));
    }
    ...
    #[test]
    fn import_name_315_drops_lazy_eager_flag_bits() {
        let lazy: CanonicalOp = shared_decode(&PyVersion::V3_15, IMPORT_NAME_315, (2 << 2) | 1);
        let eager: CanonicalOp = shared_decode(&PyVersion::V3_15, IMPORT_NAME_315, (2 << 2) | 2);
        assert_eq!(lazy, CanonicalOp::ImportName(2));
        assert_eq!(eager, CanonicalOp::ImportName(2));
    }

    #[test]
    fn duplicate_pair_opcodes_preserve_pair_width() {
        let dup_two: CanonicalOp = decode_by_name("DUP_TOP_TWO", 0, 0, 0, 0, 0, 0);
        let dup_topx_two: CanonicalOp = decode_by_name("DUP_TOPX", 0, 2, 0, 0, 0, 0);
        let dup_topx_other: CanonicalOp = decode_by_name("DUP_TOPX", 0, 3, 0, 0, 0, 0);
        assert_eq!(dup_two, CanonicalOp::DupTwo);
        assert_eq!(dup_topx_two, CanonicalOp::DupTwo);
        assert_eq!(dup_topx_other, CanonicalOp::Dup);
    }
}
