...
///
/// Emits `a, b = c, d` rather than `(a, b) = (c, d)`. The empty and single-element forms keep their
/// parentheses (`()` and `(x,)`) so the trailing-comma semantics stay explicit. Only used for the
/// simultaneous subscript/attribute assignment recovered from the 3.11+ `SWAP` idiom, where pycdc
/// leaves the tuples bare.
#[must_use]
...
///
/// Matches the 3.11+ `SWAP`-folded shape `a[i], a[j] = a[j], a[i]`. pycdc renders exactly this
/// without enclosing parentheses, in contrast to name-target unpacks (`(a, b) = x`) and mid-chain
/// tuple targets, which it keeps parenthesized. Requires a single tuple target of length >= 2 with
/// no `Name` element and a tuple value of the same arity.
#[must_use]
