...
use crate::codegen::expr::{Precedence, aug_symbol, emit_arguments, emit_expr, emit_type_params};
use crate::codegen::flow;
use crate::codegen::format_string_literal;
...
pub fn emit_stmt(em: &DefaultEmitter, s: &Stmt, indent: u32, version: &PyVersion) -> String {
    ...
    match s {
        ...
        Stmt::Expr(e) => format!("{pad}{}", emit_expr(em, e, version, Precedence::Lowest)),
...
fn emit_function_def(
    ...
    if let Some(doc) = docstring {
        ...
        out.push_str(&format_string_literal(doc, em.use_double_quotes));
        if !body.is_empty() {
            out.push('\n');
        }
    }
