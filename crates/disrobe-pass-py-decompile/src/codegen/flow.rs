...
pub fn emit_for(
    ...
    let render_async: bool = is_async && version_dispatch::supports_async(version);
    if render_async {
        head.push_str("async ");
    }
    head.push_str("for ");
    head.push_str(&emit_for_target(em, target, render_async, version));
    head.push_str(" in ");
...
fn emit_for_target(
    ...
    match target {
        Expr::Tuple { elts, .. } if !render_async => match elts.as_slice() {
            [] => "[]".to_owned(),
            [single] => format!("{},", emit_expr(em, single, version, Precedence::Lambda)),
            many => many
                .iter()
        ...
        _ => emit_expr(em, target, version, Precedence::Lowest),
    }
}
