...
impl EmitPipeline {
    ...
    pub fn run(
        ...
        let raw: String = if self.preserve_blank_lines {
            self.emitter.emit_module(module, version)
        } else {
...
#[inline]
#[must_use]
fn is_python_acceptable(s: &str) -> bool {
    !s.is_empty()
}
