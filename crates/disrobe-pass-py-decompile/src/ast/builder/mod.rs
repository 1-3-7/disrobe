...
use crate::bytecode::opcode::{
    CanonicalOp, CmpOp, LoadGlobalNullOrder, OpcodeMap, map_for, shared_load_global_null_order,
};
...
impl DecodedStream {
    fn index_for_offset(&self, byte_offset: u32) -> Option<usize> {
        let idx: usize = self
            .offsets
            .partition_point(|&offset: &u32| offset < byte_offset);
        self.offsets
            .get(idx)
            .is_some_and(|&offset: &u32| offset == byte_offset)
            .then_some(idx)
    }
...
fn decode_wordcode_with_offsets(
    ...
    while cursor + 1 < bytes.len() {
        ...
        let load_global_null_order: LoadGlobalNullOrder =
            shared_load_global_null_order(version, raw, arg);
        if matches!(load_global_null_order, LoadGlobalNullOrder::BeforeValue) {
            offsets.push(here);
            ops.push(CanonicalOp::Push(0));
        }
        offsets.push(here);
        ops.push(opmap.decode(raw, arg));
        if matches!(load_global_null_order, LoadGlobalNullOrder::AfterValue) {
            offsets.push(here);
            ops.push(CanonicalOp::Push(0));
        }
