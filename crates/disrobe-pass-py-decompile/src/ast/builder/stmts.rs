...
fn legacy_with_return_matches_handler_tail(
    stream: &DecodedStream,
    body_end: usize,
    region_end: usize,
    cleanup_idx: usize,
    ...
    let exit_return: &[CanonicalOp] = &stream.ops[ret_start..=ret_idx];
    let Some(tail_start): Option<usize> = hi
        .checked_sub(exit_return.len())
        .filter(|start: &usize| *start >= cleanup_idx)
    else {
        return false;
    };
    stream.ops.get(tail_start..hi) == Some(exit_return)
}
...
fn structure_legacy_with(
    ...
    let mut continuation: Vec<Stmt> = Vec::new();
    if let Some(ret) = trailing_return {
        if legacy_with_return_matches_handler_tail(stream, body_end, region_end, cleanup_idx, hi) {
            continuation.push(ret);
        } else {
            body.push(ret);
        }
...
fn guard_body_completes_statement(ops: &[CanonicalOp], idx: usize) -> bool {
    if is_walrus_store_shape(ops, idx) {
        return false;
    }
    matches!(
        ops[idx],
...
fn value_form_guard_holds_statement(stream: &DecodedStream, idx: usize, hi: usize) -> bool {
    ...
    (idx + 1..target).any(|k: usize| guard_body_completes_statement(&stream.ops, k))
}

fn loop_is_then_arm_with_else(
    stream: &DecodedStream,
    lo: usize,
    hi: usize,
    region: &LoopRegion,
) -> bool {
    if !matches!(region.kind, LoopKind::While) {
        return false;
    }
    (lo..region.header).any(|guard: usize| {
        ...
        if false_target <= region.back_edge || false_target > hi {
            return false;
        }
        let Some(then_jump): Option<usize> = then_terminating_jump(stream, guard + 1, false_target)
        else {
            return false;
        };
        then_jump + 1 == false_target
            && region.header > guard
            && region.back_edge < then_jump
            && resolve_jump_target(stream, then_jump, &stream.ops[then_jump])
                .is_some_and(|join: usize| join > false_target && join <= hi)
    })
}
...
pub(super) fn structure_stmts(
    ...
    if !stream.is_pre_311()
        && let Some(index) = (lo..hi).find(|&index: &usize| {
            is_forward_cond_jump(&stream.ops[index])
                && !is_chain_cond_jump(&stream.ops, index)
                && resolve_jump_target(stream, index, &stream.ops[index]) == Some(index + 1)
        })
    {
        preflight_guarded_finally_raise_mismatch(stream, lo, hi, index)?;
        return Err(crate::error::DecompileError::AstDesync {
            offset: stream
                .offsets
                .get(index)
                .map_or(0, |offset: &u32| *offset as usize),
            reason: "conditional jump targets its fallthrough".to_owned(),
        });
    }
    ...
    if let Some(loop_region) = find_loop(stream, lo, hi)
        ...
        && !loop_is_then_arm_with_else(stream, lo, hi, &loop_region)
...
fn try_structure_terminating_guard_else(
    code: &CodeObject,
    stream: &DecodedStream,
    lo: usize,
    hi: usize,
    ...
    let Some(target): Option<usize> = resolve_jump_target(stream, guard, &stream.ops[guard])
        .filter(|&t: &usize| t > guard && t < hi && t <= region.try_start)
    else {
        return Ok(None);
    };
    if try_recover_compound_if(code, stream, lo, hi)?.is_some() {
    ...
    if then_arm_opens_handler_after(stream, guard + 1, target)
        || !region_ends_in_hard_terminator(stream, guard + 1, target)
    {
        return Ok(None);
    }
    let Some(else_end): Option<usize> =
        hard_terminator_else_end(code, stream, lo, hi, guard, target, target)
            .filter(|&end: &usize| target < end)
    else {
        return Ok(None);
    };
    let (head, residual): (Vec<Stmt>, Vec<Expr>) =
    ...
    let else_body: Vec<Stmt> = structure_stmts(code, stream, target, else_end)?;
    ...
    out.extend(structure_stmts(code, stream, else_end, hi)?);
    Ok(Some(out))
}
