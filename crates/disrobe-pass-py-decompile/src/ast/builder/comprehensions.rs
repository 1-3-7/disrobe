...
use super::function_meta::{
    ...
    nested_code_object_at, slice_bound, try_build_lambda_expr,
};
    ...
    while idx < ops.len() {
        ...
        match op {
            ...
            CanonicalOp::DictMerge(_) | CanonicalOp::DictUpdate(_) => {
                let mapping: Expr = sim.pop_or_synth(nested, idx);
                let base: Option<Expr> = sim.try_pop();
                sim.push(merge_extend(base, mapping, true));
            }
            ...
            CanonicalOp::CallFunctionEx(has_kw) => {
                let kwargs_on_314: bool =
                    active_version().is_some_and(|v: PyVersion| (v.major(), v.minor()) >= (3, 14));
                let kwargs: Option<Expr> = if kwargs_on_314 {
                    let top: Expr = sim.pop_or_synth(nested, idx);
                    (!is_null_marker(&top)).then_some(top)
                } else if *has_kw {
                    Some(sim.pop_or_synth(nested, idx))
                } else {
                    None
                };
                let args_iter: Expr = sim.pop_or_synth(nested, idx);
                let (func, _implicit_self): (Expr, Option<Expr>) = sim.pop_call_target(nested, idx);
                sim.push(Expr::Call {
                    func: Box::new(func),
                    args: call_ex_args(args_iter),
                    keywords: kwargs.map(call_ex_kwargs).unwrap_or_default(),
                });
            }
