...
fn a_harness_handed_the_wrong_interpreter_refuses_instead_of_measuring() {
    ...
    for (harness, extra) in [
        ...
        let release_run: Output = Command::new(&python)
            .arg(harness)
            .args(&extra)
            .arg("--require-version")
            .arg(absent_release)
            .stdin(Stdio::null())
            .output()
            .expect("spawn the harness with a patch release it cannot satisfy");
        assert_refusal(
            &release_run,
            &name,
            "an interpreter patch release it is not",
        );

        println!(
            "{name} refuses a wrong magic, version, and patch release before measuring anything"
        );
    }
}
