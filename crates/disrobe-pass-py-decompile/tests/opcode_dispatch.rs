...
#[test]
fn specialized_pep_659_ops_classified_for_312() {
    let map: Box<dyn OpcodeMap> = map_for(PyVersion::V3_12);
    let mut found_any: bool = false;
    for op in 0u8..=255u8 {
        let name: &'static str = map.opname(op);
        if name.starts_with("CALL_PY_") || name.starts_with("BINARY_OP_") {
            found_any = true;
            assert!(
                matches!(map.decode(op, 0), CanonicalOp::Specialized(_)),
                "{name} should be Specialized"
            );
        }
    }
