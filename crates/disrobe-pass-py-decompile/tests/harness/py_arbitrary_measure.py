...
Per-code-object normalized opcode-structure comparator. For each pinned stdlib module:
...
an opcode-normalized diff. It drops jump targets and most operands, removes and merges selected
opcodes, omits __annotate__ code objects, and does not reject extra recovered objects. It does
not establish semantic or emitted-bytecode equivalence.
...
def interpreter_refusal(
    required_magic: Optional[str],
    required_version: Optional[str],
    required_optimize: Optional[int] = None,
) -> Optional[str]:
    version: str = running_version()
    if required_version is not None and required_version.count(".") >= 2:
        version = running_release()
    ...
    if required_optimize is not None and required_optimize != sys.flags.optimize:
        return (
            f"REFUSED: caller requires optimize {required_optimize} but this interpreter runs "
            f"with optimize {sys.flags.optimize}; a band measured with different compiler flags is "
            f"a different population, so nothing was measured"
        )
    return None
...
def runtime_library() -> Optional[str]:
    ...
    loaded: int = ctypes.windll.kernel32.GetModuleFileNameW(
        ctypes.c_void_p(sys.dllhandle), path_buffer, len(path_buffer)
    )
    if loaded == 0 or loaded >= len(path_buffer):
        return None
...
def main():
    ...
    ap.add_argument("--candidate-source-identity", default=None)
    ap.add_argument("--original-executable", default=None)
    ap.add_argument("--require-magic", default=None)
    ap.add_argument("--require-version", default=None)
    ap.add_argument("--require-optimize", type=int, choices=(0, 1, 2), default=None)
    ap.add_argument("--require-complete-sources", action="store_true")
    args = ap.parse_args()

    refusal: Optional[str] = interpreter_refusal(
        args.require_magic, args.require_version, args.require_optimize
    )
    if refusal is not None:
        print(refusal, file=sys.stderr)
        raise SystemExit(2)

    measured_executable_sha256: str = sha256_file(args.disrobe)
    original_executable_sha256: Optional[str] = None
    if args.original_executable is not None:
        original_executable_sha256 = sha256_file(args.original_executable)
        if original_executable_sha256 != measured_executable_sha256:
            raise SystemExit("the supplied executable changed while its snapshot was prepared")
    runtime_path: Optional[str] = runtime_library()
    runtime_sha256: Optional[str] = sha256_file(runtime_path) if runtime_path else None
    ...
    for path in files:
        ...
        try:
            with open(path, "rb") as source:
                source_bytes: bytes = source.read()
        except OSError as error:
            source_read_errors.append(f"{path}: {error}")
            continue
        relative: str = os.path.relpath(path, args.lib).replace(os.sep, "/")
        source_files.append((path, relative, source_bytes, hashlib.sha256(source_bytes).hexdigest()))
    if args.require_complete_sources and (missing_from_lib or source_read_errors):
        raise SystemExit(
            f"the fixed population has {len(missing_from_lib)} missing sources and "
            f"{len(source_read_errors)} unreadable sources"
        )
    source_files.sort(key=lambda source: source[1])
    source_manifest: list[dict[str, str]] = [
        {"path": relative, "sha256": digest}
        for _path, relative, _bytes, digest in source_files
    ]
    manifest_text: str = "".join(
        f"{entry['path']}\t{entry['sha256']}\n" for entry in source_manifest
    )
    manifest_digest: str = hashlib.sha256(manifest_text.encode("utf-8")).hexdigest()
    ...
    for f, _relative, source_bytes, _source_digest in source_files:
        try:
            a = compile(
    ...
    result = {
        ...
        "module_list_sha256": sha256_file(args.modules),
        "source_manifest_sha256": manifest_digest,
        "source_manifest": source_manifest,
        "candidate_source_identity": args.candidate_source_identity,
        "candidate_source_identity_attestation": "caller-supplied",
        "original_executable_sha256": original_executable_sha256,
        "measured_executable_sha256": measured_executable_sha256,
        "interpreter_sha256": sha256_file(sys.executable),
