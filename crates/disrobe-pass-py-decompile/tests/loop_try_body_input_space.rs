...
#[test]
fn parser_parse_keeps_its_rotated_walrus_loop() {
    let band: Vec<BandInterpreter> = resolve_band(&["3.14.5"], &[]);
    assert_eq!(
        band.len(),
        1,
        "CPython 3.14.5 is required for the email.parser regression"
    );
    let scratch: PathBuf = band_scratch("email_parser_parse_rotated_walrus");
    let (outcome, recovered): (BandOutcome, String) = recompile_equiv_inline(
        &band[0],
        PARSER_PARSE,
        "email_parser_parse_rotated_walrus",
        &scratch,
    );
    match outcome {
