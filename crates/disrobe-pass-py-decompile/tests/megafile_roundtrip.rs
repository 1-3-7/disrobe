...
/// Recompile emitted source with a version-matched interpreter; `Ok(())` means it parsed and
/// compiled (no `SyntaxError`). `Err` carries the syntax/compile error signature.
fn recompile_syntactic(interpreter: &Path, source: &str, tag: &str) -> Result<(), String> {
...
fn megafile_roundtrip_full_matrix() {
    ...
    for o in &outcomes {
        ...
        if let Err(sig) = recompile_syntactic(&interpreter, &o.source, &tag) {
...
/// Honest ceiling on emitted modules that raise a syntax error across the whole-module matrix.
/// The engine must emit valid Python; this counts modules the interpreter refuses to recompile.
/// Ratchet DOWN toward 0 as the engine improves; never up. Every band now recompiles: the 3.14
/// `__conditional_annotations__` seed is stripped so `from __future__` stays first, and a `match`
/// head nested inside a `try` protected body no longer folds the `except*` dispatch into duplicate
/// wildcard arms (the enclosing `try` is structured first).
