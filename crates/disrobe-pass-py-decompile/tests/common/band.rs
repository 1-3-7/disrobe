...
/// One in-band python interpreter the band harness will recompile against. `alias` is the `X.Y`
/// resolver key (`uv python find`/python.org fallback); `is_prerelease` flips on the 3.15 beta so
/// the band harness applies jump-index-aware tolerance and never lets a beta-only codegen drift fail
/// the gate.
#[derive(Debug, Clone)]
...
/// Outcome of driving one in-band fixture: either a real recompile-equivalence verdict against a fresh
/// in-band `.pyc`, or a token-match fallback against the vendored ORIGINAL source when no in-band
/// interpreter exists for that sub-band.
#[derive(Debug, Clone)]
...
/// `uv python find` first, then the canonical python.org Windows install locations so the band can
/// exercise interpreters `uv` refuses to resolve (2.7) or does not index.
#[must_use]
...
/// Resolve every requested in-band alias that is actually installed. `prerelease_aliases` flips the
/// jump-index-aware flag for beta builds (3.15) so the band harness treats their verdict leniently.
#[must_use]
...
/// Hard placeholder-leak guard. Any surviving `__DR_*__` sentinel means the emitter degenerated
/// instead of recovering real source, so the band must reject it even if recompile happened to pass.
...
/// Drive one construct fixture through the real recompile-equivalence oracle on `interp`: compile the
/// ORIGINAL source to a fresh in-band `.pyc`, decompile it, recompile the recovered source, and assert
/// the round-trip is `Perfect`/`Semantic` AND leaks no placeholder. The original `.pyc` is the
/// independent ground truth - disrobe never emits it.
...
/// Drive one vendored legacy `.pyc` through the recompile-equivalence oracle on `interp`. The legacy
/// `.pyc` itself is the ground truth (already on disk, disrobe never emits it): decompile it, recompile
/// the recovered source on the in-band interpreter, and assert round-trip equivalence + no placeholder.
...
/// Token-match fallback for a sub-band with NO installed interpreter (pre-3.6 / pre-2.7): decompile a
/// vendored legacy `.pyc` and assert the recovered source tokenizes equal to the vendored ORIGINAL
/// `.py`. Recompile is impossible here, so the caller MUST have emitted a loud `eprintln!` SKIP for the
/// recompile leg before relying on this. Still placeholder-guarded - a degenerate emit cannot pass.
...
/// Enumerate vendored legacy `.pyc` files whose marshalled `(major, minor)` falls within
/// `[low, high]` (inclusive), returning `(pyc_path, vtag, stem)` sorted for determinism. `stem` is the
/// fixture name with the trailing `.X.Y` version tag stripped, matching the legacy source layout.
#[must_use]
...
/// Resolve the vendored ORIGINAL source for a legacy fixture stem, honouring the `_py3` demotion the
/// legacy corpus uses (`test_functions_py3` shares `test_functions.py`).
#[must_use]
