...
pub(crate) fn find_disrobe() -> Option<PathBuf> {
    let configured: Option<std::ffi::OsString> = std::env::var_os("DISROBE_MEASUREMENT_EXECUTABLE");
    if let Some(configured) = configured {
        let candidate: PathBuf = PathBuf::from(configured);
        assert!(
            candidate.is_file(),
...
pub(crate) struct EvidenceRequest<'a> {
    ...
    pub object_ledger: &'a Path,
    pub candidate_source_identity: &'a str,
    pub require_version: &'a str,
    pub require_magic: &'a str,
    pub require_optimize: u8,
}

#[must_use]
pub(crate) fn run_measure_with_evidence(
    python: &Path,
    disrobe: &Path,
    lib: &Path,
    modules: &Path,
    ...
    command
        .arg("--object-ledger")
        .arg(request.object_ledger)
        .arg("--candidate-source-identity")
        .arg(request.candidate_source_identity)
        .arg("--original-executable")
        .arg(request.original_executable)
        .arg("--require-version")
        .arg(request.require_version)
        .arg("--require-magic")
        .arg(request.require_magic)
        .arg("--require-optimize")
        .arg(request.require_optimize.to_string())
        .arg("--require-complete-sources");
    let output: std::process::Output = command
        .stdin(Stdio::null())
        .output()
        .expect("spawn recompile-equivalence harness");
