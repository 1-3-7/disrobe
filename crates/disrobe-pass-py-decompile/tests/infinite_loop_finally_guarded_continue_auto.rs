...
const SOURCE: &str = r"def consume(next_item, should_continue, sink):
    while True:
        try:
            sink(next_item())
        finally:
...
assert len(loop.body) == 2 and isinstance(loop.body[0], ast.Try)
assert isinstance(loop.body[1], ast.Break)
region = loop.body[0]
...
#[test]
fn auto_preserves_guarded_continue_inside_infinite_loop_finally() {
    let interpreter: PathBuf = find_interpreter("3.12").unwrap_or_else(|| {
        panic!("CPython 3.12 is required for the finally-arm guarded-continue fixture")
    });
    ...
    let scratch: PathBuf = band_scratch("infinite-loop-finally-guarded-continue-auto");
    ...
    assert_eq!(
        recovered.matches("if should_continue():").count(),
        1,
        "{recovered}"
    );
    assert_eq!(
    ...
    assert_eq!(
        recovered.matches("sink(\"stop\")").count(),
        1,
        "{recovered}"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "['one', 'cleanup', 'two', 'cleanup', 'stop', 'done']"
    );
}
