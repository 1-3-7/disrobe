...
const SOURCE: &str = r"def consume(next_item, should_stop, sink):
    while True:
        try:
            sink(next_item())
        finally:
...
events = []
consume(values(['one', 'two']), values([False, True]), events.append)
print(events)
";
...
assert len(guard.body) == 1 and isinstance(guard.body[0], ast.Break)
...
#[test]
fn auto_preserves_guarded_break_inside_infinite_loop_finally() {
    let interpreter: PathBuf = find_interpreter("3.12").unwrap_or_else(|| {
        panic!("CPython 3.12 is required for the finally-arm guarded-break fixture")
    });
    ...
    let scratch: PathBuf = band_scratch("infinite-loop-finally-guarded-break-auto");
    ...
    assert_eq!(
        recovered.matches("if should_stop():").count(),
        1,
        "{recovered}"
    );
    ...
    assert_eq!(
        ...
        "['one', 'cleanup', 'continue', 'two', 'cleanup', 'done']"
    );
}
