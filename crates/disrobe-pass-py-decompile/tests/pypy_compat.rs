...
fn pypy_magic_dispatch() {
    ...
    DecompilePass::dispatch_runtime(runtime).expect("PyPy runtime dispatches to full pipeline");
}
...
fn pypy_full_pipeline() {
    ...
    let runtime: AltRuntime = detect_runtime(PYPY_MAGIC_310_MARKER);
    assert_eq!(runtime, AltRuntime::PyPy);
    DecompilePass::dispatch_runtime(runtime).expect("PyPy full pipeline runtime dispatches");
