...
fn py_decompile_band_3_12_to_3_15() {
    ...
    let mut prerelease_tolerated: usize = 0;
                ...
                BandOutcome::Tolerated(detail) => {
                    if interp.is_prerelease {
                        ...
                        failures.push(format!(
                            "py{} {construct}: Tolerated outcome from a stable interpreter is a real failure: {detail}",
                            interp.alias
                        ));
                    }
                }
    ...
    println!(
        "=== BAND 3.12-3.15 SUMMARY: stable {recompiled_stable}/{checked_stable} recompile-equiv; \
         prerelease {prerelease_evaluated} evaluated, {prerelease_tolerated} CodeDiff tolerated \
         (jump-index-aware, non-gating) ==="
    );
    assert!(
        failures.is_empty(),
