...
use common::band_gate::{
    BandPopulation, CPYTHON_311, PINNED_MODULE_COUNT, PINNED_MODULE_LIST,
...
const OBJECT_PCT_FLOOR: f64 = 96.36;
const BAND_OBJECTS_OK: u64 = 5_433;
const BAND_CODE_OBJECTS: u64 = 5_638;
const BAND_MODULES: u64 = 172;
const BAND_MODULES_EXACT_FLOOR: u64 = 101;
const BAND_MISSING_FROM_LIB: u64 = 28;
const BAND_CPYTHON: &str = CPYTHON_311.release;
...
#[test]
fn published_311_band_bar_agrees_with_the_counts_this_gate_enforces() {
    let doc: serde_json::Value = recovery_document();
    let bar: PublishedBar = published_band_bar(&doc, BAND_LABEL);

    println!("=== PUBLISHED CPYTHON 3.11 BAND ===");
    println!(
        "{}",
    ...
    println!(
        "this case reads xtask/data/recovery.json and compares it against the counts \
         `arbitrary_recompile_equivalence_gate_311` enforces. It decompiles nothing, so it proves \
    ...
    assert_eq!(
        ...
        "this gate expects {BAND_MODULES} measured modules and {BAND_MISSING_FROM_LIB} absent from \
         the 3.11 Lib, which does not account for all {PINNED_MODULE_COUNT} pinned module paths"
    );
    ...
    assert!(
        ...
        "the `{BAND_LABEL}` detail never names the interpreter release the counts were measured \
         on, so a re-measurement on another 3.11 patch cannot be told apart from this one: {detail}"
    );
...
#[test]
fn a_shrunken_311_band_population_is_rejected() {
    assert_population_pin_rejects_shrinkage(&published(), OBJECT_PCT_FLOOR, BAND_POPULATION);
}

#[test]
fn arbitrary_recompile_equivalence_gate_311() {
    ...
    let Some(python): Option<PathBuf> = resolve_band_interpreter(&CPYTHON_311, &graded()) else {
        return;
    };
    ...
    assert_eq!(
        (maj, min),
        (3, 11),
        "resolved interpreter at {} is {maj}.{min}, not 3.11",
        python.display()
    );
    ...
    println!("=== ARBITRARY RECOMPILE-EQUIVALENCE HARNESS (3.11) ===");
    ...
    assert_measurement_is_comparable(&m.cpython_version, &CPYTHON_311, &graded());
    println!(
        "{}",
    ...
    assert_eq!(
        ...
        "{} of the {PINNED_MODULE_COUNT} pinned modules are absent from this 3.11 Lib, not the \
    ...
    assert_eq!(
        ...
        "only {} of the pinned modules were measured on 3.11, not {BAND_MODULES}; a run that \
    ...
    assert!(
        ...
        "per-code-object recompile-equivalence regressed on 3.11: {:.2}% < floor {OBJECT_PCT_FLOOR}% \
    ...
    assert!(
        ...
        "whole-module exact recovery regressed on 3.11: {} of the {} measured modules came back \
    ...
    assert!(
        disagreements.is_empty(),
        "xtask/data/recovery.json publishes {} / {} over {} modules for the 3.11 band and the \
