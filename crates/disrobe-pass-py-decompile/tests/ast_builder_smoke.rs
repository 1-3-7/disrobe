...
fn smoke_load_const_str_then_return_value_312() {
    ...
    assert_eq!(
        ...
        "no LOAD_CONST/STORE_NAME __doc__ prologue means no module docstring"
    );
}
...
#[test]
fn import_from_level_is_bounded_before_ast_output() {
    let mut code: CodeObject = CodeObject::new(CodeEra::Py311Plus);
    code.consts = vec![
        Object::Int64(i64::from(u32::MAX)),
        Object::Tuple(vec![Object::String {
            value: "name".to_owned(),
            interned: false,
        }]),
        Object::None,
    ...
    let module: AstModule = builder
        .build_module(&code, &tree, &PyVersion::V3_12)
        .expect("build succeeds");
    let Stmt::ImportFrom { level, .. } = &module.body[0] else {
        panic!("expected ImportFrom, got {:?}", module.body[0])
    };
    assert!(*level <= 32);
}
