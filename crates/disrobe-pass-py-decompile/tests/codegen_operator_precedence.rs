...
#[test]
fn nonfinite_float_constants_render_as_fold_expressions() {
    let em: DefaultEmitter = DefaultEmitter::new();
    let v: PyVersion = PyVersion::V3_8;
    let cases: [(f64, &str); 4] = [
        (f64::from_bits(0xFFF8_0000_0000_0000), "(1e309 * 0)"),
        (f64::from_bits(0x7FF8_0000_0000_0000), "-(1e309 * 0)"),
        (f64::INFINITY, "1e309"),
        (f64::NEG_INFINITY, "-1e309"),
    ];
    for (bits, expected) in cases {
        let e: Expr = Expr::Constant {
            value: ConstValue::Float(bits),
            line: None,
        };
        assert_eq!(em.emit_expr(&e, &v), expected);
    }
}

#[test]
fn nonfinite_complex_constant_renders_parseable() {
    let em: DefaultEmitter = DefaultEmitter::new();
    let v: PyVersion = PyVersion::V3_8;
    let e: Expr = Expr::Constant {
        value: ConstValue::Complex {
            real: 0.0,
            imag: f64::from_bits(0xFFF8_0000_0000_0000),
        },
        line: None,
    };
    assert_eq!(em.emit_expr(&e, &v), "complex(0.0, (1e309 * 0))");
}
