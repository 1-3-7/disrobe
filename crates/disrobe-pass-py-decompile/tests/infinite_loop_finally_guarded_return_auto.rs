...
const SOURCE: &str = r"def consume(next_item, should_return, sink):
    while True:
        try:
            sink(next_item())
        finally:
...
events = []
result = consume(values(['one', 'two']), values([False, True]), events.append)
print(result, events)
";
...
assert isinstance(guard.body[0].value, ast.Constant) and guard.body[0].value.value == "returned"
...
#[test]
fn auto_preserves_guarded_return_inside_infinite_loop_finally() {
    let interpreter: PathBuf = find_interpreter("3.12").unwrap_or_else(|| {
        panic!("CPython 3.12 is required for the finally-arm guarded-return fixture")
    });
    ...
    let scratch: PathBuf = band_scratch("infinite-loop-finally-guarded-return-auto");
    ...
    assert_eq!(
        recovered.matches("return \"returned\"").count(),
        1,
        "{recovered}"
    );
    assert_eq!(
        recovered.matches("sink(next_item())").count(),
        1,
        "{recovered}"
    );
    assert_eq!(
    ...
    assert_eq!(
        recovered.matches("sink(\"continue\")").count(),
        1,
        "{recovered}"
    );
    ...
    assert_eq!(
        ...
        "returned ['one', 'cleanup', 'continue', 'two', 'cleanup']"
    );
}
