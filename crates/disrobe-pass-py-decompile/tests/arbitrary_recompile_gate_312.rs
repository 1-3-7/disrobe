...
use common::band_gate::{
    BandPopulation, CPYTHON_312, PINNED_MODULE_COUNT, PINNED_MODULE_LIST,
...
const OBJECT_PCT_FLOOR: f64 = 95.56;
const BAND_OBJECTS_OK: u64 = 5_408;
const BAND_CODE_OBJECTS: u64 = 5_659;
const BAND_MODULES: u64 = 177;
const BAND_MODULES_EXACT_FLOOR: u64 = 107;
...
#[test]
fn published_312_band_bar_agrees_with_the_counts_this_gate_enforces() {
    let doc: serde_json::Value = recovery_document();
    let bar: PublishedBar = published_band_bar(&doc, BAND_LABEL);

    println!("=== PUBLISHED CPYTHON 3.12 BAND ===");
    println!(
        "{}",
    ...
    println!(
        "this case reads xtask/data/recovery.json and compares it against the counts \
         `arbitrary_recompile_equivalence_gate_312` enforces. It decompiles nothing, so it proves \
    ...
    assert_eq!(
        ...
        "this gate expects {BAND_MODULES} measured modules and {BAND_MISSING_FROM_LIB} absent from \
         the 3.12 Lib, which does not account for all {PINNED_MODULE_COUNT} pinned module paths"
    );
    ...
    assert!(
        ...
        "the `{BAND_LABEL}` detail never names the interpreter release the counts were measured \
         on, so a re-measurement on another 3.12 patch cannot be told apart from this one: {detail}"
    );
...
#[test]
fn a_shrunken_312_band_population_is_rejected() {
...
fn arbitrary_recompile_equivalence_gate_312() {
    ...
    let Some(python): Option<PathBuf> = resolve_band_interpreter(&CPYTHON_312, &graded()) else {
        return;
    };
    ...
    assert_measurement_is_comparable(&m.cpython_version, &CPYTHON_312, &graded());
    println!(
        "{}",
    ...
    assert_eq!(
        ...
        "the 3.12 measurement matched multiple original sibling code objects to one recovered \
         sibling {} times",
        m.sibling_collisions
    );
    assert_eq!(
        m.missing_from_lib, BAND_MISSING_FROM_LIB,
        "{} of the {PINNED_MODULE_COUNT} pinned modules are absent from this 3.12 Lib, not the \
    ...
    assert_eq!(
        ...
        "only {} of the pinned modules were measured on 3.12, not {BAND_MODULES}; a run that \
    ...
    assert!(
        ...
        "whole-module exact recovery regressed on 3.12: {} of the {} measured modules came back \
    ...
    assert!(
        disagreements.is_empty(),
        "xtask/data/recovery.json publishes {} / {} over {} modules for the 3.12 band and the \
