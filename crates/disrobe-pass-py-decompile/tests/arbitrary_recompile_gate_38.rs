...
const BAND_LABEL: &str = "CPython 3.8 (154 of the pinned modules)";
const BAND_LABEL_PREFIX: &str = "CPython 3.8 (";
const BAND_POPULATION: &str = "cpython-38-band";

const OBJECT_PCT_FLOOR: f64 = 87.79;
const BAND_OBJECTS_OK: u64 = 4_467;
    ...
    serde_json::json!({
        "groups": [{
            "bars": [
                ...
                {
                    "label": "CPython 3.10 (161 of the pinned modules)",
                    "value": 94.72,
                    "num": 5170,
                    "den": 5458,
                    "modules": 161,
                    "detail": "5170 / 5458 code objects over 161 modules on CPython 3.10.20.",
                },
            ],
        }],
    })
...
#[test]
fn published_38_band_bar_agrees_with_the_counts_this_gate_enforces() {
    let doc: serde_json::Value = recovery_document();

    println!("=== CPYTHON 3.8 BAND ===");
    println!(
        "{}",
        population_line(
            BAND_POPULATION,
    ...
    println!(
        "this case decompiles nothing. It proves the counts \
         `arbitrary_recompile_equivalence_gate_38` enforces describe one population, and that they \
    ...
    assert_eq!(
        ...
        "this gate expects {BAND_MODULES} measured modules and {BAND_MISSING_FROM_LIB} absent from \
         the 3.8 Lib, which does not account for all {PINNED_MODULE_COUNT} pinned module paths"
    );
    const {
        assert!(
...
#[test]
fn a_bar_published_under_this_band_prefix_is_read_whatever_its_module_count_says() {
    let none: serde_json::Value = serde_json::json!({ "groups": [{ "bars": [] }] });
    assert!(
        matches!(
            band_publication(&none, BAND_LABEL_PREFIX),
            BandPublication::Unpublished
        ),
        "a chart with no 3.8 bar has to read as unpublished, not as a bar with zero counts"
    );

    let published: serde_json::Value = doc_with_bar(
        "CPython 3.8 (999 of the pinned modules)",
        OBJECT_PCT_FLOOR,
        BAND_OBJECTS_OK,
        BAND_CODE_OBJECTS,
        999,
    ...
    let found: BandPublication = band_publication(&published, BAND_LABEL_PREFIX);
    match found {
        BandPublication::Published { label, bar } => {
            assert_eq!(label, "CPython 3.8 (999 of the pinned modules)");
            assert_eq!(bar.num, BAND_OBJECTS_OK);
            assert_eq!(bar.den, BAND_CODE_OBJECTS);
            assert_eq!(
                bar.modules, 999,
                "the prefix lookup has to return the bar as published, wrong module count and \
                 all, so the caller can reject it rather than never finding it"
            );
        }
        BandPublication::Unpublished => panic!(
            "a bar labelled with the `{BAND_LABEL_PREFIX}` prefix but a different module count \
             read as unpublished, so a mislabelled 3.8 bar could sit on the chart with no gate \
             enforcing it"
        ),
    }
}
...
#[test]
#[should_panic(expected = "is not the published")]
fn a_published_38_bar_that_disagrees_with_the_measured_numerator_is_rejected() {
    let wrong: serde_json::Value = doc_with_bar(
        BAND_LABEL,
        OBJECT_PCT_FLOOR,
        BAND_OBJECTS_OK + 1,
        BAND_CODE_OBJECTS,
    ...
    drop(assert_publication_matches_the_gate(
        &wrong,
        BAND_LABEL_PREFIX,
        BAND_LABEL,
        &enforced(),
...
#[test]
#[should_panic(expected = "bars whose label starts")]
fn two_bars_under_the_38_prefix_are_rejected() {
    let duplicated: serde_json::Value = serde_json::json!({
        "groups": [{
            "bars": [
                { "label": BAND_LABEL, "value": OBJECT_PCT_FLOOR, "num": BAND_OBJECTS_OK,
                  "den": BAND_CODE_OBJECTS, "modules": BAND_MODULES, "detail": "detail" },
                { "label": "CPython 3.8 (12 of the pinned modules)", "value": 50.0, "num": 1,
                  "den": 2, "modules": 12, "detail": "detail" },
            ],
        }],
    });
    drop(band_publication(&duplicated, BAND_LABEL_PREFIX));
}

#[test]
fn a_shrunken_38_band_population_is_rejected() {
    assert_population_pin_rejects_shrinkage(&enforced_bar(), OBJECT_PCT_FLOOR, BAND_POPULATION);
}

#[test]
fn the_38_band_population_carries_the_bytecode_only_3_8_emits() {
    let Some(python): Option<PathBuf> = resolve_band_interpreter(&CPYTHON_38, &graded()) else {
        return;
    };
    ...
    println!("=== CPYTHON 3.8 BYTECODE REACH ===");
    println!(
    ...
    assert_eq!(
        ...
        "{} of the pinned modules are absent from this 3.8 Lib, not the {BAND_MISSING_FROM_LIB} \
         this band records",
    ...
    assert_eq!(
        ...
        "{} of the measured code objects declare positional-only parameters, not the \
         {BAND_POSONLY_OBJECTS} this band records. PEP 570 landed in 3.8 and the equivalence check \
         compares co_posonlyargcount, so this band has to keep reaching them. A CPython 3.8 patch \
         release whose Lib uses the syntax differently needs a re-measured band, not an edited \
         count",
        reach.posonly_objects
    );
...
#[test]
fn arbitrary_recompile_equivalence_gate_38() {
    ...
    assert_eq!(
        (maj, min),
        (3, 8),
        "resolved interpreter at {} is {maj}.{min}, not 3.8",
        python.display()
    );
    ...
    println!("=== ARBITRARY RECOMPILE-EQUIVALENCE HARNESS (3.8) ===");
    ...
    assert_eq!(
        ...
        "{} of the {PINNED_MODULE_COUNT} pinned modules are absent from this 3.8 Lib, not the \
    ...
    assert_eq!(
        ...
        "only {} of the pinned modules were measured on 3.8, not {BAND_MODULES}; a run that \
    ...
    assert!(
        ...
        "per-code-object recompile-equivalence regressed on 3.8: {:.2}% < floor \
    ...
    assert!(
        ...
        "whole-module exact recovery regressed on 3.8: {} of the {} measured modules came back \
    ...
    assert!(
        ...
        "this gate enforces {BAND_OBJECTS_OK} / {BAND_CODE_OBJECTS} over {BAND_MODULES} modules \
         for the 3.8 band, but this run measured {} / {} over {} modules on CPython {}: \
         {disagreements:?}",
        m.objects_ok,
        m.code_objects,
        m.modules,
