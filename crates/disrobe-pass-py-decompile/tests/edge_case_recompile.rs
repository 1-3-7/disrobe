...
fn assert_recompiles(label: &str, program: &str) {
    ...
    assert_recompiles_with(label, program, &band);
}

fn assert_recompiles_with(label: &str, program: &str, band: &[BandInterpreter]) {
...
#[test]
fn global_calls_with_direct_and_starred_arguments() {
    assert_recompiles(
        "edge_global_star_call",
        "values = (1, 2)\ndef target(*items):\n    return items\ndef call():\n    direct = target(0)\n    return (direct, target(*values))\n",
    );
}

#[test]
fn python_314_branch_targets_preserve_global_callables() {
    let band: Vec<BandInterpreter> = resolve_band(&["3.14"], &[]);
    assert_eq!(
        band.len(),
        1,
        "edge_314_global_call_targets: Python 3.14 is required"
    );
    let scratch: PathBuf = band_scratch("edge_314_global_call_targets");
    let program: &str = "def helper(*, value):\n    return value\n\ndef keyword_after_guard(value):\n    if not value:\n        raise ValueError('missing')\n    return helper(value=value)\n\ndef builtin_in_try(value):\n    try:\n        converted = int(value)\n    except (TypeError, ValueError):\n        return 0\n    return converted\n";
    let (outcome, source): (BandOutcome, String) =
        recompile_equiv_inline(&band[0], program, "edge_314_global_call_targets", &scratch);
    assert!(
        source.contains("return helper(value=value)"),
        "keyword call target changed in recovered source:\n{source}"
    );
    assert!(
        source.contains("converted = int(value)"),
        "builtin call target changed in recovered source:\n{source}"
    );
    assert!(
        matches!(outcome, BandOutcome::RecompileEquiv),
        "expected recompile-equivalent recovery, got {outcome:?}\n--- recovered:\n{source}"
    );
}

#[test]
fn generator_expression_starred_call_without_keywords() {
    assert_recompiles(
        "edge_generator_star_call",
        "def call(fn, rows):\n    return tuple(fn(*row) for row in rows)\n",
    );
}

#[test]
fn generator_expression_starred_call_with_keywords() {
    assert_recompiles(
        "edge_generator_star_kw_call",
        "def call(fn, rows, options):\n    return tuple(fn(*row, **options) for row in rows)\n",
    );
}
...
#[test]
fn context_manager_with_nested_exception_handler() {
    let interpreter: BandInterpreter = BandInterpreter {
        alias: "3.14.5",
        path: find_interpreter("3.14.5").expect(
            "context-manager branch regression requires CPython 3.14.5; install it with `uv python install 3.14.5`",
        ),
        is_prerelease: false,
    };
    assert_recompiles_with(
        "edge_with_nested_except",
