...
/// Honest recovery floor across the per-version construct matrix. Green means real round-trip
/// recovery (`Perfect`+`Semantic`); ratchet UP per commit as the engine improves, never down.
...
/// 3.15 ships only as a pre-release (3.15.0b1) whose `with`/`async`/generator lowering still diverges
/// from the GA-frozen 3.14 shapes the frame-tree models. Its rows are evaluated and reported for early
/// signal on the real PY315 decode path, but excluded from the gating recovery floor so a beta-only
/// codegen drift cannot retroactively lower the proven stable-matrix guarantee.
...
/// `uv` refuses to resolve interpreters older than 3.6 and does not always index manually-installed
/// python.org builds. Fall back to the canonical Windows install locations so the construct matrix can
/// exercise 2.7/3.4/3.5/3.6/3.7 when the user has them on disk.
...
/// Regression for the 3.12+ dropped-tail-return bug: `try/except/else/finally` returns its post-finally
/// fall-through value (`return total`). Before the fix the decompiler dropped the construct-exit return
/// on 3.12-3.14, so the recovered function fell off the end and returned `None` (orig `f([1,2,3]) == 106`
/// vs recovered `None`). Asserts the recovered source carries `return total` and that the recovered
/// function still evaluates to 106 across 3.12/3.13/3.14.
#[test]
