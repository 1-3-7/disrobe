...
#[test]
fn open_coded_any_genexpr_in_try_recovers_3_14() {
    ...
    let (source, verdict): (String, Verdict) = decompile_and_roundtrip_3_14(
        &interpreter,
        "open_coded_any_genexpr_try.py",
        "open-coded-any-genexpr-scratch",
    );
    assert!(
        !source.contains("None(None())") && !source.contains("__DR_CODE_CONST_"),
        "open-coded any/all genexpr collapsed to a placeholder; got:\n{source}"
    );
    assert_eq!(
        source.matches("for component in version_number").count(),
        2,
        "both open-coded any(genexpr) code objects must survive as genexprs; got:\n{source}"
    );
    assert!(
        source.contains("any((not component.isdigit()")
            && source.contains("any((len(component) > 10"),
        "open-coded any(genexpr) must recover the enclosing any() call; got:\n{source}"
    );
    assert!(
        matches!(verdict, Verdict::Perfect | Verdict::Semantic),
        "open-coded any(genexpr)-in-try round-trip not equivalent: {verdict:?}\nsource:\n{source}"
    );
}
