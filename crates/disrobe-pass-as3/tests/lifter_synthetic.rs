...
fn measures_method_body_recovery_rate() {
    ...
    assert!(
        skel.contains("function init(arg1: Object)")
            && skel.contains("function scale(arg1: Object)"),
        "unnamed parameters must use the same identifiers in signatures and bodies: {skel}"
    );
    assert!(
