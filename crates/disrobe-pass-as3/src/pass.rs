...
impl LegacyPass for As3Pass {
    ...
    fn run(&self, artifact: &Artifact) -> CoreResult<Artifact> {
        ...
        let parsed: Swf = swf::parse(&input.bytes)
            .map_err(|e| CoreError::PassFailure(format!("DR-AS3-PASS: {e}")))?;
        ...
        let payload: Vec<u8> = serde_json::to_vec(&report)
            .map_err(|e| CoreError::PassFailure(format!("DR-AS3-PASS: serialize: {e}")))?;
