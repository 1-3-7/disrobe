//! Read-only obfuscation detection for AS3/ABC.

use std::collections::BTreeMap;

use serde::{Deserialize, Serialize};

use crate::abc::{AbcFile, InstanceInfo};
...
/// A detection-only report of observed obfuscation signals, tools, and ratios.
...
fn identifier_mangle_ratio(abc: &AbcFile) -> u8 {
    ...
    for tr in abc.instances.iter().flat_map(|i: &InstanceInfo| &i.traits) {
...
/// Density of stack-shuffle opcodes (`pop`/`dup`/`swap`), per mille capped at 100.
...
/// Fraction of pool strings that look like an encrypted/rebuilt blob.
fn string_pool_rebuild_ratio(abc: &AbcFile) -> u8 {
    let strings: &[String] = &abc.cpool.strings;
    let total: usize = strings.iter().filter(|s: &&String| !s.is_empty()).count();
    if total == 0 {
        return 0;
    }
...
/// Fingerprint commercial obfuscators from pool vendor markers and decrypt stubs.
