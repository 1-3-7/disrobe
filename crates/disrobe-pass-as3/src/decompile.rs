...
/// Index from a method-info slot to its parsed body, so a method trait can be
/// resolved to the bytecode the lifter walks. Built once per ABC file.
...
fn write_constructor(
    ...
    match lifted_method_body(abc, instance.iinit, bodies) {
        Some(body) if !body.trim().is_empty() => {
            let _ = writeln!(out, "    public function {class_name}({params}) {{");
            out.push_str(&body);
            out.push_str("    }\n");
        }
        _ => {
...
fn write_trait(
    ...
    match kind {
        0 => {
            ...
            let _ = writeln!(out, "    public var {raw_name}: {ty};");
        }
        6 => {
            let ty: String = render_type_or_star(abc, trait_info.type_name);
            let _ = writeln!(out, "    public const {raw_name}: {ty};");
        }
        ...
        4 => {
            let _ = writeln!(out, "    public class {raw_name} {{ /* nested */ }}");
        }
        other => {
            let _ = writeln!(out, "    /* trait kind {other} {raw_name} */");
        }
    }
    Ok(())
}
