...
impl<'a> Reader<'a> {
    ...
    fn u30(&mut self) -> Result<u32> {
        let v: u32 = self.u32_var()?;
        if v >= (1u32 << 30) {
            return Err(Error::AbcU30Overflow(v));
        }
        Ok(v)
    }
    ...
    fn s32_var(&mut self) -> Result<i32> {
        ...
        for i in 0..5 {
            ...
            acc |= u32::from(byte & 0x7F) << shift;
            if (byte & 0x80) == 0 {
                if shift < 32 && (byte & 0x40) != 0 && i < 4 {
                    acc |= !0u32 << (shift + 7);
                }
                return Ok(acc.cast_signed());
            }
            shift += 7;
        }
        Ok(acc.cast_signed())
...
fn parse_trait(r: &mut Reader<'_>) -> Result<TraitInfo> {
    ...
    match kind {
        0 | 6 => {
            ...
            let _vindex: u32 = r.u30()?;
            let vindex_present: bool = matches!(kind, 0 | 6);
            let _vkind: u8 = if vindex_present { r.u8()? } else { 0 };
        }
        1..=5 => {
            let _index_a: u32 = r.u30()?;
            let _index_b: u32 = r.u30()?;
        }
        _ => {}
    }
    if (kind_byte & 0x40) != 0 {
...
#[must_use]
fn opcode_operand_count(op: u8) -> u8 {
    match op {
        0x08
        | 0x10..=0x1A
        | 0x24
        ...
        | 0xC3
        | 0xF1 => 1,
        0x06 | 0x41 | 0x42 | 0x43 | 0x45 | 0x46 | 0x4A | 0x4C | 0x4E | 0x4F | 0x53 | 0x65
        | 0x32 => 2,
        0xEF => 4,
        _ => 0,
    }
...
pub fn disasm(code: &[u8]) -> Result<Vec<DisasmLine>> {
    ...
    while r.pos < code.len() {
        ...
        match op {
            ...
            _ => {
                let n: u8 = opcode_operand_count(op);
                for _ in 0..n {
                    operands.push(i64::from(r.u30()?));
                }
            }
...
mod tests {
    ...
    #[test]
    fn u30_overflow_rejected() {
        let mut r: Reader<'_> = Reader::new(&[0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
        let err: Error = r.u30().expect_err("overflow must reject");
