...
impl LiftedBody {
    /// A human-readable fidelity warning, or `None` when fully recovered.
    #[must_use]
...
impl Lifter<'_> {
    ...
    /// Resolve a slot id to its declared trait name, falling back to `slot{n}`.
    ...
    /// Resolve a `newclass` class index to the class's name, else `class{n}`.
...
fn step(lifter: &mut Lifter<'_>, line: &DisasmLine, next_off: usize, end_off: usize) {
    ...
    match op {
        ...
        0x04 => emit_getsuper(lifter, ops.first().copied().unwrap_or(0)),
        0x05 => emit_setsuper(lifter, ops.first().copied().unwrap_or(0)),
        0x6A => emit_deleteproperty(lifter, ops.first().copied().unwrap_or(0)),
        0x59 => emit_getdescendants(lifter, ops.first().copied().unwrap_or(0)),
        0x53 => emit_applytype(lifter, ops.first().copied().unwrap_or(0)),
        0x40 => emit_newfunction(lifter, ops.first().copied().unwrap_or(0)),
        0xB2 => emit_istype(lifter, ops.first().copied().unwrap_or(0)),
        0xB3 => emit_istypelate(lifter),
        0x58 => emit_newclass(lifter, ops.first().copied().unwrap_or(0)),
        ...
        0x02 | 0x09 | 0x1D | 0x82 | 0xEF | 0xF0 | 0xF1 => {}
...
fn emit_hasnext2(lifter: &mut Lifter<'_>, ops: &[i64]) {
    let obj_reg: i64 = ops.first().copied().unwrap_or(0);
    let idx_reg: i64 = ops.get(1).copied().unwrap_or(0);
...
/// True when `callee` is the lexical-scope object for the same multiname.
...
/// Count `If`/`Jump` statements in `stmts` targeting `label`.
...
/// Re-structure a single-entry forward conditional skip into an `if`-block.
...
/// Build a slot-id to trait-name map for `getslot`/`setslot` resolution.
...
/// Lift one method body into recovered statements via AVM2 stack interpretation.
pub fn lift_body(
    abc: &AbcFile,
    body: &MethodBody,
    ...
    let reached_terminator: bool = statements
        .iter()
        .any(|s: &Stmt| matches!(s, Stmt::Return(_) | Stmt::Throw(_)));
    let fully_recovered: bool =
        reached_terminator && dropped_opcodes.is_empty() && opaque_operands == 0;
    Ok(LiftedBody {
        statements,
        fully_recovered,
        reached_terminator,
...
/// Render a lifted body to indented AS3 pseudocode.
...
mod tests {
    ...
    fn unmodelled_opcode_marks_not_fully_recovered() {
        ...
        assert!(
            ...
            "dxnslate (0x07) is unmodelled so the body must not claim full recovery"
        );
        assert_eq!(lifted.dropped_opcodes, vec![0x07]);
        let warning: String = lifted.fidelity_warning().expect("warning present");
        assert!(
            warning.contains("0x07"),
