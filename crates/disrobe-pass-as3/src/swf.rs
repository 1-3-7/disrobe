...
impl Swf {
    ...
    pub fn file_attributes(&self) -> Option<FileAttributes> {
        self.tags
            .iter()
            .find(|t| t.code == TagCode::FILE_ATTRIBUTES)
            .and_then(|t| parse_file_attributes(t).ok())
    }
    ...
    pub fn symbol_classes(&self) -> Vec<SymbolClassEntry> {
        self.tags
            .iter()
            .filter(|t| t.code == TagCode::SYMBOL_CLASS)
            .filter_map(|t| parse_symbol_class(t).ok())
            .flatten()
            .collect()
    }
}
...
fn decompress_body(
    ...
    match compression {
        ...
        SwfCompression::Zlib => {
            ...
            let mut out: Vec<u8> = Vec::with_capacity(file_length as usize);
            decoder
                .read_to_end(&mut out)
        ...
        SwfCompression::Lzma => {
            ...
            let cap: usize = usize::try_from(uncompressed_size).unwrap_or(usize::MAX);
