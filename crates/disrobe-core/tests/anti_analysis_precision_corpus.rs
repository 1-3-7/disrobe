...
#[cfg(windows)]
fn configure_windows_msvc_linker(command: &mut Command) {
    const LINKER_ENV: &str = "CARGO_TARGET_X86_64_PC_WINDOWS_MSVC_LINKER";
    let Some(linker): Option<std::ffi::OsString> = std::env::var_os(LINKER_ENV) else {
        return;
    };
    let linker: PathBuf = PathBuf::from(linker);
    assert!(
        linker.is_absolute(),
        "{LINKER_ENV} must be an absolute path when configured: {}",
        linker.display()
    );
    assert!(
        linker.is_file(),
        "{LINKER_ENV} must name an existing linker when configured: {}",
        linker.display()
    );
    command.arg(format!("-Clinker={}", linker.display()));
}
...
fn compile_wine_fixture(dir: &std::path::Path, kind: WineFixtureKind, opt_level: &str) -> Vec<u8> {
    ...
    let mut command: Command = Command::new(pinned_rustc());
    configure_windows_msvc_linker(&mut command);
...
fn benign_rust_binary_yields_zero_verdicts() {
    ...
    for (label, opt) in [("debug", "0"), ("release", "2")] {
        ...
        let mut command: Command = Command::new("rustc");
        #[cfg(windows)]
        configure_windows_msvc_linker(&mut command);
        let ok: bool = command
