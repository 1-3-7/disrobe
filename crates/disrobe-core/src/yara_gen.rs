...
pub enum YaraGenError {
    ...
    #[error(
        "DR-YARAGEN-0003: cannot generate a signature from empty input; provide a nonempty sample"
    )]
    EmptyInput,
}
...
fn build_rule(bytes: &[u8], opts: &GenerateOptions) -> Rule {
    ...
    let condition: String = build_condition(selected.len());
...
fn build_condition(string_count: usize) -> String {
    let mut clauses: Vec<String> = vec!["$magic at 0".to_owned()];
    match string_count {
        0 => {}
...
pub fn generate(bytes: &[u8], opts: &GenerateOptions) -> Result<GeneratedRule> {
    ...
    if bytes.is_empty() {
        return Err(YaraGenError::EmptyInput);
    }
...
mod tests {
    ...
    fn empty_input_is_rejected() {
        assert!(matches!(
            ...
            Err(YaraGenError::EmptyInput)
        ));
    }

    #[test]
    fn unknown_format_signature_rejects_different_prefixes() {
        let bytes: [u8; 99] = [b'A'; 99];
        let generated: GeneratedRule =
            generate(&bytes, &GenerateOptions::default()).expect("generate");
        assert_eq!(generated.rule.condition, "$magic at 0");
        let ruleset: crate::yara::YaraRuleset = parse_ruleset(&generated.source).expect("parse");
        let compiled: crate::yara_match::CompiledRuleset =
            crate::yara_match::CompiledRuleset::compile(&ruleset).expect("compile");
        assert_eq!(compiled.scan(&bytes).matches.len(), 1);
        let mut different: [u8; 99] = bytes;
        different[0] = b'B';
        assert!(compiled.scan(&different).matches.is_empty());
        assert!(
            compiled
                .scan(b"unrelated nonempty input")
                .matches
                .is_empty()
        );
        assert!(compiled.scan(b"").matches.is_empty());
    }
