...
mod tests {
    ...
    #[test]
    fn plain_ioc_offsets_preserve_original_utf8_byte_positions() {
        let url: &str = "https://example.invalid/café";
        let prefixes: &[&[u8]] = &[b"\xff\xfe\0", b"\xe2\x82\0", "λ\0".as_bytes()];
        for prefix in prefixes {
            let mut bytes: Vec<u8> = prefix.to_vec();
            bytes.extend_from_slice(url.as_bytes());
            bytes.push(0xff);
            let indicators: Vec<Indicator> = extract(&bytes);
            let urls: Vec<&Indicator> = indicators
                .iter()
                .filter(|indicator: &&Indicator| {
                    indicator.kind == IocKind::Url && indicator.encoding == Encoding::Plain
                })
                .collect();
            assert_eq!(urls.len(), 1);
            assert_eq!(urls[0].value, url);
            assert_eq!(urls[0].offset, prefix.len());
            assert_eq!(
                &bytes[urls[0].offset..urls[0].offset + urls[0].value.len()],
                urls[0].value.as_bytes()
            );
        }
    }
