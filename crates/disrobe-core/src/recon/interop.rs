...
/// Identifies which disrobe artifact schema a JSON document carries, by its `schema`
/// tag first and its array shape (`findings` / `indicators` / `urls`) as a fallback.
#[must_use]
...
impl IndicatorAggregator {
    ...
    /// Ingests one JSON artifact (recon / ioc / prowl), merging its indicators into the
    /// running table with source provenance. Returns the schema it was parsed as, or
    /// `None` if the document matches no known disrobe artifact shape.
    ...
    /// Pulls the bare follow-up targets (network indicators reduced to a host or address)
    /// out of the aggregated table, the same set prowl would re-harvest. This is the
    /// reverse of `targets_from_disrobe_report`: aggregate first, then re-seed.
    #[must_use]
...
/// Aggregates a set of disrobe artifact JSON documents into one versioned indicator
/// bundle with per-indicator source provenance. Unrecognized documents are skipped.
#[must_use]
