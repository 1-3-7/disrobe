...
//!
//! Following spec §12.4 the registry is **explicit** (no `inventory`
//! link-time magic). PR-B..PR-E register concrete passes by calling
//! [`PassRegistry::register`] at boot. Until then the registry exists,
//! compiles, and is empty.
//!
//! Determinism: passes are keyed by `PassId` in a `BTreeMap`, so
//! iteration order is the lexicographic order of `pass_id` strings, which
//! makes both `run_all_and_pick` and `chain.json` rendering reproducible.
...
impl PassRegistry {
    ...
    /// Run every registered detector against `ctx` (sequentially — parallel
    /// invocation is a v2 perf optimisation that requires Rayon plumbing
    /// at the cli layer).
    #[must_use]
