//! `chain.json` serde model (spec §9, schema `disrobe.chain/v1`).
//!
//! The shape mirrors `schemas/chain.v1.json`. Every numeric is integer;
//! every byte hash is hex-encoded. The schema is **additive-only** within
//! v1 (new optional fields allowed; renames / removals bump to v2).
...
impl ChainDocument {
    /// Render a [`ChainPlan`] into the wire-format document.
    ///
    /// ```
    /// # #[cfg(feature = "chain")] {
    /// use disrobe_core::chain::{ChainDocument, ChainPlan, Node, NodeId,
    ///     Verdict, ChainSpec};
    /// let plan: ChainPlan = ChainPlan {
    ///     nodes: vec![Node {
    ///         id: 0, parent_id: None, depth: 0, branch_id: "a".into(),
    ...
    ///     }],
    ///     root_id: 0,
    ///     verdict: Verdict::Complete { format: "Python".into() },
    ///     final_format: Some("Python".into()),
    ///     total: std::time::Duration::from_millis(5),
    ///     detector_calls: 1, rejected_passes: 0, topology_is_tree: false,
    /// };
