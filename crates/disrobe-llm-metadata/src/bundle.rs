...
//!
//! Aggregates per-pass `BTreeMap<&'static str, PerPassEnvelope>` outputs into
//! the canonical `schemas/disrobe-metadata-llm-v1.json` shape and serializes
//! to JSON / JSONL / CBOR / `MessagePack` on disk.
...
/// Aggregator over per-pass `emit_metadata` outputs. Each call to
/// [`Self::record_pass`] takes one pass's envelope map (the JSON object returned
/// by `LlmMetadataEmitter::emit_metadata`) and registers a [`PipelineStep`].
...
fn build_categories(per_pass: &[Json]) -> Map<String, Json> {
    ...
    for (label, entries) in grouped {
        ...
        if label == Category::Provenance.label() {
            ...
            for (k, v) in manifest.as_object().cloned().unwrap_or_default() {
                category_obj.insert(k, v);
            }
...
/// Render & write `AGENTS.md` + `SKILL.md` into `dir`, returning their paths.
///
/// Both briefs are derived deterministically from `bundle` (a finalized
/// `disrobe.metadata.llm.v1` value). Creates `dir` as needed.
...
///
/// Useful for the CLI orchestrator that wants to build a synthetic envelope map
/// for passes that do not implement [`crate::LlmMetadataEmitter`].
#[must_use]
