...
use disrobe_llm_metadata::{
    Category, LlmMetadataEmitter, MetadataCapability, MetadataSelection, Pack, SelectionBuilder,
};
...
fn supported_category_yields_applicable_envelope() {
    let sel: MetadataSelection = SelectionBuilder::new().category(Category::Disasm).build();
...
fn unsupported_category_yields_not_applicable_with_reason() {
    ...
    assert_eq!(
        envelope.get("reason").unwrap().as_str(),
        Some("pass `disrobe-pass-py-disasm` declares no support for `ast`")
    );
    assert!(envelope.get("value").unwrap().is_null());
}

#[test]
fn supported_empty_category_yields_input_specific_reason() {
    let sel: MetadataSelection = SelectionBuilder::new().category(Category::Strings).build();
    let v: Json = FakePyDisasm.emit_metadata(&sel);
    let envelope: &Json = v.get("strings").expect("strings must be present");
    assert_eq!(envelope.get("applicable").unwrap().as_bool(), Some(false));
    assert_eq!(
        envelope.get("reason").unwrap().as_str(),
        Some(
            "pass `disrobe-pass-py-disasm` supports `strings` but produced no data for this input"
        )
    );
    assert!(envelope.get("value").unwrap().is_null());
}
...
fn pack_request_returns_envelope_per_resolved_category() {
    let sel: MetadataSelection = SelectionBuilder::new().pack(Pack::Pack1).build();
    let v: Json = FakePyDisasm.emit_metadata(&sel);
