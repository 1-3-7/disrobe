...
pub fn walk_python_sources(app_dir: &Path) -> Result<Vec<BriefcaseSourceEntry>> {
    ...
    visit(app_dir, app_dir, 0, &mut out)?;
    Ok(out)
}
...
fn visit(
    ...
    current: &Path,
    depth: usize,
    out: &mut Vec<BriefcaseSourceEntry>,
) -> Result<()> {
    if depth >= MAX_WALK_DEPTH {
        return Err(Error::BriefcaseWalkBounded {
            path: current.display().to_string(),
            reason: format!("directory nesting exceeds depth cap {MAX_WALK_DEPTH}"),
        });
    }
    ...
    for entry in read {
        if out.len() >= MAX_WALK_ENTRIES {
            return Err(Error::BriefcaseWalkBounded {
                ...
                reason: format!("source entry count exceeds cap {MAX_WALK_ENTRIES}"),
            });
        }
