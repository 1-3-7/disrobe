...
const MAX_CIRCULAR_FILES_SCANNED: usize = 16_384;
const MAX_CIRCULAR_FILE_BYTES: u64 = 1024 * 1024;
...
pub fn scan_circularity(roots: &[PathBuf]) -> CircularityReport {
    ...
    'roots: for root in roots {
            ...
            .filter_map(core::result::Result::ok)
        {
            ...
            if files_scanned >= MAX_CIRCULAR_FILES_SCANNED {
                break 'roots;
            }
            files_scanned += 1;
...
fn scan_file(root: &Path, path: &Path, findings: &mut Vec<CircularityFinding>) {
    let Some(text): Option<String> = read_text_bounded(path, MAX_CIRCULAR_FILE_BYTES) else {
        return;
    };
