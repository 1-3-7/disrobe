...
#[test]
fn dotfuscator_ce_signature_detected() {
    let mut img: Vec<u8> = synth_minimal_dotnet_pe("v4.0.30319");
    embed_signature(&mut img, b"DotfuscatorCE");
    let report: DetectionReport = detect_all(&img);
    assert!(report.matches.contains_key(&Protector::DotfuscatorCe));
}

#[test]
#[ignore = "FIXTURE PENDING: real Dotfuscator Community Edition output"]
fn dotfuscator_ce_full_round_trip() {
