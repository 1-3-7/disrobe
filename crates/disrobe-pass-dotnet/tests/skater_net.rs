...
#[test]
fn skater_published_signature_vector_detected_in_managed_pe() {
    ...
    assert!(
        report.matches.contains_key(&Protector::Skater),
        "grading the published RustemSoft.Skater watermark vector embedded in a faithful managed \
         PE, not a captured vendor sample; the signature string must be recognized"
    );
}

#[test]
fn bare_managed_carrier_is_not_flagged_as_skater() {
    let img: Vec<u8> = synth_minimal_dotnet_pe("v4.0.30319");
    let report: DetectionReport = detect_all(&img);
    assert!(
        !report.matches.contains_key(&Protector::Skater),
        "the identical synth carrier without the published watermark must not detect Skater, \
