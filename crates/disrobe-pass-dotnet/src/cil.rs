...
/// Kind of an exception-handling clause (§II.25.4.6, `CorExceptionFlag`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub enum ExceptionClauseKind {
    /// Typed catch; `class_token_or_filter` is the caught type's metadata token.
    Catch,
    /// Filtered catch; `class_token_or_filter` is the filter-block start offset.
    Filter,
    Finally,
    Fault,
}

/// One structured exception handler over a method body (§II.25.4.6).
...
/// Parse the method data sections following the IL body (§II.25.4.5). Sections are 4-byte aligned;
/// each is either fat or small EH-table format. Other section kinds are skipped via their length.
