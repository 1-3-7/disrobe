...
use crate::signature::{
    ...
    parse_field_sig_strict, parse_field_sig_validated, parse_field_sig_with_modifiers,
    parse_method_sig, parse_method_sig_strict, parse_method_sig_validated,
};
...
impl Resolver {
    ...
    pub fn validate_definition_signatures(&self) -> Result<()> {
        for row in &self.tables.methods {
            let blob: &[u8] = self
                .blob(row.signature)
                .ok_or(Error::BadCompressedUint(row.signature as usize))?;
            let _: MethodSig = parse_method_sig_validated(blob)?;
        }
        for row in &self.tables.fields {
            let blob: &[u8] = self
                .blob(row.signature)
                .ok_or(Error::BadCompressedUint(row.signature as usize))?;
            let _: TypeSig = parse_field_sig_validated(blob)?;
        }
        Ok(())
    }
...
fn assembly_public_key_token(public_key_or_token: &[u8], flags: u32) -> Option<[u8; 8]> {
    ...
    let digest: sha1::digest::Output<Sha1> = Sha1::digest(public_key_or_token);
...
mod tests {
    ...
    #[test]
    fn definition_validation_accepts_real_init_setters() {
        let resolver: Resolver =
            resolver_for("../../corpus/dotnet/megafile/EdgeCases.baseline.dll");
        let method: &MethodDefRow = resolver.tables.methods.get(0x4f - 1).expect("init setter");
        assert_eq!(resolver.string(method.name), "set_Id");
        assert_eq!(
            resolver.blob(method.signature).expect("init signature"),
            &[0x20, 0x01, 0x1f, 0x50, 0x01, 0x0a]
        );
        resolver
            .validate_definition_signatures()
            .expect("valid EdgeCases definition signatures");
    }

    #[test]
    fn definition_validation_preserves_native_defaults_and_rejects_invalid_blobs() {
        let mut resolver: Resolver = resolver_for("../../corpus/dotnet/shapes/Shapes.dll");
        resolver
            .validate_definition_signatures()
            .expect("valid Shapes signatures");
        let original: u32 = resolver.tables.methods[0].signature;
        for index in [0u32, u32::MAX] {
            resolver.tables.methods[0].signature = index;
            assert!(resolver.validate_definition_signatures().is_err());
            let model: AssemblyModel = resolver.model();
            let method: &MethodModel = model
                .types
                .iter()
                .flat_map(|ty: &TypeModel| &ty.methods)
                .find(|method: &&MethodModel| method.token == 0x0600_0001)
                .expect("first method remains in native model");
            assert_eq!(method.signature, MethodSig::default());
        }
        resolver.tables.methods[0].signature = original;
        resolver
            .validate_definition_signatures()
            .expect("restored signatures");
        resolver.tables.fields.push(FieldRow {
            flags: 0,
            name: 0,
            signature: 0,
        });
        assert!(resolver.validate_definition_signatures().is_err());
        resolver.tables.fields.pop();
        resolver
            .validate_definition_signatures()
            .expect("valid followup");
    }
