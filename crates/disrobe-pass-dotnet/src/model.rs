...
impl MethodModel {
    ...
    /// Render a C#-style method header from the decoded signature.
    #[must_use]
    ...
    /// Render a VB.NET method header (`Sub` when the return type is `void`).
    #[must_use]
...
impl Resolver {
    ...
    /// Decode the `#US` user-string at a heap byte offset (§II.24.2.4).
    #[must_use]
    ...
    /// Resolve a metadata token (`table << 24 | rid`) to a printable member/type name.
    #[must_use]
    ...
    /// Resolve a `TypeSpec` to a readable type name (e.g. `List<int>`, `Circle[]`).
    #[must_use]
    ...
    /// Replace every `type(0xNNNNNNNN)` placeholder with the resolved token name.
    #[must_use]
    ...
    /// Find the declaring type of a 1-based `MethodDef` rid (§II.22.37).
    #[must_use]
    ...
    /// Resolve a `MethodDef`/`MemberRef` token to its decoded [`MethodSig`].
    #[must_use]
...
/// Strip the CLI generic-arity suffix (`` `N ``) from a type name.
#[must_use]
