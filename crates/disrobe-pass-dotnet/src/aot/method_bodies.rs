...
enum BodyOutcome {
    Lifted(Box<LeafRecovery>),
    Refused(AotMethodBody),
}
...
fn recover_body(
...
) -> crate::error::Result<BodyOutcome> {
    ...
    if let Err(outcome) = budget.claim_input(range, length) {
        return Ok(BodyOutcome::Refused(outcome));
    }
    let bytes: &[u8] = pe
    ...
    let Some(base): Option<u64> = pe.image_base.checked_add(u64::from(range.start_rva)) else {
        return Ok(BodyOutcome::Refused(refused(
            range,
            "method virtual address overflowed",
        )));
    };
...
pub(super) fn attach_method_bodies(
    ...
    while cursor < ranges.len() {
        ...
        let recovered: AotMethodBody = match recover_body(image, pe, range, &mut budget)? {
