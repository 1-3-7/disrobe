...
struct LoopFrame {
    header: BlockId,
    exit: Option<BlockId>,
    /// In-loop continue block (e.g. a `for` increment) whose sole successor is the header.
...
impl<'a, N: TokenNamer> Structurer<'a, N> {
    ...
    /// Emit the region beginning at `start` and ending before `stop` (exclusive follow).
    ...
    /// Emit a single block (and any structure it heads) into `seq`, returning the next block.
    fn emit_block_into(
        &mut self,
        bid: BlockId,
        stop: Option<BlockId>,
    ...
    /// Resolve where control goes after a block whose only exit is `next`.
    fn flow_to(
        &mut self,
        next: BlockId,
        stop: Option<BlockId>,
    ...
    /// Emit a `continue` that runs the loop's continue block statements then loops back.
    ...
    /// Emit the body following an infinite-loop header's terminator.
    ...
    /// Whether `bid` is a pure-condition block safe to inline into a compound condition.
    ...
    /// Greedily fold short-circuit `&&`/`||` chains rooted at a conditional block.
    fn fold_condition(
        &mut self,
        bid: BlockId,
        taken: BlockId,
    ...
    /// Emit one branch of an `if`, recursing up to the join.
    fn branch_region(
        &mut self,
        target: BlockId,
        join: Option<BlockId>,
    ...
    /// Emit a `try`/handler region, returning the follow block.
    ...
    /// Emit a jump to `bid`, inlining a small terminal block when duplicable, else a labeled `goto`.
    ...
    /// Produce an inline copy of a small duplicable terminal block for a jump site, else `None`.
    ...
    /// Whether `bid`'s statements are a short sequence safe to duplicate at a jump site.
    ...
    /// Whether `bid` is a short block whose successor `next` makes it a duplicable `continue` tail.
    ...
    /// Blocks reachable forward from `start` (inclusive).
    ...
    /// Whether the loop header is a pure `while (cond)` guard.
    fn is_loop_guard(
        &self,
        lp_idx: usize,
        taken: BlockId,
    ...
    /// The loop's single continue block (an in-loop back-edge pre-block), or `None`.
    ...
    /// The loop's single exit block (lowest-offset successor outside the loop body).
...
/// Recover structured pseudo-source for a method body.
#[must_use]
...
/// Push a block's linear statements, dropping any trailing `return`/`throw`.
...
/// Parenthesize a sub-condition that already contains a boolean operator.
...
/// Rewrite a single top-level relational comparison to its negation (`a <= b` -> `a > b`).
