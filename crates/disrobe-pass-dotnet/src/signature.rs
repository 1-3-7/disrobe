...
//!
//! Decodes the compressed type-encoding grammar (§II.23.1.16 element types) used by `Field`,
//! `MethodDef`, `Property`, and `LocalVar` signatures. Produces a [`TypeSig`] tree with C#-style
//! rendering so the decompiler can print real parameter and return types. Unknown / vendor-extension
//! element types degrade to [`TypeSig::Unknown`] rather than erroring.
...
impl<'a> SigReader<'a> {
    ...
    /// Decode a `TypeDefOrRef` coded index (§II.23.2.8): a compressed integer whose low 2 bits select
    /// `{TypeDef, TypeRef, TypeSpec}` and the rest form the rid. Re-encoded here as a
    /// `0x02`/`0x01`/`0x1B` metadata token for the resolver.
