...
impl<'a> SigReader<'a> {
    ...
    const fn new(bytes: &'a [u8]) -> Self {
        Self {
            ...
            depth: 0,
            nodes: 0,
            policy: SignaturePolicy::Recovery,
        }
    }
    ...
    const fn new_strict(bytes: &'a [u8]) -> Self {
        Self {
            policy: SignaturePolicy::Evaluation,
            ..Self::new(bytes)
        }
    }

    const fn new_definition_validation(bytes: &'a [u8]) -> Self {
        Self {
            policy: SignaturePolicy::DefinitionValidation,
            ..Self::new(bytes)
        }
    }
    ...
    fn type_sig_inner(&mut self) -> Result<TypeSig> {
        ...
        while leading_modifiers {
            match self.peek() {
                Some(et::CMOD_REQD | et::CMOD_OPT) => {
                    if self.policy == SignaturePolicy::Evaluation {
                        return Err(Error::BadCompressedUint(self.reader.position()));
                    }
                    if self.policy == SignaturePolicy::DefinitionValidation {
                        self.consume_node()?;
                    }
                    let _: u8 = self.byte()?;
        ...
        Ok(match elem {
            ...
            et::ARRAY => {
                ...
                if self.policy != SignaturePolicy::Recovery && (rank == 0 || num_sizes > rank) {
                    return Err(Error::BadCompressedUint(self.reader.position()));
                }
                ...
                if self.policy != SignaturePolicy::Recovery && num_lo > rank {
                    return Err(Error::BadCompressedUint(self.reader.position()));
                }
                for _ in 0..num_lo {
                    let _ = self.compressed()?;
            ...
            et::GENERICINST => {
                ...
                if self.policy != SignaturePolicy::Recovery
                    && !matches!(base, TypeSig::NamedType { .. })
                {
                    return Err(Error::BadCompressedUint(base_position));
                }
                let argc: u32 = self.compressed()?;
                if self.policy != SignaturePolicy::Recovery && argc == 0 {
                    return Err(Error::BadCompressedUint(self.reader.position()));
                }
            ...
            _ if self.policy != SignaturePolicy::Recovery => {
...
pub(crate) fn parse_method_sig_validated(blob: &[u8]) -> Result<MethodSig> {
    parse_method_sig_with_reader(SigReader::new_definition_validation(blob))
}
...
pub(crate) fn parse_field_sig_validated(blob: &[u8]) -> Result<TypeSig> {
    parse_field_sig_with_reader(SigReader::new_definition_validation(blob))
}
...
fn parse_local_sig_with_reader(mut r: SigReader<'_>) -> Result<Vec<TypeSig>> {
    ...
    for _ in 0..count {
        ...
        if r.policy != SignaturePolicy::Recovery && !valid_strict_local_type(&local) {
            return Err(Error::BadCompressedUint(r.reader.position()));
        }
        locals.push(local);
    }
...
mod tests {
    ...
    #[test]
    fn validated_definition_accepts_init_setter_without_changing_evaluator_policy() {
        let blob: [u8; 6] = [0x20, 0x01, 0x1f, 0x50, 0x01, 0x0a];
        let signature: MethodSig = parse_method_sig_validated(&blob).expect("init setter");
        assert_eq!(signature.calling_convention, SIG_HASTHIS);
        assert!(signature.has_this);
        assert!(!signature.explicit_this);
        assert_eq!(signature.generic_param_count, 0);
        assert_eq!(signature.return_type, TypeSigOrVoid::Void);
        assert_eq!(signature.params, vec![TypeSig::I8]);
        assert_eq!(parse_method_sig(&blob).expect("native recovery"), signature);
        assert!(matches!(
            parse_method_sig_strict(&blob),
            Err(Error::BadCompressedUint(2))
        ));
    }

    #[test]
    fn validated_definition_accepts_required_optional_and_nested_modifiers() {
        for modifier in [element_type::CMOD_REQD, element_type::CMOD_OPT] {
            let field: [u8; 4] = [SIG_FIELD, modifier, 0x05, element_type::I4];
            assert_eq!(
                parse_field_sig_validated(&field).expect("modified field"),
                TypeSig::I4
            );
            assert!(parse_field_sig_strict(&field).is_err());
            let method: [u8; 5] = [SIG_DEFAULT, 0, modifier, 0x05, element_type::VOID];
            assert_eq!(
                parse_method_sig_validated(&method)
                    .expect("modified return")
                    .return_type,
                TypeSigOrVoid::Void
            );
            let nested: [u8; 8] = [
            ...
            assert_eq!(
                parse_field_sig_validated(&nested).expect("nested modifiers"),
                TypeSig::SzArray(Box::new(TypeSig::Ptr(Box::new(TypeSig::I4))))
            );
        }
    }

    #[test]
    fn validated_definition_rejects_bad_modifiers_unknown_types_and_trailing_bytes() {
        let fields: &[&[u8]] = &[
            &[0x06, 0x1f],
            &[0x06, 0x1f, 0x80],
            &[0x06, 0x1f, 0xff, 0, 0, 0],
            &[0x06, 0x1f, 0x00, 0x08],
            &[0x06, 0x1f, 0x07, 0x08],
            &[0x06, 0x1f, 0x05],
            &[0x06, 0x1f, 0x05, 0x17],
            &[0x06, 0x0f, 0x17],
            &[0x06, 0x20, 0x05, 0x08, 0x08],
        ];
        for &blob in fields {
            assert!(
                parse_field_sig_validated(blob).is_err(),
                "invalid field accepted: {blob:02x?}"
            );
        }
        let methods: &[&[u8]] = &[
            &[0x20, 0x01, 0x1f, 0x50, 0x01],
            &[0x20, 0x01, 0x1f, 0x50, 0x01, 0x0a, 0x00],
            &[0x20, 0x01, 0x1f, 0x50, 0x01, 0x17],
            &[0x20, 0x01, 0x1f, 0x00, 0x01, 0x0a],
        ];
        for &blob in methods {
            assert!(
                parse_method_sig_validated(blob).is_err(),
                "invalid method accepted: {blob:02x?}"
            );
        }
    }

    #[test]
    fn validated_definition_retains_array_and_generic_shape_checks() {
        let fields: &[&[u8]] = &[
            &[0x06, 0x14, 0x08, 0x00, 0x00, 0x00],
            &[0x06, 0x14, 0x08, 0x01, 0x02, 0x01, 0x01, 0x00],
            &[0x06, 0x14, 0x08, 0x01, 0x00, 0x02, 0x00, 0x00],
            &[0x06, 0x15, 0x08, 0x01, 0x08],
            &[0x06, 0x15, 0x12, 0x05, 0x00],
        ];
        for &blob in fields {
            assert!(
                parse_field_sig_validated(blob).is_err(),
                "invalid shape accepted: {blob:02x?}"
            );
        }
        assert_eq!(
            parse_field_sig_validated(&[0x06, 0x14, 0x08, 1, 1, 4, 1, 0])
        ...
        assert!(parse_field_sig_validated(&[0x06, 0x15, 0x12, 0x05, 1, 0x08]).is_ok());
    }

    #[test]
    fn validated_definition_charges_modifiers_to_the_node_limit() {
        let mut blob: Vec<u8> = vec![SIG_FIELD];
        for _ in 1..MAX_SIGNATURE_NODES {
            blob.extend_from_slice(&[element_type::CMOD_REQD, 0x05]);
        }
        blob.push(element_type::I4);
        assert_eq!(
            parse_field_sig_validated(&blob).expect("node ceiling"),
            TypeSig::I4
        );
        blob.pop();
        blob.extend_from_slice(&[element_type::CMOD_REQD, 0x05, element_type::I4]);
        assert!(matches!(
            parse_field_sig_validated(&blob),
            Err(Error::SignatureTooManyNodes(MAX_SIGNATURE_NODES))
        ));
    }

    #[test]
    fn validated_definition_retains_the_depth_limit() {
        let mut blob: Vec<u8> = vec![SIG_FIELD];
        blob.extend(std::iter::repeat_n(element_type::PTR, MAX_SIG_DEPTH - 1));
        blob.push(element_type::I4);
        assert!(parse_field_sig_validated(&blob).is_ok());
        blob.insert(1, element_type::PTR);
        assert!(matches!(
            parse_field_sig_validated(&blob),
            Err(Error::SignatureTooDeep(MAX_SIG_DEPTH))
        ));
