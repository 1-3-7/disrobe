...
/// Numeric table identifiers per ECMA-335 §II.22.
...
/// Coded-index schemes per ECMA-335 §II.24.2.6.
...
impl CodedIndex {
    ...
    /// Byte width of this coded index under the given row counts (§II.24.2.6).
    #[must_use]
...
/// Byte width of each heap index, from the `HeapSizes` flag byte (§II.24.2.6).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// A `MethodSpec` row (§II.22.29): a generic-method instantiation.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
/// `ManifestResource` table row (§II.22.24).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
impl Sizing {
    ...
    /// Simple index width: 4 bytes if the target table has >= 2^16 rows, else 2.
    #[inline]
    ...
    /// Byte width of one row of the given table under the current sizing.
...
/// Parse the `#~`/`#-` table stream into typed rows.
