//! ECMA-335 Partition II §22 metadata-table decoding.
//!
//! Parses the `#~` (or `#-`) compressed/uncompressed table stream into typed rows for every table
//! defined by the spec, computing per-table row widths from the heap-size flags and the
//! cross-table coded-index schemes (§II.24.2.6). All offsets are bounds-checked; a malformed row
//! count or stream length yields [`Error::Truncated`] rather than a panic.
...
/// Numeric table identifiers per ECMA-335 §II.22. Values are the bit position in the `Valid`
/// bitvector of the `#~` header.
...
/// Coded-index schemes per ECMA-335 §II.24.2.6. Each carries the set of tables it can address and
/// the number of tag bits used to discriminate them.
...
impl CodedIndex {
    ...
    /// Coded indexes are 2 bytes unless the largest addressed table needs more than
    /// `16 - tag_bits` rows to index, in which case they widen to 4 bytes (§II.24.2.6).
    #[must_use]
    ...
    /// Decode a raw coded-index value into a (table, row) pair. Row is 1-based; 0 means null.
    #[must_use]
...
/// Width (in bytes) of each heap index, derived from the `HeapSizes` flag byte (§II.24.2.6).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// A decoded reference to another metadata row (a resolved token).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct RowRef {
    pub table: TableId,
    /// 1-based row index; 0 = null reference.
    pub row: u32,
}

/// Selected, fully-decoded high-value table rows.
///
/// Not every spec table needs a struct here; the ones that carry names, signatures, or method RVAs
/// are materialized, the rest are skipped over using their computed row width (so token resolution
/// into later tables stays correct).
...
/// `ClassLayout` table row.
///
/// `TableId` 0x0F, ECMA-335 §II.22.8: explicit struct layout metadata. `class_size` is the
/// absolute byte size of the type; `parent` is a 1-based [`TableId::TypeDef`] row index.
/// `ConfuserEx2` leverages `class_size` to size the field-RVA backing store of its
/// encrypted-resource blob (`packing=1` value type with `class_size = encryptedBuffer.Length`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
/// `FieldRVA` table row.
///
/// `TableId` 0x1D, ECMA-335 §II.22.18: maps a field with `HasFieldRVA` to the RVA holding its
/// initial bytes. `ConfuserEx2` stores encrypted constants + encrypted resources as
/// `FieldRVA`-backed bytes on private nested `ValueType`-typed fields.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
///
/// `TableId` 0x28, ECMA-335 §II.22.24: the in-PE catalogue of embedded or linked resources.
/// `ConfuserEx2`'s `ResourceProtection` strips every `EmbeddedResource` and replaces it with an
/// `AssemblyLinkedResource` pointing at a synthetic stub assembly, so an honest peeling reports
/// the link-only entries that remain.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
impl<'a> Cursor<'a> {
    ...
    /// Read an index of the given byte width (2 or 4) into a u32.
    #[inline]
...
impl Sizing {
    ...
    /// Simple (single-table) index width: 4 bytes if the target table has >= 2^16 rows.
    #[inline]
    ...
    /// Compute the byte width of one row of the given table under the current sizing. Arms are kept
    /// one-per-table for direct traceability to ECMA-335 §II.22, even where widths coincide.
...
/// Parse the `#~`/`#-` table stream into typed rows. `metadata_bytes` is the full metadata blob
/// (starting at the `BSJB` signature); `header` locates the table stream within it.
