//! Basic-block control-flow graph over a CIL instruction stream, with dominance and natural-loop
//! analysis.
//!
//! Clean-room reimplementation of the dominance / natural-loop approach used by `ILSpy` (MIT). The
//! immediate-dominator solver is the iterative Cooper-Harvey-Kennedy "A Simple, Fast Dominance
//! Algorithm" (postorder dataflow + `intersect` walking postorder numbers); natural loops are
//! recovered from back-edges (an edge `t -> h` where `h` dominates `t`). No `ILSpy` source is
//! copied; the algorithms are reimplemented from understanding.
...
/// One basic block: a maximal straight-line instruction run entered only at its head and left only
/// at its tail.
#[derive(Debug, Clone)]
...
/// A recovered natural loop: a header block plus the set of blocks in its body, and the block ids
/// that are tails of its back-edges.
#[derive(Debug, Clone)]
...
impl Cfg {
    /// Build the CFG from a branch-normalized method body (operands already absolute via
    /// `normalize_branches`). EH clause boundaries are treated as block leaders so handlers begin
    /// their own blocks.
    #[must_use]
    ...
    /// Compute immediate post-dominators using Cooper-Harvey-Kennedy over the reverse CFG, where a
    /// single virtual exit collects every `Return`/`Throw`/`EndFinally` block. `ipdom[b]` is the
    /// nearest block through which all forward paths from `b` must pass to leave the method; an
    /// unreachable or exit-less block maps to [`usize::MAX`].
    ///
    /// Clean-room port of the same dominance solver applied to the transposed graph (ref `ILSpy` MIT
    /// `PrepareReverseCFG`/post-dominance), reimplemented from understanding.
    #[must_use]
...
/// Block leaders per ECMA-335: the entry, every branch/switch target, the instruction following any
/// branch/return/throw, and each EH clause's try/handler/filter boundary.
