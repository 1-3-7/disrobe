...
//!
//! Many .NET obfuscators (`ConfuserEx2` constant protection, .NET Reactor / Eazfuscator pre-VM
//! string decrypters) implement decoding as a pure CIL method over integers and `byte[]`/`char[]`:
//! XOR / ADD / ROL keyed by an embedded blob. This evaluator executes such a method body against a
//! caller-supplied input buffer to recover the plaintext, mirroring the clean-room bytecode-emulation
//! approach already used by the JVM pass - no fabricated cipher, no .NET runtime.
//!
//! Safety: a hard step limit (`STEP_LIMIT`) bounds runtime; any external call, unsupported opcode, or
//! malformed stack shape aborts with [`EmulationError`] rather than looping or panicking. Heap arrays
//! are capped (`MAX_ARRAY`, `MAX_HEAP`) to prevent allocation blow-ups on adversarial input.
...
/// The element width of a heap array, tracked so `ldelem`/`stelem` honour byte vs. 16-bit element
/// semantics for `byte[]` vs `char[]` decrypt buffers.
#[derive(Debug, Clone)]
struct HeapArray {
    bytes: Vec<u8>,
    elem_size: usize,
...
/// `newarr` element size is type-dependent; without resolving the token we default to a 1-byte
/// element (`byte[]`), the dominant decrypt-buffer shape. Callers wanting `char[]` set element size
/// via [`StubInput`]-seeded arrays instead.
...
///
/// Argument slots are filled in order: integer args first map to the method's leading parameters,
/// then byte-array args become heap
/// arrays bound to subsequent parameter slots.
