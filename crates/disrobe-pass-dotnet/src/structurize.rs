...
fn double_literal(v: f64, lang: TargetLang) -> String {
    let type_name: &str = match lang {
        TargetLang::CSharp => "double",
        TargetLang::FSharp | TargetLang::VbNet => "System.Double",
    };
    if v.is_nan() {
        format!("{type_name}.NaN")
    } else if v.is_infinite() {
        if v.is_sign_negative() {
            format!("{type_name}.NegativeInfinity")
        } else {
            format!("{type_name}.PositiveInfinity")
        }
    } else {
        match lang {
            TargetLang::CSharp => format!("{v}D"),
            TargetLang::FSharp => format!("{v:?}"),
            TargetLang::VbNet => format!("{v}R"),
        }
    }
}

fn single_literal(v: f32, lang: TargetLang) -> String {
    let type_name: &str = match lang {
        TargetLang::CSharp => "float",
        TargetLang::FSharp | TargetLang::VbNet => "System.Single",
    };
    if v.is_nan() {
        format!("{type_name}.NaN")
    } else if v.is_infinite() {
        ...
        match lang {
            TargetLang::CSharp => format!("{v}f"),
            TargetLang::FSharp => format!("{v:?}f"),
            TargetLang::VbNet => format!("{v}F"),
        }
    }
}
...
impl<'a, N: TokenNamer> Lifter<'a, N> {
    ...
    fn float_const(&self, ins: &Instruction) -> String {
        match ins.operand {
            OperandValue::F32Bits(b) => single_literal(f32::from_bits(b), self.lang),
            OperandValue::F64Bits(b) => double_literal(f64::from_bits(b), self.lang),
            _ => "0".to_owned(),
        }
    }
...
mod tests {
    ...
    fn fsharp_lifts_addition_to_return() {
        ...
        assert_eq!(out.recovered_branches, 0);
    ...
    #[test]
    fn double_literals_preserve_target_type_and_negative_zero() {
        let cases: [(u64, &str, &str); 6] = [
            (0x3ff4_0000_0000_0000, "1.25", "1.25R"),
            (0x8000_0000_0000_0000, "-0.0", "-0R"),
            (0x4008_0000_0000_0000, "3.0", "3R"),
            (
        ...
        for (bits, fsharp, vbnet) in cases {
            let mut bytes: Vec<u8> = vec![0x23];
            bytes.extend_from_slice(&bits.to_le_bytes());
            bytes.push(0x2a);
            let body: MethodBody = body_from(&bytes);
            for (lang, signature, expected) in [
                (TargetLang::FSharp, "member M() : float", fsharp),
                (TargetLang::VbNet, "Public Function M() As Double", vbnet),
            ] {
                let output: StructuredMethod =
                    decompile_method_in(signature, &body, &HexNamer, lang);
                let expression: String = if lang == TargetLang::VbNet {
                    format!("Return {expected}")
                } else {
                    expected.to_owned()
                };
                assert!(
                    output.body.lines().any(|line| line.trim() == expression),
                    "{lang:?}: {}",
                    output.body
                );
            }
    ...
    #[test]
    fn single_literals_preserve_target_type_and_negative_zero() {
        let cases: [(u32, &str, &str); 5] = [
            (0x4040_0000, "3.0f", "3F"),
            (0x8000_0000, "-0.0f", "-0F"),
            (0x7fc0_0000, "System.Single.NaN", "System.Single.NaN"),
            (
                0x7f80_0000,
        ...
        for (bits, fsharp, vbnet) in cases {
            ...
            let body: MethodBody = body_from(&bytes);
            for (lang, signature, expected) in [
                (TargetLang::FSharp, "member M() : float32", fsharp),
                (TargetLang::VbNet, "Public Function M() As Single", vbnet),
            ] {
                let output: StructuredMethod =
                    decompile_method_in(signature, &body, &HexNamer, lang);
                let expression: String = if lang == TargetLang::VbNet {
