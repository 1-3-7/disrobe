...
/// Resolves a metadata token to a printable name and, when available, a [`CallInfo`].
pub trait TokenNamer {
    fn name(&self, token: u32) -> String;

    /// Resolve the callee's call shape, or `None` for shape-only rendering.
    fn call_info(&self, _token: u32) -> Option<CallInfo> {
        None
    }

    /// Whether the method being lifted has an implicit `this` (affects `ldarg.0`).
...
/// Pairs a [`Resolver`] with the `has_this` flag of the method being decompiled.
...
impl<'a, N: TokenNamer> Lifter<'a, N> {
    ...
    /// Pop two operands and render `a op b` as a comparison-branch condition.
...
/// A linear, label-free statement: the structured-emitter form of a lifted instruction.
#[derive(Debug, Clone)]
...
/// One basic block lifted to linear statements plus its terminator condition/selector.
...
/// Lift one basic block's instruction range `[first, last]` to linear statements.
...
/// Pre-resolve every branch operand to be relative to the instruction's own start.
...
/// Emit the closing token per language.
