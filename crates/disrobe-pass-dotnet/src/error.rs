...
pub enum Error {
    ...
    #[error("DR-DOTNET-0026: signature nesting exceeds maximum depth {0}")]
    SignatureTooDeep(usize),

    #[error("DR-DOTNET-0027: external backend '{tool}' {stream} capture failed: {message}")]
    BackendCapture {
        tool: &'static str,
        stream: &'static str,
        message: String,
