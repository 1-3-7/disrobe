...
//!
//! The C# compiler caches a lambda in a static `<>9__N_M` field guarded by a null check:
//! `if (!(<>9__N_M)) { <>9__N_M = new D(<>9, <Method>b__N_M); }`. This adds noise to every method
//! that passes a lambda. This pass detects the guard, maps each cached-delegate field to the bare
//! lambda method name, strips the guards, and rewrites references to the lambda.
//!
//! Clean-room: the recognized shape is the documented Roslyn caching idiom; the matching logic is
//! original line-level pattern matching over disrobe's rendered output. No decompiler source copied.

use std::collections::BTreeMap;
use std::fmt::Write as _;

/// Fold cached-delegate boilerplate in a rendered C# method body. Returns the cleaned body and the
/// number of cached delegates folded.
#[must_use]
...
/// Scan for cached-delegate guards, returning a `field -> lambda-name` map and the set of line
/// indices that make up the guard blocks (to be removed).
...
/// Extract the lambda method name from `<>9__N_M = new <D>(<>9, <Method>b__N_M);`. Returns the
/// `<Method>b__N_M` token.
