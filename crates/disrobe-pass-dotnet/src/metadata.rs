...
impl RuntimeLabel {
    ...
    pub fn classify(version: &str) -> Self {
        ...
        if v.starts_with("1.") {
            Self::NetFramework1
        } else if v.starts_with("2.0") || v.starts_with("2.1") || v.starts_with("3.") {
            Self::NetFramework2
            ...
            Self::Net10OrLater
        } else if v.contains("3.0") || v.contains("3.1") {
            Self::NetCore3
        } else {
            Self::Unknown
        }
...
pub fn parse_metadata_root(image: &[u8], pe: &PeImage, clr: &ClrHeader) -> Result<MetadataRoot> {
    let slice: &[u8] = pe.slice_at_rva(image, clr.metadata.rva, clr.metadata.size as usize)?;
...
mod tests {
    ...
    fn signature_matches_ecma_335_bsjb() {
        ...
        assert_eq!(&METADATA_SIGNATURE.to_be_bytes(), b"BJSB");
    }
