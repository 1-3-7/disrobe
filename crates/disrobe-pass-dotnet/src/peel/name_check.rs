//! Heuristic classifier for "obviously renamed" vs "plausibly human" identifiers.
//!
//! Clean-room port of de4dot's `RandomNameChecker` (de4dot/de4dot.code/deobfuscators/RandomNameChecker.cs).
//! Used across every .NET renamer (Reactor / Eazfuscator / Babel / SmartAssembly / Spices / Skater /
//! CryptoObfuscator / Confuser / Obfuscar) to decide whether a metadata name string came from an
//! obfuscator or from the original developer.
...
/// Detect Confuser-family unprintable / surrogate-pair / `_<num>` style names.
#[must_use]
...
/// Detect the SmartAssembly literal `#=q...` / `#=z...` namespace tag.
#[must_use]
