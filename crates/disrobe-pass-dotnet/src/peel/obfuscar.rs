...
//!
//! Algorithm summary (clean-room from obfuscar/obfuscar `src/Obfuscar/NameMaker.cs`):
//! * Renamer - the marquee (and, in default config, only) transform. Types/methods/fields/
//!   properties/events are renamed by `NameMaker.UniqueName(index)`, a base-N odometer over the
//!   default alphabet `AaBbCcDd…Zz` (52 alternating-case ASCII letters). Index 0 -> "A",
//!   1 -> "a", 2 -> "B", …, 51 -> "z", 52 -> "aA", 53 -> "aa", … Renaming starts at index 0 in
//!   each freshly-allocated `NameGroup`, so the output is a dense, contiguous low-index cluster.
//! * Watermark - NONE. Obfuscar embeds no `ObfuscarAttribute` nor any literal `Obfuscar` marker in
//!   default output; the legacy `b"Obfuscar.Obfuscator"` signature is absent from real samples
//!   (verified: `grep -c Obfuscar` == 0 on `HelloAppLegacy.obfuscar.dll` /
//!   `EdgeCases.obfuscar.dll`). Detection therefore keys on the deterministic odometer naming.
//! * String encryption - optional, off by default; never present on the default-config fixtures.
//! * No control-flow flattening, no VM, no native stub. Obfuscar is pure managed metadata rewrite.
//!
//! Recovery scope - Obfuscar does not embed a name-mapping table in the assembly (the optional
//! `Mapping.txt` is written to disk beside the build, not into the PE), so original identifiers are
//! NOT statically recoverable from the bytes alone. The honest peel is: detect, classify the
//! renamed slots, and report. No fabricated original names.
...
/// Highest odometer index counted as a member. 20k spans every name an obfuscated assembly of
/// realistic size allocates (52^3 == 140_608 four-char names dwarfs reality), while excluding long
/// human identifiers whose base-52 decode lands far above any from-zero renamer's output.
const ODOMETER_CEILING: u32 = 20_000;

/// Minimum contiguous odometer run that, on its own, proves Obfuscar renaming. A from-zero
/// sequential renamer trivially clears this on any non-degenerate assembly; human code and other
/// obfuscators (Confuser `_NNN`/unicode, SmartAssembly `#=q…`) never produce 4+ consecutive
/// base-52 odometer indices.
const STRONG_RUN: usize = 4;

/// Minimum contiguous run for the small-assembly path, paired with the density + tight-window
/// guards below so isolated coincidental matches (a lone human `Cat`/`Dog`) cannot trip it.
const WEAK_RUN: usize = 2;

const RADIX: u32 = OBFUSCAR_ALPHABET.len() as u32;

/// Clean-room port of `NameMaker.UniqueName(int index)`: base-52 conversion over
/// [`OBFUSCAR_ALPHABET`], most-significant digit first.
#[must_use]
...
/// Closed-form inverse of [`unique_name`].
///
/// Decodes a candidate identifier back to its odometer index, or `None` if it is not a pure
/// [`OBFUSCAR_ALPHABET`] string or decodes past [`ODOMETER_CEILING`]. Single char maps to its
/// alphabet position; multi-char folds MSB-first as `acc = acc * RADIX + digit` (the exact
/// arithmetic inverse of the `UniqueName` stack push).
#[must_use]
...
///
/// Discriminator (validated against the real fixtures + clean/Confuser controls): the names form a
/// dense, contiguous, low-index run of the base-52 odometer. We require either a long contiguous
/// run (`STRONG_RUN`) or a short run backed by high low-index density (`WEAK_RUN` + the window
/// guard), which separates `HelloAppLegacy.obfuscar` (run 2, density 1.0) and
/// `EdgeCases.obfuscar` (run 23, density 0.53) from every non-Obfuscar control (run <= 1).
#[must_use]
...
/// Heap-aware Obfuscar detection. Parses the #Strings heap and runs the odometer discriminator.
/// Fails safe to `false` on any parse error (callers still have the literal-signature path).
#[must_use]
...
/// Honest report-only peel: classify the renamed slots and report. Obfuscar embeds no in-PE name
/// map, so original identifiers are not statically recoverable; no fabricated recovery.
