...
/// A single recovered constant string, with the call-site id it was loaded by and the byte offset
/// into the decompressed pool it was decoded from.
...
/// Locate, extract, and decrypt the ConfuserEx2 constants pool in `image`, then recover every
/// string reachable from a constant call-site id. Returns `Ok(None)` for a well-formed managed PE
/// that carries no constants-protection blob.
...
/// Decrypt a ConfuserEx2 constants blob under `seed`: per 16-`uint32` block, `pt[l] = enc[l] ^
/// key[l]` emitted little-endian, then chaining `key[l] ^= pt[l]` for the next block. Returns
/// `None` when the blob length is not a multiple of [`CONSTANTS_BLOCK_BYTES`].
#[must_use]
...
/// Apply a ConfuserEx2 decoder's id mutation: `id = (id * mul) ^ xor`, then return
/// `(tag, byte_offset)` where `tag = id >>> 30` and `byte_offset = (id & 0x3FFF_FFFF) << 2`.
#[must_use]
...
/// (`[4 bytes count LE][count bytes UTF-8]`). Returns `None` on out-of-bounds or invalid UTF-8.
#[must_use]
