//! ConfuserEx2 "Ki.Resources" protection - clean-room port of the documented FOSS algorithm.
//!
//! Algorithm summary (clean-room from github.com/mkaring/ConfuserEx
//! `Confuser.Protections/Resources/{InjectPhase,MDPhase,NormalMode}.cs` and
//! `Confuser.Runtime/Resource.cs`).
//!
//! Pipeline at build time:
//! 1. Every [`EmbeddedResource`] is removed from the protected module and re-packed into a new
//!    synthetic dnlib module (random ASCII name).
//! 2. That synthetic module is serialized to bytes and LZMA-compressed
//!    (`[5 bytes LZMA1 properties][4 bytes uncompressed length LE][raw LZMA1 stream]`).
//! 3. The compressed bytes are zero-padded out to a `uint32` count divisible by `0x10`, treated as
//!    a `uint32[]`, then encrypted with [`NormalMode`]: per 16-uint32 block,
//!    `enc[j] = pt[j] ^ key[j]; key[j] ^= pt[j]`.
//! 4. The encrypted bytes are stored as a `HasFieldRVA` field on the global `<Module>` type whose
//!    field-type is a private nested `ValueType` with [`ClassLayout`] `packing=1, class_size =
//!    encryptedBuffer.Length`.
//! 5. A static `Initialize` method is injected on `<Module>` and prepended to `.cctor`; it embeds
//!    the per-build `keySeed` and `size/4` as two `ldc.i4` placeholders patched by
//!    [`MutationHelper::InjectKeys`].
//!
//! Key schedule (xorshift32 seeded by `keySeed`, runtime equivalent in `Confuser.Runtime/Resource.cs`):
//!
//! ```text
...
//! ```
//!
//! Real-fixture caveat: when ConfuserEx2 is invoked with the documented "normal" preset (i.e.
//! Resources + Constants + ControlFlow + Rename, which is the default and what
//! [`corpus/dotnet/HelloAppLegacy.confuserex2.dll`] uses), the `keySeed` `ldc.i4` lives inside an
//! injected `Initialize` method that the Constants protection's `IConstantService::ExcludeMethod`
//! call is supposed to spare - but in practice the Constants encoder rewrites it anyway because it
//! runs as a writer-event hook ordered later than the exclude registration. Recovering the seed
//! against the layered "normal" preset therefore requires emulating the Constants bootstrap, which
//! is a strictly larger effort; see [`peel_confuserex_resources`] for the honest tiered result we
//! emit instead (byte-exact blob extraction + best-effort seed bruteforce against the LZMA oracle).
//!
//! Cipher correctness is proven by the synthetic round-trip in this module's tests.
...
/// LZMA1 properties byte that ConfuserEx2 always writes for its resource compressor:
/// `(pb * 5 + lp) * 9 + lc = (2 * 5 + 0) * 9 + 3 = 0x5D` (see `Confuser.Core/Services/CompressionService.cs::Compress`).
pub const CONFUSEREX_LZMA_PROPS: u8 = 0x5D;

/// LZMA1 dictionary size that ConfuserEx2 writes - `1 << 23`, little-endian.
pub const CONFUSEREX_LZMA_DICT: u32 = 1 << 23;

/// ConfuserEx2 encrypts in blocks of 16 `uint32` (= 64 bytes).
pub const CONFUSEREX_BLOCK_BYTES: usize = 64;

/// Hard cap on the LZMA uncompressed size we are willing to allocate for a single decrypted
/// resource blob. ConfuserEx2 never embeds more than the protected assembly's own resources
/// (post-compression), so 64 MiB is a generous DoS-safe ceiling.
pub const MAX_DECRYPTED_RESOURCE_BYTES: usize = 64 * 1024 * 1024;

/// Maximum total encrypted-blob bytes we will scan for; protects against malformed input claiming a
/// `class_size` larger than the PE itself.
pub const MAX_ENCRYPTED_BLOB_BYTES: usize = 64 * 1024 * 1024;

/// Cap on distinct `ldc.i4` candidate seeds we will attempt against the LZMA oracle. Bounds the
/// brute search by the actual number of immediates ever emitted in the method bodies, which is in
/// the thousands for the largest known sample (`EdgeCases.confuserex2.dll`).
pub const MAX_SEED_CANDIDATES: usize = 65_536;

/// Outcome of attempting to recover a ConfuserEx2-protected resource blob.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum ConfuserExRecovery {
    /// The encrypted blob was located byte-exactly and the per-build `keySeed` was recoverable
    /// from a clean `ldc.i4` immediate in some method body, producing a fully-decrypted LZMA
    /// stream whose properties matched the documented ConfuserEx2 encoder.
    FullyDecrypted {
        blob_rva: u32,
        blob_size: u32,
        blob_sha256: [u8; 32],
    ...
    /// The encrypted blob was located and extracted byte-exactly, but the per-build `keySeed`
    /// could not be recovered from any `ldc.i4` immediate in the assembly. This is the expected
    /// outcome when Confuser's Constants protection nests over Resources (the "normal" preset);
    /// it confirms the blob is real and is the work-product we hand off, while documenting the
    /// remaining keyed wall.
    ...
    /// No `(FieldRVA, packing=1 ClassLayout)` pair was found in shape - the protection is absent
    /// or the metadata is malformed.
    NoEncryptedResourceFound,
}

impl ConfuserExRecovery {
    /// Was the encrypted blob actually located in the PE? (True for both [`Self::FullyDecrypted`]
    /// and [`Self::BlobExtractedKeyedWall`].)
    #[must_use]
    ...
    /// Returns the SHA-256 of the encrypted blob if it was located.
    #[must_use]
...
/// Top-level entry: locate, extract, and (best-effort) decrypt the ConfuserEx2-encrypted resource
/// blob in `image`. Returns a [`ConfuserExRecovery`] describing exactly what we accomplished. On a
/// well-formed managed PE without ConfuserEx2 resource protection the result is
/// [`ConfuserExRecovery::NoEncryptedResourceFound`].
pub fn peel_confuserex_resources(image: &[u8]) -> Result<ConfuserExRecovery> {
    ...
    let seed_pool: Vec<u32> = collect_ldc_i4_immediates(image, &pe, &tables.methods);
    let mut tried: u32 = 0;
...
/// LZMA-decompress a ConfuserEx2-decrypted resource buffer. Returns the inner ManifestModule bytes
/// (a complete managed assembly whose `<Module>` re-hosts the original embedded resources).
///
/// Format on the wire: `[1 byte LZMA1 properties][4 bytes LE dictionary size][4 bytes LE uncompressed
/// size][raw LZMA1 stream]`. lzma-rs takes the standard 13-byte header (1 byte props + 4 bytes dict
/// + 8 bytes uncompressed size); ConfuserEx writes a 4-byte size, so we widen it to 8 bytes by
/// zero-extending the upper half (uncompressed sizes never exceed `u32::MAX` here).
...
/// Encrypt a clear-text uint32 buffer with ConfuserEx2's [`NormalMode`] under the given `key_seed`.
/// Block size is fixed at 16 uint32 (= 64 bytes); the caller is responsible for zero-padding the
/// underlying byte buffer to a multiple of 64 bytes before passing it in.
///
/// Used by the synthetic round-trip test; matches `Confuser.Protections/Resources/NormalMode.cs`
/// (`Encrypt`) + `Confuser.Protections/Resources/MDPhase.cs::OnWriterEvent` chaining loop.
#[must_use]
...
/// Decrypt a ConfuserEx2 NormalMode resource blob under the given `key_seed`. Returns `None` when
/// the blob's length is not a multiple of [`CONFUSEREX_BLOCK_BYTES`]; that constraint is enforced
/// by ConfuserEx itself via `compressedLen = (compressedLen + 0xfu) & ~0xfu`.
#[must_use]
...
/// Derive the per-block key schedule from `key_seed` using ConfuserEx2's runtime xorshift32
/// (`Confuser.Runtime/Resource.cs`). The schedule is the same in encrypt and decrypt; the chaining
/// step (key[j] ^= plaintext[j]) is applied by the block loop, not here.
#[must_use]
...
/// LZMA1 oracle: the decrypted bytes must begin with the exact ConfuserEx2 header
/// (`[0x5D][0x00 0x00 0x80 0x00][uncompressed size LE u32]`) followed by at least one byte of LZMA
/// stream, and the embedded uncompressed size must be in DoS-safe bounds.
#[must_use]
...
/// Walk every `ManifestResource` row in the metadata, returning the resolved
/// `(name_offset_in_strings, has_external_link)` pairs. ConfuserEx2 strips all `EmbeddedResource`
/// rows in favour of `AssemblyLinkedResource` links to a synthetic stub assembly; reporting that
/// transform is part of the honest peel.
#[must_use]
...
/// Summary of `ManifestResource` table rows used by the honest peel report.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct ManifestResourceClassification {
    pub total: u32,
    /// Rows with `implementation == null` - i.e. true embedded resources in this PE.
    pub embedded: u32,
    /// Rows pointing at an [`crate::tables::TableId::AssemblyRef`] (or `File`/`ExportedType`) - the
    /// signature of Confuser's resource protection re-link.
    pub linked: u32,
}
...
/// Identify the ConfuserEx2 encrypted-resource blob via its shape:
/// 1. A `ClassLayout` row with `packing_size == 1` (the resources protection always writes
///    `ClassLayoutUser(1, 0)` and patches `class_size` at write-time; the Constants protection
///    uses `packing_size == 0`).
/// 2. `class_size > 0` and a multiple of [`CONFUSEREX_BLOCK_BYTES`] (= 64).
/// 3. A `FieldRVA` row whose `field` index belongs to a field whose declared type is that
///    `ClassLayout`'s parent TypeDef.
///
/// In the current Tables decoding we don't carry per-field type signatures, so the closest
/// shape-check we run is: the `FieldRVA.rva` actually maps to a readable PE region of exactly
/// `class_size` bytes. The combination of `packing=1`, `class_size % 64 == 0`, and that the same
/// `class_size` reappears as a contiguous PE region matches the ConfuserEx2 invariant uniquely.
