...
/// LZMA1 properties byte ConfuserEx2 always writes for its resource compressor (`0x5D`).
...
/// Hard cap on the LZMA uncompressed size for a single decrypted resource blob.
pub const MAX_DECRYPTED_RESOURCE_BYTES: usize = 64 * 1024 * 1024;

/// Maximum total encrypted-blob bytes we will scan for.
pub const MAX_ENCRYPTED_BLOB_BYTES: usize = 64 * 1024 * 1024;

/// Cap on distinct `ldc.i4` candidate seeds attempted against the LZMA oracle.
...
pub enum ConfuserExRecovery {
    /// Blob located and `keySeed` recovered, yielding a fully-decrypted LZMA stream.
    FullyDecrypted {
        blob_rva: u32,
        blob_size: u32,
        blob_sha256: [u8; 32],
    ...
    /// Blob extracted byte-exactly, but `keySeed` not recoverable from any `ldc.i4` immediate.
    ...
    /// No `(FieldRVA, packing=1 ClassLayout)` pair found; protection absent or metadata malformed.
    NoEncryptedResourceFound,
}

impl ConfuserExRecovery {
    /// Whether the encrypted blob was located in the PE.
    #[must_use]
...
/// Locate, extract, and best-effort decrypt the ConfuserEx2-encrypted resource blob in `image`.
...
/// LZMA-decompress a ConfuserEx2-decrypted resource buffer to the inner ManifestModule bytes.
...
/// Encrypt a clear-text uint32 buffer with ConfuserEx2's NormalMode under `key_seed`.
#[must_use]
...
/// Decrypt a ConfuserEx2 NormalMode resource blob under `key_seed`.
#[must_use]
...
/// Derive the per-block key schedule from `key_seed` via ConfuserEx2's xorshift32.
#[must_use]
...
/// LZMA1 oracle: whether the decrypted bytes begin with the exact ConfuserEx2 header.
#[must_use]
...
/// Classify every `ManifestResource` row as embedded or externally linked.
#[must_use]
...
/// Summary of `ManifestResource` table rows used by the peel report.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct ManifestResourceClassification {
    pub total: u32,
    /// Rows with `implementation == null` (true embedded resources).
    pub embedded: u32,
    /// Rows pointing at an external implementation (Confuser's resource re-link).
    pub linked: u32,
}
...
/// Identify the ConfuserEx2 encrypted-resource blob via its `packing=1` `ClassLayout` + `FieldRVA` shape.
