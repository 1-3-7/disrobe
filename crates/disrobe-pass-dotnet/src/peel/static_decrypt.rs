//! Self-contained static-decrypter recovery via CIL emulation.
//!
//! Locates `MethodDef`s whose bodies are pure integer/array transforms (no external calls, no field
//! or token loads) - the canonical shape of constant/string decoders emitted by ConfuserEx2 and
//! pre-VM Reactor/Eazfuscator tiers - and emulates them with [`crate::cil_emulator`] to recover the
//! decoded output. This is real reversal by executing the protector's own code, not a re-implemented
//! cipher. Methods that touch external state are skipped (they need full resource + call resolution).
...
/// One recovered constant from a successfully-emulated pure decrypter method.
...
/// Summary of static-decrypter recovery over an assembly.
...
/// Whether a method body is a pure self-contained transform safe to emulate: it must contain no
/// external-state opcodes (calls, field access, string/token loads, object creation).
#[must_use]
...
/// Scan an image for pure static decoders and emulate each over a small range of integer inputs,
/// recovering any constants/strings they produce. Bounded by `MAX_PROBE_ARGS` probes per method.
...
/// Count the `ldstr` call sites in a method body that pass through a decoder (a rough metric of how
/// many obfuscated strings a recovered decoder would resolve). Used for reporting only.
#[must_use]
