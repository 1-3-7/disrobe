...
pub struct PeelReport {
    ...
    pub bytes_in: u32,
    pub bytes_out: u32,
    /// Pure static decoder methods recovered by CIL emulation.
...
pub enum PeelStrategy {
    ...
    /// `ConfuserEx2` encrypted-resource blob extracted byte-exactly; key recovered or keyed-wall.
...
/// Parse + materialize the strings/us heaps.
...
/// Universal report-only peel: identify watermark attributes, classify names, emit a [`PeelReport`].
...
/// Encrypted-resource family peel: locate suspicious resources, report findings, leave bytes intact.
...
/// Error out when the bytes do not parse as a managed PE.
...
/// Dispatch helper: peel by protector enum value, or `None` if unregistered.
...
/// `ConfuserEx`/`ConfuserEx2` peel: run the Resources-protection extractor into a [`PeelReport`].
