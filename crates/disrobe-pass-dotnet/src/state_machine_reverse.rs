...
/// Rewrite a rendered `MoveNext` body, undoing the state-machine lowering.
#[must_use]
...
/// Rename hoisted-local, captured-this, and current backing fields to source-like names.
...
/// Replace parameter-backing fields `this.<>N__name` -> `name`.
...
/// Parse a `N__name` parameter-field suffix, returning the bare `name` and bytes consumed.
...
/// Replace `this.<ident>5__N` occurrences with `ident`.
...
/// Parse a `this.<ident>5__N` prefix, returning the bare `ident` and bytes consumed.
...
/// Fold iterator yield idioms into `yield return`/`yield break`.
...
/// Match `<current> = X;` then `state = N;` then `return true|1;`.
...
/// Fold async await idioms, rewriting `GetAwaiter()`/`GetResult()` lines into `await` annotations.
...
/// Match `X = <expr>.GetAwaiter();`, capturing target `X` and `<expr>`.
...
/// Rewrite `(&this.<>t__builder).SetResult(X);` -> `return X;`.
...
/// Remove the state-machine resume plumbing (state stores, awaiter fields, builder bookkeeping).
