//! Structural-recovery metric for the in-house CIL decompiler against `EdgeCases.baseline.dll`.
...
fn main() -> std::io::Result<()> {
    ...
    let mut recompile_ready: usize = 0;
    ...
    println!(
        "recompile_ready(structural)={recompile_ready}/{total} ({:.1}%) [precondition only, NOT fidelity]",
        pct(recompile_ready, total)
    );
    println!(
...
/// Structural recompile-readiness precondition (no residual goto/underflow/unresolved tokens).
fn is_recompile_ready(body: &str) -> bool {
    !residual_goto(body)
        && !body.contains("__stack_underflow")
        && !body.contains("type(0x")
        && !body.contains("TypeSpec[")
        && !body.contains("MethodSpec[")
        && !body.contains("__token")
}
