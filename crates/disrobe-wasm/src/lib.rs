...
fn pack_result_with_cap(payload: &[u8], cap: usize) -> *mut u8 {
    pack_prefixed_result(payload, &[], cap)
}

fn pack_prefixed_result(payload: &[u8], prefix: &[u8], cap: usize) -> *mut u8 {
    let Some(clamped): Option<usize> = payload.len().checked_add(prefix.len()) else {
        return pack_error("result buffer length overflow");
    };
    if clamped > cap {
        return pack_error("result exceeds wasm bridge output cap");
    }
    let Ok(payload_len): Result<u32, _> = u32::try_from(clamped) else {
...
fn pack_json(value: &impl Serialize) -> *mut u8 {
    pack_json_with_cap(value, MAX_RESULT_PAYLOAD)
}

fn pack_json_with_cap(value: &impl Serialize, cap: usize) -> *mut u8 {
    serde_json::to_vec(value).map_or_else(
        |err: serde_json::Error| pack_error(&format!("serialize result: {err}")),
        |bytes: Vec<u8>| pack_result_with_cap(&bytes, cap),
    )
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn pyarmor_unpack(
    ptr: *const u8,
    len: usize,
    runtime_ptr: *const u8,
    runtime_len: usize,
) -> *mut u8 {
    if len > entry::pyarmor::MAX_WRAPPER_BYTES || runtime_len > entry::pyarmor::MAX_RUNTIME_BYTES {
        return pack_error("PyArmor supports wrappers up to 1 MiB and runtimes up to 16 MiB");
    }
    let wrapper: &[u8] = match unsafe { input_slice(ptr, len) } {
        Ok(bytes) => bytes,
        Err(message) => return pack_error(message),
    };
    let runtime: &[u8] = match unsafe { input_slice(runtime_ptr, runtime_len) } {
        Ok(bytes) => bytes,
        Err(message) => return pack_error(message),
    };
    let output: entry::pyarmor::ModuleOutput = match entry::pyarmor::unpack(wrapper, runtime) {
        Ok(output) => output,
        Err(message) => return pack_error(&message),
    };
    let Ok(metadata_len): Result<u32, _> = u32::try_from(output.metadata.len()) else {
        return pack_error("PyArmor metadata length exceeds its frame header");
    };
    let mut prefix: Vec<u8> = Vec::with_capacity(5 + output.metadata.len());
    prefix.push(0);
    prefix.extend_from_slice(&metadata_len.to_le_bytes());
    prefix.extend_from_slice(&output.metadata);
    pack_prefixed_result(
        &output.bytes,
        &prefix,
        entry::pyarmor::MAX_WRAPPER_BYTES + entry::pyarmor::MAX_METADATA_BYTES + 5,
    )
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn hermes_analyze(ptr: *const u8, len: usize) -> *mut u8 {
    unsafe { dispatch(ptr, len, entry::hermes_analyze) }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn apk_analyze(ptr: *const u8, len: usize) -> *mut u8 {
    unsafe { dispatch(ptr, len, entry::apk_analyze) }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn dart_kernel(ptr: *const u8, len: usize) -> *mut u8 {
    unsafe { dispatch(ptr, len, entry::dart_kernel) }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn phar_list(ptr: *const u8, len: usize) -> *mut u8 {
    unsafe { dispatch(ptr, len, entry::phar_list) }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_metadata(ptr: *const u8, len: usize) -> *mut u8 {
    let bytes: &[u8] = match unsafe { input_slice(ptr, len) } {
        Ok(bytes) => bytes,
        Err(message) => return pack_error(message),
    };
    entry::go_metadata(bytes).map_or_else(
        |message: String| pack_error(&message),
        |result: entry::GoResult| pack_json_with_cap(&result, 32 * 1024 * 1024),
    )
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn js_unbundle(ptr: *const u8, len: usize) -> *mut u8 {
    let bytes: &[u8] = match unsafe { input_slice(ptr, len) } {
        Ok(bytes) => bytes,
        Err(message) => return pack_error(message),
    };
    entry::js_unbundle(bytes).map_or_else(
        |message: String| pack_error(&message),
        |result: entry::BundleResult| pack_json_with_cap(&result, 8 * 1024 * 1024),
    )
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn jvm_class(ptr: *const u8, len: usize) -> *mut u8 {
    let bytes: &[u8] = match unsafe { input_slice(ptr, len) } {
        Ok(bytes) => bytes,
        Err(message) => return pack_error(message),
    };
    entry::jvm_class(bytes).map_or_else(
        |error: String| pack_error(&error),
        |result: entry::JvmClassResult| pack_json_with_cap(&result, 8 * 1024 * 1024),
    )
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn dotnet_analyze(ptr: *const u8, len: usize) -> *mut u8 {
    let bytes: &[u8] = match unsafe { input_slice(ptr, len) } {
        Ok(bytes) => bytes,
        Err(message) => return pack_error(message),
    };
    entry::dotnet_analyze(bytes).map_or_else(
        |error: String| pack_error(&error),
        |result: entry::DotnetResult| pack_json_with_cap(&result, 8 * 1024 * 1024),
    )
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn source_map_recover(ptr: *const u8, len: usize) -> *mut u8 {
    let bytes: &[u8] = match unsafe { input_slice(ptr, len) } {
        Ok(bytes) => bytes,
        Err(message) => return pack_error(message),
    };
    entry::source_map_recover(bytes).map_or_else(
        |message: String| pack_error(&message),
        |result: entry::SourceMapResult| pack_json_with_cap(&result, 8 * 1024 * 1024),
    )
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn phar_extract(ptr: *const u8, len: usize, index: u32) -> *mut u8 {
    let bytes: &[u8] = match unsafe { input_slice(ptr, len) } {
        Ok(bytes) => bytes,
        Err(message) => return pack_error(message),
    };
    entry::phar_extract(bytes, index).map_or_else(
        |message: String| pack_error(&message),
        |output: Vec<u8>| pack_prefixed_result(&output, &[0], MAX_RESULT_PAYLOAD),
    )
}
