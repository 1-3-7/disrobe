mod source_map;
pub use source_map::{SourceMapResult, source_map_recover};
mod unbundle;
pub use unbundle::{BundleResult, js_unbundle};
mod jvm;
pub use jvm::{JvmClassResult, analyze as jvm_class};
mod dotnet;
pub use dotnet::{DotnetResult, analyze as dotnet_analyze};
...
use disrobe_pass_mobile::{
    DecompileReport as HermesDecompileReport, DetectedKind as MobileKind, HermesHeader,
    HermesModule, HermesStringKind, decompile_hermes_module, detect_kind as mobile_detect_kind,
    extract_android_bundle_children, extract_android_dex_children, hermes_disasm_function,
    parse_hermes_module,
};
...
mod apk;
mod go;
pub use apk::{ApkResult, analyze as apk_analyze};
pub use go::{GoResult, analyze as go_metadata};
mod phar;
pub use phar::{PharResult, extract as phar_extract, list as phar_list};
pub mod pyarmor;

#[derive(Debug, Serialize)]
pub struct DartKernelResult {
    ok: bool,
    format: &'static str,
    kernel: disrobe_pass_mobile::DartKernel,
}

pub fn dart_kernel(bytes: &[u8]) -> Result<DartKernelResult, String> {
    let kernel: disrobe_pass_mobile::DartKernel = disrobe_pass_mobile::parse_dart_kernel(bytes)
        .map_err(|error: disrobe_pass_mobile::Error| format!("Dart Kernel: {error}"))?;
    if kernel.sources.len() > 4096 {
        return Err("Dart Kernel: the playground supports up to 4096 source records; use the CLI for this component".to_string());
    }
    Ok(DartKernelResult {
        ok: true,
        format: "dart-kernel",
...
pub struct WasmSignaturesResult {
    ...
    boundary_link_collection_status: disrobe_pass_wasm_deob::BoundaryLinkCollectionStatus,
    summary: ModuleSummary,
...
pub fn wasm_signatures(bytes: &[u8]) -> Result<WasmSignaturesResult, String> {
    ...
    Ok(WasmSignaturesResult {
        ...
        boundary_links,
        boundary_link_collection_status: *sigs.boundary_link_collection_status(),
        summary,
...
#[derive(Debug, Serialize)]
pub struct HermesInstructions {
    index: usize,
    listing: String,
}

#[derive(Debug, Serialize)]
pub struct HermesStringEntry {
    index: usize,
    kind: HermesStringKind,
    value: String,
}
...
pub struct HermesResult {
    ...
    format: &'static str,
    header: HermesHeader,
    string_table: Vec<HermesStringEntry>,
    report: HermesDecompileReport,
    disassembly: Vec<HermesInstructions>,
}

pub fn hermes_analyze(bytes: &[u8]) -> Result<HermesResult, String> {
    let module: HermesModule = parse_hermes_module(bytes)
        .map_err(|error: disrobe_pass_mobile::Error| format!("Hermes bytecode: {error}"))?;
    let string_count: usize = module.identifiers.len() + module.strings.len();
    let identifier_count: usize = module
        .string_kinds
        .iter()
        .filter(|kind: &&HermesStringKind| **kind == HermesStringKind::Identifier)
        .count();
    if module.string_kinds.len() != string_count
        || u32::try_from(string_count).ok() != Some(module.header.string_count)
        || u32::try_from(identifier_count).ok() != Some(module.header.identifier_count)
    {
        return Err("Hermes bytecode: string kinds and string table counts disagree".to_string());
    }
    let report: HermesDecompileReport = decompile_hermes_module(&module);
    let disassembly: Vec<HermesInstructions> = (0..module.functions.len())
        .map(|index: usize| HermesInstructions {
            index,
            listing: hermes_disasm_function(&module, index).join("\n"),
        })
        .collect();
    let string_table: Vec<HermesStringEntry> = module
        .identifiers
        .into_iter()
        .chain(module.strings)
        .zip(module.string_kinds)
        .enumerate()
        .map(
            |(index, (value, kind)): (usize, (String, HermesStringKind))| HermesStringEntry {
                index,
                kind,
                value,
            },
...
pub fn auto_route(bytes: &[u8]) -> AutoRouteResult {
    ...
    if bytes.starts_with(b"MZ")
        && disrobe_pass_dotnet::parse(bytes)
            .and_then(|pe| disrobe_pass_dotnet::parse_clr_header(bytes, &pe))
            .is_ok()
    {
        push_candidate(
            &mut candidates,
            "dotnet",
            "dotnet_analyze",
            ".NET assembly: recover methods and inspect CIL and managed metadata".to_owned(),
        );
    }
    if bytes.starts_with(&[0xca, 0xfe, 0xba, 0xbe])
        && disrobe_core::structural::validate_java_class(bytes)
    {
        push_candidate(
            &mut candidates,
            "jvm",
            "jvm_class",
            "Java class: recover source and inspect method bytecode".to_owned(),
        );
    }
    if go::recognizes(bytes) {
        push_candidate(
            &mut candidates,
            "go",
            "go_metadata",
            "Go executable: inspect build information, functions, and runtime types".to_string(),
        );
    }
    let php: PhpDetection = detect_php(bytes);
    if php.kind == disrobe_pass_php::PhpKind::PharArchive {
        push_candidate(
            &mut candidates,
            "php",
            "phar_list",
            "PHAR archive: list and extract individual members".to_string(),
        );
    }
    ...
    if bytes.len() <= 1024 * 1024
        ...
        && let Some(detection) = disrobe_pass_js_deob::detect_bundle(text)
    {
        push_candidate(
            &mut candidates,
            "javascript",
            "js_unbundle",
            format!("{} bundle markers", detection.kind.as_str()),
        );
    }
    ...
    if !matches!(
        shell_detection.family,
        disrobe_pass_shell::Family::Plain | disrobe_pass_shell::Family::Unknown
    ...
    if disrobe_pass_mobile::is_dart_kernel(bytes) {
        push_candidate(
            &mut candidates,
            "mobile",
            "dart_kernel",
            "Dart Kernel: extract embedded source files and inspect program structure".to_string(),
        );
    }
    if apk::recognizes(bytes) {
        push_candidate(
            &mut candidates,
            "mobile",
            "apk_analyze",
            "Android package: inspect its manifest, resources, and signing metadata".to_string(),
        );
    }
    if mobile_kind == MobileKind::HermesRawBytecode {
        push_candidate(
            &mut candidates,
            "mobile",
            "hermes_analyze",
            "Hermes bytecode: recover JavaScript structure and function instructions".to_string(),
        );
    }
