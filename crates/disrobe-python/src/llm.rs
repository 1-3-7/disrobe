...
/// Serialize a report struct, attach `"llm": null`, & pythonize to a dict.
///
...
/// Extract the finalized LLM bundle from a decompile/deob result dict.
///
/// Accepts either a bare bundle (with a top-level `"categories"` key) or a
/// pass result dict carrying the bundle under an `"llm"` key.
...
fn depythonize_value(obj: &Bound<'_, PyAny>) -> PyResult<Json> {
    pythonize::depythonize(obj)
        .map_err(|e: pythonize::PythonizeError| DisrobeError::new_err(format!("depythonize: {e}")))
}

/// Render the `AGENTS.md` reconstruction brief for a result dict (or bundle).
#[pyfunction]
#[pyo3(name = "agents_md")]
...
/// Render the `SKILL.md` reconstruction brief for a result dict (or bundle).
#[pyfunction]
#[pyo3(name = "skill_md")]
