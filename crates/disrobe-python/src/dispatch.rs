...
///
/// Source-input languages (compile-then-disasm via the host interpreter):
///   - `python` / `py` / `python3` — uses the embedded `dis` module
///
/// Bytecode-input languages (decode-then-render):
///   - `python-bytecode` / `pyc` — parses a `.pyc` and renders the typed listing
///   - `jvm-class` / `class` — `disrobe-pass-jvm` classfile dump
///   - `dex` — `disrobe-pass-jvm` DEX dump
///   - `beam` — `disrobe-pass-beam` instruction listing
///   - `hermes` / `hermes-bundle` / `hbc` — `disrobe-pass-mobile::hermes` report
///   - `wasm` — `disrobe-pass-wasm-deob` module summary
///
/// `source` accepts either `str` (for source-input languages) or `bytes`
/// (for bytecode-input languages). Raises `UnsupportedLanguage` for any
...
/// Parse source or bytes into the language's structured IR (AST / bytecode /
/// metadata). Returns a dict representation.
///
/// Languages supported:
///   - `python` / `py` / `python3` — uses host `ast.parse` then `ast.dump`
///   - `python-bytecode` / `pyc` — returns the same dict as
///     `py_decompile(...)` minus the `llm` key
///   - `jvm-class` / `class` — same dict as `jvm_parse_class(...)`
///   - `dex` — same dict as `jvm_parse_dex(...)`
///   - `wasm` — same dict as `wasm_analyze(...)`
///   - `hermes` / `hermes-bundle` / `hbc` — disassembly report dict
///   - `beam` — disassembly listing dict
#[pyfunction]
#[pyo3(signature = (language, source))]
#[pyo3(text_signature = "(language, source)")]
fn parse<'py>(
...
///
/// Only meaningful for source languages where disrobe has access to a
/// backing compiler. Currently supports `python` via the host interpreter
/// (returns `code.co_code` plus marshalled module). Other languages raise
/// `UnsupportedLanguage`.
#[pyfunction]
