...
/// Run the auto chain detector against `input`. Walks the registered pass
/// tree (pyarmor, pyinstaller, nuitka, js, jvm, dotnet, wasm, mobile, swift,
/// container, py-decompile, py-deob, native-packer) up to `max_depth`
/// layers and returns the full `chain.json` plan as a Python dict.
#[pyfunction]
...
fn auto<'py>(
    ...
    if max_depth == 0 || max_depth > 16 {
        return Err(DisrobeError::new_err(
            "max_depth must be between 1 and 16 inclusive".to_owned(),
        ));
    }
    ...
    report_with_null_bundle(py, &doc)
}
