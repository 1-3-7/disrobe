...
///
/// Walks the TOC, decrypts AES-CTR encrypted modules when the key is
/// recoverable, inflates zlib streams, and prepends pyc headers to bare
/// code objects. Returns a report dict with `cookie`, `entries`,
/// `bare_pyc_paths`, and `llm` (always `None` — LLM metadata for the
/// PyInstaller pass lands in v0.10). Entry payload bytes themselves are
/// returned as blake3 hashes plus byte counts (use `pyinstaller_entry_bytes`
/// to fetch a single payload as `bytes`).
#[pyfunction]
...
/// Return the raw bytes for a single entry by name, re-running the
/// extraction for the supplied image. For large archives prefer caching
/// the result of `pyinstaller_extract` and dispatching by `entries[i].name`.
#[pyfunction]
