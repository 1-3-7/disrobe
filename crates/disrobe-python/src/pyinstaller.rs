...
/// recoverable, inflates zlib streams, & prepends pyc headers to bare
/// code objects. Returns a report dict with `cookie`, `entries`,
/// `bare_pyc_paths`, & `llm` (always `None` - LLM metadata for the
...
/// the result of `pyinstaller_extract` & dispatching by `entries[i].name`.
#[pyfunction]
