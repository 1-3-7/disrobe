...
/// Create a `DISROBE\\0`-magic envelope wrapping `payload` as a Raw-rung
/// artifact. Returns the encoded envelope bytes ready to write to disk or
/// hand to `envelope_verify`.
#[pyfunction]
...
fn envelope_create<'py>(
    ...
    let sidecar: Sidecar = Sidecar {
        produced_by: produced_by.unwrap_or(PRODUCED_BY_DEFAULT).to_owned(),
...
/// Decode and verify a disrobe envelope. Returns the version, rung, hot
/// and cold byte counts, and blake3 root hash. Raises `DisrobeError` on
/// malformed input.
#[pyfunction]
#[pyo3(text_signature = "(envelope_bytes)")]
fn envelope_verify<'py>(py: Python<'py>, envelope_bytes: &[u8]) -> PyResult<Bound<'py, PyAny>> {
