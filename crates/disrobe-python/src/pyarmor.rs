...
fn pyarmor_detect(source: &str, pack: Option<&str>) -> PyResult<PyPyarmorDetection> {
    ...
    Ok(PyPyarmorDetection::from_value(value))
}
...
#[derive(Debug, Clone, Serialize)]
struct PyarmorClassifyReport {
    script_type: String,
    bootstrap_import: String,
    rft_enabled: bool,
    ecc_enabled: bool,
...
impl From<&ModeClassification> for PyarmorClassifyReport {
    fn from(c: &ModeClassification) -> Self {
        Self {
            script_type: c.script_type.label().to_owned(),
            bootstrap_import: c.bootstrap_import.label().to_owned(),
            rft_enabled: c.rft_enabled,
            ecc_enabled: c.ecc_enabled,
            mix_str_enabled: c.mix_str_enabled,
            disposition: c.disposition.label().to_owned(),
            min_format_version: c.min_format_version.to_owned(),
...
#[pyfunction]
#[pyo3(text_signature = "(source, payload)")]
fn pyarmor_classify(source: &str, payload: &[u8]) -> PyResult<PyarmorClassification> {
    let classification: ModeClassification = classify_modes(source, payload);
    let report: PyarmorClassifyReport = PyarmorClassifyReport::from(&classification);
    Ok(PyarmorClassification::from_value(to_value(&report)?))
}
