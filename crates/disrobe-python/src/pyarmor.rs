...
/// Detect PyArmor wrappers in a Python source string. Returns the typed
/// `Detection` (version, protection mode, serial, payload offsets) along
/// with an `llm` bundle. `pack` selects the LLM metadata pack
/// (`pack-1`..`pack-4`); defaults to `pack-1`.
///
/// Raises `DisrobeError` if no PyArmor signature is present.
#[pyfunction]
...
///
/// This is the pure-Rust path: no Python interpreter is launched. Supports
/// v6/v7/v8/v9 wrappers; legacy v3/v4/v5 reports detect-only with a
/// best-effort diagnostic. BCC (super-mode native bytecode) blobs are
/// surfaced but not lifted here.
///
/// Returns a unpack report dict including an `llm` bundle. `pack` selects
/// the LLM metadata pack (`pack-1`..`pack-4`); defaults to `pack-1`.
#[pyfunction]
