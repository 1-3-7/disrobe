...
/// Parse a .NET PE image (managed DLL/EXE) & return the PE layout
...
/// Parse the CLR header & metadata root of a .NET assembly. Returns
...
/// Natively decompile every method body in a .NET PE to structural pseudo-C#
/// using the built-in stack-lifting decompiler (no .NET runtime, no external
/// tool). Returns per-method resolved signatures + bodies and recovery counts.
#[pyfunction]
#[pyo3(text_signature = "(pe_bytes)")]
fn dotnet_decompile<'py>(py: Python<'py>, pe_bytes: &[u8]) -> PyResult<Bound<'py, PyAny>> {
    let asm: DecompiledAssembly = decompile_assembly(pe_bytes).map_err(map("dotnet decompile"))?;
    report_with_null_bundle(py, &asm)
}

/// Recover self-contained static decrypter/decoder methods by emulating their
/// CIL against probe inputs (constant + pre-VM string decoders). Returns the
/// decoded constants and the count of pure decoders found.
#[pyfunction]
#[pyo3(text_signature = "(pe_bytes)")]
fn dotnet_recover_decoders<'py>(py: Python<'py>, pe_bytes: &[u8]) -> PyResult<Bound<'py, PyAny>> {
