...
fn analyze_origin(origin: Origin) -> &'static NativeLangAnalysis {
    let rate: &ModeRate = MODE_RATES
        .iter()
        .find(|rate| rate.origin == origin)
        .expect("every graded origin has a measured mode");
    rate.analysis.get_or_init(|| {
...
fn crystal_pairing_heap_body_matches_the_reference() {
    ...
    let dir: PathBuf = scratch_dir("crystal-pairing-heap");
    let source: PathBuf = dir.join("recovered.c");
    let reference: PathBuf = dir.join("reference.c");
    ...
    std::fs::write(
        &reference,
        include_str!("support/crystal_pairing_heap_reference.c"),
    )
    .expect("write the independent pairing-heap model");
    let compile: Output = Command::new(&compiler)
        .args(["-std=c11", "-O0", "-o"])
        .arg(&exe)
        .arg(&source)
        .arg(&reference)
        .output()
        .expect("compile the recovered pairing heap and reference");
    assert!(
        compile.status.success(),
        "{compiler} rejected the pairing-heap grade:\n{}",
        String::from_utf8_lossy(&compile.stderr)
    );
...
fn zig_float_rounding_preserves_branch_predecessor_conditions() {
    ...
    let body: &FunctionBody = analysis
        ...
        .bodies
        .iter()
        .find(|body| body.start == 0x10b_df30)
        .expect("the committed Zig extended-to-float conversion must be carved");
    ...
    let dir: PathBuf = scratch_dir("zig-float-rounding");
    let source: PathBuf = dir.join("recovered.c");
    let reference: PathBuf = dir.join("reference.c");
    let exe: PathBuf = dir.join(if cfg!(windows) { "grade.exe" } else { "grade" });
    ...
    std::fs::write(
        &reference,
        include_str!("support/zig_float_rounding_reference.c"),
    )
    .expect("write the rounding boundary vectors");
    let compile: Output = Command::new(&compiler)
        .args(["-std=c11", "-O0", "-o"])
        .arg(&exe)
        .arg(&source)
        .arg(&reference)
        .output()
        .expect("compile the recovered conversion and rounding vectors");
    assert!(
        compile.status.success(),
        "{compiler} rejected the rounding grade:\n{}",
        String::from_utf8_lossy(&compile.stderr)
    );
    let observed: String = run_bounded(Command::new(&exe), "zig-float-rounding");
    std::fs::remove_dir_all(&dir).expect("remove the rounding grade scratch directory");
