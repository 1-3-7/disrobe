...
fn analyze_fixture(fixture: &'static Fixture) -> &'static NativeLangAnalysis {
    let analysis: &NativeLangAnalysis = fixture.analysis.get_or_init(|| {
...
fn body_recovery_is_byte_identical_across_runs() {
    for fixture in &FIXTURES {
        let bytes: Vec<u8> = fixture_or_fail(fixture.relative_path);
        let first: &NativeLangAnalysis = analyze_fixture(fixture);
...
fn no_committed_fixture_is_clipped_by_the_aggregate_carve_budget() {
    for fixture in &FIXTURES {
        let relative: &str = fixture.relative_path;
        let analysis: &NativeLangAnalysis = analyze_fixture(fixture);
