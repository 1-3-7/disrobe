...
mod tests {
    ...
    #[test]
    fn compiler_builtin_function_names_keep_their_recovered_bodies() {
        ...
        let recovery: BodyRecovery = recover_bodies(
            &image,
            NativeLang::Zig,
            &[func("__clear_cache", 0x1000, Some(0x1004))],
        );
        assert_eq!(recovery.recovered, 1);
        assert_eq!(recovery.rust_bodies, 1);
        let body: &FunctionBody = &recovery.bodies[0];
        assert_eq!(body.name, "__clear_cache");
        assert_eq!(body.emitted_name, "__clear_cache_1000");
        let BodyStatus::Recovered {
            pseudo_c,
            pseudo_rust: RustBody::Emitted(rust),
        } = &body.status
        else {
            panic!("expected both recovered languages, got {body:?}");
        };
        assert!(
            pseudo_c.contains("uint64_t __clear_cache_1000("),
            "{pseudo_c}"
        );
        assert!(rust.contains("fn __clear_cache_1000("), "{rust}");
    }
