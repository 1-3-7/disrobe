...
///
/// The commercial encoders (`ionCube` / `SourceGuardian` / Zend Guard) each serialize a
/// compiled `zend_op_array` in a proprietary, version-keyed framing. Once the outer
/// envelope is decrypted (see [`crate::encoder`]) the plaintext is a Zend `op_array` whose
/// in-memory shape is documented in `Zend/zend_compile.h` (`struct _zend_op`) and
/// `Zend/zend_vm_opcodes.h` (the opcode table). This container is disrobe's own
/// spec-accurate, deterministic re-serialization of that structure: it captures exactly the
/// fields a decompiler needs (opcodes, operand triples + types, literals, line numbers,
/// nested `op_arrays` for functions/classes/methods) without depending on a target PHP
/// version's pointer-laden binary image.
...
impl Op {
    ...
    ///
    /// Encoders preserve the opcode stream verbatim (only names and the outer envelope are
    /// scrambled), so the branch targets remain truthful and reconstruct an exact CFG.
    #[must_use]
...
///
/// Encoders keep string and numeric literals intact (they are needed at runtime), so these
/// survive decryption and are the single richest signal for skeleton reconstruction.
...
pub mod op {
    ...
    //!
    //! These are the stable core opcodes whose numbering is consistent across the encoder
    //! era (PHP 5.2 through 7.4) for the control-flow and declaration opcodes used here.
    pub const NOP: u8 = 0;
    pub const ADD: u8 = 1;
    pub const SUB: u8 = 2;
    pub const MUL: u8 = 3;
...
///
/// This is the structural decoder the encoder pipeline feeds once an outer envelope is
/// decrypted: it never executes anything, it deterministically walks the serialized op
/// stream + literal pool + nested children. Bounds are checked on every field; corrupt or
/// hostile input yields a typed error, never a panic.
...
///
/// Block leaders are: op 0, every branch target, and every instruction following a branch.
/// Targets are absolute op indices, exactly as the Zend compiler emits them (and exactly as
/// encoders preserve them), so the recovered CFG is faithful, not heuristic.
#[must_use]
...
///
/// Reconstructs declarations (functions / classes / methods), control-flow structure
/// (if / while / for / foreach / switch derived from the CFG), and per-statement intent
/// recovered from the opcode stream. Variable names are emitted as `$vN` where the encoder
/// erased the compiled-variable names; literals and called symbol names are recovered
...
impl Structurer<'_> {
    ...
    /// Loop headers are CFG block leaders that are the destination of a back edge: a
    /// successor index less than or equal to the source block's start. The Zend compiler
    /// emits exactly these for `while`/`for`/`do` constructs, so this is faithful, not a guess.
