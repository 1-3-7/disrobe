...
/// Resolves an inner call argument to raw bytes: a quoted literal or a single nested `base64_decode('...')`.
...
/// Evaluates a nested decoder expression to cleartext bytes, applying each transform inner-first up to [`RESOLVE_DEPTH_CAP`].
...
/// Applies a single classified transform to its inner-resolved body.
fn apply_transform(kind: EvalKind, body: Vec<u8>, depth: u32) -> Option<Vec<u8>> {
    match kind {
        ...
        EvalKind::StrReplace { from, to } => Some(str_replace_bytes(&body, &from, &to)),
        ...
        EvalKind::PackHex => decode_hex_stream(&body),
...
/// Resolves a wrapping transform's inner argument, descending through nested decoders first.
...
/// Decodes a gzip member (`gzdecode`), the gzip-framed variant of deflate.
fn gunzip(body: &[u8], depth: u32) -> Result<Vec<u8>> {
    inflate_bounded(flate2::read::GzDecoder::new(body), depth)
}

/// Decodes PHP `urldecode` (`+` -> space, `%NN` -> byte) or `rawurldecode` (no `+` handling).
...
/// Decodes PHP double-quoted `\xNN` (and `\NNN` octal) escape sequences embedded in a literal.
...
/// Decodes a contiguous hex string (`pack('H*', ...)`) into bytes. Returns `None` if not
...
/// Decodes a PHP `convert_uudecode` payload (classic Unix uuencode body lines).
...
/// XOR a buffer against a repeating key, the lossless inverse of single-key XOR obfuscation.
...
/// Recognizes a helper-call single-key XOR (`xor_dec(base64_decode('...'), 'KEY')`), returning `(key, ciphertext_bytes)`.
#[allow(clippy::expect_used)]
...
/// Recovers a `chr(N).chr(N)...` concatenation chain back to its raw bytes.
#[allow(clippy::expect_used)]
...
/// Decodes a bare double-quoted literal that is entirely `\xNN` escapes to cleartext bytes.
