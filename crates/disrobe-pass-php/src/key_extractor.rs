...
///
/// The three commercial PHP encoders place key material very differently, and only some of
/// it is statically present in the encoded file. This enum keeps disrobe honest: it never
/// claims to hold a key it cannot actually extract from bytes on disk.
...
/// AES forward S-box, transcribed from FIPS-197 (the canonical Rijndael substitution table).
///
/// `SourceGuardian` ships an AES implementation in its loader; the presence of this exact
/// 256-byte table inside an envelope is a strong, original-bytes signal that an AES round
/// pipeline is embedded, which lets us classify the scheme and locate the crypto region
/// without ever copying any encoder source.
...
///
/// `Rcon` appears in any AES key-expansion routine; locating this short, distinctive run
/// near the S-box confirms an embedded key schedule rather than a decrypt-only table.
...
/// Documented Zend Guard legacy XOR-key marker: the payload header carries a fixed-width key
/// at a known offset past the `@Zend;` banner in pre-cryptographic-integrity builds.
const ZG_XOR_KEY_LEN: usize = 8;

/// Scans an encoder envelope for statically recoverable key material, dispatching by family.
///
/// This never decrypts; it classifies key provenance and recovers only what the bytes
/// actually contain. ionCube symmetric keys are loader-derived and are honestly reported as
/// unrecoverable; `SourceGuardian` AES sessions are runtime-derived (we locate the round
/// pipeline but do not fabricate a key); Zend Guard legacy XOR keys are statically recovered.
#[must_use]
...
///
/// In pre-cryptographic-integrity Zend Guard builds the `@Zend;` banner is followed by a
/// version byte and a fixed-width key block before the ciphertext begins. This locates that
/// block by structure, returning its absolute offset and bytes. Returns `None` when the
/// banner is absent or the header is too short to hold a key (modern builds).
...
///
/// This is the one commercial scheme whose legacy framing is fully static: once the key is
/// recovered by [`recover_zend_guard_xor_key`], the ciphertext is a straight repeating-key
/// XOR. Modern builds are not covered (their keys are runtime-derived).
#[must_use]
...
/// Decrypts an AES-CBC blob using the existing `aes` + `cbc` crates.
///
/// `SourceGuardian` session keys are not statically recoverable, so this is the bring-your-
/// own-key path used when a key is supplied out of band (or by a future loader-emulation
/// stage). Key must be 16/24/32 bytes; IV must be 16 bytes; ciphertext must be a multiple of
/// the 16-byte block size. Never panics: malformed inputs return [`AesOutcome::BadInput`].
#[must_use]
