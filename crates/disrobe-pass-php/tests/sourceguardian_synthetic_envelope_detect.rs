//! Detect-only coverage for the `SourceGuardian` protector against synthetic envelopes.
//!
//! These envelopes carry the real `SourceGuardian` banner bytes but opaque filler in
//! place of the `ixed.*` loader's encrypted Zend opcode stream. There is no real
//! `SourceGuardian` sample and no source recovery here; the verdict is detect-only
