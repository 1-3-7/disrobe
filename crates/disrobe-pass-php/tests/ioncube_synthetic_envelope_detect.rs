//! Detect-only coverage for the `ionCube` protector against synthetic envelopes.
//!
//! These envelopes carry the real `ionCube` era marker bytes but opaque filler in
//! place of the licensed loader's encrypted Zend opcode arrays. There is no real
//! `ionCube` sample and no source recovery here; the crate's verdict is detect-only
