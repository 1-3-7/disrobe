...
/// Builder for a spec-accurate disrobe canonical op_array container.
///
/// This hand-assembles the exact byte layout the parser expects, transcribing the Zend
/// `_zend_op` field order (op1/op2/result types, the three operand u32s, extended_value,
/// lineno). The corpus produced here is non-circular: tests assert recovered control flow
/// and literals against intent that is encoded ONLY in raw bytes, never reusing the emitter.
#[derive(Default)]
...
fn cfg_distinguishes_while_loop_via_back_edge() {
    ...
    b.op(op::JMP, T_UNUSED, T_UNUSED, T_UNUSED, 0, 0, 0, 0, 3);
...
fn foreach_skeleton_from_fe_fetch() {
    ...
    b.op(op::FE_FETCH_R, T_VAR, T_UNUSED, T_CV, 0, 4, 1, 0, 2);
    ...
    let decomp: Decompilation = decompile_oparray(&parsed);
    assert!(
        decomp.php_skeleton.contains("foreach ("),
        "skeleton: {}",
        decomp.php_skeleton
    );
}
...
fn reconstructs_class_method_skeleton() {
    ...
    assert!(skel.contains("return ~2;"), "skeleton: {skel}");
}
...
fn recovers_function_call_name_from_literal_pool() {
    ...
    let decomp: Decompilation = decompile_oparray(&parsed);
    assert!(
        decomp.php_skeleton.contains("strlen(...);"),
        "skeleton: {}",
        decomp.php_skeleton
    );
}
