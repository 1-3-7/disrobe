...
fn emit_real_dump_versioned(sample: &str, force_version: Option<u8>) -> Option<RealDump> {
    ...
    let source: Vec<u8> = std::fs::read(&canonical_src).expect("read sample to stage");
    write_opcache_source(&src, &source).expect("stage sample at a settled modification time");
