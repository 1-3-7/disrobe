...
#[test]
fn zend_optimizer_obfuscation_key_is_statically_recovered() {
    let key: &[u8] = b"OPTKEY-1234";
    let mut envelope: Vec<u8> = b"<?php @Zend;\n4".to_vec();
    envelope.push(0x00);
    envelope.extend_from_slice(b"ZOBF");
    envelope.extend_from_slice(&u16::try_from(key.len()).unwrap().to_le_bytes());
    envelope.extend_from_slice(key);
    envelope.extend_from_slice(b"obfuscated-opcode-body");
    let scan: KeyScan = scan_key(&envelope, EncoderFamily::ZendGuard);
    assert_eq!(scan.provenance, KeyProvenance::StaticEmbedded);
    assert_eq!(scan.key, key.to_vec());
    assert!(scan.note.contains("Zend Optimizer"));
}

#[test]
fn zend_optimizer_zero_length_key_is_not_accepted() {
    let mut envelope: Vec<u8> = b"<?php @Zend;\n4".to_vec();
    envelope.push(0x00);
    envelope.extend_from_slice(b"ZOBF");
    ...
    assert_eq!(
        scan.provenance,
        KeyProvenance::RuntimeDerived,
        "a zero-length ZOBF key must not be treated as a recovered key"
    );
    assert!(scan.key.is_empty());
}
