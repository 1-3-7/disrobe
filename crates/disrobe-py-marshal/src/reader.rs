...
fn object_node_count_capped(
        ...
        match obj {
            ...
            | Object::FrozenSet(items) => {
                for item in items {
                    ...
                    walk(item, cap, acc, depth + 1, depth_limit)?;
                }
            }
            Object::Dict(map) | Object::FrozenDict(map) => {
                for (k, v) in map {
                    ...
                    walk(k, cap, acc, depth + 1, depth_limit)?;
                    walk(v, cap, acc, depth + 1, depth_limit)?;
                }
            }
            Object::Slice { lower, upper, step } => {
                walk(lower, cap, acc, depth + 1, depth_limit)?;
                walk(upper, cap, acc, depth + 1, depth_limit)?;
                walk(step, cap, acc, depth + 1, depth_limit)?;
            }
            Object::Code(co) => {
                for item in co
                    .consts
                ...
                walk(&co.filename, cap, acc, depth + 1, depth_limit)?;
                walk(&co.name, cap, acc, depth + 1, depth_limit)?;
                walk(&co.qualname, cap, acc, depth + 1, depth_limit)?;
            }
            Object::None
    ...
    let mut acc: u64 = 0;
    walk(obj, cap, &mut acc, depth, depth_limit)?;
    Ok(acc)
}
    ...
    fn walk(obj: &Object, cap: u64, acc: &mut u64) {
        ...
        match obj {
            ...
            Object::Code(co) => {
                ...
                add_capped(acc, co.localspluskinds.len(), cap);
...
fn charged_totals_after_clone(
    ...
    let node_limit: u64 = limits.map_or(NODE_BUDGET, |value: Limits| value.nodes);
    let byte_limit: u64 = limits.map_or(BYTE_BUDGET, |value: Limits| value.reference_bytes);
    let remaining_nodes: u64 = node_limit.saturating_sub(already_nodes);
    let nodes: u64 = object_node_count_capped(
        obj,
        remaining_nodes.saturating_add(1),
        depth,
        limits.map(|value: Limits| value.depth),
    )?;
    if nodes > remaining_nodes {
        return Err(Error::NodeBudget { limit: node_limit });
    }
    let remaining_bytes: u64 = byte_limit.saturating_sub(already_bytes);
    let bytes: u64 = object_byte_count_capped(obj, remaining_bytes.saturating_add(1));
    if bytes > remaining_bytes {
        return Err(Error::ByteBudget { limit: byte_limit });
    }
    Ok((
        already_nodes.saturating_add(nodes),
...
pub fn load_with_limits(
    ...
    let mut reader: Reader<'_> = Reader::new(data, version, false);
    reader.limits = Some(Limits {
        input_bytes: limits.input_bytes,
        nodes: limits.nodes.min(NODE_BUDGET),
        depth: limits.depth.min(MAX_DEPTH),
        collection_items: limits.collection_items.min(MAX_COLLECTION_ITEMS),
        dict_entries: limits.dict_entries.min(MAX_DICT_ENTRIES),
        reference_bytes: limits.reference_bytes.min(BYTE_BUDGET),
    });
    let object: Object = reader.read_object(0)?;
    Ok((object, reader.cursor.position()))
}
...
impl<'a> Reader<'a> {
    ...
    fn clone_obj_charged(&mut self, obj: &Object, depth: usize) -> Result<Object> {
        let (total_nodes, total_bytes): (u64, u64) = charged_totals_after_clone(
            obj,
    ...
    fn read_object(&mut self, depth: usize) -> Result<Object> {
        let depth_limit: usize = self.limits.map_or(MAX_DEPTH, |value: Limits| value.depth);
        if depth >= depth_limit {
            return Err(Error::DepthLimit(depth_limit));
        }
        ...
        if let Some(limits) = self.limits
            && !matches!(tag, b'r' | b'R')
        {
            if self.materialized_nodes >= limits.nodes {
                return Err(Error::NodeBudget {
                    limit: limits.nodes,
                });
            }
        ...
        if let Some(idx) = ref_idx {
            let stored: Object = self.clone_obj_charged(&obj, depth)?;
    ...
    fn decode_tag(
        ...
        match tag {
            ...
            b'r' => self.decode_back_ref(ref_override, depth),
    ...
    fn decode_string_ref(&mut self, ref_override: &mut Option<u32>) -> Result<Object> {
        ...
        let byte_limit: u64 = self
            .limits
            .map_or(BYTE_BUDGET, |value: Limits| value.reference_bytes);
        let node_limit: u64 = self.limits.map_or(NODE_BUDGET, |value: Limits| value.nodes);
        if value_len > byte_limit.saturating_sub(self.materialized_bytes) {
            return Err(Error::ByteBudget { limit: byte_limit });
        }
        if self.materialized_nodes >= node_limit {
            return Err(Error::NodeBudget { limit: node_limit });
        }
    ...
    fn decode_back_ref(&mut self, ref_override: &mut Option<u32>, depth: usize) -> Result<Object> {
        ...
        let (total_nodes, total_bytes): (u64, u64) =
            charged_totals_after_clone(entry, already_nodes, already_bytes, self.limits, depth)?;
    ...
    fn read_n_objects(&mut self, n: usize, depth: usize) -> Result<Vec<Object>> {
        let limit: usize = self
            .limits
            .map_or(MAX_COLLECTION_ITEMS, |value: Limits| value.collection_items);
        if n > limit {
            let truncated: u32 = u32_saturating_from_usize(n);
            return Err(Error::LengthOverflow(truncated));
        }
        let capacity: usize = object_prealloc_capacity(n, self.remaining());
        let capacity: usize = self.limits.map_or(capacity, |value: Limits| {
            capacity.min(
                usize::try_from(value.nodes.saturating_sub(self.materialized_nodes))
                    .unwrap_or(usize::MAX),
            )
        });
        let mut v: Vec<Object> = Vec::with_capacity(capacity);
        for _ in 0..n {
    ...
    fn push_interned_string(&mut self, value: &str) -> Result<()> {
        ...
        if let Some(limits) = self.limits {
            let value_len: u64 = u64::try_from(value.len()).unwrap_or(u64::MAX);
            if value_len
                > limits
                    .reference_bytes
            ...
            self.materialized_bytes += value_len;
            self.materialized_nodes += 1;
        }
        self.interned_strings.push(value.to_owned());
        Ok(())
    }
    ...
    fn read_dict(&mut self, depth: usize) -> Result<IndexMap<Object, Object>> {
        ...
        let limit: usize = self
            .limits
            .map_or(MAX_DICT_ENTRIES, |value: Limits| value.dict_entries);
        loop {
            if self.limits.is_none() && entry_count >= MAX_DICT_ENTRIES {
...
mod tests {
    ...
    const fn bounded_limits() -> Limits {
        Limits {
            ...
            collection_items: 16,
            dict_entries: 8,
            reference_bytes: 1024,
        }
    }

    #[test]
    fn bounded_input_limit_is_inclusive_and_precedes_parsing() {
        let mut limits: Limits = bounded_limits();
        limits.input_bytes = 0;
        assert!(matches!(
            load_with_limits(b"?", PyVersion::PY312, limits),
        ...
        assert_eq!(
            load_with_limits(b"N", PyVersion::PY312, limits).unwrap(),
            (Object::None, 1)
        );
    }

    #[test]
    fn bounded_reader_returns_exact_consumed_length_without_rejecting_trailing_bytes() {
        let data: &[u8] = b"NT";
        let (object, consumed): (Object, usize) =
            load_with_limits(data, PyVersion::PY312, bounded_limits()).unwrap();
        let (reference, trace): (Object, RefTableDump) =
            load_with_reftable(data, PyVersion::PY312).unwrap();
        assert_eq!(object, Object::None);
        assert_eq!(object, reference);
        assert_eq!(consumed, 1);
        assert_eq!(consumed, trace.total_bytes);
        assert!(consumed < data.len());
        assert_eq!(load(data, PyVersion::PY312).unwrap(), Object::None);
    }

    #[test]
    fn bounded_direct_nodes_are_charged_before_decoding_children() {
        let data: &[u8] = b")\x02NT";
        let mut limits: Limits = bounded_limits();
        limits.nodes = 3;
        assert_eq!(
            load_with_limits(data, PyVersion::PY312, limits).unwrap(),
            (Object::Tuple(vec![Object::None, Object::True]), data.len())
        );
        limits.nodes = 2;
        assert!(matches!(
            load_with_limits(data, PyVersion::PY312, limits),
            Err(Error::NodeBudget { limit: 2 })
        ));
        limits.nodes = 0;
        assert!(matches!(
            load_with_limits(b"N", PyVersion::PY312, limits),
            Err(Error::NodeBudget { limit: 0 })
        ));
    }

    #[test]
    fn bounded_collection_limit_rejects_declared_count_before_children() {
        let mut limits: Limits = bounded_limits();
        limits.collection_items = 2;
        assert!(load_with_limits(b")\x02NT", PyVersion::PY312, limits).is_ok());
        limits.collection_items = 1;
        assert!(matches!(
            load_with_limits(b")\x02", PyVersion::PY312, limits),
            Err(Error::LengthOverflow(2))
        ));
    }

    #[test]
    fn bounded_dict_limit_counts_pairs_and_allows_exact_limit() {
        let mut limits: Limits = bounded_limits();
        limits.dict_entries = 2;
        let (object, consumed): (Object, usize) =
            load_with_limits(b"{NTNF0", PyVersion::PY312, limits).unwrap();
        let Object::Dict(entries) = object else {
            panic!("expected dictionary");
        };
        assert_eq!(entries.len(), 1);
        assert_eq!(entries.get(&Object::None), Some(&Object::False));
        assert_eq!(consumed, 6);
        limits.dict_entries = 1;
        assert!(matches!(
            load_with_limits(b"{NTN", PyVersion::PY312, limits),
            Err(Error::LengthOverflow(2))
        ));
        limits.dict_entries = 0;
        assert!(load_with_limits(b"{0", PyVersion::PY312, limits).is_ok());
    }

    #[test]
    fn bounded_depth_rejects_nested_objects_at_the_boundary() {
        let data: &[u8] = b")\x01)\x01N";
        let mut limits: Limits = bounded_limits();
        limits.depth = 3;
        assert!(load_with_limits(data, PyVersion::PY312, limits).is_ok());
        limits.depth = 2;
        assert!(matches!(
            load_with_limits(data, PyVersion::PY312, limits),
    ...
    #[test]
    fn bounded_nodes_include_reference_storage_and_resolved_objects() {
        let data: &[u8] = &[b')', 2, FLAG_REF | b'N', b'r', 0, 0, 0, 0];
        let mut limits: Limits = bounded_limits();
        limits.nodes = 4;
        assert_eq!(
            load_with_limits(data, PyVersion::PY312, limits).unwrap().0,
            Object::Tuple(vec![Object::None, Object::None])
        );
        limits.nodes = 3;
        assert!(matches!(
            load_with_limits(data, PyVersion::PY312, limits),
            Err(Error::NodeBudget { limit: 3 })
        ));
    }

    #[test]
    fn bounded_reference_bytes_include_storage_and_later_resolution() {
        let data: &[u8] = &[
            b')',
            2,
            FLAG_REF | b's',
        ...
        let mut limits: Limits = bounded_limits();
        limits.reference_bytes = 6;
        assert_eq!(
            load_with_limits(data, PyVersion::PY312, limits).unwrap().0,
            Object::Tuple(vec![
                Object::Bytes(b"abc".to_vec()),
                Object::Bytes(b"abc".to_vec())
            ])
        );
        limits.reference_bytes = 5;
        assert!(matches!(
            load_with_limits(data, PyVersion::PY312, limits),
            Err(Error::ByteBudget { limit: 5 })
        ));
    }

    #[test]
    fn bounded_interned_strings_charge_storage_and_reference_copies() {
        let data: &[u8] = b")\x02t\x03\x00\x00\x00abcR\x00\x00\x00\x00";
        let mut limits: Limits = bounded_limits();
        limits.reference_bytes = 6;
        limits.nodes = 4;
        assert!(load_with_limits(data, PyVersion::PY312, limits).is_ok());
    ...
    #[test]
    fn bounded_reference_depth_counts_resolved_children() {
        let data: &[u8] = &[
            b')',
            3,
            FLAG_REF | b')',
        ...
        assert!(matches!(
            load_with_limits(data, PyVersion::PY312, limits),
            Err(Error::DepthLimit(3))
        ));
        assert!(load(data, PyVersion::PY312).is_ok());
    }

    #[test]
    fn bounded_code_clone_bytes_include_localspluskinds() {
        let mut code: CodeObject = CodeObject::new(CodeEra::Py311Plus);
        code.localspluskinds = vec![1, 2, 3];
        let object: Object = Object::Code(Box::new(code));
        let mut limits: Limits = bounded_limits();
        limits.reference_bytes = 3;
        let (_, bytes): (u64, u64) =
            charged_totals_after_clone(&object, 0, 0, Some(limits), 0).unwrap();
        assert_eq!(bytes, 3);
        limits.reference_bytes = 2;
        assert!(matches!(
            charged_totals_after_clone(&object, 0, 0, Some(limits), 0),
            Err(Error::ByteBudget { limit: 2 })
        ));
    }
