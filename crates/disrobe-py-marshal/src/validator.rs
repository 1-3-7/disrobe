...
pub fn validate_roundtrip_strict(obj: &Object, version: PyVersion) -> Result<Vec<u8>> {
    ...
    require_clean_roundtrip(&report, version)?;
    crate::writer::dump(obj, version)
}

const fn require_clean_roundtrip(report: &RoundTripReport, version: PyVersion) -> Result<()> {
    if report.is_clean() {
        Ok(())
    } else {
        Err(crate::error::Error::RoundtripMismatch {
            version,
            object_matches: report.object_matches,
            bytes_match: report.bytes_match,
...
mod tests {
    ...
    #[test]
    fn strict_gate_rejects_byte_drift() {
        let report: RoundTripReport = RoundTripReport {
            encoded_len: 7,
            reencoded_len: 7,
            object_matches: true,
            bytes_match: false,
        ...
        let err: crate::error::Error =
            require_clean_roundtrip(&report, PyVersion::PY312).unwrap_err();
        assert!(matches!(
            err,
            crate::error::Error::RoundtripMismatch {
                version: PyVersion::PY312,
                object_matches: true,
                bytes_match: false,
                first_diff_offset: Some(3),
            }
