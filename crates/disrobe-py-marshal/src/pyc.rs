...
impl PycHeader {
    pub fn deterministic(version: PyVersion) -> Result<Self> {
        let magic: u16 = magic_for(version).ok_or(Error::UnsupportedPyVersion(version))?;
        Ok(Self {
            version,
            magic,
...
pub fn write_pyc(file: &PycFile) -> Result<Vec<u8>> {
    ...
    if let Some(bf) = file.header.bit_field {
        ...
        out.extend_from_slice(&file.header.source_size.unwrap_or(0).to_le_bytes());
...
pub fn read_pyc(bytes: &[u8]) -> Result<PycFile> {
    ...
    let version: PyVersion = pyversion_from_magic(magic).ok_or_else(|| Error::UnknownPycMagic {
