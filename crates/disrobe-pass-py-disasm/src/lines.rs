//! Per-version source-line resolution. Decodes `lnotab` (`CPython` <= 3.9), the 3.10 `linetable`,
//! and the PEP 626 location table (3.11+) into byte-offset regions so the disassembler reproduces
//! the `dis` `starts_line` / `L N` column exactly.
...
/// Resolves the source line for every byte offset of a code object, matching `CPython`'s
/// `co_lines()` regions, then exposes them through [`LineMap::start_line`] so the disassembler
/// can reproduce `dis`'s `starts_line` / `L N` column exactly.
#[derive(Debug, Clone)]
