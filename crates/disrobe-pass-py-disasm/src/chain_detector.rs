...
impl Pass for PyDisasmPass {
    ...
    fn output_kind(&self, output: &Artifact) -> OutputKind {
        let recovered: bool = serde_json::from_slice::<PyDisasmExtract>(output.envelope.as_slice())
            .is_ok_and(|e: PyDisasmExtract| e.instruction_count > 0);
        if recovered {
            OutputKind::Source {
...
fn extract_for(bytes: &[u8]) -> CoreResult<PyDisasmExtract> {
    if let Some(rt) = detect_runtime(bytes) {
        return Ok(PyDisasmExtract {
            ...
            py_version: None,
            disasm_text: format!(
                "; alt-runtime {} detected; native disassembly not implemented (detection only, no instructions recovered)\n",
                alt_label(rt)
            ),
...
mod tests {
    ...
    fn pass_output_kind_reflects_recovery() {
        let recovered: PyDisasmExtract = PyDisasmExtract {
            runtime: "cpython".to_owned(),
            py_version: Some("3.11".to_owned()),
            disasm_text: "  0 RESUME 0\n".to_owned(),
            instruction_count: 5,
        };
        let payload: Vec<u8> = serde_json::to_vec(&recovered).expect("serialize");
        let a: Artifact = Artifact::new(Rung::Disasm, payload, [0u8; 32]);
        match PY_DISASM_PASS.output_kind(&a) {
            ...
            _ => panic!("expected Source for recovered disasm"),
        }

        let detection_only: PyDisasmExtract = PyDisasmExtract {
            runtime: "pypy".to_owned(),
            py_version: None,
            disasm_text: "; alt-runtime pypy detected; native disassembly not implemented (detection only, no instructions recovered)\n".to_owned(),
            instruction_count: 0,
        };
        let payload: Vec<u8> = serde_json::to_vec(&detection_only).expect("serialize");
        let a: Artifact = Artifact::new(Rung::Disasm, payload, [0u8; 32]);
        match PY_DISASM_PASS.output_kind(&a) {
            OutputKind::Bytes { format_tag, .. } => {
                assert_eq!(format_tag, "py.runtime-detected");
            }
            _ => panic!("expected Bytes for detection-only result"),
        }
    }
