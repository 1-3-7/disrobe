...
pub fn decode_exception_table(bytes: &[u8]) -> Result<Vec<ExceptionEntry>, DecodeError> {
    ...
    while cursor < bytes.len() {
        ...
        let start_offset: u32 = start_units.saturating_mul(BYTECODE_UNIT_BYTES);
        let end_offset: u32 = start_units
            .saturating_add(length_units)
            .saturating_mul(BYTECODE_UNIT_BYTES);
        let target_offset: u32 = target_units.saturating_mul(BYTECODE_UNIT_BYTES);
