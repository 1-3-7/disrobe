//! Probe Windows for `python3.lib` so MSVC links the pyo3 extension-module abi3 stub.
//! Order: explicit `PYO3_PYTHON`, then `VIRTUAL_ENV`, then well-known install roots.
//! Falls back to an actionable `cargo:warning` when nothing is found.
...
fn emit_python_libs_search_path() {
    ...
    println!(
        "cargo:warning=disrobe-pyarmor-pytrace: no python3.lib found on PATH. Set PYO3_PYTHON=<path-to-python.exe> or install Python 3.9+ from https://www.python.org/downloads/ to build this crate on Windows."
    );
}
