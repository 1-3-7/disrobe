...
fn run_pyinstaller(entry: &CorpusEntry, bytes: &[u8], blake_in: &str) -> Vec<SampleMetrics> {
    ...
    match cookie {
        Ok(c) => vec![SampleMetrics {
            ...
            blake3_output: None,
            message: Some(format!(
                "detected python={}.{} toc_entries~{} (cookie only, no extraction)",
...
fn run_nuitka(entry: &CorpusEntry, bytes: &[u8], blake_in: &str) -> Vec<SampleMetrics> {
    ...
    match det {
        Ok(d) => vec![SampleMetrics {
            ...
            blake3_output: None,
            message: Some(format!(
                "detected flavor={:?} sigs={} (detection only, no body recovery)",
                d.flavor,
                d.hits.len()
            )),
        }],
...
fn run_pyfreeze(entry: &CorpusEntry, bytes: &[u8], blake_in: &str) -> Vec<SampleMetrics> {
    ...
    let micros: u128 = start.elapsed().as_micros();
    let detected: bool = !matches!(det.kind, disrobe_pass_pyfreeze::FreezerKind::Unknown);
    vec![SampleMetrics {
        entry: entry.clone(),
        ...
        blake3_output: None,
        message: Some(format!(
            "kind={:?} confidence={:.2} (detection only, no extraction)",
