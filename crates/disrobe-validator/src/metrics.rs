...
mod tests {
    ...
    fn sample(pass: &str, ok: bool, recovered: bool) -> SampleMetrics {
        SampleMetrics {
            entry: CorpusEntry {
    ...
    fn aggregates_correctly() {
        let samples: Vec<SampleMetrics> = vec![
            sample("js", true, true),
            sample("js", true, false),
            sample("js", false, false),
            sample("pyarmor", true, false),
        ];
        ...
        assert_eq!(js.samples_recovered, 1);
        assert_eq!(js.samples_failed, 1);
        let pyarmor: &PassMetrics = agg.iter().find(|m| m.pass_name == "pyarmor").unwrap();
        assert_eq!(pyarmor.samples_ok, 1);
        assert_eq!(pyarmor.samples_recovered, 0);
    }
}
