...
fn main() {
    ...
    let module_docs = beam
        .chunks
        .docs
        .as_ref()
        .and_then(|d| disrobe_pass_beam::parse_docs(&d.term));
    let rec = disrobe_pass_beam::recover_elixir_with_docs(
        beam.module_name().unwrap(),
        &info,
        module_docs.as_ref(),
    )
    .unwrap();
    let _ = recover_elixir;
