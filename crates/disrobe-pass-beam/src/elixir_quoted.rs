//! Renders the Elixir quoted AST stored in `Dbgi` `elixir_v1` metadata back to idiomatic Elixir source.
...
/// Renders a 3-element quoted tuple as either a variable `{name, meta, ctx}` or a call.
...
/// Whether an expanded `:erlang.fun/arity` is auto-imported into Elixir's `Kernel`.
