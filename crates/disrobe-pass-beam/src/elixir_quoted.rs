//! Renders the Elixir quoted AST stored in `Dbgi` `elixir_v1` metadata back to
//! idiomatic Elixir source.
//!
//! The metadata holds each definition's clauses as the *expanded* quoted form
//! `{name | {:., _, [mod, fun]}, meta, args}`, the same shape `Code.string_to_quoted`
//! produces. This module is a clean-room reimplementation of the `Macro.to_string`
//! rendering contract (studied from the documented quoted grammar, no source
//! copied): three-element tuples are calls or variables, two-element tuples are
//! literal pairs, and a curated operator/sigil table recovers infix surface.

use crate::etf::Term;

/// One recovered `def`/`defp` clause: argument patterns, optional `when` guard,
...
/// Renders a single definition clause `{meta, args, guards, body}` from the
/// metadata `definitions` list.
#[must_use]
...
/// Strips the `Elixir.` namespace prefix from a module atom, e.g.
/// `Elixir.EdgeCases.MyServer` -> `EdgeCases.MyServer`.
#[must_use]
...
/// Renders a body that may be a `__block__` of several statements, one per line.
#[must_use]
...
/// The crux: a 3-element tuple is either a quoted *variable* `{name, meta, ctx}`
/// (when the context is an atom) or a quoted *call* `{target, meta, args}` (when
/// args is a list). A 2-element tuple is a literal Elixir pair. Everything else
/// is an n-tuple literal rendered with brace syntax.
...
/// `{:., meta, [mod, fun]}` -> `(mod, fun)`. The module may be an atom alias
/// (`:Elixir.String`) or a nested expression (the `expr.field` access form).
...
/// Maps the expanded `:erlang`/`:elixir` operator atoms back to surface infix
/// operators with the precedence that drives parenthesization.
...
/// Whether an expanded `:erlang.fun/arity` is auto-imported into Elixir's
/// `Kernel`, so it renders as a bare call (`is_atom(x)`, `byte_size(b)`,
/// `tuple_size(t)`) rather than `erlang.fun(...)`.
...
/// Splits a call's args into the trailing `[do: body, ...]` keyword block and
/// the leading positional qualifiers (generators/filters of `for`/`with`).
