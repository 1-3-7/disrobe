...
const MAX_DISASM_DEPTH: usize = 500;
...
pub fn disassemble(code: &CodeChunk) -> Result<Disassembly> {
    dbg_section("opcode stream");
    dbg_kv("code_header", || {
        format!(
            "instr_set={} opcode_max={} labels={} functions={} bytes={}",
    ...
    while !reader.is_empty() {
        ...
        let spec: OpcodeSpec = opcodes::opcode_spec(opcode).ok_or_else(|| {
            dbg_line(|| format!("unknown opcode {opcode} at offset {offset}"));
            Error::UnknownOpcode {
                opcode,
                offset,
        ...
        for _ in 0..spec.arity {
            operands.push(decode_operand(&mut reader, 0)?);
        }
    ...
    dbg_kv("decoded", || {
        format!(
            ...
            instructions
                .last()
                .map_or("<none>", |i: &Instruction| i.name)
        )
    });
...
pub fn decode_compact_simple(reader: &mut Reader<'_>) -> Result<(u8, u32)> {
    let op: Operand = decode_operand(reader, 0)?;
...
fn operand_u32(op: &Operand) -> Option<u32> {
    match op {
        ...
        Operand::SignedInteger(v) => u32::try_from(*v).ok(),
        Operand::Atom(v)
        | Operand::XReg(v)
        | Operand::YReg(v)
        | Operand::Label(v)
...
fn list_size_u32(reader: &Reader<'_>, op: &Operand) -> Result<u32> {
    let Some(size): Option<u32> = operand_u32(op) else {
        return Err(Error::Truncated {
            offset: reader.position(),
            needed: 1,
            had: reader.remaining(),
        });
    };
    Ok(size)
}

fn decode_operand(reader: &mut Reader<'_>, depth: usize) -> Result<Operand> {
    if depth > MAX_DISASM_DEPTH {
        return Err(Error::DepthExceeded {
            kind: "compact term",
            limit: MAX_DISASM_DEPTH,
        });
    }
    let byte: u8 = reader.u8()?;
    let tag: u8 = byte & 0b0000_0111;
    if tag == TAG_EXTENDED {
        return decode_extended(reader, byte, depth);
    }
...
fn decode_compact_value(reader: &mut Reader<'_>, byte: u8) -> Result<CompactValue> {
    ...
    let nbytes: usize = if high_nibble == 0b111 {
        let length_tag: u8 = reader.u8()?;
        let extra: CompactValue = decode_compact_value(reader, length_tag)?;
        match extra {
...
fn decode_extended(reader: &mut Reader<'_>, byte: u8, depth: usize) -> Result<Operand> {
    match byte {
        EXT_LIST => {
        ...
        EXT_FPREG => {
            let inner: Operand = decode_operand(reader, depth + 1)?;
        ...
        EXT_TYPED_REG => {
            let reg: Operand = decode_operand(reader, depth + 1)?;
            let type_op: Operand = decode_operand(reader, depth + 1)?;
            let type_index: u32 =
...
mod tests {
    ...
    #[test]
    fn operand_u32_rejects_out_of_range_literals() {
        assert_eq!(
            operand_u32(&Operand::Literal(u64::from(u32::MAX) + 1)),
            None
        );
        assert_eq!(operand_u32(&Operand::SignedInteger(-1)), None);
        assert_eq!(
            operand_u32(&Operand::SignedInteger(i64::from(u32::MAX) + 1)),
            None
        );
        assert_eq!(
            operand_u32(&Operand::Literal(u64::from(u32::MAX))),
            Some(u32::MAX)
        );
    }
}
