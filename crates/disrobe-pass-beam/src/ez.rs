...
///
/// Guards against decompression bombs and central-directory size lies. `.ez`
/// archives ship OTP application `ebin/` trees, so the safe defaults are tight.
...
impl EzArchive {
    ...
    /// Parses an `.ez` archive, enforcing `quota` against entry count,
    /// per-entry size, aggregate size, and aggregate expansion ratio. Declared
    /// central-directory sizes are never trusted for allocation: each entry is
    /// streamed through a hard byte ceiling so a lying header cannot trigger an
    /// out-of-memory before the real (over-cap) read is rejected.
