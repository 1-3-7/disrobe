...
///
/// Uses a register-SSA value model over the X/Y registers plus structural
/// control-flow reconstruction (`case`, `if`, `receive`, `try`, recursion). The
/// instruction slice must include the function's `label`/test ops.
#[must_use]
pub fn lift_body(
    instrs: &[Instruction],
    arity: u32,
    ...
    LiftedBody {
        stmts,
        fully_recovered: !flags.degraded && !unresolved,
    }
}

/// Lifts a function body into reconstructed Erlang clauses.
///
/// Recovers multi-clause function heads from the single merged decision tree that
/// the compiler emits per `func_info`: binary-matching functions walk their
/// `bs_match` fail chain into one clause per binary pattern; value- and
/// guard-dispatched functions flatten their top-level `case`/`if` into clauses
/// with recovered argument patterns. A function with no recoverable dispatch
/// yields a single all-variable clause carrying the lifted body.
#[must_use]
...
impl Lifter<'_> {
    ...
    /// Models `trim N Remaining`: the BEAM stack-frame trim discards the bottom
    /// `N` Y-register slots and renumbers the survivors, so `y(i)` becomes
    /// `y(i - N)`. The value model must shift the Y-register environment to match.
    ...
    /// Builds the conditional for an in-body `bs_match` whose fail branch is a real
    /// clause (the recursion base of a lowered binary comprehension): `case Subject
    /// of <<Segs>> -> Cont; _ -> <fail clause> end`. A binary match cannot live in a
    /// guard, so a `case` is the faithful, recompilable surface.
    ...
    /// Builds the conditional for a `get_map_elements` whose fail branch is a real
    /// clause: `if is_map_key(K, Src) andalso ... -> #{K := V} = Src, Cont; true ->
    /// <fail clause> end`, recovering the `#{key := V}` head guard that the
    /// compiler split into a presence test plus extraction.
    ...
    /// binding fresh variables. This keeps the extracted values usable in guards
    /// (where `maps:get/2` is illegal) and recovers the `#{key := V}` surface.
    ...
    /// Reconstructs binary-matching function clauses when the entry block opens a
    /// match context. Each clause's binary pattern is the segment sequence its
    /// `bs_match` chain consumes; the chain's first fail label is the next
    /// clause's entry. Saved match positions (`bs_get_position`/`bs_set_position`)
    /// are tracked so a clause that rewinds shares the matched prefix and the
    /// trailing `ensure_exactly 0` directive marks an exact (non-`Rest`) pattern.
    ...
    /// Walks one binary-match clause starting at `label`. Segment matches grow the
    /// shared `all_segments`/`seg_vars` ledger (so rewinding clauses reuse the same
    /// pattern variables); saved positions index into that ledger. Returns the
    /// clause's full pattern, its lifted body, and every `bs_match` fail label.
    ...
    /// Recognizes a heap-GC retry stub: a block that only restores the tail and
    /// jumps back to an already-emitted clause, rather than starting a new clause.
    ...
    /// Lifts an in-body `bs_match` into a `Pattern = Subject` binding so the
    /// matched segment variables are bound for the rest of the clause (the helper
    /// recursion in lowered comprehensions relies on these bindings).
    ...
    /// Expands a compile-time `string` segment (bytes stored in the `StrT` chunk)
    /// into individual `Byte:8` integer segments, recovering the `<<1:8, ...>>`
    /// surface the compiler folded into a packed string.
    ...
    /// Converts the lifted handler dispatch (an `if Class =:= <c> -> ...` chain
    /// terminated by an implicit re-raise) into idiomatic `Class:Reason[:Stack]`
    /// catch clauses.
...
/// Splits a catch-handler guard into the matched class atom and any residual
/// guard. `Class =:= error` yields `("error", None)`; a conjunction keeps the
/// rest as a clause guard.
...
/// Whether any statement in `body` references the variable `name`. Used by clause
/// reconstruction to decline rewrites that would leave the dispatch argument
/// unbound in a clause head.
#[must_use]
...
/// Finalizes a binary-match clause's segment list. The match `source` and `ctx`
/// are threaded for completeness; the surface pattern is the segment sequence as
/// accumulated (an empty list renders as `<<>>`).
#[derive(Debug)]
struct BinaryClause {
    segments: Vec<BinSegment>,
    body: Vec<Stmt>,
...
/// Shared binary-match ledger across the clauses of one function. `all_segments`
/// holds every matched segment in offset order; `seg_vars` the pattern variable
/// bound to each; `pos_len` maps a saved-position register to its segment count.
#[derive(Debug)]
struct BinShared {
...
/// Closes a binary clause pattern: when the subject is not exhausted, binds the
/// match context to a named tail variable so `byte_size(ctx)` in the body recovers
/// as `byte_size(Rest)`, and appends the `Rest/binary` segment.
fn close_pattern(
    prefix: &[BinSegment],
    exact: bool,
    ctx: Option<Reg>,
...
/// Re-establishes the destination-register bindings for a retained match prefix
/// after a position rewind, so the clause body references the shared segment
/// variables rather than stale registers.
...
/// The exception class for a re-raise. The 2-arg `raise Trace Reason` op restores
/// the class from the cooked stacktrace; in a recovered catch handler that class
/// is the bound `Class` variable.
...
/// Whether an `erlang:` function may be called unqualified. The vast majority of
/// `erlang` BIFs are auto-imported, but a handful (`raise`, `raw_raise`,
/// `apply/2`, ...) are not and must keep the `erlang:` qualifier to recompile.
