...
fn tagged_tuple_pattern(guard: &Expr) -> Option<(Expr, Expr)> {
    ...
    let arity: u32 = arity.min(crate::chunks::MAX_FUN_ARITY);
    if arity == 0 {
        return None;
    }
    let arity: usize = usize::try_from(arity).ok()?;
    let mut elements: Vec<Expr> = Vec::with_capacity(arity);
    elements.push(tag);
    for _ in 1..arity {
...
mod tests {
    ...
    fn andalso(lhs: Expr, rhs: Expr) -> Expr {
        Expr::BinOp {
            op: "andalso".to_owned(),
            lhs: Box::new(lhs),
            rhs: Box::new(rhs),
    ...
    #[test]
    fn tagged_tuple_pattern_huge_arity_is_capped() {
        let subject: Expr = Expr::Var("X0".to_owned());
        let guard_expr: Expr = andalso(
            guard("is_tuple", vec![subject.clone()]),
            andalso(
                eq(
                    guard("tuple_size", vec![subject.clone()]),
                    Expr::Int(i64::from(u32::MAX)),
                ),
                eq(
                    guard("element", vec![Expr::Int(1), subject.clone()]),
                    Expr::Atom("tag".to_owned()),
                ),
            ),
        );
        let recovered: Option<(Expr, Expr)> = tagged_tuple_pattern(&guard_expr);
        assert!(recovered.is_some(), "expected tagged tuple pattern");
        let Some((pattern, value)): Option<(Expr, Expr)> = recovered else {
            return;
        };
        assert_eq!(value, subject);
        let elements: Vec<Expr> = match pattern {
            Expr::Tuple(elements) => elements,
            _ => Vec::new(),
        };
        assert!(!elements.is_empty(), "expected tuple pattern");
        assert_eq!(elements.len(), crate::chunks::MAX_FUN_ARITY as usize);
        assert_eq!(elements.first(), Some(&Expr::Atom("tag".to_owned())));
    }
}
