...
/// Rewrites the synthetic decision tree emitted by the lifter into idiomatic surface Erlang.
#[must_use]
...
/// Detects the compiled `Pattern = Subject` idiom.
...
/// Recovers a `{Tag, _, ..}` pattern from the compiled tuple-test sequence.
