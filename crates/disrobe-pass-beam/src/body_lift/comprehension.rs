...
struct ComprehensionShape {
    ...
    filters: Vec<Expr>,
    source_reg: String,
    /// Generator pattern arity when the generator destructures a tuple.
...
/// Analyzes a list-comprehension helper, recognizing the capture-free arity-1 shape.
...
/// Whether an expression references a register variable other than the recovered generator.
...
/// Whether any `hd(reg)` / `element(_, hd(reg))` reference survives the full substitution.
...
/// Detects a tuple-pattern generator guard.
...
/// Whether a `hd(<reg>)` reference survives substitution, signalling an unrecovered generator.
...
/// Rewrites every `helper(Src, ..captured)` call in a statement tree to the surface comprehension.
...
/// Builds the `[Elem || Pattern <- Src, Filters]` surface string.
...
/// The recovered generator head pattern: `G` for a plain generator, or
/// `{T0, .., Tn-1}` for a recovered tuple destructuring.
...
/// Substitutes the generator references throughout an expression.
...
/// Tuple-pattern substitution: `element(i+1, hd(reg))` -> `Ti`,
/// `hd(reg)` -> `{T0, .., Tn-1}`.
...
/// Applies `f` to every direct sub-expression, rebuilding the node. Variants
/// without sub-expressions are cloned unchanged.
