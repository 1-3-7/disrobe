...
/// Renders a statement sequence as an Erlang clause body at the given indent
/// (number of 4-space levels). Statements are comma-joined; the final statement
/// terminates the body without a trailing comma.
#[must_use]
...
/// Renders an expression where the Erlang grammar requires a primary (the base of
/// an `Expr#{...}` map update, or a binary-segment value/size). Literals, vars,
/// and bracketed constructors pass through; calls, operators, and other composite
/// expressions are parenthesized so the surrounding construct parses.
...
/// Renders a binary-segment size. Erlang permits only an integer or a variable
/// bare in size position; any other expression (call, arithmetic) is parenthesized.
...
/// Renders an arbitrary-precision integer from its little-endian magnitude bytes
/// via repeated base-256 -> base-10 long division, so values beyond 128 bits
/// (e.g. `999999999999999999999 * 999999999999999999999`) print exactly.
