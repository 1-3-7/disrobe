...
/// The modern OTP `{debug_info_v1, erl_abstract_code, {Forms, Opts}}` chunk must decode to source.
#[test]
...
/// `recover_erlang` on the real OTP-29 megafile routes through the abstract-code path.
#[test]
...
/// The abstract-form renderer must recover full bodies with original variable names.
#[test]
...
/// With the `Dbgi` chunk stripped, recovery falls back to the register-named core lift.
#[test]
...
/// In the no-Dbgi core-lift, an arity-1 capture-free list comprehension is re-sugared.
#[test]
...
/// Every lowered list-comprehension helper that is still called must remain present.
...
/// Extracts the first `'-...-lc$^N/M-K-'` quoted helper token in a line, if any.
