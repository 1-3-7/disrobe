...
const RANDOM_CASES: usize = 8_000;
const MUTATIONS_PER_SEED: usize = 1_200;
...
const BATCH_PATH_ENV: &str = "DISROBE_BEAM_FUZZ_BATCH";
const WORKSPACE_PATH_ENV: &str = "DISROBE_BEAM_FUZZ_WORKSPACE";
const WORKER_TOKEN_ENV: &str = "DISROBE_BEAM_FUZZ_TOKEN";
...
fn beam_seed() -> Vec<u8> {
    ...
    let form_len: u32 = u32::try_from(4 + chunks_blob.len()).map_or(u32::MAX, |value: u32| value);
...
fn deeply_nested_etf_seed() -> Vec<u8> {
...
fn exercise_entrypoints(bytes: &[u8]) {
    ...
    let quota: EzQuota = EzQuota::default();
    consume(EzArchive::parse_with_quota(bytes, quota));
    ...
    let term: Result<Term> = decode_etf(bytes);
    if let Ok(term) = term {
        exercise_term_entrypoints(&term);
    }

    let raw: Result<RawBeam> = RawBeam::parse(bytes);
    if let Ok(raw) = raw {
        consume(BeamFile::from_raw(&raw));
    }

    let beam: Result<BeamFile> = BeamFile::parse(bytes);
    if let Ok(beam) = beam {
        consume(lift(&beam));
        consume(recover_erlang(&beam));
        consume(symbolic_disassemble(&beam));
        if let Some(dbgi) = &beam.chunks.dbgi {
            exercise_term_entrypoints(&dbgi.term);
        }
        if let Some(docs) = &beam.chunks.docs {
            exercise_term_entrypoints(&docs.term);
        }
    }
}
...
impl TempWorkspace {
    fn create() -> std::io::Result<Self> {
        ...
        Err(std::io::Error::new(
            ...
            "unable to create unique beam fuzz workspace",
        ))
    }
...
fn workspace_dir() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "disrobe-beam-fuzz-{}-{sequence}",
        std::process::id()
    ))
}

fn build_cases() -> Vec<Vec<u8>> {
    let mut rng: Xorshift64 = Xorshift64::new(0x4245_414d_0010_0001);
...
#[test]
fn bounded_public_beam_parse_entrypoints_accept_malformed_inputs_without_panicking()
-> std::io::Result<()> {
