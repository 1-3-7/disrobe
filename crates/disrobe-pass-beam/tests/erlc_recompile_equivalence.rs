...
fn stripped_core_lift_is_recompile_equivalent() {
    ...
    assert_eq!(
        full_version, CI_OTP_VERSION,
        "the CI claim-backing gate requires OTP {CI_OTP_VERSION}, but the OTP_VERSION file reports {}",
        full_version
    );
    let measurement: CorpusMeasurement = measure_corpus(erlang);
    assert_eq!(
        measurement.release, "27",
        "OTP_VERSION reports {CI_OTP_VERSION}, but erlang:system_info(otp_release) reports major release {}",
        measurement.release
    );
    let bar: serde_json::Value = published_bar();
