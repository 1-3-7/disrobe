...
fn is_recovered(core: &CoreModule, name: &str, arity: u32) -> bool {
    find_fn(core, name, arity)
        .clauses
        .iter()
        .all(|c| c.body.fully_recovered)
}
...
#[test]
fn megafile_all_functions_fully_recovered() {
    let core: CoreModule = lift(&megafile!()).expect("lift");
    let total: usize = core.functions.len();
    let unrecovered: Vec<String> = core
        .functions
        .iter()
        .filter(|f: &&CoreFunction| !f.clauses.iter().all(|c| c.body.fully_recovered))
        .map(|f: &CoreFunction| format!("{}/{}", f.name, f.arity))
        .collect();
    assert!(
        unrecovered.is_empty(),
        "every one of {total} functions must be fully recovered; partials: {unrecovered:?}"
    );
    assert_eq!(
        total, 84,
...
fn megafile_binary_comprehension_helper_resugars_to_byte_match_recursion() {
    ...
    assert!(
        f.clauses.iter().all(|c| c.body.fully_recovered),
        "lbc helper must be fully recovered"
    );
}
...
fn megafile_bit_syntax_decode_reconstructs_binary_pattern_clauses() {
    ...
    assert!(is_recovered(&core, "bit_syntax_decode", 1));
}
...
fn megafile_bit_syntax_encode_recovers_literal_string_segment() {
    ...
    assert!(is_recovered(&core, "bit_syntax_encode", 1));
}
...
fn megafile_error_with_stacktrace_recovers_try_catch_class_and_stack() {
    ...
    assert!(is_recovered(&core, "error_with_stacktrace", 0));
}
...
fn megafile_catch_old_school_recovers_catch_value_binding() {
    ...
    assert!(is_recovered(&core, "catch_old_school", 1));
}
...
fn megafile_dets_demo_resugars_badmatch_chain_into_match_bindings() {
    ...
    assert!(is_recovered(&core, "dets_demo", 0));
}
...
fn megafile_nested_map_match_recovers_map_key_patterns() {
    ...
    assert!(is_recovered(&core, "nested_map_match", 1));
}
...
fn megafile_handle_cast_recovers_record_update_via_setelement() {
    ...
    assert!(is_recovered(&core, "handle_cast", 2));
}
...
fn megafile_map_update_declines_unsafe_clause_split_keeps_map_pattern() {
    ...
    assert!(is_recovered(&core, "map_update", 1));
}
...
fn megafile_exception_chain_recovers_throw_error_exit() {
    ...
    assert!(is_recovered(&core, "exception_chain", 1));
}
