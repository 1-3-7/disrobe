...
impl<'a> BodyCtx<'a> {
    ...
    fn lift(mut self) -> LiftOutput {
        let mut i: usize = 0usize;
        while i < self.lines.len() {
            if let Some(consumed) = self.try_match_at(i) {
                i += consumed.max(1);
                continue;
            }
            ...
            if !should_skip(line) {
                self.unrecognized.push(line.to_owned());
            }
            i += 1;
        }
        LiftOutput {
    ...
    fn try_match_at(&mut self, i: usize) -> Option<usize> {
        if let Some((stmt, consumed, nested)) = self.try_if_block(i) {
            self.stmts.push(stmt);
            self.unrecognized.extend(nested);
        ...
        if let Some((stmt, consumed, nested)) = self.try_for_loop(i) {
            self.stmts.push(stmt);
            self.unrecognized.extend(nested);
            return Some(consumed);
        }
    ...
    fn try_if_block(&self, start: usize) -> Option<(PythonStmt, usize, Vec<String>)> {
        ...
        let yes_out: LiftOutput = self.lift_range(yes_start + 1, no_start.min(self.lines.len()));

        let yes_ends_with_return: bool = yes_out
            .stmts
            .last()
        ...
        if yes_ends_with_return {
            ...
            Some((stmt, consumed, yes_out.unrecognized))
        } else {
            let no_out: LiftOutput = self.lift_range(no_start + 1, self.lines.len());
            let no_end: usize = find_end_of_branch(self.lines, no_start + 1)
                .unwrap_or_else(|| self.lines.len().saturating_sub(1));
            let mut nested: Vec<String> = yes_out.unrecognized;
            nested.extend(no_out.unrecognized);
            let stmt: PythonStmt = PythonStmt::If {
                test,
                body: yes_out.stmts,
                orelse: no_out.stmts,
            ...
            Some((stmt, consumed, nested))
        }
    }

    fn lift_range(&self, from: usize, to: usize) -> LiftOutput {
    ...
    fn try_for_loop(&self, start: usize) -> Option<(PythonStmt, usize, Vec<String>)> {
        let line: &str = self.lines[start].trim();
        ...
        let body_out: LiftOutput = self.lift_range(loop_start_idx + 1, loop_end_idx);
        ...
        Some((stmt, consumed, body_out.unrecognized))
    }
...
fn should_skip(line: &str) -> bool {
    ...
    let skip_prefixes: &[&str] = &[
        ...
        "static PyObject *impl_",
        "static PyObject *MAKE_FUNCTION_",
        "if (cache_frame_",
        "if (frame_",
        "if (exception_tb",
...
fn is_scratch_assignment(t: &str) -> bool {
    ...
    let scratch: bool = lhs.starts_with("tmp_")
        || lhs.starts_with("var_")
        || lhs.starts_with("par_")
        || lhs.starts_with("exception_")
        || lhs.starts_with("type_description");
    scratch && !lhs.contains('(') && !lhs.contains('[')
}
...
fn is_attach_locals_fragment(t: &str) -> bool {
    ...
    let is_ident: bool = !core.is_empty()
        && core
            .chars()
            .all(|c: char| c.is_ascii_alphanumeric() || c == '_');
    is_ident
        && (core.starts_with("par_") || core.starts_with("var_") || core.starts_with("tmp_"))
        && (t.ends_with(',') || t.ends_with(';') || t == core)
}
...
pub fn lift_body_detailed(c_body: &str, params: &[String], pool: &ConstantsPool) -> BodyLift {
    ...
    let out: LiftOutput = ctx.lift();
    let _ = params;

    let fidelity: LiftFidelity = if out.stmts.is_empty() {
        LiftFidelity::Skeleton
    } else if !out.unrecognized.is_empty() || out.stmts.iter().any(stmt_has_unresolved) {
    ...
    BodyLift {
        stmts: out.stmts,
        fidelity,
        unrecognized_lines: out.unrecognized,
    }
}
...
mod tests {
    ...
    #[test]
    fn unrecognized_semantic_line_downgrades_fullbody_to_partial() {
        let body: &str = r"{
...
goto frame_return_exit_1;
        ...
        assert_eq!(
            lift.fidelity,
            LiftFidelity::PartialBody,
            "a dropped semantic C line must downgrade fidelity, never stay FullBody"
        );
        assert_eq!(lift.unrecognized_lines.len(), 1);
        assert!(
            lift.unrecognized_lines[0].contains("SOME_UNMODELED_NUITKA_HELPER"),
            "the dropped construct must be recorded, not silently skipped"
        );
    }

    #[test]
    fn fully_scaffolded_body_stays_fullbody_with_zero_unrecognized() {
        let body: &str = r"{
CHECK_OBJECT(par_n);
tmp_return_value = par_n;
...
goto frame_return_exit_1;
        ...
        let pool: ConstantsPool = ConstantsPool::default();
        let lift: BodyLift = lift_body_detailed(body, &[], &pool);
        assert!(lift.unrecognized_lines.is_empty());
