//! Real Nuitka `--onefile` payload extraction.
//!
//! Format reverse-engineered from Nuitka 4.1.1 `OnefileBootstrap.c` (the runtime
//! reader) and `tools/onefile_compressor/OnefileCompressor.py` (the writer), then
//! verified byte-for-byte against a real Windows onefile build.
//!
//! Layout: the payload is a 3-byte header `KA` + a compression indicator
//! (`X` = stored, `Y` = zstd), followed by the entry stream. With compression the
//! whole entry stream is one (or, for multi-threaded encodes, several concatenated)
//! zstd frame(s) - NOT per-file framing, unless the build used `--onefile-as-archive`.
//!
//! Per entry, in the (decompressed) stream:
//! - filename: NUL-terminated, UTF-16LE on Windows builds, UTF-8 elsewhere.
//! - POSIX builds only: a 1-byte flags field directly after the name; if `flags & 2`
//!   the entry is a symlink and a second NUL-terminated target name follows with no
//!   size/data.
//! - `u64` little-endian file size.
//! - optional `u32` little-endian CRC32 - present only for cached-mode builds
//!   (`--onefile-cache-mode=cached`); absent for the default temp-dir mode.
//! - the file bytes (`size` of them).
//! - the stream ends with an empty filename (a lone NUL unit).
//!
//! Because CRC presence is not encoded in the payload, both layouts are attempted and
//! the one that walks cleanly to the terminator (consuming the whole stream) is taken.

use std::io::Read;

use crate::error::{Error, Result};

/// Hard ceiling on a single contained file, guarding against a corrupt/adversarial
/// size field before any allocation keyed on it.
...
/// Decompression-bomb guard: a compressed payload may not expand beyond this multiple
/// of its compressed size, nor beyond [`MAX_DECOMPRESSED_ABS`] in absolute terms.
...
/// Longest filename (in bytes) accepted before declaring the stream malformed; the
/// bootstrap's own buffer is 1024 `filename_char_t` units.
...
/// Decode a Nuitka `--onefile` payload that begins at `payload_offset` (the `K` of the
/// `KA` magic) within `image`.
...
///
/// Three facts drive this loop, all verified against a real Nuitka 4.1.1 onefile:
/// - The default one-shot decoder enables cross-frame reading, which treats the COFF
///   blob's trailing alignment padding as a malformed second frame and fails with
///   "Unknown frame descriptor" - the exact symptom the prior parser hit.
/// - The multi-threaded encoder may emit several concatenated frames, so a single
///   `single_frame()` decode is insufficient.
/// - The embedded blob is padded after the final frame, so the loop must stop at the
///   first tail that does not begin a fresh zstd (or skippable) frame rather than error.
...
/// Whether `bytes` begins a regular zstd frame or a skippable frame (magics
/// `0x184D2A50..=0x184D2A5F`), used to distinguish a real frame from trailing padding.
#[inline]
...
/// Walk the decompressed entry stream, selecting the layout (encoding × checksum-present)
/// whose walk consumes the entire stream and terminates on the empty filename.
...
/// Attempt one layout, returning the entries and the number of bytes consumed (which a
/// caller compares against the stream length to validate the layout choice).
fn try_walk(
    stream: &[u8],
    encoding: FilenameEncoding,
    has_checksums: bool,
...
/// Read a NUL-terminated filename in the given encoding, returning the decoded string and
/// the cursor just past the terminator. Rejects names whose code units are not plausible
/// path characters, which lets the layout autodetector discard a mis-aligned walk.
...
mod tests {
    ...
    /// Build an uncompressed Windows-layout (`KAX`, UTF-16LE, no checksum) payload that is
    /// byte-identical to what Nuitka's writer emits for the default temp-dir onefile mode.
