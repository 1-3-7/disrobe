//! Shared, validated locator for a Nuitka `--onefile` payload inside an image.
//!
//! The payload magic `KA` + indicator (`X`/`Y`) can occur coincidentally inside an
//! arbitrary binary, so a bare magic scan (as the previous code did) yields false
//! positives. Each candidate is validated: a `KAY` candidate must be immediately followed
//! by a real zstd frame magic, and a `KAX` candidate must begin a plausible UTF-16LE or
//! UTF-8 filename. On Windows builds the blob lives in a `.rdata` section rather than as an
//! EOF overlay, so there is no reliable trailing-size record to trust; the validated
//! forward scan is the portable ground truth and matches what the bootstrap consumes.
...
/// A stored (`KAX`) payload begins with a filename; require the first 1-2 code units to be
/// a plausible printable path start in either encoding, with at least one terminator
/// reachable in a bounded window. An empty payload (immediate terminator) is also valid.
#[inline]
