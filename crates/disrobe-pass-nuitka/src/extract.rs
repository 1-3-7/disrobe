...
mod tests {
    ...
    fn extract_onefile_kax_returns_payload() {
        ...
        let mut onefile_payload: Vec<u8> = b"KAX".to_vec();
        onefile_payload.extend_from_slice(&synth_entry("hello.txt", 0x00, b"hi"));
        onefile_payload.extend_from_slice(&[0u8, 0u8]);
        bytes.extend_from_slice(&onefile_payload);
        bytes.extend_from_slice(b"loadConstantsBlob");
        bytes.extend_from_slice(b"Nuitka_FunctionObject");
