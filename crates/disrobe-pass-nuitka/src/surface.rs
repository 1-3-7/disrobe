...
use crate::body::{LiftFidelity, PythonExpr, PythonStmt, extract_impl_body_text, lift_body};
...
pub fn build_surface(
    ...
    for body in bodies {
        ...
        let (body_stmts, lift_fidelity): (Vec<PythonStmt>, LiftFidelity) = c_source
            .and_then(|src: &str| {
            ...
            .map_or_else(
                || (Vec::new(), LiftFidelity::Skeleton),
                |slice: &str| lift_body(slice, &body.params, pool),
            );
        let body_recovered: bool = !body_stmts.is_empty();
