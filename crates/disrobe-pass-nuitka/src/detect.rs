...
const PYTHON_ABI_MAJORS: [u8; 2] = [3, 2];
const MAX_PYTHON_ABI_MINOR: u8 = 20;
...
pub(crate) fn find_python_version_strings(bytes: &[u8]) -> Option<(u8, u8)> {
    find_python_runtime_name(bytes).or_else(|| find_python_version_marker(bytes))
}

fn find_python_runtime_name(bytes: &[u8]) -> Option<(u8, u8)> {
    for major in PYTHON_ABI_MAJORS {
        let major: u8 = major;
        for minor in (0u8..=MAX_PYTHON_ABI_MINOR).rev() {
            let minor: u8 = minor;
            if python_runtime_marker_present(bytes, major, minor) {
...
fn find_python_version_marker(bytes: &[u8]) -> Option<(u8, u8)> {
    let mut found: Option<(u8, u8)> = None;
    for major in PYTHON_ABI_MAJORS {
        let major: u8 = major;
        for minor in (0u8..=MAX_PYTHON_ABI_MINOR).rev() {
            let minor: u8 = minor;
            if python_version_marker_present(bytes, major, minor) {
                let pair: (u8, u8) = (major, minor);
                match found {
...
fn python_runtime_marker_present(bytes: &[u8], major: u8, minor: u8) -> bool {
    let markers: [String; 9] = [
        format!("python{major}{minor}.dll"),
        format!("Python{major}{minor}.dll"),
        format!("python{major}.{minor}.dll"),
        format!("Python{major}.{minor}.dll"),
        format!("libpython{major}.{minor}.so"),
        format!("libpython{major}.{minor}.dylib"),
        format!("Python.framework/Versions/{major}.{minor}/Python"),
        format!("Python.framework\\Versions\\{major}.{minor}\\Python"),
        format!("/lib/python{major}.{minor}/"),
    ];
    markers
        .iter()
        .any(|marker: &String| marker_present_ascii_or_utf16(bytes, marker.as_str()))
}

fn python_version_marker_present(bytes: &[u8], major: u8, minor: u8) -> bool {
    let markers: [String; 4] = [
        format!("Python {major}.{minor}."),
        format!("PY_VERSION={major}.{minor}."),
        format!("PYTHON_VERSION={major}.{minor}."),
        format!("cpython-{major}.{minor}."),
    ];
    markers
        .iter()
        .any(|marker: &String| marker_present_ascii_or_utf16(bytes, marker.as_str()))
...
fn marker_present_ascii_or_utf16(bytes: &[u8], marker: &str) -> bool {
    if find_subslice(bytes, marker.as_bytes()).is_some() {
        return true;
    }
    let marker_utf16: Vec<u8> = utf16le_bytes(marker);
    find_subslice(bytes, &marker_utf16).is_some()
}

fn utf16le_bytes(text: &str) -> Vec<u8> {
    let mut bytes: Vec<u8> = Vec::with_capacity(text.len().saturating_mul(2));
