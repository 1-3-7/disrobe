//! Nuitka build detection from real binary contents.
//!
//! Signatures were frozen from a real Nuitka 4.1.1 build on Python 3.14: a `--standalone`
//! exe, a `--module` `.pyd`, and a `--onefile` exe. The onefile bootstrap is a thin C
//! launcher that contains almost none of the compiled-Python C-API symbols, so it is
//! recognised by its runtime env-var names plus a validated `KA[XY]` payload; the
//! standalone/module binaries are recognised by the Nuitka loader/runtime strings that
//! actually survive into release output.
...
/// Strings verified present in a real Nuitka 4.1.1 `--standalone` exe and `--module` pyd.
/// The invented C-API symbol names used previously (`Nuitka_FunctionObject`,
/// `loadConstantsBlob`, `MAKE_FUNCTION_`, …) do not survive optimised compilation and were
/// removed.
...
/// Env-var names baked into the onefile bootstrap C (`OnefileBootstrap.c`), present in a
/// real `--onefile` exe whose payload-side Nuitka strings live only inside the compressed
/// blob.
