...
/// Hand-build a minimal but valid PE32+ standalone image with a multi-hundred-MB `.text` and a
/// `.rdata` carrying the Nuitka standalone marker strings. The `object` crate's PE *writer* does
/// not support arbitrary section emission, so the headers are laid out by hand; `object::File::parse`
/// and disrobe's PE reader both accept the result. No onefile KA payload is present, so it routes as
/// standalone.
