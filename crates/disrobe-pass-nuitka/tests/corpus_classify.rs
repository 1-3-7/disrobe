...
/// Resolve the real onefile fixture, building it on the fly when Nuitka is on PATH and the
/// fixture is absent. Returns `None` only when neither the fixture nor Nuitka is available,
/// in which case the caller hard-ignores (never silently passes).
...
/// The decisive end-to-end proof: decode a REAL Nuitka onefile payload and assert recovered
/// file names, sizes, and content. This is not a synthetic round-trip - it reads the actual
/// shipped `corpus/.../onefile/hello.exe` and must extract genuine embedded files.
#[test]
