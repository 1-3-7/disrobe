...
//! Fast, always-on tests against small checked-in artifacts derived byte-for-byte from a
//! real Nuitka 4.1.1 `--onefile` payload. The `.kax` fixture is the real decompressed entry
//! stream with each file body truncated to its leading bytes (and sizes adjusted to match),
//! so it exercises the genuine entry walker without committing multi-megabyte PE images.
