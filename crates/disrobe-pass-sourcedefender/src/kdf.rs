...
mod tests {
    ...
    #[test]
    fn derive_key_self_consistency_regression_snapshot_not_upstream_vectors() {
        let snapshots: [(&str, &str); 4] = [
            (
                "mymod",
        ...
        for (basename, snapshot_hex) in snapshots {
            let derived: DerivedKey = derive_aes_key(basename).unwrap_or(DerivedKey([0u8; 32]));
            assert_eq!(
                crate::codec::hex_encode(derived.as_bytes()),
                snapshot_hex,
                "regression snapshot of our own blake2b(key,salt) output for {basename}; \
                 these are self-generated and pin determinism only -- they are NOT \
                 upstream sourcedefender known-answer vectors and cannot validate the \
                 algorithm against the real .pye format (no real corpus available)",
            );
        }
    }
}
