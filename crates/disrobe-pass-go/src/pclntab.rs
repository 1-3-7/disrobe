...
/// Strategy-2 scan stride: pointer-aligned (8).
const SIG_SCAN_STEP: usize = 8;

/// Evaluate one header position under one magic, keeping the best by resolvable-name count.
...
/// Validate the bytes at `data[pos..]` as a pclntab header under `magic`.
...
/// Count how many of the first [`SIG_NAME_SAMPLE`] functab entries resolve to a plausible symbol name.
...
/// Whether a NUL-terminated funcname entry is a printable, separator-bearing symbol.
...
/// Like [`parse_header`] but tolerant of malformed fields and with a forced version.
...
/// Structural acceptance test that rejects coincidental magic runs.
...
/// Whether the funcname table opens with a printable, separator-bearing Go symbol.
...
/// Whether a Go funcname carries a package or pseudo-symbol separator (`.`, `/`, or `:`).
