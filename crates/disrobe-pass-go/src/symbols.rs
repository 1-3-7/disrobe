...
fn parse_go12(
    ...
    let n: usize = usize::try_from(header.n_funcs).unwrap_or(0);
    let tab_off: usize = 8 + ps;
    let stride: usize = 2 * ps;
    if n == 0 || tab_off + n.saturating_mul(stride) > body.len() {
        return Ok(());
    }
    for i in 0..n {
        ...
        let Ok(funcoff) = usize::try_from(funcoff_word) else {
            continue;
        };
...
fn parse_go116(
    ...
    let ftab_size: usize = (n + 1) * stride;
    if ftab_off + ftab_size > body.len() {
        return Ok(());
    }
    for i in 0..n {
        ...
        let Ok(off_native) = usize::try_from(funcoff_word) else {
            continue;
        };
        ...
        let name: String = read_cstring(body, funcname_off + nameoff as usize);
        if name.is_empty() {
            continue;
        }
        record_package(&name, packages);
...
fn parse_go118_plus(
    ...
    let stride: usize = 8;
    let ftab_size: usize = (n + 1) * stride;
    if funcdata_off + ftab_size > body.len() {
        return Ok(());
    }
    for i in 0..n {
...
fn collect_files_go116(
    ...
    let ftab_off: usize = usize::try_from(header.filetab_off).unwrap_or(0);
    let n: usize = usize::try_from(header.n_files).unwrap_or(0);
    if ftab_off == 0 || n == 0 || ftab_off + n * 4 > body.len() {
        return;
    }
    for i in 0..n {
        let pos: usize = ftab_off + i * 4;
        let Ok(off) = read_u32(body, pos, header.endian) else {
            continue;
        };
        let path: String = read_cstring(body, ftab_off + off as usize);
        if !path.is_empty() {
            files.insert(path);
        }
    }
