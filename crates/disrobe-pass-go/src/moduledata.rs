...
fn walk_moduledata(image: &GoImage<'_>, base: u64, via: ModuledataSource) -> Option<Moduledata> {
    ...
    let text_va: u64 = image.read_ptr(base.wrapping_add(ps * 22)).unwrap_or(0);
    let etext_va: u64 = image.read_ptr(base.wrapping_add(ps * 23)).unwrap_or(0);
    let types_va: u64 = image.read_ptr(base.wrapping_add(ps * 35)).unwrap_or(0);
    let etypes_va: u64 = image.read_ptr(base.wrapping_add(ps * 36)).unwrap_or(0);
    let typelinks_va: u64 = image.read_ptr(base.wrapping_add(ps * 41)).unwrap_or(0);
    let typelinks_len: u64 = image.read_ptr(base.wrapping_add(ps * 42)).unwrap_or(0);
    let itablinks_va: u64 = image.read_ptr(base.wrapping_add(ps * 44)).unwrap_or(0);
    let itablinks_len: u64 = image.read_ptr(base.wrapping_add(ps * 45)).unwrap_or(0);
    Some(Moduledata {
        pclntab_va,
        typelinks_va,
        typelinks_len,
