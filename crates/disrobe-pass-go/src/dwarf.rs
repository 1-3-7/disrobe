...
/// Recover parameter/local/type-parameter names from a binary's DWARF.
///
/// Handles both plain `.debug_*` and Go's legacy zlib-compressed `.zdebug_*` (a
/// `"ZLIB"` magic + 8-byte big-endian length + zlib stream). Returns an absent report
/// when no debug sections survive (e.g. `-ldflags=-w`), never erroring on malformed
/// debug data — DWARF recovery is strictly additive.
#[must_use]
