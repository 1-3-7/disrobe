...
fn walk_embed_files(image: &GoImage<'_>) -> Vec<EmbedFile> {
    ...
    for sec in &image.sections {
        ...
        scan_section_for_runs(image, sec, &mut acc);
    }
...
fn scan_section_for_runs(
    ...
    let mut pos: u64 = 0;
    while pos + EMBED_FILE_STRIDE <= len {
        let Some(first) = read_embed_entry(image, base + pos) else {
            pos += 8;
            continue;
        };
        let mut run: Vec<EmbedEntry> = vec![first];
        let mut k: u64 = pos + EMBED_FILE_STRIDE;
        while k + EMBED_FILE_STRIDE <= len && run.len() < MAX_RUN_ENTRIES {
            let Some(entry) = read_embed_entry(image, base + k) else {
                break;
            };
            run.push(entry);
            k += EMBED_FILE_STRIDE;
        ...
        if run.len() >= 2 && is_embed_run(&run) {
            for entry in &run {
                let preview: String = if entry.is_dir {
                    String::new()
                } else {
                    read_preview(image, entry.data_va, entry.data_len)
                };
...
fn read_embed_entry(image: &GoImage<'_>, va: u64) -> Option<EmbedEntry> {
    ...
    let name_len: u64 = image.read_ptr(va.wrapping_add(8))?;
    let data_ptr: u64 = image.read_ptr(va.wrapping_add(16))?;
    let data_len: u64 = image.read_ptr(va.wrapping_add(24))?;
...
fn read_preview(image: &GoImage<'_>, data_va: u64, data_len: u64) -> String {
    ...
    let Some(bytes) = image.data_at_va(data_va, take) else {
        return String::new();
    };
