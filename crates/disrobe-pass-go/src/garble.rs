...
/// Per-scheme counts of literals recovered by static byte-stream deobfuscation.
...
/// One of garble's `simple` byte transforms (XOR/ADD/SUB).
...
/// Recover literals under a single-byte transform, accepting only a sharp outlier key.
...
/// True when `best_hits` dominates the key histogram by a wide margin.
...
/// Recover literals under repeating-key XOR (period 2..=8), accepting only a sharp outlier key.
...
/// Common English words and Go-ish tokens a decoded literal must contain to be committed.
