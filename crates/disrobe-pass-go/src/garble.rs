...
pub struct GarbleReport {
    ...
    pub renamed_stdlib_hits: usize,
    pub seed_hash: Option<String>,
    pub renamed_funcs: BTreeMap<String, String>,
...
pub fn analyze(image: &GoImage<'_>, syms: &GoSymbols) -> GarbleReport {
    ...
    let renamed: BTreeMap<String, String> = recover_renamed_stdlib(syms);
    let recovered_strings: Vec<String> = recover_strings(image);
    let renamed_stdlib_hits: usize = renamed.len();
    let total_stdlib: usize = STDLIB_FINGERPRINT_NAMES.len();
    let quality: GarbleQuality = match (detection_score, renamed_stdlib_hits) {
        (0, _) => GarbleQuality::None,
        (s, n) if s >= 4 && n * 2 >= total_stdlib => GarbleQuality::Full,
        (s, n) if s >= 2 && n > 0 => GarbleQuality::Partial,
    ...
    GarbleReport {
        ...
        detection_score,
        renamed_stdlib_hits,
        seed_hash: extract_seed_hash(image),
        renamed_funcs: renamed,
        recovered_strings,
    }
}
...
fn recover_renamed_stdlib(syms: &GoSymbols) -> BTreeMap<String, String> {
    ...
    let observed: BTreeSet<&str> = syms
        ...
        .collect();
    for canonical in STDLIB_FINGERPRINT_NAMES {
        if observed.contains(canonical) {
            out.insert((*canonical).to_owned(), (*canonical).to_owned());
        }
    }
    out
}
