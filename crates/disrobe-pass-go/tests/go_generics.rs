...
/// The generics fixture instantiates `Sum[int]`, `Sum[float64]`,
/// `MapKeys[string,int]`, and the `Box[T].Describe` method for `int`/`string`. With
/// inlining disabled these land in the pclntab funcname table, so the structured
/// recovery must surface them with the correct base and (shape) type-args.
#[test]
