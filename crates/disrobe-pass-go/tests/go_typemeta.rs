...
/// Asserts the embed fixture's typelinks expose the `embed.*` type family and an `*embed.FS` itab.
#[test]
fn typemeta_recovers_embed_types_on_embed_fixture() {
    let Some(bytes): Option<Vec<u8>> = common::fixture_or_skip(common::HELLO_EMBED) else {
        return;
    };
