...
/// A binary built without `-ldflags=-w` keeps DWARF (Go emits the zlib-compressed
/// `.zdebug_*` form on Windows). Recovery must decompress it, surface DWARF v5
/// compile units, and pull parameter/local/type-parameter names the pclntab lacks.
#[test]
