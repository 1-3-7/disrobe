...
fn locate_cpython312() -> Option<CPython312> {
    ...
    for (program, args) in candidates {
        ...
        let ver: &str = parts.next().map_or("", |value: &str| value);
        let magic_hex: &str = parts.next().map_or("", |value: &str| value);
...
fn compile_pyc_zipper_wrapper_marshal(py: &CPython312, method: &str) -> Option<Vec<u8>> {
    let src: &str = REFERENCE_SOURCE;
    let name: &str = REFERENCE_FILENAME;
    let code: String = format!(
        "import marshal,sys,zlib,bz2,lzma\n\
         src={src:?}\n\
         orig=compile(src,{name:?},'exec')\n\
         body=marshal.dumps(orig)\n\
         method={method:?}\n\
         if method=='zlib':\n\
        \x20   packed=zlib.compress(body,9); call='import zlib, marshal\\nexec(marshal.loads(zlib.decompress(__p__)))\\n'\n\
         elif method=='bz2':\n\
        \x20   packed=bz2.compress(body,9); call='import bz2, marshal\\nexec(marshal.loads(bz2.decompress(__p__)))\\n'\n\
         else:\n\
        \x20   packed=lzma.compress(body); call='import lzma, marshal\\nexec(marshal.loads(lzma.decompress(__p__)))\\n'\n\
         wrapper_src=call.replace('__p__', repr(packed))\n\
         wrapper=compile(wrapper_src, '<pyc-zipper>', 'exec')\n\
         sys.stdout.buffer.write(marshal.dumps(wrapper))",
    );
    py.run_capture(&code)
}
...
#[test]
fn pi_carchive_pyc_zipper_layer_recovers_original_bytecode() {
    let Some(py): Option<CPython312> = locate_cpython312() else {
        eprintln!(
            "SKIP: on-box CPython 3.12 not found; cannot build a genuine pyc-zipper-wrapped CArchive"
        );
        return;
    };
    ...
    for method in ["zlib", "bz2", "lzma"] {
        let wrapper_marshal: Vec<u8> = compile_pyc_zipper_wrapper_marshal(&py, method)
            .unwrap_or_else(|| panic!("cpython must build a {method} pyc-zipper wrapper"));

        let entries: Vec<CArchiveEntry> = vec![CArchiveEntry {
            type_byte: b's',
            name: "hello",
            payload: wrapper_marshal,
        }];
        let archive: Vec<u8> = assemble_carchive(&entries);

        let output: ExtractOutput =
            extract_archive(&archive).expect("extract the pyc-zipper-wrapped carchive");
        assert_eq!(
            output.pyc_unzipped_count, 1,
            "the {method} pyc-zipper layer must be peeled off exactly once"
        );
        ...
        assert!(
            script.pyc_unzipped,
            "the script entry must be flagged as pyc-zipper-unwrapped for {method}"
        );

        let recovered_body: &[u8] = recovered_marshal_body(script);
        assert_eq!(
            recovered_body,
            oracle_marshal.as_slice(),
            "the recovered .pyc body after peeling the {method} pyc-zipper layer must byte-match an INDEPENDENT cpython recompile of the original source"
        );
    }
}

fn build_base_library_zip(py: &CPython312) -> Option<Vec<u8>> {
    let src: &str = REFERENCE_SOURCE;
    let name: &str = REFERENCE_FILENAME;
    let code: String = format!(
        "import io,marshal,zipfile,importlib.util,sys\n\
         src={src:?}\n\
         co=compile(src,{name:?},'exec')\n\
         body=marshal.dumps(co)\n\
         pyc=importlib.util.MAGIC_NUMBER + b'\\x00'*12 + body\n\
         buf=io.BytesIO()\n\
         z=zipfile.ZipFile(buf,'w')\n\
         z.writestr('_bootstrap_alpha.pyc', pyc, compress_type=zipfile.ZIP_DEFLATED)\n\
         z.writestr('encodings/__init__.pyc', pyc, compress_type=zipfile.ZIP_STORED)\n\
         z.close()\n\
...
fn cpython_marshal_loads_typename(py: &CPython312, body: &[u8]) -> Option<String> {
    const HEX: &[u8; 16] = b"0123456789abcdef";

    let mut hex: String = String::with_capacity(body.len().checked_mul(2)?);
    for &byte in body {
        hex.push(char::from(HEX[usize::from(byte >> 4)]));
        hex.push(char::from(HEX[usize::from(byte & 0x0f)]));
    }
    let code: String = format!(
        "import marshal,binascii,sys; d=binascii.unhexlify({hex:?}); co=marshal.loads(d); sys.stdout.write(type(co).__name__)",
    );
    let raw: Vec<u8> = py.run_capture(&code)?;
    Some(String::from_utf8_lossy(&raw).trim().to_owned())
}

#[test]
fn pi_carchive_base_library_zip_modules_surface_and_marshal_load() {
    let Some(py): Option<CPython312> = locate_cpython312() else {
        eprintln!(
            "SKIP: on-box CPython 3.12 not found; cannot build a genuine base_library.zip of real .pyc members"
        );
        return;
    };
    ...
    let oracle_marshal: Vec<u8> = compile_reference_marshal(&py)
        .expect("independent cpython 3.12 recompile of the reference must succeed");
    let base_library_zip: Vec<u8> =
        build_base_library_zip(&py).expect("cpython must build a real base_library.zip");
    assert_eq!(
        &base_library_zip[..2],
        b"PK",
        "the fixture must be a real pkzip archive produced by cpython zipfile"
    );
    ...
    let entries: Vec<CArchiveEntry> = vec![
        CArchiveEntry {
            type_byte: b's',
            name: "hello",
            payload: compile_reference_marshal(&py).expect("marshal the reference script"),
        },
        CArchiveEntry {
            type_byte: b'x',
            name: "base_library.zip",
            payload: base_library_zip,
        },
    ];
    let archive: Vec<u8> = assemble_carchive(&entries);

    let output: ExtractOutput =
        extract_archive(&archive).expect("extract the carchive carrying base_library.zip");

    assert_eq!(
        output.base_library_module_count, 2,
        "both bootstrap .pyc members inside base_library.zip must be surfaced (was 0 before this pass)"
    );

    let still_opaque: bool = output.entries.iter().any(|e: &ExtractedEntry| {
        e.toc.name == "base_library.zip" && e.toc.entry_type == EntryType::Data
    });
    assert!(
        still_opaque,
        "the original base_library.zip data entry must be preserved unchanged (additive-safe)"
    );

    let expected_names: [&str; 2] = [
        "base_library.zip_extracted/_bootstrap_alpha.pyc",
        "base_library.zip_extracted/encodings/__init__.pyc",
    ];
    for expected in expected_names {
        let member: &ExtractedEntry = output
            .entries
            .iter()
            .find(|e: &&ExtractedEntry| e.toc.name == expected)
            .unwrap_or_else(|| panic!("base_library member '{expected}' must surface as an entry"));
        assert!(
            member.toc.entry_type.is_pyc_carrier(),
            "surfaced base_library member '{expected}' must be a pyc carrier"
        );
        let recovered_body: &[u8] = recovered_marshal_body(member);
        assert_eq!(
            recovered_body,
            oracle_marshal.as_slice(),
            "the recovered marshal body of '{expected}' must byte-match an INDEPENDENT cpython recompile of the reference source (non-circular oracle)"
        );
        let typename: String = cpython_marshal_loads_typename(&py, recovered_body)
            .expect("cpython must marshal-load the recovered body");
        assert_eq!(
            typename, "code",
            "real cpython marshal.loads of the recovered '{expected}' body must yield a code object"
        );
    }
    ...
    assert!(
        output
            .bare_pyc_paths
            .iter()
            .any(|p: &String| p == "base_library.zip_extracted/_bootstrap_alpha.pyc"),
        "surfaced base_library members must be routed into bare_pyc_paths for py-decompile"
    );
}
...
fn parse_version_string(text: &str) -> Option<(u32, u32)> {
    let first_line: &str = text.lines().next().map_or(text, |value: &str| value);
