...
pub enum Error {
    ...
    #[error(
        "DR-PYINST-0006: AES decryption failed (wrong key or unsupported legacy encrypted build)"
    )]
    DecryptFailed,
    ...
    #[error(
        "DR-PYINST-0011: PYZ pyc-magic {0:#010x} maps to no known Python version; refusing to guess a marshal format"
    )]
    UnknownPyzMagic(u32),

    #[error(
        "DR-PYINST-0012: no pyc magic known for Python {major}.{minor}; refusing to stamp a wrong (e.g. 3.12) header"
    )]
    UnknownPycVersion { major: u8, minor: u8 },

    #[error("DR-PYINST-0013: input file is {len} bytes, exceeding the {limit} byte read cap")]
    InputFileTooLarge { len: u64, limit: u64 },
}
