...
//!
//! A component is loaded only when every gate passes, evaluated in order:
//!
//! 1. The detached minisign signature parses and was produced by the trusted key.
//! 2. The signature verifies over the exact component bytes.
//! 3. The component validates and compiles under wasmtime's component model.
//! 4. Every WIT capability the component imports is granted by the manifest.
//!
//! Ground truth for each gate is an independent library: [`minisign`] for the
//! cryptography and [`wasmtime`] for component validation and import
//! introspection. The loader never trusts a format it emits itself.
...
pub enum LoaderError {
    /// The signature failed to parse or did not verify over the component bytes
    /// under the trusted key.
    #[error("signature verification failed: {0}")]
    BadSignature(String),

    /// The signature is well-formed but was produced by a key other than the
    /// trusted one.
...
/// Verify a detached minisign signature against `trusted_key`, validate the
/// component, enforce the manifest's capability grants, and return the compiled
/// [`Component`] on success.
///
/// `signature` is the textual `.minisig` content (as produced by `minisign -S`)
/// supplied as raw bytes. `trusted_key` is the single key permitted to sign
/// loadable plugins.
///
/// # Errors
///
/// Returns [`LoaderError::BadSignature`] when the signature is malformed or does
/// not verify, [`LoaderError::Untrusted`] when it was made by a different key,
/// [`LoaderError::Malformed`] when the bytes are not a valid component, and
/// [`LoaderError::CapabilityDenied`] when an imported capability is not granted.
pub fn load_signed(
    component: &[u8],
    signature: &[u8],
