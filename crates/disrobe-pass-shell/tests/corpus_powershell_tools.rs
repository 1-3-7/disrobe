...
fn fixture_chameleon_hello_decodes_base64_payload() {
    ...
    assert!(
        r.decoded_payload,
        "chameleon must actually decode the payload"
    );
    let lowered: String = r.output.to_ascii_lowercase();
    assert!(
        lowered.contains("write-host") && lowered.contains("hello world"),
        "chameleon must produce decoded plaintext, got: {}",
        r.output
    );
    assert!(
        !lowered.contains("frombase64string"),
        "still-encoded output is not recovery: {}",
        r.output
    );
}

#[test]
fn fixture_chameleon_rename_real_detects_long_random_identifiers() {
    let src: String = read_corpus("powershell/chameleon/rename_real.ps1");
    let det: Detection = detect(src.as_bytes());
    assert_eq!(det.dialect, Dialect::PowerShell);
    let r: ChameleonReport = reverse_chameleon(&src);
    assert!(
        r.renamed_variables >= 2,
        "real Chameleon renames every identifier to a long high-entropy base62 slot; \
         reverse_chameleon must detect them (it reported 0 before the fix - the heuristic only \
         matched names <= 4 chars): got {}",
        r.renamed_variables
    );
}
...
fn fixture_invoke_stealth_rev_b64_extracts_payload() {
    ...
    assert!(
        r.decoded_payloads >= 1,
        "invoke-stealth must actually decode at least one payload"
    );
    let lowered: String = r.output.to_ascii_lowercase();
    assert!(
        lowered.contains("write-host") && lowered.contains("hello world"),
        "invoke-stealth must produce decoded plaintext, got: {}",
        r.output
    );
    assert!(
        !lowered.contains("frombase64string"),
...
#[test]
fn fixture_psobf_hello_decodes_percent_insertion() -> disrobe_pass_shell::Result<()> {
    let src: String = read_corpus("powershell/psobf/hello.ps1");
    let r: PsobfReport = reverse_psobf(&src)?;
    let lowered: String = r.output.to_ascii_lowercase();
    assert!(
        lowered.contains("write-host") && lowered.contains("hello world"),
        "psobf must produce decoded plaintext, got: {}",
        r.output
    );
    assert!(
        !r.output.contains("<obfus") && !r.output.contains('%'),
        "obfus wrappers must be stripped, got: {}",
        r.output
    );
    Ok(())
}
