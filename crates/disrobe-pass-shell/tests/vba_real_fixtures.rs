...
fn real_vba_project_pcode_parses_header_with_honest_wall() -> disrobe_pass_shell::Result<()> {
    ...
    assert!(
        d.walls
            .iter()
            .any(|w: &disrobe_pass_shell::PCodeWallDetail| w.kind == PCodeWall::UnsupportedVersion),
        "p-code disasm must surface an honest UnsupportedVersion wall (per-VBA-version opcode tables not in scope); walls={:?}",
        d.walls
    );
    assert!(
