...
use disrobe_pass_shell::{
    RealModuleDisasm, RealPCodeLine, RealPCodeReport, SemanticLift, disassemble_pcode_real,
    semantic_lift,
};
...
fn module1_disasm_text(report: &RealPCodeReport) -> String {
    let module: &RealModuleDisasm = report
        .modules
        .iter()
        .find(|m: &&RealModuleDisasm| m.name == "Module1")
    ...
    module
        .lines
        .iter()
        .map(|l: &RealPCodeLine| l.text.clone())
        .collect::<String>()
}

#[test]
fn real_vbaproject_disasm_matches_pcodedmp_ground_truth() -> disrobe_pass_shell::Result<()> {
    let bin: Vec<u8> = std::fs::read(corpus_path("vba/vbaProject.bin"))
        .unwrap_or_else(|e: std::io::Error| panic!("read vbaProject.bin: {e}"));
    let report: RealPCodeReport = disassemble_pcode_real(&bin)?;
    ...
    assert_eq!(
        module1_disasm_text(&report),
        expected,
        "disrobe disassembly must match pcodedmp 1.2.6 reference output byte-for-byte; \
         func_ resolved to the indirect-table signature and ArgsCall opType-16 marker handled"
    );
    Ok(())
}

#[test]
fn real_docm_disasm_matches_pcodedmp_ground_truth() -> disrobe_pass_shell::Result<()> {
    let bin: Vec<u8> = vbaproject_from_docm("vba/hello.docm");
    let report: RealPCodeReport = disassemble_pcode_real(&bin)?;
    let expected: &str = concat!(
        "FuncDefn (Sub Main())\n",
    ...
    assert_eq!(module1_disasm_text(&report), expected);
    Ok(())
}
