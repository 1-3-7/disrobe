...
/// Normalise whitespace and case so a produced-plaintext match ignores layout, never encoding.
...
/// True only when the deobfuscated `produced` text contains the known `cleartext`.
