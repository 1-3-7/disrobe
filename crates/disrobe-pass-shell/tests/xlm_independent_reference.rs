...
use xlm_reference::{
    FORMULA_REFERENCE as REFERENCE_TOOL, FORMULA_REFERENCE_VERSION as REFERENCE_VERSION, Fixture,
    Manifest, PYTHON_VAR, find_interpreter, fixture_bytes, manifest, normalize,
};
...
fn refuse_or_announce(defect: &str) {
    assert!(
        ...
        "{REQUIRE_VAR} makes {REFERENCE_TOOL} mandatory for this run, so {GRADED} cannot be \
         ...
         at one that has it. On an interpreter past 3.11, {REFERENCE_TOOL}.deobfuscator imports the \
         removed stdlib distutils module, so also install setuptools, which reinstates it as an \
         importable shim; to permit a run that measures nothing here, clear {REQUIRE_VAR}."
    );
    println!(
...
fn run(python: &Path, args: &[&str]) -> Result<Option<CapturedOutput>, String> {
    run_captured(python, args, CALL_TIMEOUT, MAX_CAPTURE)
        .map_err(|err| format!("cannot run `{}`: {err}", python.display()))
}
...
fn grade_fixture(
    ...
    let out: Option<CapturedOutput> = match run(
        python,
        &[
            "-m",
        ...
        Ok(out) => out,
        Err(error) => {
            faults.push(format!("{}: {error}", fixture.file));
            return Graded {
                cells: 0,
                faults: Vec::new(),
            };
...
fn recovered_formulas_match_an_independent_deobfuscator() {
    let Some(python): Option<PathBuf> = find_interpreter() else {
        refuse_or_announce(
