...
fn real_compress_hello_recovers_gzip_payload() -> disrobe_pass_shell::Result<()> {
    ...
    assert!(
        out_lower.contains("echo") || out_lower.contains("hello") || out_lower.contains("world"),
        "expected gzip recovery to yield ECHO-payload, got: {}",
        r.output
    );
    assert!(
        r.steps
...
#[test]
fn real_token_hello_wall_or_progress() -> disrobe_pass_shell::Result<()> {
    ...
    assert!(
        !r.walls.is_empty()
            || r.output.len() < src.len()
            || r.steps
                .iter()
                .any(|s: &String| s.starts_with("strip-noise-expansions")),
        "expected at least some peeling progress or honest wall, got steps={:?} walls={:?}",
        r.steps,
        r.walls
    );
    Ok(())
...
#[test]
fn real_string_hello_wall_or_progress() -> disrobe_pass_shell::Result<()> {
