...
const PCODEDMP_TOOL: &str = "pcodedmp";
const PCODEDMP_VERSION: &str = "1.2.6";
const PCODEDMP_PROVENANCE: &str = "vba_pcodedmp.provenance.json";
const PINNED_DUMPS: usize = 3;
...
fn provenance() -> Provenance {
    let path: PathBuf = golden_path(PCODEDMP_PROVENANCE);
...
fn sha256_hex(bytes: &[u8]) -> String {
    ...
    hasher
        .finalize()
        .iter()
        .fold(String::with_capacity(64), |mut acc: String, b: &u8| {
...
#[test]
fn every_reference_dump_is_version_pinned_and_hash_pinned() {
    let p: Provenance = provenance();
    assert_eq!(p.tool, PCODEDMP_TOOL);
    assert_eq!(
        p.version, PCODEDMP_VERSION,
        "the reference tool version is pinned; a dump regenerated by another version would \
         silently change the expected bytes"
    );
    assert!(
        p.command.contains(PCODEDMP_TOOL),
        "the provenance must record the command that produced the dumps; got {:?}",
        p.command
    );
    assert_eq!(
        p.dump.len(),
        PINNED_DUMPS,
        "the number of pinned reference dumps is fixed so one cannot quietly leave the gate"
    );
    for dump in &p.dump {
        let path: PathBuf = golden_path(&dump.file);
        let bytes: Vec<u8> = std::fs::read(&path)
            .unwrap_or_else(|e: std::io::Error| panic!("read {}: {e}", path.display()));
        assert_eq!(
            bytes.len(),
            dump.bytes,
            "{}: byte length moved away from the pinned provenance",
            dump.file
        );
        assert_eq!(
            sha256_hex(&bytes),
            dump.sha256,
            "{}: content hash moved away from the pinned provenance, so it is no longer the \
             {PCODEDMP_TOOL} {PCODEDMP_VERSION} dump this gate grades against",
            dump.file
        );
        let container: PathBuf = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(|p: &std::path::Path| p.parent())
            .expect("workspace root")
            .join(&dump.container);
        assert!(
            container.is_file(),
            "{}: the container the dump was taken from must be committed at {}",
            dump.file,
            container.display()
        );
    }
...
#[test]
fn the_hash_pin_rejects_an_edited_dump() {
    let p: Provenance = provenance();
    ...
    assert_ne!(
        sha256_hex(&bytes),
        first.sha256,
        "appending a byte must change the pinned hash, otherwise the pin cannot detect an edit"
    );
}
