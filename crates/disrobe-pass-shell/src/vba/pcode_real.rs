...
mod tests {
    ...
    /// MS-OVBA 2.4.1.3.19.3: at `DifferenceDecompressed` 0, `BitCount = 4` and `LengthMask = 0x0FFF`.
    #[test]
    ...
    /// MS-OVBA 2.4.1.3.19.3: at `DifferenceDecompressed` 16, `BitCount` is still 4 and `LengthMask = 0x0FFF`.
    #[test]
    ...
    /// Hand-built MS-OVBA 2.4.1 `CompressedChunk` for "abcabcabc" exercising `CopyToken` overlap-copy.
    #[test]
    ...
    /// MS-OVBA 2.4.1.1.5: an uncompressed `CompressedChunk` (header 0x3FFF) copies its body verbatim.
    #[test]
