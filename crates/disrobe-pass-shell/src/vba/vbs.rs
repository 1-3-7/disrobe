...
/// Statically deobfuscate a `VBScript` snippet under the default static-only policy.
#[must_use]
pub fn deobfuscate_vbs(input: &str) -> VbsReport {
    deobfuscate_vbs_with_policy(input, DynamicPolicy::default())
}

/// Deobfuscate `VBScript`, unwrapping nested `Execute` layers up to the [`DynamicPolicy`] cap.
#[must_use]
