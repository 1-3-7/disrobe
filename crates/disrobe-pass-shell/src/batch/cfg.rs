...
/// Kind of control-flow edge leaving a basic block in a resolved batch CFG.
...
/// A directed edge from one basic block to another (or to a named-but-missing label).
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct CfgEdge {
    pub kind: EdgeKind,
...
/// A maximal run of straight-line batch statements ending at a control-flow transfer.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
/// Resolved control-flow graph for a batch script: blocks, label table, and per-label classification.
...
/// Resolve a batch script's goto/call/label control flow into a typed basic-block CFG.
///
/// Blocks split at every label definition and after any unconditional transfer (`goto`,
/// `exit /b`, `exit`). `call :label` edges carry an implicit `CallReturn` to the following
/// block; conditional `if ... goto` produces both a `ConditionalGoto` and a fall-through.
/// Targets that resolve to no label are retained on the edge and surfaced in
/// `unresolved_targets` so dynamic/computed dispatch (`call :SUB_%VAR%`) is never silently dropped.
#[must_use]
...
impl BatchCfg {
    /// Bridge the resolved label table into disrobe-ir disassembly symbols.
    ///
    /// Labels that are ever the target of `call :label` become [`DisasmSymbolKind::Function`];
    /// pure `goto` targets become [`DisasmSymbolKind::Label`]. Addresses are the defining line
    /// number of each label, giving downstream IR consumers a stable ordering key.
    #[must_use]
