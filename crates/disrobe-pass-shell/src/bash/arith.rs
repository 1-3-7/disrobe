...
fn bash_shift_count(count: i64) -> Option<u32> {
    u32::try_from(count.rem_euclid(i64::from(i64::BITS))).ok()
}
...
impl<'a> ArithParser<'a> {
    ...
    fn parse_expression(&mut self) -> Option<i64> {
        ...
        let value: Option<i64> = self.parse_bitor();
        self.depth -= 1;
        value
    }
    ...
    fn parse_shift(&mut self) -> Option<i64> {
        ...
        loop {
            ...
            {
                self.pos += 2;
                let right: u32 = bash_shift_count(self.parse_add()?)?;
                left = left.wrapping_shl(right);
...
mod tests {
    ...
    fn deeply_nested_parens_does_not_overflow() {
        ...
        let inner: String = format!("{}1{}", "(".repeat(depth), ")".repeat(depth));
        let value: Option<i64> = eval_arith(&inner);
        assert!(value.is_none(), "over-deep nesting must degrade to None");
    }

    #[test]
    fn long_unary_chain_does_not_overflow() {
        let inner: String = format!("{}5", "-".repeat(50_000));
        let value: Option<i64> = eval_arith(&inner);
        assert!(
            value.is_none(),
            "over-deep unary chain must degrade to None"
        );
    }

    #[test]
    fn bounded_depth_still_evaluates() {
        assert_eq!(eval_arith("(((((7)))))"), Some(7));
        assert_eq!(eval_arith("----5"), Some(5));
    }

    #[test]
    fn unbalanced_close_parens_do_not_underflow() {
        let mut steps: Vec<String> = Vec::new();
        let out: String = evaluate_arithmetic("$(( )x)1)", &mut steps);
        assert_eq!(out, "$(( )x)1)");
        let mut steps2: Vec<String> = Vec::new();
        let out2: String = evaluate_arithmetic("$[ ]x]1]", &mut steps2);
        assert_eq!(out2, "$[ ]x]1]");
        assert!(steps.is_empty() && steps2.is_empty());
    }

    #[test]
    fn oversized_shift_counts_match_bash_word_width() {
        assert_eq!(eval_arith("1 << 99"), Some(34_359_738_368));
        assert_eq!(eval_arith("1 << -1"), Some(i64::MIN));
        assert_eq!(eval_arith("1 >> 99"), Some(0));
        assert_eq!(eval_arith("-8 >> -1"), Some(-1));
    }
}
