...
fn reverse_then_b64_decode(literal: &str) -> Option<String> {
    if literal.len() > MAX_BASE64_INPUT {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn oversized_reverse_b64_literal_is_not_decoded() {
        let oversized: String = "A".repeat(MAX_BASE64_INPUT + 1);
        let src: String = format!(
            "$best64code = \"{oversized}\" ; [array]::Reverse($base64) ; [System.Convert]::FromBase64String($Stripped)"
        );
        let r: InvokeStealthReport = reverse_invoke_stealth(&src);
        assert_eq!(r.decoded_payloads, 0);
