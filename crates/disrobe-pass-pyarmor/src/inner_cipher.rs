...
pub fn decrypt_module(code: &mut Object, module_state: &PyarmorModuleState) -> DecryptionStats {
    ...
    if let Object::Code(co) = code {
        decrypt_code_object(co, module_state, &mut metrics, None);
    }
    metrics
}
...
pub fn decrypt_module_with_cache(
    ...
    let mut metrics: DecryptionStats = DecryptionStats::default();
    if let Object::Code(co) = code {
        decrypt_code_object(co, module_state, &mut metrics, Some(cache));
    }
    metrics
}
...
fn decrypt_code_object(
    ...
    if co.flags & CO_PYARMOR_OBFUSCATED != 0 && !co.pyarmor_trailer.is_empty() {
        ...
        if metrics.first_trailer_hex.is_none() {
            metrics.first_trailer_hex = Some(co.pyarmor_trailer.iter().fold(
                String::with_capacity(co.pyarmor_trailer.len() * 2),
                |mut acc, b| {
    ...
    for cnst in &mut co.consts {
        if let Object::Code(inner) = cnst {
            decrypt_code_object(inner, module_state, metrics, cache_slot.as_deref_mut());
        }
    }
...
fn apply_descriptor(
    ...
    let nonce_index: usize = if desc.short_code() {
        ...
        desc.short_nonce_index as usize
            + desc.decrypt_begin_index as usize
            + desc.decrypt_length as usize
    };

    if nonce_index + 12 > co.code.len() {
        return;
    }
    let mut nonce: [u8; 12] = [0u8; 12];
    nonce.copy_from_slice(&co.code[nonce_index..nonce_index + 12]);
    ...
    if let Some(keystream) = cached_keystream {
        ...
        for (target, k) in co.code[begin..begin + len].iter_mut().zip(keystream.iter()) {
            *target ^= *k;
        }
    } else {
    ...
    if desc.copy_prologue() {
        ...
        if len + begin <= co.code.len() {
            let src_start: usize = len;
            if src_start + begin <= co.code.len() {
                let mut prologue: Vec<u8> = vec![0u8; begin];
                prologue.copy_from_slice(&co.code[src_start..src_start + begin]);
                co.code[..begin].copy_from_slice(&prologue);
                for b in &mut co.code[src_start..src_start + begin] {
                    *b = nop;
                }
...
pub fn parse_plaintext_xor_procedure(plaintext: &[u8]) -> ([u8; 12], bool) {
    ...
    if proc_length == 0 || code_offset + proc_length > plaintext.len() {
        return ([0u8; 12], false);
    }
    let buf: &[u8] = &plaintext[code_offset..code_offset + proc_length];
    let key: [u8; 12] = run_xor_key_vm(buf);
    (key, true)
}
...
fn run_xor_key_vm(program: &[u8]) -> [u8; 12] {
    ...
    while cursor < program.len() {
        ...
        let Some((operand2, advance)): Option<(i64, usize)> =
...
mod tests {
    ...
    fn xor_vm_empty_program_returns_zeros() {
        assert_eq!(run_xor_key_vm(&[]), [0u8; 12]);
    }
