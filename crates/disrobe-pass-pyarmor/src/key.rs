...
pub(crate) fn extract_runtime_key(runtime_bytes: &[u8]) -> Result<RuntimeKeyMaterial> {
    ...
    if u32_le(&buf, 0x4C)? != 0 {
        ...
        if buf[xor_flag] == 1 {
            ...
            let _: &[u8] = sub(&buf, xor_source, xor_length)?;
            for i in 0..xor_length {
                buf[xor_target + i] ^= buf[xor_source + i];
            }
        }
    }
...
fn checked_offset(base: usize, offset: usize) -> Result<usize> {
    base.checked_add(offset)
        .ok_or_else(|| Error::KeyExtraction("runtime data offset overflow".to_owned()))
}

fn sub(buf: &[u8], offset: usize, len: usize) -> Result<&[u8]> {
    let end: usize = checked_offset(offset, len)?;
...
mod tests {
    ...
    #[test]
    fn runtime_subslice_rejects_overflow_without_panicking() {
        let bytes: [u8; 4] = [0x12, 0x34, 0x56, 0x78];
        assert!(sub(&bytes, usize::MAX, 1).is_err());
        assert!(sub(&bytes, 1, usize::MAX).is_err());
    ...
    #[test]
    fn runtime_relative_offsets_reject_overflow() {
        assert!(checked_offset(0x60, usize::MAX).is_err());
        assert!(checked_offset(usize::MAX, 1).is_err());
        assert_eq!(checked_offset(0x60, 0x20).unwrap(), 0x80);
        assert_eq!(checked_offset(usize::MAX, 0).unwrap(), usize::MAX);
    }
    ...
    pub(crate) fn synth_elf64_with_rodata(rodata: &[u8], text: &[u8]) -> Vec<u8> {
