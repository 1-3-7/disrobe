...
use disrobe_py_marshal::{Object, PyVersion, PycFile, PycHeader, code_era_for, load, write_pyc};
...
use crate::v3v4v5::{self, LegacyDecryptedPayload};
use crate::v6v7;
...
pub fn unpack_wrapper_text_with_options(
    ...
    if matches!(
        ...
        let best_effort: Result<LegacyDecryptedPayload> =
            v3v4v5::decrypt_legacy(&payload, &detection);
        let mut legacy_output: UnpackOutput =
            legacy_detection_only_output(&detection, wrapper_path, &payload);
        match best_effort {
            Ok(decrypted) => {
                legacy_output.plaintext.clone_from(&decrypted.plaintext);
                let combined_reason: String = format!(
                    "{:?} best-effort decryption (unverified-against-real-sample={})",
                    decrypted.version, decrypted.unverified_against_real_sample
                );
                legacy_output.fallback_reason = Some(combined_reason);
                let mut diag_combined: Vec<String> = legacy_output.detection.diagnostics.clone();
                diag_combined.extend(decrypted.diagnostics);
                legacy_output.detection.diagnostics = diag_combined;
            }
            Err(err) => {
                let combined_reason: String = format!(
                    "{:?} detect-only; legacy best-effort failed: {err}",
                    detection.version
                );
                legacy_output.fallback_reason = Some(combined_reason);
            }
        ...
        if options.strict {
            return Err(Error::StrictPartial(format!(
                "{:?} detect-only; no validated decryption (legacy era)",
                detection.version
            )));
        }
    ...
    if matches!(detection.protection, ProtectionKind::Bcc)
        && options.allow_bcc
        && options.strict
        && which_ghidra_for_lift().is_none()
    {
        return Err(Error::BccGhidraMissing);
    }
...
fn which_on_path(exe: &str) -> Option<PathBuf> {
    ...
    for dir in std::env::split_paths(&path_var) {
        let candidate_exact: PathBuf = dir.join(exe);
        if candidate_exact.is_file() {
            return Some(candidate_exact);
        }
        for ext in ["exe", "bat", "cmd"] {
            let candidate: PathBuf = dir.join(format!("{exe}.{ext}"));
...
fn attempt_dynamic_fallback(
    ...
    let primary_pyc_bytes: Option<Vec<u8>> = primary
        .as_ref()
        .and_then(|c| std::fs::read(&c.pyc_path).ok());
...
fn classify_candidates_with_stem(
    ...
    for (source, entry) in entries {
        ...
        let Ok(bytes): std::io::Result<Vec<u8>> = std::fs::read(&pyc_path) else {
...
fn finalize_v8v9(
    ...
    if !bcc_blobs.is_empty() && options.allow_bcc {
        ...
        match ghidra_path {
            ...
            None => {
                out.bcc_lift_skipped_reason = Some(
                    "ghidra-headless not on PATH; v9 BCC blobs surfaced but not lifted".to_owned(),
                );
            }
        }
...
fn which_ghidra_for_lift() -> Option<PathBuf> {
    if let Some(envp) = std::env::var_os("DISROBE_GHIDRA") {
        let p: PathBuf = PathBuf::from(envp);
        if p.is_file() || p.is_dir() {
            return Some(p);
        }
    }
    for name in ["analyzeHeadless", "ghidraRun", "ghidra"] {
