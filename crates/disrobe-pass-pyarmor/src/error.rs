...
pub enum Error {
    #[error("outer-module recovery requires a PyArmor 8 or 9 payload")]
    ModuleFormatUnsupported,

    #[error("outer-module recovery requires a PyArmor 8 or 9 runtime descriptor")]
    ModuleRuntimeUnsupported,
    #[error("outer-module recovery does not support protection type {0}")]
    ModuleProtectionUnsupported(u32),
    #[error("payload format {payload} does not match runtime descriptor {runtime}")]
    ModuleVersionMismatch { payload: u8, runtime: u8 },

    #[error("runtime serial {runtime} does not match payload serial {payload}")]
    ModuleRuntimeMismatch { payload: String, runtime: String },
