...
pub struct UnpackedModule {
    pub runtime_version: PyarmorVersion,
    pub header: HeaderMetadata,
    pub runtime_arch: runtime::RuntimeArch,
    pub bytes: Vec<u8>,
}

pub fn unpack_module_bytes(payload: &[u8], runtime_bytes: &[u8]) -> Result<UnpackedModule> {
    let magic: WrapperMagic = sniff(payload)?;
    if magic != WrapperMagic::Py8Or9 {
        return Err(Error::ModuleFormatUnsupported);
    }
    let header: HeaderMetadata = parse_header(payload, magic)?;
    let protection: u32 = u32::from_le_bytes([payload[20], payload[21], payload[22], payload[23]]);
    match protection {
        8 => {}
        9 => return Err(Error::BccPartialOnly),
        other => return Err(Error::ModuleProtectionUnsupported(other)),
    }
    let runtime: RuntimeInfoSummary = load_runtime_info(runtime_bytes)?;
    let version: PyarmorVersion = match runtime.descriptor_version {
        Some(version @ (PyarmorVersion::V8 | PyarmorVersion::V9)) => version,
        _ => return Err(Error::ModuleRuntimeUnsupported),
    };
    if let Some(serial) = header.serial.as_ref()
        && serial != &runtime.serial
    {
        return Err(Error::ModuleRuntimeMismatch {
            payload: serial.clone(),
            runtime: runtime.serial,
        });
    }
    if let Some(serial) = header.serial.as_deref()
        && let Some(payload_version) = crate::key_class::classify_serial(serial).format_version
    {
        let runtime_version: u8 = if version == PyarmorVersion::V8 { 8 } else { 9 };
        if payload_version != runtime_version {
            return Err(Error::ModuleVersionMismatch {
                payload: payload_version,
                runtime: runtime_version,
            });
        }
    }
    let result: VersionedOutcome =
        decrypt_v8::decrypt_with_runtime_key(payload, &runtime.aes_key, DecryptStatus::Functional)?;
    Ok(UnpackedModule {
        runtime_version: version,
        header,
        runtime_arch: runtime.arch,
