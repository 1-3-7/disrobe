...
const OPAQUE_FIXTURES: [&str; 2] = ["v8/obfmod0", "v9/obfmod0"];
const RECOVERY_FLOOR: usize = 70;
...
fn recovers_real_source_from_v8_and_v9_corpus() {
    ...
    for fx in &fixtures {
        ...
        if fx.is_opaque {
            opaque_seen += 1;
            assert!(
                locate_real_code_object(&out.plaintext).is_none(),
                "{}: obfmod0 inner module transform is genuinely opaque; a marshal recovery here would be a corpus regression",
                fx.relative_id
            );
            continue;
        }
    ...
    assert_eq!(
        opaque_seen,
        OPAQUE_FIXTURES.len(),
        "expected exactly the {} documented opaque obfmod0 fixtures",
        OPAQUE_FIXTURES.len()
    );
    assert!(
        markers_seen >= 36,
...
fn gather(dir: &Path, corpus_dir: &Path, expected_version: PyarmorVersion, out: &mut Vec<Fixture>) {
        ...
        && !wrappers.is_empty()
    {
        for wrapper_py in wrappers {
            ...
            let is_opaque: bool = OPAQUE_FIXTURES
                .iter()
                .any(|o: &&str| relative_id.starts_with(o));
            out.push(Fixture {
                wrapper_py,
