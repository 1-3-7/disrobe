...
#[test]
fn luajit_underreported_proto_len_cannot_consume_following_bytes() {
    let bytes: Vec<u8> = vec![
        0x1B, b'L', b'J', 0x02, 0x02, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x11, 0x22,
        0x33, 0x44, 0x00,
    ];
    let result: Result<_, Error> = luajit::read(&bytes);
    assert!(
        matches!(result, Err(Error::Truncated { .. })),
        "a proto must not read code bytes past its declared length; got {result:?}"
    );
    let decompile_result = decompile_luajit(&bytes);
    assert!(
        decompile_result.is_err(),
        "the lifter must also reject underreported proto bodies"
    );
}
...
fn luau_oversized_string_count_does_not_oom() {
    ...
    let result: Result<_, Error> = luau::read(&bytes);
    assert!(
        matches!(result, Err(Error::LimitExceeded { .. })),
        "an oversized string count must hit the parser cap before allocation; got {result:?}"
    );
}
