...
pub enum Error {
    ...
    #[error("DR-LUA-0025: prototype nesting too deep ({0}); refusing to recurse (possible bomb)")]
    ProtoNestingTooDeep(usize),

    #[error("DR-LUA-0026: bootstrap init emulation failed: {0}")]
    BootstrapEmulationFailed(&'static str),

    #[error("DR-LUA-0027: {section} count {count} exceeds parser limit {limit}")]
    LimitExceeded {
        section: &'static str,
        count: u64,
        limit: usize,
