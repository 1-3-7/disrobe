...
mod tests {
    ...
    fn chain_run_devirtualizes_real_prometheus_greeting() {
        ...
        let children: Vec<ChildArtifact> =
            LUA_PASS.extract_children(&a).expect("recovery sidecars");
        let manifest: &ChildArtifact = children
            .iter()
            .find(|child: &&ChildArtifact| child.handle.relative_path == "lua.manifest.json")
            .expect("Lua recovery manifest");
        let report: serde_json::Value =
            serde_json::from_slice(&manifest.bytes).expect("parse Lua recovery manifest");
        assert_eq!(report["fully_recovered"], true);
        assert!(
            report["passes_run"]
                .as_array()
                .expect("recovery passes")
                .iter()
                .any(|pass: &serde_json::Value| pass == "prometheus-vmify-container-devirt")
        );
    }
    ...
    #[test]
    fn catalog_detect_misses_native_code_with_lua_keyword_strings() {
        let bytes: &[u8] = include_bytes!("../../../corpus/native/packers/upx/hello.original.exe");
        assert!(prometheus::detect(bytes).is_none());
        assert!(ObfuscatorCatalog::detect(&LuaDetector, &ctx(bytes)).is_none());
    }

    #[test]
    fn catalog_detect_misses_malformed_lua_with_dispatch_keywords() {
        let bytes: &[u8] = b"function( while ";
        assert!(prometheus::detect(bytes).is_none());
        assert!(ObfuscatorCatalog::detect(&LuaDetector, &ctx(bytes)).is_none());
    }
}
