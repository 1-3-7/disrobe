...
use crate::reader::common::{
    LUAJIT_SIGNATURE, LuaChunk, LuaConstant, LuaDialect, LuaProto, LuaUpvalueName, capped_i64,
    capped_u32,
};
...
pub fn read(bytes: &[u8]) -> Result<LuaChunk> {
    ...
    let stripped: bool = flags & u64::from(FLAG_STRIPPED) != 0;
    if !stripped {
        let src_len: u64 = c.read_uleb128()?;
        if src_len > 0 {
            let _src: &[u8] = c.read_bytes(c.checked_len("luajit source length", src_len)?)?;
        }
    }
    ...
    loop {
        ...
        let proto_size: usize = c.checked_len("luajit proto length", proto_len)?;
        let proto_bytes: &[u8] = c.read_bytes(proto_size)?;
        let mut proto_cursor: ByteCursor<'_> = ByteCursor::new(proto_bytes);
        let proto: LuaProto = read_proto(&mut proto_cursor, stripped, dialect, proto_size)?;
        main = proto;
    }
    Ok(LuaChunk {
        dialect,
...
fn skip_ktab_entry(c: &mut ByteCursor<'_>) -> Result<()> {
    ...
    match tp {
        ...
        _ => {
            ...
            let n: usize = c.checked_len("luajit table string length", len)?;
...
fn skip_ktab(c: &mut ByteCursor<'_>) -> Result<()> {
    ...
    let narray: usize = c.checked_count::<u8>("luajit table array", narray, 1)?;
    let nhash: usize = c.checked_count::<u8>("luajit table hash", nhash, 2)?;
    for _ in 0..narray {
        skip_ktab_entry(c)?;
    }
    for _ in 0..nhash {
...
fn read_proto(
    ...
    let size_bc: usize = c.checked_count::<u32>("luajit code", size_bc, 4)?;
    let mut code: Vec<u32> = Vec::with_capacity(size_bc);
    for _ in 0..size_bc {
    ...
    let size_kgc: usize = c.checked_count::<LuaConstant>("luajit gc constant", size_kgc, 1)?;
    let size_kn: usize = c.checked_count::<LuaConstant>("luajit numeric constant", size_kn, 1)?;
    let total_constants: usize = size_kgc.checked_add(size_kn).ok_or(Error::LimitExceeded {
        section: "luajit constant",
        count: u64::MAX,
        limit: c.remaining(),
    })?;
    let mut constants: Vec<LuaConstant> = Vec::with_capacity(total_constants);
    for _ in 0..size_kgc {
        ...
        match tag {
            ...
            _ => {
                ...
                let strlen: usize = c.checked_len("luajit constant string length", strlen_raw)?;
    ...
    for _ in 0..size_kn {
        ...
        if lo & 1 != 0 {
            ...
            constants.push(LuaConstant::Integer(capped_i64(lo >> 1)));
        }
    }
    ...
    Ok(LuaProto {
        source: None,
        line_defined: capped_u32(first_line),
        last_line_defined: capped_u32(first_line.saturating_add(num_line)),
        num_params,
