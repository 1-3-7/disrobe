...
fn read_string(c: &mut ByteCursor<'_>, size_size_t: u8) -> Result<Option<String>> {
    ...
    let raw: &[u8] = c.read_bytes(c.checked_len("lua52 string length", len)?)?;
...
fn read_proto(
    ...
    let code_len: usize = c.checked_count::<u32>("lua52 code", code_len, size_instr.into())?;
    ...
    let const_count: usize = c.checked_count::<LuaConstant>("lua52 constant", const_count, 1)?;
    ...
    let proto_count: usize = c.checked_count::<LuaProto>("lua52 proto", proto_count, 1)?;
    ...
    let upval_count: usize = c.checked_count::<LuaUpvalueName>("lua52 upvalue", upval_count, 2)?;
    ...
    let line_count: usize =
        c.checked_count::<u32>("lua52 line info", line_count, size_int.into())?;
    ...
    let local_count: usize = c.checked_count::<LuaLocal>("lua52 local", local_count, 1)?;
    ...
    let upval_names: usize = c.checked_count::<u8>("lua52 upvalue name", upval_names, 1)?;
