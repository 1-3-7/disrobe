...
impl LiftState {
    ...
    /// Stable synthetic `loc{slot}` name for a register slot with no debug name or live expression.
    #[inline]
...
/// Resolves the real source name of parameter slot `slot`, falling back to `p{slot}`.
#[must_use]
