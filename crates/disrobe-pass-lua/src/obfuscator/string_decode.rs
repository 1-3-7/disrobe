...
/// Extracts every `string.char(a, b, c, ...)` numeric byte array in `text`, one byte vector per call.
#[must_use]
...
/// Recovers a string-pool byte array XOR'd against a single fixed key byte (`MoonSec` V2).
#[must_use]
pub fn xor_decode_fixed(encoded: &[u8], key: u8) -> Vec<u8> {
    encoded.iter().map(|b: &u8| b ^ key).collect()
}

/// Recovers a string-pool byte array XOR'd against an index-advancing key: `pool[i] = plain[i] ~ ((key + i) & 0xFF)`.
#[must_use]
