...
pub fn peel(src: &[u8], _opts: &DeobfOptions) -> Result<PeelResult> {
    ...
    if let Some(payload) = embedded_payload {
        return devirt_to_peel(src, &text, &payload, "moonsec-v2");
    }
    ...
    let Some((pool, variant)): Option<(Vec<String>, &'static str)> = recover_xor_pool(&arrays, key)
        ...
        return Ok(PeelResult::passthrough(
            src,
            vec![
                "moonsec v2: this sample carries neither a static char-array string pool nor an inline XOR key literal, so the decrypt material is absent from the artifact (the normal path recovers when either is present)"
                    .to_owned(),
            ],
        ));
    };
