...
fn two_exit_states_that_rejoin_reloop_under_wasmtime() {
    ...
    let obf_bytes: Vec<u8> = assemble_fixture("cff_multi_exit_join.obf.wat");
    let recovered: RecoveredModule =
        recover_module(&obf_bytes).expect("recover the rejoining multi-exit dispatch loop");
    assert_eq!(
        recovered.report.flattened_dispatchers_walled, 0,
        "two exit states that fall through to a shared continuation must structure: {:?}",
        recovered.report
    );
    assert_reloops_to_clean_behavior(
        &clean_bytes,
...
fn runtime_differential_rejects_swapped_rejoining_exit_bodies() {
    ...
    let recovered: RecoveredModule =
        recover_module(&mutant_bytes).expect("recover the mutated rejoining dispatch loop");
    assert_eq!(
        recovered.report.flattened_conditional_restructured, 1,
        "the mutant must reach the runtime battery instead of being refused: {:?}",
        recovered.report
