...
use disrobe_pass_wasm_deob::{
    BlockId, BlockTarget, ConstVal, DispatcherInfo, FunctionCfg, IntegrityStripStats, LiftResult,
    LiftTarget, LoadKind, NameStrategy, OpKind, OpaquePredStats, SideEffect, SsaBlock, SsaFunction,
    SsaMemArg, SsaTerm, StoreKind, StructuredFunction, StubInfo, UnflattenStats, ValueDef, ValueId,
    WasmDetection, WasmObfuscator, WobfuscatorTable, build_function_cfg, build_ssa,
    classify_export_strategy, detect, detect_decrypt_stubs, detect_dispatcher, extract_optable,
    kill_opaque_predicates, lift_op_to_rust_fn, lift_with_ssa, reloop_inverse,
...
fn full_pipeline_smoke_detect_through_lift() {
    ...
    for payload in Parser::new(0).parse_all(&bytes) {
        ...
        if let Payload::CodeSectionEntry(body) = payload {
            ...
            let params: &[WpValType] = &[WpValType::I32];
            ...
            assert!(
                structured.block_count >= 1,
                "structured func must report >=1 block"
            );

            let lifted: LiftResult = lift_with_ssa(&structured, &ssa, LiftTarget::Rust);
            assert!(
                !lifted.pseudo_source.is_empty(),
                "lifted Rust pseudo-source must be non-empty"
            );
            assert!(
                lifted.pseudo_source.contains("fn lifted()"),
                "Rust lift must emit the fn lifted() wrapper"
            );
            assert!(lifted.blocks_emitted >= 1);

            let lifted_ts: LiftResult = lift_with_ssa(&structured, &ssa, LiftTarget::TypeScript);
            assert!(
                lifted_ts.pseudo_source.contains("function lifted(): void"),
                "TS lift must emit the function lifted(): void wrapper"
            );
            visited = true;
        }
    }
