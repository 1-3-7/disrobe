...
#[test]
fn typescript_loop_uses_javascript_labels() {
    let wat: &str = r"(module (func (param i32)
        block
            loop
                local.get 0
                i32.eqz
    ...
    let out: LiftResult = lift_first(
        wat,
        &sig("count_down", vec![ValType::I32], Vec::new()),
        LiftTarget::TypeScript,
    );
    assert!(out.pseudo_source.contains(": while (true) {"));
    assert!(out.pseudo_source.contains("break b"));
    assert!(out.pseudo_source.contains("continue b"));
    assert!(!out.pseudo_source.contains("'b"));
    assert!(!out.pseudo_source.contains(": loop {"));
}
