...
//! End-to-end name recovery: a DWARF/source-map-style name table is attached to the
//! module signatures, then the structured lifter is asserted to emit those real names
//! (`lhs`/`rhs`/`acc`) in place of positional `p0`/`p1`/`l2`. Ground truth is the
//! independently-supplied name table, never the lifter's own re-emit.
