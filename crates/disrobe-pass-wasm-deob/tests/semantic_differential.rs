...
#[cfg(feature = "sandbox")]
const STORE_MEMORY_LIMIT_BYTES: usize = 2 * 1024 * 1024;
#[cfg(feature = "sandbox")]
const STORE_TABLE_ELEMENT_LIMIT: usize = 1_024;
#[cfg(feature = "sandbox")]
const STORE_INSTANCE_LIMIT: usize = 4;
#[cfg(feature = "sandbox")]
...
#[cfg(feature = "sandbox")]
#[test]
fn sandbox_limits_linear_memory_growth() {
    const GROW_WAT: &str = r#"
        (module
          ...
          (func (export "grow") (param i32) (result i32)
            ...
            memory.grow))
    "#;
    let bytes: Vec<u8> = wat::parse_str(GROW_WAT).expect("assemble bounded linear-memory module");
    let eng: Engine = engine();
    let mut sandbox: Sandbox = instantiate(&eng, &bytes).expect("instantiate bounded module");
    let grow: wasmtime::TypedFunc<i32, i32> = sandbox
        .instance
        .get_typed_func(&mut sandbox.store, "grow")
        .expect("find linear-memory grow export");

    assert_eq!(
        grow.call(&mut sandbox.store, 31)
            .expect("grow to the configured limit"),
        1
    );
    let memory: wasmtime::Memory = first_memory(&mut sandbox).expect("find exported memory");
    assert_eq!(memory.data(&sandbox.store).len(), STORE_MEMORY_LIMIT_BYTES);
    assert_eq!(
        grow.call(&mut sandbox.store, 1)
            .expect("WebAssembly reports denied growth"),
        -1
    );
    assert_eq!(memory.data(&sandbox.store).len(), STORE_MEMORY_LIMIT_BYTES);
}
...
#[cfg(feature = "sandbox")]
#[test]
fn sandbox_limits_gc_heap_growth() {
    const GC_WAT: &str = r#"
        (module
          (type $row (array (mut i64)))
          (func (export "allocate") (param i32) (result i32)
            ...
            array.len))
    "#;
    let bytes: Vec<u8> = wat::parse_str(GC_WAT).expect("assemble bounded GC module");
    let eng: Engine = engine();
    let mut sandbox: Sandbox = instantiate(&eng, &bytes).expect("instantiate bounded GC module");
    let allocate: wasmtime::TypedFunc<i32, i32> = sandbox
        .instance
        .get_typed_func(&mut sandbox.store, "allocate")
        .expect("find GC allocation export");

    assert_eq!(
        allocate
            .call(&mut sandbox.store, 16_384)
            .expect("allocate within the configured limit"),
        16_384
    );
    let error: wasmtime::Error = allocate
        .call(&mut sandbox.store, 1_000_000)
        .expect_err("a GC allocation larger than the store memory limit must trap");
    assert!(
        error
            .downcast_ref::<wasmtime::GcHeapOutOfMemory<()>>()
            .is_some(),
        "expected Wasmtime GC heap capacity error, got {error:#}"
    );
}
