...
//! Real compile-the-output tests: lift the committed multi-function `.wasm` fixture and
//! prove the emitted Rust ACTUALLY compiles (`rustc --emit=metadata`), the emitted WAT
//! round-trips through `wat::parse_str`, and (when a C compiler is on PATH) the emitted C
//! compiles (`cc -c`). No substring-only false-green.
