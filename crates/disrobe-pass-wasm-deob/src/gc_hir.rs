...
fn emit_rust(
    ...
    for kind in abstract_refs {
        crate::push_string_line(
            &mut out,
            format_args!("pub struct {alias};", alias = abstract_alias_rust(*kind)),
        );
    }
...
fn emit_rust_struct(s: &GcHirStruct, out: &mut String) {
    crate::push_string_line(out, format_args!("#[derive(Debug, Clone)]"));
    crate::push_string_line(
        out,
        format_args!("pub struct {name} {{", name = s.rust_name),
    );
    for f in &s.fields {
        crate::push_string_line(
            out,
...
fn emit_ts(
    ...
    for s in structs.values() {
        crate::push_string_line(
            &mut out,
            format_args!("export interface {name} {{", name = s.rust_name),
        );
        for f in &s.fields {
