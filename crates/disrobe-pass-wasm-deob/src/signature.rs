...
/// Hard ceiling on locals a single function body may expand to.
...
pub struct FunctionSig {
    ...
    pub exported: bool,
    pub imported: bool,
    /// Local names indexed by wasm local index (params first, then declared locals).
...
/// An exported function and any additional export names aliasing the same index.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
impl ModuleSignatures {
    ...
    /// Per-function-index `(params, results)` for `CallSignatures::new`.
    #[must_use]
    ...
    /// Export aliases carrying more than one name.
    #[must_use]
    ...
    /// Attaches local names from a provider keyed by defined-function index, returning the count of functions named.
