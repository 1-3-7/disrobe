...
pub use lift::{LiftResult, LiftTarget, lift, lift_with_ssa};
...
pub use ssa::{
    BlockTarget, ConstVal, LocalId, OpKind, SideEffect, SsaBlock, SsaFunction, SsaMemArg, SsaTerm,
    SsaValue, ValueDef, ValueId, build_ssa, promote_locals_to_ssa,
};
    ...
    for block in &ssa.blocks {
        ...
        for store in &block.stores {
            ...
            out.push(AccessPattern {
                ...
                width,
                alignment: 1u32 << store.memarg.align,
...
fn classify_base_origin(v: ValueId, ssa: &SsaFunction) -> BaseOrigin {
    ...
    match def {
        ...
        ValueDef::Op { args, .. } => args
            .first()
            .copied()
            .map_or(BaseOrigin::Unknown, |a| classify_base_origin(a, ssa)),
        ValueDef::Load { addr, .. } => {
            let inner: BaseOrigin = classify_base_origin(*addr, ssa);
        ...
        ValueDef::Phi { .. } => BaseOrigin::Unknown,
    }
}
    ...
    match def {
        ValueDef::Op { kind, args, .. } => {
            matches!(kind, OpKind::I32Shl | OpKind::I32Mul)
                || args.iter().any(|a| address_is_indexed(*a, ssa))
        }
        _ => false,
    }
}
