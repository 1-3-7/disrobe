...
/// Lifts a function body from its structured WebAssembly operator stream to compilable source.
...
impl Translator<'_> {
    ...
    fn translate_numeric_or_memory(&mut self, op: &Operator<'_>) -> Result<()> {
        ...
        match op {
            ...
            other => {
                self.emit_untranslated(other);
                Ok(())
            }
        }
    }

    /// Records an unlowerable operator and emits a marked stub, counting it as untranslated.
    fn emit_untranslated(&mut self, op: &Operator<'_>) {
        let mnemonic: String = operator_mnemonic(op);
        match self.lang {
            HighLang::Rust => {
                self.emit_stmt(&format!(
                    "todo!(\"DR-WASMDEOB: untranslated op {mnemonic}\");"
                ));
            }
            HighLang::TypeScript => {
                self.emit_stmt(&format!(
                    "throw new Error(\"DR-WASMDEOB: untranslated op {mnemonic}\");"
                ));
            }
            HighLang::C => {
                self.emit_stmt(&format!(
                    "__builtin_trap(); /* untranslated op {mnemonic} */"
                ));
            }
        }
...
/// Sanitizes a debug-info local name into a valid lowered identifier.
