...
impl Translator<'_, '_> {
        ...
        match self.lang {
            ...
            HighLang::TypeScript => {
                if idiomatic {
                    ...
                    push_line!(self.out, "{pad}b{label}: while (true) {{");
                }
                self.indent += 1;
            }
            HighLang::C => {
    ...
    fn branch_action(&self, frame: &Frame) -> String {
        if let Some(loop_label) = frame.merged_into {
            return match self.lang {
                HighLang::Rust => format!("break 'b{loop_label};"),
                HighLang::TypeScript => format!("break b{loop_label};"),
        ...
        match self.lang {
            ...
            HighLang::TypeScript => match frame.kind {
                FrameKind::Loop => format!("continue b{};", frame.label),
                FrameKind::Block | FrameKind::If => format!("break b{};", frame.label),
            },
    ...
    fn emit_catch_handler(&mut self, catch: &wasmparser::Catch) {
        ...
        match self.lang {
            ...
            HighLang::C | HighLang::TypeScript => {
                self.emit_stmt(&format!("if ({predicate}) {{ {action} }}"));
            }
        }
    }
...
mod merge_tests {
    ...
    fn counted_loop_collapses_to_single_labeled_loop_typescript() {
        ...
        assert_eq!(
            source.matches(": while (true) {").count(),
            1,
            "the block+loop counted idiom must collapse to ONE labeled loop:\n{source}"
        );
