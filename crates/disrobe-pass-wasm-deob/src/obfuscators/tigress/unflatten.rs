...
///
/// Each round rewrites every case block that branches back to the dispatcher with a
/// known next-state write into a direct branch to the successor case, then re-scans
/// for cases that became chainable. Iterates until no further case inlines or the
/// safety bound is hit. The dispatcher header is marked unreachable once any case is
/// inlined so downstream DCE drops it.
...
/// Repeatedly detects and flattens dispatchers until the function reaches a fixed
/// point (no dispatcher remains, or no dispatcher yields a new inline).
///
/// Handles stacked / nested Tigress dispatchers: each detected dispatcher is flattened
/// to its own fixed point, then the function is re-scanned for a freshly exposed inner
/// dispatcher. Returns the aggregate statistics across all dispatchers.
...
/// Most recent constant write to the state variable in `block_id`, tracking both
/// linear-memory stores and global writes (Tigress emits either depending on the
/// state-variable storage class). Constant folding through a `local.tee`-style copy
/// is resolved by following single-const definitions.
