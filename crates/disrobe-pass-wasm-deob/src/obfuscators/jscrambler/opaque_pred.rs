...
fn fold_const_binop(ssa: &SsaFunction, v: ValueId) -> Option<i32> {
    let ValueDef::Op { kind, args, .. } = ssa.value_def(v)? else {
        return None;
    };
    let lhs: i32 = const_i32(ssa, *args.first()?)?;
    let rhs: i32 = const_i32(ssa, *args.get(1)?)?;
    Some(eval_i32_binop(*kind, lhs, rhs))
}
...
fn eval_i32_binop(kind: OpKind, a: i32, b: i32) -> i32 {
...
mod tests {
    ...
    fn eval_eq_two_equal_consts_is_one() {
        assert_eq!(eval_i32_binop(OpKind::I32Eq, 7, 7), 1);
    }

    #[test]
    fn eval_eq_two_distinct_consts_is_zero() {
        assert_eq!(eval_i32_binop(OpKind::I32Eq, 7, 9), 0);
    }

    #[test]
    fn eval_unsigned_lt_treats_negatives_as_large() {
        assert_eq!(eval_i32_binop(OpKind::I32LtU, -1, 1), 0);
        assert_eq!(eval_i32_binop(OpKind::I32LtS, -1, 1), 1);
    }
}
