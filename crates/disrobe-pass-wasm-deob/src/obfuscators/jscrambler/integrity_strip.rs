...
/// Eliminates CFG-level integrity re-entry guard blocks left after stubbing the checks.
...
/// Classifies a guard block, returning `(continue_arm, trap_block_id)` if it is one.
fn classify_guard(ssa: &SsaFunction, bidx: usize) -> Option<(BlockTarget, BlockId)> {
    let block: &crate::ssa::SsaBlock = ssa.blocks.get(bidx)?;
    let SsaTerm::BrIf { then_t, else_t, .. }: &SsaTerm = &block.terminator else {
        return None;
    };
