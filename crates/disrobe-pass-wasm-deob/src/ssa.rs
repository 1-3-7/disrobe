...
/// Per-function-index call signature (parameter + result types).
///
/// Recovered from the module type + function sections. Lets `build_ssa` pop exactly the
/// callee arity for a `call` (instead of draining the whole value stack) and assign the
/// real result type.
