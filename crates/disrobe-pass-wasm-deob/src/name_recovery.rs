    ...
    F: FnMut(u32) -> Option<(Vec<Option<String>>, Vec<Option<String>>)>,
{
    ...
    crate::debug::dbg_kv("dwarf-names", || {
        format!("functions_with_names={attached} defined_functions={defined_function_count}")
    });
    NameRecoveryStats {
    ...
    F: FnMut(u32) -> Option<(u32, u32)>,
{
    ...
    crate::debug::dbg_kv("sourcemap-names", || {
        format!(
            "functions_with_names={attached} defined_functions={defined_function_count} map_segments={}",
            map.segment_count()
        )
    });
    NameRecoveryStats {
