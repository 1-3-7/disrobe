...
/// WASM custom-section name carrying the `.wasm.map` source-map URL.
...
impl SourceMap {
    ...
    /// Resolves a segment's original source path, prepending `sourceRoot`.
    #[must_use]
    ...
    /// Floor lookup: the segment with the greatest generated offset not exceeding `byte_offset`.
    #[must_use]
...
/// Parses a source-map v3 JSON document into a resolved [`SourceMap`].
