...
fn emit_rust(
    ...
    let _: std::fmt::Result = writeln!(out, "pub trait Imports{world_ty} {{");
    for item in imports {
        match item.kind {
    ...
    let _: std::fmt::Result = writeln!(out, "pub trait Exports{world_ty} {{");
    for item in exports {
        match item.kind {
            ComponentBindingKind::Func => {
                let _: std::fmt::Result = writeln!(
...
fn emit_ts(
    ...
    let mut out: String = String::with_capacity(512);
    let world_ty: String = pascal(world);
    let _: std::fmt::Result = writeln!(out, "export interface Imports{world_ty} {{");
    for item in imports {
        match item.kind {
            ComponentBindingKind::Func => {
                let _: std::fmt::Result = writeln!(out, "  {name}(): void;", name = item.ts_ident);
            }
            _ => {
                let _: std::fmt::Result =
                    writeln!(out, "  readonly {name}: unknown;", name = item.ts_ident);
            }
        }
    }
    out.push_str("}\n\n");

    let _: std::fmt::Result = writeln!(out, "export interface Exports{world_ty} {{");
    for item in exports {
        match item.kind {
            ComponentBindingKind::Func => {
                let _: std::fmt::Result = writeln!(out, "  {name}(): void;", name = item.ts_ident);
...
fn emit_wit(
    ...
    let _: std::fmt::Result = writeln!(out, "package disrobe:recovered;");
    let _: std::fmt::Result = writeln!(out);
    let _: std::fmt::Result = writeln!(out, "world {world} {{");
    for item in imports {
        match item.kind {
            ComponentBindingKind::Func => {
                let _: std::fmt::Result =
                    writeln!(out, "  import {name}: func();", name = item.name);
            }
            ComponentBindingKind::Instance => {
                let _: std::fmt::Result =
                    writeln!(out, "  import {name}: interface {{}}", name = item.name);
            }
            ComponentBindingKind::Type => {
                let _: std::fmt::Result = writeln!(out, "  import {name}: type;", name = item.name);
            }
            _ => {
                let _: std::fmt::Result = writeln!(
                    out,
                    "  // import {name} (unsupported kind)",
                    name = item.name
                );
            }
        }
    ...
    for item in exports {
        match item.kind {
            ComponentBindingKind::Func => {
                let _: std::fmt::Result =
                    writeln!(out, "  export {name}: func();", name = item.name);
            }
            _ => {
                let _: std::fmt::Result = writeln!(
                    out,
                    "  // export {name} (kind {kind:?})",
                    name = item.name,
                    kind = item.kind
                );
            }
...
mod tests {
    ...
    fn hello_component_lifts_to_typed_bindings() {
        ...
        assert!(bindings.wit_source.contains("world hello_world"));
