...
`tests/word_sweep.rs` grades AArch64 decode word by word against LLVM 19.1.7, which does not read the vendored Ghidra language files. The sweep covers 2830 distinct words: every word of the three committed AArch64 corpora, field-boundary variants of each distinct corpus word, and 192 seeded random words in each of the nine top-level encoding groups. The decoder matches a constructor for 1382 words, and 1373 of those 1382 mnemonics agree with the reference after one declared alias equivalence, which is 99.35 percent. The remaining 9 accepted words are 5 mnemonics in one recorded disagreement class and 4 encodings that the reference rejects. The disagreement is the MOV alias applied to an `orr` with a shifted or rotated second source, where the emitted P-code still applies the shift. Each alias equivalence, disagreement class, and over-accepted encoding is a declared constant with a reason, so a new one requires a deliberate edit. A further 168 words that the decoder declines are accepted by the reference; that number is reported separately and never counted as agreement. All 206 comparable branch and address targets match the reference exactly.
