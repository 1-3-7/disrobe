...
`tests/word_sweep.rs` grades AArch64 decode word by word against LLVM 19.1.7, which does not read the vendored Ghidra language files. The sweep covers 2830 distinct words: every word of the three committed AArch64 corpora, field-boundary variants of each distinct corpus word, and 192 seeded random words in each of the nine top-level encoding groups. The decoder matched a constructor for 1315 of those words and 1272 of the 1315 mnemonics agree with the reference, which is 96.73 percent. The remaining 43 accepted words are 39 mnemonics that disagree in nine recorded classes and 4 encodings that the reference rejects. Each class and each over-accepted encoding is a declared constant with a reason, so a new one needs a deliberate edit. A further 235 words that the decoder declines are accepted by the reference; that number is reported separately and never counted as agreement. All 206 comparable branch and address targets match the reference exactly. The nine recorded mnemonic classes are the `tb`, `bc`, and unscaled load and store constructor stems that keep the stem and drop the suffix, and the MOV alias applied to a shifted or rotated `orr`, where the emitted P-code still applies the shift.
