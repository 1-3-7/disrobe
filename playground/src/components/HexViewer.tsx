...
export function HexViewer({
  ...
  function onGridKeyDown(event: KeyboardEvent<HTMLDivElement>): void {
    ...
    const step: Record<string, number> = {
      ...
      PageUp: -effectiveBytesPerRow * viewportRows,
      PageDown: effectiveBytesPerRow * viewportRows,
      Home: event.ctrlKey ? -selection.head : -(selection.head % effectiveBytesPerRow),
      End: event.ctrlKey ? total - 1 - selection.head : effectiveBytesPerRow - 1 - selection.head % effectiveBytesPerRow,
    };
  ...
  for (let row: number = firstVisibleRow; row < lastVisibleRow; row += 1) {
    ...
    for (let col: number = 0; col < effectiveBytesPerRow; col += 1) {
      ...
      if (offset >= total) {
        hexCells.push(
          <span key={`h-${offset}`} aria-hidden="true" className="inline-block w-[1.25rem]" />,
        );
        continue;
      }
      ...
      hexCells.push(
        <span
          ...
          id={`${gridId}-${offset}`}
          aria-colindex={col + 2}
          aria-label={`Offset ${offsetHex(offset, offsetWidth)}: ${byteHex(value)}${value >= 0x20 && value <= 0x7e ? ` (${printable(value)})` : ""}`}
    ...
    rows.push(
      ...
      >
        <span aria-colindex={1} className="shrink-0 text-ink-faint" role="rowheader">{offsetHex(base, offsetWidth)}</span>
        <span className="shrink-0" role="presentation">{hexCells}</span>
        <span aria-hidden="true" className="shrink-0 border-l border-hairline pl-3">{asciiCells}</span>
      </div>,
    );
  }
  ...
  return (
    ...
    >
      ...
      <div
        ref={scrollRef}
        aria-activedescendant={selection !== null && selection.head >= firstVisibleRow * effectiveBytesPerRow && selection.head < lastVisibleRow * effectiveBytesPerRow ? `${gridId}-${selection.head}` : undefined}
        aria-colcount={effectiveBytesPerRow + 1}
        ...
        tabIndex={0}
        onFocus={(): void => {
          if (selection === null && total > 0) setSelection({ anchor: 0, head: 0 });
        }}
        onKeyDown={onGridKeyDown}
        onScroll={onScroll}
      >
        <div className="relative w-max min-w-full" role="presentation" style={{ height: totalHeight }}>
          {rows}
        </div>
      </div>
