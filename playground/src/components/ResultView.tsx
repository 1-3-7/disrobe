...
import { DartKernelView } from "@/components/DartKernelView";
import { GoView } from "@/components/GoView";
import { HermesView } from "@/components/HermesView";
import { JvmClassView } from "@/components/JvmClassView";
import { DotnetView } from "@/components/DotnetView";
import { Metric } from "@/components/Metric";
import { PharView } from "@/components/PharView";
import { PyarmorView } from "@/components/PyarmorView";
import { RubyView } from "@/components/RubyView";
import { SourceMapView } from "@/components/SourceMapView";
import { StatusChip, type StatusTone } from "@/components/StatusChip";
import { Button } from "@/components/ui/button";
import { downloadBlob } from "@/lib/download";
import { downloadMetaFor, languageForSourcePath, type EditorLanguage } from "@/lib/editor";
...
export function ResultView({ mode, data, onJumpToEntry, input, onUseInput }: ResultViewProps): ReactElement {
  if (mode.render === "source-format") {
    const result: FormatResult = data as FormatResult;
    const filename: string = `formatted.${result.language}`;
    const language: EditorLanguage = languageForSourcePath(filename);
    const shortened: boolean = result.source.length > 64 * 1024;
    return (
      <section aria-label="Formatted source" className="flex min-w-0 flex-col gap-2 sm:gap-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <StatusChip label="Formatted source" tone="accent" />
          <Button aria-label="Download formatted source" onClick={(): void => downloadBlob(filename, new Blob([result.source], { type: downloadMetaFor(language).mime }))}>Download</Button>
        </div>
        {shortened ? <p className="font-sans text-xs text-ink-dim">Preview shortened. Downloads contain the complete output.</p> : null}
        {result.source.length === 0 ? <p role="status" className="font-sans text-sm text-ink-dim">The source file is empty.</p> : <CodeEditor code={shortened ? result.source.slice(0, 64 * 1024) : result.source} label="formatted source" language={language} />}
      </section>
    );
  }
  if (mode.render === "source-map") return <SourceMapView result={data as SourceMapResult} />;
  if (mode.render === "js-unbundle") return <BundleView result={data as BundleResult} />;
  if (mode.render === "go") return <GoView result={data as GoResult} />;
  if (mode.render === "phar") return <PharView result={data as PharResult} input={input} onUseInput={onUseInput} />;
  if (mode.render === "apk") return <ApkView result={data as ApkResult} />;
  if (mode.render === "dart-kernel") return <DartKernelView result={data as DartKernelResult} />;
  if (mode.render === "hermes") return <HermesView result={data as HermesResult} />;
  if (mode.render === "jvm-class") return <JvmClassView result={data as JvmClassResult} />;
  if (mode.render === "dotnet") return <DotnetView result={data as DotnetResult} />;

  if (mode.render === "detect") {
    const result: DetectResult = data as DetectResult;
    return (
      <div className="flex flex-col gap-4">
        <ChipRow><StatusChip label={result.format} tone={result.format === "unknown" ? "muted" : "accent"} /></ChipRow>
        <p className="font-sans text-sm text-ink">{result.detail}</p>
        {onJumpToEntry !== undefined ? <Button onClick={(): void => { onJumpToEntry("auto_route"); }}>Find recovery passes</Button> : null}
        <CodeEditor code={prettyJson(result)} downloadName="format.json" label="format identification" language="json" />
      </div>
    );
  }

  if (mode.render === "pyarmor-module") return <PyarmorView result={data as PyarmorModuleResult} />;

  if (mode.render === "pyarmor") {
    const result: PyarmorDetectResult | PyarmorClassifyResult = data as PyarmorDetectResult | PyarmorClassifyResult;
    return (
      <div className="flex flex-col gap-4">
        <ChipRow>
          <StatusChip label={`PyArmor ${result.detection.version}`} tone="accent" />
          <StatusChip label={result.detection.protection} tone="muted" />
          <StatusChip label={`${result.detection.confidence} confidence`} tone="muted" />
        </ChipRow>
        <MetricGrid>
          <Metric label="Python version" value={result.detection.python_version ?? "Not identified"} />
          <Metric label="Runtime serial" value={result.detection.serial ?? "Not identified"} />
          <Metric label="Initialization vector" value={result.detection.has_iv ? "Present" : "Absent"} />
        </MetricGrid>
        {"classification" in result ? (
          <>
            <MetricGrid>
              <Metric label="Script type" value={result.classification.script_type} />
              <Metric label="Bootstrap" value={result.classification.bootstrap_import} />
              <Metric label="Disposition" value={result.classification.disposition} />
            </MetricGrid>
            <ChipRow>
              <StatusChip label={`RFT ${result.classification.rft_enabled ? "enabled" : "not detected"}`} tone="muted" />
              <StatusChip label={`ECC ${result.classification.ecc_enabled ? "enabled" : "not detected"}`} tone="muted" />
              <StatusChip label={`Mixed strings ${result.classification.mix_str_enabled ? "enabled" : "not detected"}`} tone="muted" />
            </ChipRow>
            <NoteList title="Classification notes" notes={result.classification.notes} />
          </>
        ) : <Metric label="Embedded payload" value={formatBytes(result.payload_len)} />}
        <NoteList title="Detection notes" notes={result.detection.diagnostics} />
        <CodeEditor code={prettyJson(result)} downloadName={`${mode.id}.json`} label="PyArmor inspection report" language="json" />
      </div>
    );
  }

  if (mode.render === "swift-objc") {
    const result: SwiftObjcResult = data as SwiftObjcResult;
    return (
      <div className="flex flex-col gap-4">
        <ChipRow>
          <StatusChip label={result.report.container} tone="accent" />
          <StatusChip label={`${result.slice_count} architecture ${result.slice_count === 1 ? "slice" : "slices"}`} tone="muted" />
        </ChipRow>
        {result.report.container === "Other" ? (
          <p className="font-sans text-sm text-ink-muted" role="status">Choose a Mach-O binary or an IPA package to inspect Swift and Objective-C metadata.</p>
        ) : null}
        {result.report.slices.map((slice, index: number): ReactElement => (
          <section key={index} className="flex flex-col gap-3">
            <h3 className="font-sans text-sm font-semibold text-ink">{slice.cpu_label} · {slice.bitness_bits}-bit</h3>
            <MetricGrid>
              <Metric label="Swift types" value={String(slice.metadata_summary.swift_nominal_types)} />
              <Metric label="Objective-C classes" value={String(slice.metadata_summary.objc_classes)} />
              <Metric label="Demangled symbols" value={String(slice.metadata_summary.swift_demangled_symbols)} />
              <Metric label="Protocols" value={String(slice.metadata_summary.swift_protocols)} />
              <Metric label="Function starts" value={String(slice.metadata_summary.function_starts_recovered)} />
              <Metric label="Exported symbols" value={String(slice.metadata_summary.exported_symbols_recovered)} />
            </MetricGrid>
            {slice.swift.type_dump.nominal_types.map((type): ReactElement => (
              <div key={type.qualified_name} className="flex min-w-0 flex-col gap-2 border-b border-hairline py-3">
                <h4 className="break-words font-mono text-[13px] text-ink">{type.qualified_name}</h4>
                <dl className="grid gap-1 font-mono text-xs text-ink-muted">
                  {type.fields.map((field): ReactElement => (
                    <div key={field.name} className="flex min-w-0 flex-wrap justify-between gap-x-4 gap-y-1">
                      <dt className="break-all">{field.name}</dt><dd className="break-all">{field.demangled_type ?? field.mangled_type}</dd>
                    </div>
                  ))}
                </dl>
              </div>
            ))}
            <NoteList title="Protocols" notes={slice.swift.type_dump.protocols.map((protocol): string => protocol.qualified_name)} />
            <NoteList title="Objective-C selectors" notes={slice.objc.unique_selectors} />
          </section>
        ))}
        <CodeEditor code={prettyJson(result)} downloadName="swift-objc.json" label="Swift and Objective-C inspection report" language="json" />
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col gap-4">
        <ChipRow>
          ...
          />
          <StatusChip
            label={result.detection.has_name_section ? "name section" : "stripped names"}
  ...
  if (mode.render === "wasm-component") {
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <CodeEditor badge="wit" code={result.bindings.wit_source} downloadName="world" label="reconstructed WIT world" language="wit" />
  ...
  if (mode.render === "wasm-signatures") {
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        {result.boundary_link_collection_status.status === "truncated" ? (
          <p className="font-mono text-[12px] text-ink-muted" role="status">
            Cross-language links: {result.boundary_link_collection_status.retained_link_count.toLocaleString()} of {result.boundary_link_collection_status.link_count.toLocaleString()} retained.
          </p>
        ) : null}
  ...
  if (mode.render === "ruby") return <RubyView result={data as RubyDetectResult} />;
  ...
  if (mode.render === "beam") {
    ...
    const isElixir: boolean = result.recovered_from === "ElixirDbgiForm";
    return (
      <div className="flex flex-col gap-4">
        <ChipRow>
          <StatusChip dot label={result.module} tone="accent" />
          <StatusChip label={result.recovered_from} tone="muted" />
          <StatusChip dot label={isElixir ? "elixir" : "erlang"} tone="accent" />
        </ChipRow>
        <CodeEditor
          badge={isElixir ? "elixir" : "erlang"}
          code={result.erlang_source}
          downloadName={isElixir ? "recovered.ex" : "recovered.erl"}
          label={isElixir ? "recovered Elixir source" : "recovered Erlang source"}
          language={isElixir ? "elixir" : "erlang"}
        />
      </div>
    );
  }
  ...
  if (mode.render === "as3") {
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <CodeEditor badge="as3" code={result.program} downloadName="recovered.as" label="decompiled ActionScript 3" language="as3" />
      </div>
    );
  }
  ...
  if (mode.render === "shell") {
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <CodeEditor badge="batch" code={result.batch.output} downloadName="deobfuscated.bat" label="deobfuscated script" language="batch" />
      </div>
    );
  }
