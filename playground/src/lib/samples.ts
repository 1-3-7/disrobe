...
async function loadFile(file: string, signal: AbortSignal, maxBytes: number): Promise<Uint8Array> {
  const cached: Uint8Array | undefined = fileCache.get(file);
  if (cached !== undefined) {
    if (cached.byteLength > maxBytes) throw new Error("Sample exceeds this input limit.");
    return cached;
  }
  const url: string = `${import.meta.env.BASE_URL}samples/${file}`;
  const timeout: AbortController = new AbortController();
  const timer: ReturnType<typeof setTimeout> = setTimeout((): void => {
    timeout.abort(new Error("Loading the sample exceeded 30 seconds. Try again."));
  }, 30_000);
  try {
    const response: Response = await fetch(url, { signal: AbortSignal.any([signal, timeout.signal]) });
    if (!response.ok) {
      throw new Error(`Could not load sample ${file}: HTTP ${response.status}. Try again.`);
    }
    if (response.body === null) throw new Error(`Sample ${file} has no response body.`);
    ...
    try {
      for (;;) {
        ...
        if (next.value.byteLength > maxBytes - total) {
          await reader.cancel();
          throw new Error("Sample exceeds this input limit.");
        }
...
export async function resolveSample(sample: Sample, signal: AbortSignal, maxBytes: number = 64 * 1024 * 1024): Promise<Uint8Array> {
  signal.throwIfAborted();
  if (sample.source.kind === "text") {
    return encoder.encode(sample.source.text);
  }
  return loadFile(sample.source.file, signal, maxBytes);
}
