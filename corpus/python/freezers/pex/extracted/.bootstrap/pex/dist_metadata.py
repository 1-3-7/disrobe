...
class MetadataFiles(object):
    metadata = attr.ib()
    _additional_metadata_files = attr.ib(default=())
    _read_function = attr.ib(default=None)
...
def requires_dists(location):
    ...
    legacy_requires = metadata_files.metadata.pkg_info.get_all("Requires", [])
    if legacy_requires:
        pex_warnings.warn(
            dedent(
                """\
...
class Constraint(object):
    ...
    name = attr.ib(eq=False)
    specifier = attr.ib(factory=SpecifierSet, order=False)
    marker = attr.ib(default=None, eq=str)

    project_name = attr.ib(init=False, repr=False)
    _str = attr.ib(init=False, eq=False, repr=False)
    _legacy_version = attr.ib(init=False, repr=False)
...
class Requirement(Constraint):
    ...
    extras = attr.ib(default=frozenset())
...
class ProjectMetadata(object):
    project_name = attr.ib()
    version = attr.ib()
    requires_dists = attr.ib(default=())
    requires_python = attr.ib(default=SpecifierSet())
...
class DistMetadata(object):
    ...
    files = attr.ib(eq=False)
    project_metadata = attr.ib()
...
class Distribution(object):
    ...
    location = attr.ib(converter=_realpath)
