...
These `.wat` (WebAssembly text) sources compile to `.wasm` via `wat2wasm` (from [WABT](https://github.com/WebAssembly/wabt)) and exercise `disrobe-pass-wasm-deob`: decompilation, defragmentation, exception-handler lifting, and opcode-table recovery.
...
`wat2wasm` lives in `wabt`; if absent the `corpus/generate.{sh,ps1}` walker skips WASM compilation & logs `skipped wasm: wat2wasm not found`.
