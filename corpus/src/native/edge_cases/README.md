...
Small source files for cross-platform native binaries that exercise the binary-format passes (`disrobe-binfmt`) and stress the layout edges (stripping, TLS callbacks, RTTI, PIE, visibility, custom sections, static linkage).
...
| `cxx_rtti_dyncast.cpp` | `cxx_rtti_dyncast.build.sh` | ELF | RTTI / `dynamic_cast` (forces `typeinfo` and vtable retention). |
...
In this workspace `clang`, `gcc`, `g++`, and the MSVC `cl.exe` are NOT on PATH. Go (`go`) and Rust (`rustc`) ARE on PATH and both `go_hello.go` and `rust_hello.rs` compile cleanly:
...
For the other recipes install the relevant toolchain (`build-essential` / `xcode-cli` / MSVC build tools) and run the matching `*.build.sh` or `*.build.ps1`. Each recipe writes into `corpus/generated/native/`.
