...
CPython source fixtures covering syntax, Unicode, typing, and control flow. They provide inputs for `disrobe-pass-py-disasm` and `disrobe-pass-py-deob`; wrapper generators can package them for the PyArmor, PyInstaller, Nuitka, and SourceDefender passes.
...
| `comments_only.py` | trivial | py-disasm | module containing only `pass`. |
...
From this directory, compile the fixtures in PowerShell:

```powershell
[string[]]$sources = Get-ChildItem -LiteralPath . -Filter '*.py' -File |
