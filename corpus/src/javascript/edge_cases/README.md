...
Hand-authored ES2022+ sources that exercise the wild edges of real-world JavaScript input. All files parse cleanly via `node --check` (Node 24). They feed `disrobe-pass-js-deob` and provide upstream input for any future JS bundler/protector wrappers.
...
| `mixed_line_endings.js` | encoding | js-deob | CRLF + LF + U+2028 + U+2029 in templates and runtime regex. |
...
| `destructuring_advanced.js` | pattern | js-deob | nested rename + default + `...rest` in both object and array patterns. |
