...
ES2022+ source fixtures covering syntax, Unicode, control flow, and runtime data structures for `disrobe-pass-js-deob`.
...
```powershell
[System.IO.FileInfo[]]$sources = Get-ChildItem -Path '*.js', '*.mjs' -File
[string[]]$failures = @()
...
if ($failures.Count -gt 0) {
    throw "Invalid JavaScript fixtures: $($failures -join ', ')"
}
```

Node 24.19.0 rejects `mixed_line_endings.js` and
`whitespace_categories.js` because they contain literal line breaks inside
double-quoted strings. Both files require correction.
