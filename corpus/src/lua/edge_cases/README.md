...
Lua 5.4 and LuaJIT sources exercising coroutines, metatables, closures, FFI, bit ops, patterns, gotos, dynamic environments. They feed any future Lua bytecode pass.
...
`luajit_ffi.lua` and `luajit_bitops.lua` require LuaJIT specifically and will syntax-check under stock 5.4 but fail at runtime on the `ffi`/`bit` requires.
