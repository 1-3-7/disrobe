...
TypeScript 5.x sources exercising the type-system edges & modern decorator syntax. They feed `disrobe-pass-js-deob` once minified/bundled & serve as a baseline for any future TS-aware passes.
...
Install the playground dependencies with `bun install --frozen-lockfile` from
`playground/`, then run this command from this directory in PowerShell:

```powershell
[string[]]$sources = Get-ChildItem -LiteralPath . -Filter '*.ts' -File |
    Select-Object -ExpandProperty FullName

node ../../../../playground/node_modules/typescript/bin/tsc `
    --noEmit --strict --target ES2022 --module ES2022 --moduleResolution Bundler `
    --lib ES2022,DOM,ESNext.Disposable --skipLibCheck `
    --typeRoots ../../../../playground/node_modules/@types --types node @sources
```

The check includes the disposable-resource and Node type declarations required by
`using_declarations.ts` and `import_type.ts`. All 21 fixtures pass with strict
checking in TypeScript 5.9.3 and 7.0.1-rc, using `@types/node` 22.19.21.
