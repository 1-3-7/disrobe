...
PyInstaller target sources live in `../python/`; PyInstaller builds run out-of-tree in `.developer/pyinst-build/` so that the heavyweight `dist/` & `build/` trees do not pollute `corpus/`.
