...
|---|---:|---|---|
| `ironbrew2/obfuscated/*.{min,max}.lua` | 9-15 KB each | `Trollicus/ironbrew-2` (MIT, C#) built from source and run on the author's benign `ironbrew2/original/*.lua` | `.min` = ControlFlow/Mutate/SuperOperators/Compress all OFF (full devirt + execution-oracle pass); `.max` = stock 2.7.0 defaults (all ON); see `ironbrew2/MANIFEST.toml` |
