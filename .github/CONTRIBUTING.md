...
`disrobe` is a multi-language deobfuscator and decompiler suite. Changes must preserve reproducible static recovery artifacts under the same inputs, options, and tool versions.
...
- `disrobe-binfmt` - container and archive detection across 103 declared formats, with extraction coverage recorded in the evidence report.
...
- `disrobe-pass-swift-objc` - Swift and Objective-C class-dump, SwiftShield mapping parser, and explicit-key single-byte XOR blob decoding.
...
Contributions are accepted only under the [Disrobe Contributor Assignment and License Agreement](../CONTRIBUTING-LICENSE.md), which implements Section 6 of the [LICENSE](../LICENSE). Before a pull request can be merged, post the complete acceptance statement from that agreement as a comment on the pull request, from the account that authored the commits it covers. Post a fresh acceptance if you add substantive commits afterwards. Opening a pull request grants you no rights in Disrobe beyond the LICENSE.
