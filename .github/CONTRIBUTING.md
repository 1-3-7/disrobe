...
- `disrobe-pass-sourcedefender` - `.pye` envelope decrypt (filename-derived password + AES + msgpack).
...
- `disrobe-pass-jvm` - classfile / jar / dex / apk decompile via CFR / Vineflower / Procyon / JADX.
- `disrobe-pass-dotnet` - .NET PE/CLR decompile via ILSpy / dnSpyEx / de4dot + protector detection.
- `disrobe-pass-native` - PE / ELF / Mach-O symbol dump + Ghidra-headless decompile + packer unpack.
