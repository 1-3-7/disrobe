...
WALKTHROUGH_BASE: str = "https://1-3-7.github.io/disrobe/latest/assets/walkthrough/"
HTML_MEDIA_RE: re.Pattern[str] = re.compile(
    r'''(?P<attribute>\b(?:src|href|poster))=(?P<quote>["'])(?P<target>[^"']+)(?P=quote)'''
)
...
def resolve_asset_url(source: Path, target: str) -> str | None:
    ...
    if not resolved.exists():
        return None
    if rel.parts[:4] == ("docs", "src", "assets", "walkthrough"):
        return f"{WALKTHROUGH_BASE}{Path(*rel.parts[4:]).as_posix()}{raw[len(path_part):]}"
...
def rewrite_links(source: Path, text: str, warnings: list[str]) -> str:
    def repl(match: re.Match[str]) -> str:
        ...
        if rewritten is None:
            asset: str | None = resolve_asset_url(source, target)
            if asset is not None and asset.startswith(WALKTHROUGH_BASE):
                rewritten = asset
        if rewritten is None:
...
def rewrite_media_attributes(source: Path, text: str) -> str:
    def repl(match: re.Match[str]) -> str:
        url: str | None = resolve_asset_url(source, match.group("target"))
        if url is None or not url.startswith(WALKTHROUGH_BASE):
            return match.group(0)
        quote: str = match.group("quote")
        return f"{match.group('attribute')}={quote}{url}{quote}"

    return HTML_MEDIA_RE.sub(repl, text)
...
def transform_page(page: Page, warnings: list[str]) -> str:
    ...
    text = rewrite_media_attributes(page.source, text)
