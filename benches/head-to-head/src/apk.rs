...
use disrobe_pass_jvm::{
    ...
    decompile_class_with_inners, parse_classfile, parse_code_items, parse_dex,
};
...
use crate::tool::{
    ...
    MAX_ZIP_ENTRIES, MAX_ZIP_ENTRY_BYTES, MAX_ZIP_TOTAL_BYTES, TOOL_TIMEOUT, ToolInvocation,
    VersionCapture, bounded_error_text, find_on_path, read_bounded_file, read_bounded_string,
    require_pinned_token, require_pinned_version, require_success, retain_source_tree,
    retain_tool_provenance, run, version_capture_checked, version_of_checked,
};
...
pub fn measure(root: &Path, run_dir: &Path) -> Result<(String, Value)> {
    ...
    let javac: PathBuf = find_on_path("javac")
        .ok_or_else(|| eyre::eyre!("javac is required for the APK publication run"))?;
    ...
    let apk_run_dir: PathBuf = run_dir.join("apk-jadx-cfr");
    std::fs::create_dir(&apk_run_dir).wrap_err_with(|| {
        format!(
            "creating unique APK artifact directory {}",
            apk_run_dir.display()
        )
    })?;
    retain_jdk_provenance(root, &apk_run_dir, &javac, &dex_sha256, &jar_sha256)
    ...
    ) = jadx_outcome(&javac, &dex_bytes, denominator, &apk_run_dir.join("jadx"))
        .map_err(|error: String| eyre::eyre!(error))?;
    let dex_leg: Leg = Leg {
        key: "dex",
        label: "DEX leg",
        disrobe_name: "disrobe (in-house Dalvik, DEX input)",
        disrobe: score_disrobe_dex(
            &javac,
            &dex_bytes,
            denominator,
            &apk_run_dir.join("disrobe-dex"),
        )
        .map_err(|error: String| eyre::eyre!(error))?,
    ...
    let jar_leg: Leg = Leg {
        ...
        disrobe: score_disrobe_jar(
            &javac,
            &jar_bytes,
            denominator,
            &apk_run_dir.join("disrobe-jar"),
        )
        .map_err(|error: String| eyre::eyre!(error))?,
        competitor_name: "cfr (JAR input)",
        competitor_short: "cfr",
        competitor: cfr_outcome(
            root,
            &javac,
            &jar_path,
            denominator,
            &apk_run_dir.join("cfr"),
        )
        .map_err(|error: String| eyre::eyre!(error))?,
    };
    ...
    let value: Value = json!({
        "id": id,
        "title": "APK / DEX decompilation: emitted-region compile yield under real javac",
        "status": "ok",
        "ecosystem": "android",
        "dataset": dataset,
        "oracle": "javac compiles the complete recovered source set against a stubbed classpath that excludes the original classes. If a parse error prevents attribution, the scorer isolates the implicated balanced method, field-initializer, or type region and reruns javac, up to 64 rounds. Methods in isolated regions fail; remaining methods are graded from compiler diagnostics. Unmapped parse errors leave the result uncertified.",
        "metric": format!("compile-clean / emitted method regions in each tool's recovered source set. The original main class declares {denominator} methods, which supplies fixture context only. Each tool emits a different population that may include nested, sibling, synthetic accessor, lambda, and bridge methods. These ratios are per-output compile yields; they do not measure source recovery, semantic equivalence, or a shared fixed method population."),
        ...
        "fairness": [
            ...
            "same compiler procedure for every tool: one source-only function performs the complete compile, diagnostic mapping, bounded region isolation, and emitted-region scoring without receiving a tool identity",
            ...
            "a tool that produces no EdgeCases source at all is a failed measurement, never an excluded sample",
            "tool-specific emitted-region denominators are reported independently and are not compared as recovery counts"
        ],
        "tools": tools,
...
fn require_competitor_result(
    ...
    match outcome {
        ...
        CompetitorOutcome::Scored {
            score: ToolScore::Uncertified { detail, .. },
            ..
        } => Err(format!(
            "required {tool} measurement was not certified: {detail}"
        )),
...
fn retain_jdk_provenance(
    ...
    let java: PathBuf = find_on_path("java").ok_or_else(|| "java is not on PATH".to_owned())?;
    let javac_version: VersionCapture = version_capture_checked(javac, &["-version"], None)?;
    let java_version: VersionCapture = version_capture_checked(&java, &["-version"], None)?;
    require_pinned_version(root, "javac", &javac_version.display)?;
    let java_banner: String = format!(
        "{}{}",
        String::from_utf8_lossy(&java_version.output.stdout),
        String::from_utf8_lossy(&java_version.output.stderr)
    );
    require_pinned_version(root, "java", &java_version.display)?;
    require_pinned_token(root, "java_build", &java_banner)?;
    let input_hashes: Vec<(String, String)> = vec![
        ("EdgeCases.dex".to_owned(), dex_sha256.to_owned()),
        ("EdgeCases-baseline.jar".to_owned(), jar_sha256.to_owned()),
    ];
    let javac_dir: PathBuf = artifact_dir.join("javac");
    let java_dir: PathBuf = artifact_dir.join("java");
    std::fs::create_dir(&javac_dir)
        .map_err(|error| format!("creating javac provenance directory: {error}"))?;
    std::fs::create_dir(&java_dir)
        .map_err(|error| format!("creating java provenance directory: {error}"))?;
    let javac_args: Vec<String> = vec!["-version".to_owned()];
    retain_tool_provenance(
        ...
        &ToolInvocation {
            ...
            args: &javac_args,
            environment: &[],
            output: Some(&javac_version.output),
        },
        &["-version"],
        &javac_version,
        &input_hashes,
    )?;
    let java_args: Vec<String> = vec!["-version".to_owned()];
    retain_tool_provenance(
        &java_dir,
        &ToolInvocation {
            route: "java-runtime",
            ...
            args: &java_args,
            environment: &[],
            output: Some(&java_version.output),
        },
        &["-version"],
        &java_version,
        &input_hashes,
...
fn jadx_outcome(
    ...
    denominator: usize,
    artifact_dir: &Path,
) -> std::result::Result<(CompetitorOutcome, std::result::Result<(), String>), String> {
    ...
    if let Err(error) = std::fs::create_dir(artifact_dir) {
        let reason: String = format!("creating JADX artifact directory failed: {error}");
        return Err(reason);
    }
    let version_capture: VersionCapture = match version_capture_checked(&jadx, &["--version"], None)
    {
        Ok(version) => version,
        Err(reason) => {
            return Ok((
                CompetitorOutcome::Scored {
                    ...
                    score: ToolScore::miss(denominator, reason.clone()),
                },
                Err(reason),
            ));
        }
    };
    let version: String = version_capture.display.clone();
    let mut measured: JadxScore = score_jadx(
        javac,
        dex_bytes,
        denominator,
        artifact_dir,
...
fn cfr_outcome(
    ...
    denominator: usize,
    artifact_dir: &Path,
) -> std::result::Result<CompetitorOutcome, String> {
    let Some(cfr): Option<CfrInvoke> = resolve_cfr(root) else {
        return Ok(CompetitorOutcome::Absent {
    ...
    if let Err(error) = std::fs::create_dir(artifact_dir) {
        return Err(format!("creating CFR artifact directory failed: {error}"));
    }
    let version: VersionCapture = match cfr_version_capture(&cfr) {
        Ok(version) => version,
        Err(reason) => {
            return Ok(CompetitorOutcome::Scored {
                version: "unavailable".to_owned(),
                score: ToolScore::miss(denominator, reason),
            });
        }
    };
    Ok(CompetitorOutcome::Scored {
        version: version.display.clone(),
        score: score_cfr(javac, jar_path, &cfr, denominator, artifact_dir, &version)?,
    })
}
...
impl Leg {
    fn to_json_rows(&self) -> Vec<Value> {
        ...
        competitor["source_artifact"] =
            Value::String(format!("apk-jadx-cfr/{}/sources", self.competitor_short));
        competitor["initial_javac_artifact"] = Value::String(format!(
            "apk-jadx-cfr/{}/initial-javac/provenance.json",
            self.competitor_short
        ));
        ...
        disrobe["source_artifact"] =
            Value::String(format!("apk-jadx-cfr/disrobe-{}/sources", self.key));
        disrobe["initial_javac_artifact"] = Value::String(format!(
            "apk-jadx-cfr/disrobe-{}/initial-javac/provenance.json",
            self.key
        ));
    ...
    fn sentence(&self) -> String {
        ...
        let (version, theirs): (&str, &ToolScore) = match &self.competitor {
            CompetitorOutcome::Absent { reason } => {
                return format!(
                    "{label}: `{short}` was not measured because {reason}. `disrobe` emitted-region \
                     compile yield: {ours}.",
                    label = self.label,
        ...
        if !self.disrobe.is_certified() || !theirs.is_certified() {
            return format!(
                "{label}: `disrobe` emitted-region compile yield: {ours}; `{short}` ({version}): \
                 {theirs_phrase}. The tool-specific emitted populations are reported separately.",
                label = self.label,
                short = self.competitor_short,
            );
        }
        format!(
            "{label}: `disrobe` emitted-region compile yield: {ours}; `{short}` ({version}): \
             {theirs_phrase}. The denominators describe different emitted-region populations and \
             do not support a cross-tool ranking.",
            label = self.label,
            short = self.competitor_short,
        )
    }
...
impl ToolScore {
    ...
    fn to_json(&self, name: &str, version: &str) -> Value {
        let mut row: Value = json!({
            ...
            "version": version,
            "metric": METRIC,
            "context_original_main_methods": self.original(),
            "status": self.status(),
            "detail": self.detail(),
        });
        match self {
...
fn score_disrobe_jar(
    ...
    if let Err(error) = std::fs::create_dir(artifact_dir) {
        return Err(format!(
            "creating disrobe JAR artifact directory failed: {error}"
        ));
    }
    match disrobe_jar_source(jar_bytes) {
        Ok(source) => {
            let sources: BTreeMap<String, String> =
                BTreeMap::from([(MAIN_CLASS_FILE.to_owned(), source)]);
            retain_sources_and_score(javac, &sources, denominator, artifact_dir)
        }
        Err(defect) => Ok(ToolScore::miss(denominator, defect)),
    }
}
...
fn score_jadx(
...
) -> std::result::Result<JadxScore, String> {
    let input_path: PathBuf = artifact_dir.join("EdgeCases.dex");
    let output_dir: PathBuf = artifact_dir.join("raw-output");
    write_new_file(&input_path, dex_bytes)?;
    if let Err(error) = std::fs::create_dir(&output_dir) {
        let reason: String = format!("creating retained JADX output directory failed: {error}");
        return Err(reason);
    }
    let invocation_args: Vec<String> = vec![
        "--no-debug-info".to_owned(),
        "--threads-count".to_owned(),
        "1".to_owned(),
        "-d".to_owned(),
        output_dir.to_string_lossy().into_owned(),
        input_path.to_string_lossy().into_owned(),
    ];
    let borrowed: Vec<&str> = invocation_args.iter().map(String::as_str).collect();
    let captured: disrobe_core::subprocess::CapturedOutput = match run(jadx, &borrowed) {
        Ok(output) => output,
        Err(reason) => {
            let retained: std::result::Result<(), String> = retain_tool_provenance(
                artifact_dir,
                &ToolInvocation {
                    route: "cli",
                    executable: jadx,
                ...
                &["--version"],
                version,
                &[("EdgeCases.dex".to_owned(), sha256_hex(dex_bytes))],
            );
            retained?;
            return Ok(JadxScore {
                score: ToolScore::miss(denominator, reason.clone()),
    ...
    let sources: BTreeMap<String, String> =
        match collect_edgecases_java(&output_dir.join("sources")) {
            Ok(sources) => sources,
            Err(reason) => {
                return Ok(JadxScore {
                    score: ToolScore::miss(denominator, reason.clone()),
    ...
    retain_source_tree(artifact_dir, &sources)?;
    let method_count: usize = source_method_regions(&sources).len();
    let output: AndroidDecompileOutput = AndroidDecompileOutput {
        ...
        method_count,
        notes: vec!["jadx external backend".to_owned()],
        sources,
    };
    let status: i32 = captured.exit_code.unwrap_or(-1);
    if status == 0 {
        return Ok(JadxScore {
            score: score_source_set_with_artifacts(
                javac,
                &output.sources,
                denominator,
                artifact_dir,
...
fn write_new_file(path: &Path, bytes: &[u8]) -> std::result::Result<(), String> {
    let mut options: OpenOptions = OpenOptions::new();
    let mut file: File = options
        .write(true)
        .create_new(true)
        .open(path)
        .map_err(|error| format!("creating retained input {}: {error}", path.display()))?;
    std::io::Write::write_all(&mut file, bytes)
        .map_err(|error| format!("writing retained input {}: {error}", path.display()))
}
...
fn score_partial_jadx_output(
    ...
    denominator: usize,
    input_path: &Path,
    captured: &disrobe_core::subprocess::CapturedOutput,
    ...
    let source_score: ToolScore = match artifact_dir {
        Some(artifact_dir) => {
            score_source_set_with_artifacts(javac, &output.sources, denominator, artifact_dir)?
        }
        None => score_source_set(javac, &output.sources, denominator),
    };
    Ok(JadxScore {
...
fn score_cfr(
    ...
    let result: Result<BTreeMap<String, String>, String> =
        cfr.run(jar_path, &out_dir, artifact_dir, version);
    let score: ToolScore = match result {
        Ok(sources) if main_class_file(&sources).is_some() => {
            score_source_set_with_artifacts(javac, &sources, denominator, artifact_dir)?
        }
    ...
    if let Err(error) = work.close() {
        return Ok(ToolScore::miss(
            denominator,
            format!("cfr work cleanup failed: {error}"),
        ));
    }
    Ok(score)
}
...
fn score_source_set_with_artifacts(
    javac: &Path,
    sources: &BTreeMap<String, String>,
    original: usize,
    artifact_dir: &Path,
...
fn retain_sources_and_score(
    ...
    retain_source_tree(artifact_dir, sources)?;
    score_source_set_with_artifacts(javac, sources, original, artifact_dir)
}

fn score_source_set_prepared(
    javac: &Path,
    sources: &BTreeMap<String, String>,
    original: usize,
    artifact_dir: Option<&Path>,
    ...
    let failures: ExplicitSourceFailures = match explicit_source_failures(sources, &regions) {
        ...
        Err(error) => {
            return Ok(ToolScore::Uncertified {
                emitted: regions.len(),
...
fn score_source_set_internal(
    ...
    if main.trim().is_empty() {
        return Ok(ToolScore::miss(
            original,
            "empty recovered source".to_owned(),
        ));
    }
    let regions: Vec<SourceMethodRegion> = source_method_regions(sources);
    if regions.is_empty() {
        return Ok(ToolScore::miss(
            original,
            "recovered source contains no methods".to_owned(),
        ));
    }
    let verdict: OracleVerdict =
        match javac_verdict_over_set_with_artifacts(javac, sources, TOOL_TIMEOUT, artifact_dir) {
            Ok(verdict) => verdict,
            Err(JavacSetError::Compiler(error)) => {
                return Ok(score_initial_javac_failure(original, regions.len(), error));
            }
            Err(JavacSetError::Artifact(error)) => return Err(error),
        };
    ...
    {
        return Ok(score_source_method_regions(
            original,
            &regions,
            &verdict,
            forced_failures,
...
impl JavacSetError {
    fn into_detail(self) -> String {
        match self {
            Self::Compiler(detail) | Self::Artifact(detail) => detail,
        }
    }
}
...
fn javac_verdict_over_set(
    ...
    javac_verdict_over_set_with_artifacts(javac, sources, TOOL_TIMEOUT, None)
        .map_err(JavacSetError::into_detail)
}
...
fn javac_verdict_over_set_with_timeout(
    ...
    javac_verdict_over_set_with_artifacts(javac, sources, timeout, None)
        .map_err(JavacSetError::into_detail)
}

fn javac_verdict_over_set_with_artifacts(
    javac: &Path,
    sources: &BTreeMap<String, String>,
    timeout: Duration,
    artifact_dir: Option<&Path>,
) -> std::result::Result<OracleVerdict, JavacSetError> {
    validate_source_set(sources).map_err(JavacSetError::Compiler)?;
    let scratch: ScratchDir =
        ScratchDir::create("disrobe_h2h_javac").map_err(|error: std::io::Error| {
            JavacSetError::Compiler(format!("could not create javac workspace: {error}"))
        })?;
    ...
    for (relative, text) in sources {
        ...
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|error: std::io::Error| {
                JavacSetError::Compiler(format!("could not create javac source directory: {error}"))
            })?;
        }
        std::fs::write(&path, text).map_err(|error: std::io::Error| {
            JavacSetError::Compiler(format!("could not write javac source: {error}"))
        })?;
        written.push(path);
    }
    if written.is_empty() {
        return Err(JavacSetError::Compiler(
            "no recovered source to compile".to_owned(),
        ));
    }
    let stub: PathBuf = dir.join("cp");
    std::fs::create_dir(&stub).map_err(|error: std::io::Error| {
        JavacSetError::Compiler(format!("could not create javac classpath: {error}"))
    })?;
    ...
    let out: disrobe_core::subprocess::CapturedOutput =
        compile(javac, &stub, &dir.join("out"), &borrowed, timeout)
            .map_err(JavacSetError::Compiler)?;
    if let Some(artifact_dir) = artifact_dir {
        retain_initial_javac(artifact_dir, sources, &out).map_err(JavacSetError::Artifact)?;
    }
    if out.exit_code == Some(0) {
        require_emitted_edgecases_class(dir).map_err(JavacSetError::Compiler)?;
...
fn retain_initial_javac(
    artifact_dir: &Path,
    sources: &BTreeMap<String, String>,
    output: &disrobe_core::subprocess::CapturedOutput,
) -> std::result::Result<(), String> {
    let javac_dir: PathBuf = artifact_dir.join("initial-javac");
    std::fs::create_dir(&javac_dir)
        .map_err(|error| format!("creating initial javac artifact directory: {error}"))?;
    write_new_file(&javac_dir.join("stdout.bin"), &output.stdout)
        .map_err(|error| format!("retaining initial javac stdout: {error}"))?;
    write_new_file(&javac_dir.join("stderr.bin"), &output.stderr)
        .map_err(|error| format!("retaining initial javac stderr: {error}"))?;
    let source_hashes: Vec<Value> = sources
        .iter()
        .map(|(name, source): (&String, &String)| {
            json!({"name": name, "sha256": sha256_hex(source.as_bytes())})
        })
        .collect();
    let mut provenance: String = serde_json::to_string_pretty(&json!({
        "exit_code": output.exit_code,
        "stdout_sha256": sha256_hex(&output.stdout),
        "stderr_sha256": sha256_hex(&output.stderr),
        "source_inputs": source_hashes,
        "processor_mode": "disabled (-proc:none)",
    }))
    .map_err(|error| format!("serializing initial javac provenance: {error}"))?;
    provenance.push('\n');
    write_new_file(&javac_dir.join("provenance.json"), provenance.as_bytes())
        .map_err(|error| format!("retaining initial javac provenance: {error}"))
}
...
fn skipped_tool(name: &str, reason: &str) -> Value {
    json!({
        "name": name,
        "version": "n/a",
        "metric": "emitted-region compile yield (clean / emitted)",
        "display": "skipped",
        "status": "skipped",
        "detail": reason,
    })
...
impl CfrInvoke {
    fn run(
        ...
        let (executable, invocation_args): (&Path, Vec<String>) = match self {
            Self::Binary(bin) => (bin, vec![jar_str, "--outputdir".to_owned(), out_str]),
            Self::Jar { java, jar } => {
        ...
        let mut input_hashes: Vec<(String, String)> = vec![(
            "EdgeCases-baseline.jar".to_owned(),
            sha256_hex(
                &read_bounded_file(jar_path, MAX_FIXTURE_BYTES)
                    .map_err(|error| format!("hashing CFR input: {error}"))?,
            ),
        )];
        if let Self::Jar { jar, .. } = self {
            input_hashes.push((
                "cfr.jar".to_owned(),
                sha256_hex(
                    &read_bounded_file(jar, MAX_FIXTURE_BYTES)
                        .map_err(|error| format!("hashing CFR jar: {error}"))?,
                ),
            ));
        }
        let version_args: Vec<String> = cfr_version_args(self);
        let version_arg_refs: Vec<&str> = version_args.iter().map(String::as_str).collect();
        let invocation_arg_refs: Vec<&str> = invocation_args.iter().map(String::as_str).collect();
        let output: disrobe_core::subprocess::CapturedOutput =
            match run(executable, &invocation_arg_refs) {
                Ok(output) => output,
                Err(reason) => {
                    let retained: std::result::Result<(), String> = retain_tool_provenance(
                        artifact_dir,
                    ...
                    return Err(match retained {
                        Ok(()) => reason,
                        Err(retention) => {
                            format!("{reason}; retaining provenance failed: {retention}")
                        }
                    });
                }
            };
        ...
        let sources: BTreeMap<String, String> = collect_edgecases_java(out_dir)?;
        retain_source_tree(artifact_dir, &sources)?;
        Ok(sources)
    }
}
...
fn resolve_cfr(root: &Path) -> Option<CfrInvoke> {
    ...
    find_on_path("cfr").map(CfrInvoke::Binary)
}

fn cfr_version_args(cfr: &CfrInvoke) -> Vec<String> {
    match cfr {
        CfrInvoke::Binary(_) => vec!["--version".to_owned()],
        CfrInvoke::Jar { jar, .. } => vec![
            "-jar".to_owned(),
...
fn cfr_version_capture(cfr: &CfrInvoke) -> std::result::Result<VersionCapture, String> {
    let args: Vec<String> = cfr_version_args(cfr);
    let borrowed: Vec<&str> = args.iter().map(String::as_str).collect();
    match cfr {
        CfrInvoke::Binary(bin) => version_capture_checked(bin, &borrowed, None),
        CfrInvoke::Jar { java, .. } => version_capture_checked(java, &borrowed, None),
    }
}

fn cfr_version_checked(cfr: &CfrInvoke) -> std::result::Result<String, String> {
    cfr_version_capture(cfr).map(|capture: VersionCapture| capture.display)
}
...
pub fn require_pinned_versions(root: &Path) -> Result<(), String> {
    ...
    let cfr: CfrInvoke =
        resolve_cfr(root).ok_or_else(|| "pinned CFR jar and java are unavailable".to_owned())?;
...
"#;
    ...
    fn nonpinned_failed_output_cannot_enter_the_shared_scorer() -> core::result::Result<(), String>
    {
        ...
        let measured: JadxScore = score_partial_jadx_output(
            ...
            &disrobe_core::subprocess::CapturedOutput {
                exit_code: Some(1),
                stdout: b"ERROR - finished with errors, count: 1".to_vec(),
                stderr: Vec::new(),
            },
            output,
            None,
    ...
    -> core::result::Result<(), String> {
        ...
        let jadx: PathBuf = find_on_path("jadx").ok_or_else(|| "jadx is not on PATH".to_owned())?;
        let version: VersionCapture = version_capture_checked(&jadx, &["--version"], None)?;
        let artifacts: ScratchDir =
            ScratchDir::create("disrobe_h2h_jadx_artifacts").map_err(|error| error.to_string())?;
        let output_dir: PathBuf = artifacts.path().join("jadx");
        std::fs::create_dir(&output_dir).map_err(|error| error.to_string())?;
        let measured: JadxScore = score_jadx(
            &javac,
            &dex_bytes,
            denominator,
            &output_dir,
    ...
    #[test]
    fn initial_javac_proof_retains_streams_exit_and_source_hashes()
    -> core::result::Result<(), String> {
        let artifacts: ScratchDir =
            ScratchDir::create("disrobe_h2h_javac_proof").map_err(|error| error.to_string())?;
        let sources: BTreeMap<String, String> = BTreeMap::from([(
            MAIN_CLASS_FILE.to_owned(),
            "class EdgeCases {}\n".to_owned(),
        )]);
        let output: disrobe_core::subprocess::CapturedOutput =
            disrobe_core::subprocess::CapturedOutput {
                stdout: b"compiler output".to_vec(),
                stderr: b"compiler detail".to_vec(),
                exit_code: Some(1),
            };
        retain_initial_javac(artifacts.path(), &sources, &output)?;
        let proof_dir: PathBuf = artifacts.path().join("initial-javac");
        assert_eq!(
            std::fs::read(proof_dir.join("stdout.bin")).map_err(|error| error.to_string())?,
            output.stdout
        );
        assert_eq!(
            std::fs::read(proof_dir.join("stderr.bin")).map_err(|error| error.to_string())?,
            output.stderr
        );
        let proof: Value = serde_json::from_slice(
            &std::fs::read(proof_dir.join("provenance.json")).map_err(|error| error.to_string())?,
        )
        .map_err(|error| error.to_string())?;
        assert_eq!(proof["exit_code"], 1);
        assert_eq!(proof["stdout_sha256"], sha256_hex(b"compiler output"));
        assert_eq!(proof["stderr_sha256"], sha256_hex(b"compiler detail"));
        assert_eq!(
            proof["source_inputs"][0]["sha256"],
            sha256_hex(sources[MAIN_CLASS_FILE].as_bytes())
        );
        Ok(())
    }

    #[test]
    fn retained_source_collision_propagates_before_compilation() -> core::result::Result<(), String>
    {
        let artifacts: ScratchDir = ScratchDir::create("disrobe_h2h_source_collision")
            .map_err(|error| error.to_string())?;
        let source_dir: PathBuf = artifacts.path().join("sources");
        std::fs::create_dir(&source_dir).map_err(|error| error.to_string())?;
        std::fs::write(source_dir.join(MAIN_CLASS_FILE), "occupied")
            .map_err(|error| error.to_string())?;
        let sources: BTreeMap<String, String> = BTreeMap::from([(
            MAIN_CLASS_FILE.to_owned(),
            "class EdgeCases { int value() { return 1; } }\n".to_owned(),
        )]);
        let result: std::result::Result<ToolScore, String> = retain_sources_and_score(
            Path::new("compiler-must-not-run"),
            &sources,
            1,
            artifacts.path(),
        );
        let error: String = result
            .err()
            .ok_or_else(|| "a retained source collision must abort the measurement".to_owned())?;
        assert!(error.contains("creating retained source"));
        Ok(())
    }
    ...
    -> core::result::Result<(), String> {
        ...
        let result: std::result::Result<ToolScore, String> =
            score_source_set_with_artifacts(&javac, &sources, 1, artifacts.path());
        let error: String = result.err().ok_or_else(|| {
            "an initial javac directory collision must abort the measurement".to_owned()
        })?;
        assert!(error.contains("creating initial javac artifact directory"));
        Ok(())
    }
    ...
    fn required_competitor_rejects_skipped_or_missing_results() {
        ...
        assert!(require_competitor_result("cfr", &uncertified).is_err());
    }
    ...
    fn the_uncertified_rule_reads_the_same_whichever_side_trips_it() {
        ...
        .sentence();
        assert!(
            sentence.contains("tool-specific emitted populations are reported separately"),
    ...
    fn the_published_note_states_the_rule_it_scored_by() {
        assert!(
            SHARED_ORACLE.contains("compiles the complete emitted source set first"),
            "a reader cannot check a rule the published note does not state"
        );
        assert!(
            SHARED_ORACLE.contains("Each ratio describes only that tool's emitted regions"),
            "the note has to state the scope of each ratio"
        );
        assert!(
    ...
    fn every_figure_in_the_summary_moves_with_the_measurement() {
        ...
        assert_eq!(
            first,
            "DEX leg: `disrobe` emitted-region compile yield: 129 clean of 132 emitted (97.7%); \
             `jadx` (1.5.5): 128 clean of 130 emitted (98.5%). The denominators describe \
             different emitted-region populations and do not support a cross-tool ranking."
        );
        ...
        assert_eq!(
            improved,
            "DEX leg: `disrobe` emitted-region compile yield: 130 clean of 132 emitted (98.5%); \
             `jadx` (1.5.5): 128 clean of 130 emitted (98.5%). The denominators describe \
             different emitted-region populations and do not support a cross-tool ranking."
        );
        assert_ne!(
            first, improved,
            "a one-method change in disrobe's emitted-region yield has to change the published sentence. \
    ...
    fn the_whole_summary_is_assembled_from_the_rows_beside_it() {
        ...
        assert_eq!(
            ...
            format!(
                "DEX leg: `disrobe` emitted-region compile yield: 129 clean of 132 emitted (97.7%); \
                 `jadx` (1.5.5): 128 clean of 130 emitted (98.5%). The denominators describe \
                 different emitted-region populations and do not support a cross-tool ranking. JAR \
                 leg: `disrobe` emitted-region compile yield: 131 clean of 131 emitted (100.0%); \
                 `cfr` (CFR 0.152): 105 clean of 106 emitted (99.1%). The denominators describe \
                 different emitted-region populations and do not support a cross-tool ranking. \
                 {SHARED_ORACLE}"
            )
        );
    ...
    #[test]
    fn different_emitted_populations_do_not_create_a_lead_claim() {
        let sentence: String = dex_leg(
            ToolScore::measured(100, 132, 106, "measured".to_owned()),
            scored("1.5.5", 128, 130),
        )
        .sentence();
        assert!(sentence.contains("100 clean of 132 emitted"));
        assert!(sentence.contains("128 clean of 130 emitted"));
        assert!(!sentence.contains("leads by"));
    }
    ...
    fn an_absent_competitor_gets_no_figure_at_all() {
        ...
        .sentence();
        assert!(
            sentence.contains("`jadx` was not measured"),
            "an unrun competitor must be marked, not narrated, got: {sentence}"
        );
        assert!(
            sentence.contains("`disrobe` emitted-region compile yield"),
