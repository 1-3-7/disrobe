...
impl FrozenApkleaks {
    pub fn attribution(&self) -> String {
        format!(
            "historical archive of `{command}` run against the committed APK with {decompiler} \
             {decompiler_version}; evidence/competitors/{CAPTURE_FILE} SHA-256 {output_sha256}. \
             Publication regeneration requires a new live run and does not replay this archive",
    ...
    if measured_input != recorded_input {
        return Err(format!(
            "{apk} hashes {measured_input} but {provenance} records the capture was taken over \
             {recorded_input}. The historical apkleaks archive describes a different file than the \
             committed fixture",
            apk = apk.display(),
    ...
    if measured_output != recorded_output {
        return Err(format!(
            "{capture} hashes {measured_output} but {provenance} records {recorded_output}. The \
             committed historical output no longer matches its recorded capture",
...
mod tests {
    ...
    #[test]
    fn the_historical_capture_matches_its_recorded_provenance() {
        let root: PathBuf = checked_workspace_root();
        let loaded: Result<FrozenApkleaks, String> = load(&root, &planted_apk(&root));
        assert!(
            loaded.is_ok(),
            "the committed historical apkleaks capture must load and match its recorded hashes: {:?}",
            loaded.as_ref().err()
        );
    }
