...
impl MeasurementKind {
    fn measure(self, root: &Path, run_dir: &Path) -> Result<(String, Value)> {
        match self {
            Self::Apk => apk::measure(root, run_dir),
            Self::Frisk => frisk::measure(root, run_dir),
            Self::Gate => gate::measure(root),
        }
    }
...
fn require_apk_tools(root: &Path) -> Result<()> {
    require_program("javac", "the APK publication run")?;
    require_program("jadx", "the APK publication run")?;
    require_program("java", "the APK publication run")?;
    let cfr_jar: PathBuf = root.join("evidence/competitors/jars/cfr.jar");
    if find_on_path("cfr").is_none() && !cfr_jar.is_file() {
        bail!(
            "the APK publication run requires `cfr` on PATH or {}, but neither is available",
            cfr_jar.display()
        );
    }
...
fn require_frisk_tools(root: &Path) -> Result<()> {
    require_program("apkleaks", "the frisk publication run")?;
    require_program("jadx", "the frisk publication run")?;
    frisk::require_pinned_versions(root).map_err(|error: String| eyre::eyre!(error))
}

fn require_program(program: &str, context: &str) -> Result<()> {
    if find_on_path(program).is_none() {
        bail!("{context} requires `{program}` on PATH, but it is unavailable");
    }
    Ok(())
}
        ...
        match argument.as_str() {
            ...
            "--artifact-root" => {
                let Some(path) = args.next() else {
                    bail!("--artifact-root requires a directory");
                };
                if options.artifact_root.replace(PathBuf::from(path)).is_some() {
                    bail!("--artifact-root was supplied more than once");
                }
            }
...
fn run(options: Options) -> Result<()> {
    ...
    let run_dir: PathBuf = create_run_dir(&root, options.artifact_root.as_deref())?;

    let outputs: Vec<(String, Value)> =
        measure_outputs(&root, &measured_dir, &run_dir, options.only.as_deref())?;
    if options.check {
    ...
    println!(
        "disrobe-bench-head-to-head: retained this run's tool output and provenance in {}",
        run_dir.display()
    );
    Ok(())
}

fn create_run_dir(root: &Path, artifact_root: Option<&Path>) -> Result<PathBuf> {
    let parent: PathBuf = artifact_root.map_or_else(
        || root.join(".developer").join("head-to-head-runs"),
        Path::to_path_buf,
    );
...
fn create_unique_run_dir(parent: &Path, name: &str) -> Result<PathBuf> {
    let run_dir: PathBuf = parent.join(name);
    fs::create_dir(&run_dir).wrap_err_with(|| {
        format!(
            "creating unique run archive {}; an existing path is never reused",
            run_dir.display()
        )
    })?;
    Ok(run_dir)
...
fn measure_outputs(
    ...
    for generator in generators {
        ...
        let output: (String, Value) = generator.kind.measure(root, run_dir)?;
...
fn render_report(outputs: &[(String, Value)]) -> String {
    ...
    md.push_str(
        "Each leg gives `disrobe` and the comparison tool the same input and scoring rule. The DEX and \
         JAR legs use their respective committed inputs. A missing, failed, or uncertified required \
         tool aborts regeneration before a result is published.\n\n",
    );
    md.push_str(
        "Regenerate with `cargo run --locked -p disrobe-bench-head-to-head`. Add `-- --check` to \
         compare a fresh run with the committed results, or `-- --check --only apk-jadx-cfr` to \
         check the APK measurement. `cargo run --locked -p xtask -- evidence` updates the public \
         evidence report.\n\n",
    );
    for (id, value) in outputs {
...
fn render_block(id: &str, value: &Value) -> String {
    ...
    if let Some(oracle) = value.get("oracle").and_then(Value::as_str) {
        let _ = writeln!(md, "- method: {oracle}");
    }
    if let Some(denom) = value.get("denominator").and_then(Value::as_str) {
        let _ = writeln!(md, "- denominator: {denom}");
    }
...
mod tests {
    ...
    #[test]
    fn artifact_root_is_explicit_and_rejects_missing_or_repeated_paths() -> Result<()> {
        assert_eq!(
            parse(&[
                "disrobe-bench-head-to-head",
                "--artifact-root",
            ...
            Options {
                check: false,
                only: None,
                artifact_root: Some(PathBuf::from("D:/benchmark-evidence")),
            }
        );
        assert!(parse(&["disrobe-bench-head-to-head", "--artifact-root"]).is_err());
        assert!(
            parse(&[
                "disrobe-bench-head-to-head",
                "--artifact-root",
    ...
    #[test]
    fn durable_run_directory_refuses_reuse() -> Result<()> {
        let scratch: ScratchDir = ScratchDir::create("disrobe-h2h-run-archive")?;
        let first: PathBuf = create_unique_run_dir(scratch.path(), "fixed-run")?;
        assert!(first.is_dir());
        assert!(create_unique_run_dir(scratch.path(), "fixed-run").is_err());
        Ok(())
    }

    #[test]
    fn explicit_artifact_root_owns_the_unique_run_directory() -> Result<()> {
        let workspace: ScratchDir = ScratchDir::create("disrobe-h2h-workspace")?;
        let archive: ScratchDir = ScratchDir::create("disrobe-h2h-external-archive")?;
        let run_dir: PathBuf = create_run_dir(workspace.path(), Some(archive.path()))?;
        assert_eq!(run_dir.parent(), Some(archive.path()));
    ...
    fn a_required_missing_competitor_is_fatal() -> Result<()> {
        ...
        assert!(failure.contains("the test publication run"));
        Ok(())
    }
}
