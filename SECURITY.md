...
**What is wired, and what remains absent.** `PluginHost::load_and_run` is a Rust-library path from signed component bytes to sandboxed output: through `disrobe-plugin-loader`, it verifies the signature, compiles the component on the host engine, and checks its imports against the manifest, then it executes that component through `PluginHost::run_component` with the same fuel, wall-clock, and memory limits. The component linker is empty, so a manifest grant permits validation but does not supply a host function. The CLI still has no option or subcommand that dispatches an analysis pass through a plugin. The WIT schema at `schemas/v0/wit/disrobe-plugin@0.1.0.wit` defines a `pass-descriptor` record carrying `id` and `version` fields intended for provenance, but no code path calls a plugin's `descriptor()` export or records a plugin's name and version in `disrobe` output. There is no `wit-bindgen` / `bindgen!` usage that binds that WIT interface to the executed `run` export. Adding descriptor fields without that output path would be cosmetic, so the absent CLI dispatch and provenance remain disclosed gaps.
