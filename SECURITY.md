...
`disrobe` is a deobfuscator and decompiler suite. By the nature of the work it parses adversarial binary input (protector output, packed PE, obfuscated bytecode, exotic encoders) and emits derived artifacts. Hardening the parsing surface is a primary concern.
...
We acknowledge reports within **72 hours** and aim to ship a fix within **30 days** for high-severity issues. Critical issues affecting parsing of untrusted input get same-week turnaround. We publish a GitHub Security Advisory and a CVE (where applicable) when the fix lands.
...
- **Decompilation output correctness on adversarial input.** `disrobe` will sometimes produce wrong output on hostile bytecode; the round-trip metric exists to flag this. A decompile result that is not byte-perfect is not a security bug; it is correctness work. Open a normal issue or PR for these.
...
- Every subprocess invocation in the table above routes its wait/capture through one shared primitive (`disrobe-core::subprocess`): a caller-set timeout kills and reaps the child if exceeded, and a caller-set byte cap truncates captured stdout/stderr instead of buffering unbounded output from a hostile or malfunctioning external tool. A pipe read error is treated the same as clean EOF, so the process's real exit code and whatever output was already captured survive rather than being discarded as an indistinguishable timeout. Argv is passed through `Command`'s own non-shell argument list, never a shell string, so a path or argument containing shell metacharacters reaches the child literally; the one exception is `mklink_junction` (see the attack surface table below), which shells through `cmd.exe` on Windows and is tracked separately.
...
`crates/disrobe-core/src/recon/git_history.rs`, `crates/disrobe-pass-native/src/pseudo_c.rs`, `crates/disrobe-pass-wasm-deob/src/structured.rs`, and `crates/disrobe-cli/src/cli/config_merge.rs` also call `std::process::Command`, but every call site found sits inside a `#[cfg(test)]` module (test-only recompile-oracle grading against a host `rustc`/`git`) and does not ship in the release binary. `crates/disrobe-pass-py-decompile/examples/decomp_one.rs` calls `Command` too; it is an `examples/` binary, not part of any shipped target, for the same reason.
