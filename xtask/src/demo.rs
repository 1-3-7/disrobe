...
fn colorize(text: &str, kind: LineKind) -> Vec<Run> {
    ...
    overlay_all(&mut colors, &chars, "OvertlyMalicious", TEXT);
    overlay_all(&mut colors, &chars, "Malicious", TEXT);
    overlay_all(&mut colors, &chars, "Suspicious", TEXT);
    overlay_all(&mut colors, &chars, "warning", TEXT);
...
fn overlay_first_token(colors: &mut [&'static str], chars: &[char], indent: usize) {
    ...
    if end > indent {
        set_range(colors, indent, end - indent, TEXT);
    }
}
...
fn overlay_arrow(colors: &mut [&'static str], chars: &[char]) {
    ...
    if let Some(byte_pos) = hay.find("-> ") {
        ...
        set_range(colors, char_start, chars.len() - char_start, TEXT);
    }
}
...
fn overlay_ok(colors: &mut [&'static str], chars: &[char]) {
    ...
    if let Some(byte_pos) = hay.find(": OK") {
        let char_start: usize = hay[..byte_pos].chars().count();
        set_range(colors, char_start + 2, 2, TEXT);
    }
}
...
fn prompt_runs(chars: &[char]) -> Vec<Run> {
    ...
    set_range(&mut colors, 0, prompt_len, TEXT_MUTED);
...
fn render_chrome(mut body: &mut String, width: f64, width_u: u32, height_u: u32) {
    ...
    let dots: [&str; 3] = [TEXT_FAINT, TEXT_MUTED, TEXT];
...
mod tests {
    ...
    fn colorize_highlights_severity_and_ok() {
        ...
        assert!(
            runs.iter()
                .any(|r: &Run| r.fill == TEXT && r.text.contains("OvertlyMalicious")),
            "severity must remain legible: {runs:?}"
        );
        let ok: Vec<Run> = colorize("native unpack: OK", LineKind::Static);
        assert!(
            ok.iter()
                .any(|r: &Run| r.fill == TEXT && r.text.trim() == "OK"),
            "OK must remain legible: {ok:?}"
        );
    }
}
