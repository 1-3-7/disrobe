...
pub(crate) fn parse_registry(registry_dir: &Path) -> Result<Vec<ErrorCode>> {
    ...
    for source in &sources {
        let text: String = fs::read_to_string(source)
            .with_context_str(|| format!("reading {}", source.display()))?;
...
pub(crate) fn render(code: &ErrorCode) -> String {
    ...
    let _ = writeln!(md, "# {}", code.code);
    md.push('\n');
    let _ = writeln!(md, "**{}**", code.title);
    md.push('\n');
    let _ = writeln!(md, "{}", code.description);
    md.push('\n');
    if !code.common_causes.is_empty() {
        md.push_str("## Common causes\n\n");
        for cause in &code.common_causes {
            let _ = writeln!(md, "- {cause}");
        }
        md.push('\n');
    }
    if !code.common_fixes.is_empty() {
        ...
        for fix in &code.common_fixes {
            let _ = writeln!(md, "- {fix}");
        }
        md.push('\n');
    }
    md.push_str("## Source\n\n");
    let _ = writeln!(md, "Emitted from `{}`.\n", code.crate_path);
    let _ = writeln!(
        md,
...
pub(crate) fn render_index(codes: &[ErrorCode]) -> String {
    ...
    for code in codes {
        let _ = writeln!(
            md,
            "| [{}](./{}.md) | {} |",
            code.code, code.code, code.title
        );
    }
    md
}
