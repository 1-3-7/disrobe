...
mod tests {
    ...
    fn test_recovery() -> Result<Recovery> {
        let raw: &str = r#"{
            "groups": [
                ...
                {
                    "heading": "Python bytecode (CPython 3.14.5 stdlib)",
                    "bars": [
                        {"label": "fixed 574-module core population", "value": 92.43, "num": 16880, "den": 18262},
                        {"label": "200-module pinned corpus (normalized opcode-structure agreement)", "value": 94.18, "num": 6051, "den": 6286}
                    ]
                },
                {
    ...
    fn of_formatters_render_the_pinned_count() -> Result<()> {
        ...
        let full: MetricValue = recovery
            .bar("Python bytecode", "fixed 574-module core population")?
            .count_ratio()?;
