...
const MAX_SCHEMA_FILES: usize = 256;
...
pub(crate) fn load_schemas(schemas_dir: &Path) -> Result<Vec<SchemaArtifact>> {
        ...
        .sort_by_file_name()
    {
        ...
        if out.len() >= MAX_SCHEMA_FILES {
            bail!(
                "too many JSON schemas in {}; max {MAX_SCHEMA_FILES}",
                schemas_dir.display()
            );
        }
        let bytes: Vec<u8> = read_bounded(path, MAX_SCHEMA_FILE_BYTES)?;
...
fn load_checksums(dir: &Path) -> Result<BTreeMap<String, String>> {
    ...
    let bytes: Vec<u8> = read_bounded(&path, MAX_CHECKSUM_FILE_BYTES)?;
...
fn read_bounded(path: &Path, max_bytes: u64) -> Result<Vec<u8>> {
    let file: fs::File =
        fs::File::open(path).wrap_err_with(|| format!("opening {}", path.display()))?;
    let reserve: usize = file
        .metadata()
        .map(|metadata: fs::Metadata| metadata.len().min(max_bytes))
        .ok()
    ...
    if len > max_bytes {
        bail!("{} exceeds max {max_bytes} bytes", path.display());
    }
    Ok(bytes)
}
...
fn render_python_def(name: &str, def: &Value, schema: &Value, defs: &Map<String, Value>) -> String {
    ...
    push_line!(out, "class {name}(TypedDict, total=False):");
...
fn render_typescript_def(
    ...
    let required: std::collections::BTreeSet<&str> = effective
        ...
        .unwrap_or_default();
    let mut out: String = String::with_capacity(256);
    push_line!(out, "export interface {name} {{");
    let Some(properties) = properties else {
        out.push_str("}\n");
        return out;
    };
    for (prop_name, prop_value) in properties {
        ...
        push_line!(out, "  {prop_name}{optional}: {ty};");
    }
    out.push_str("}\n");
    out
}
...
mod tests {
    ...
    fn read_bounded_rejects_oversized_file() {
        ...
        let path: PathBuf = tmp.path().join("big.json");
        fs::write(&path, b"abcd").expect("write");
        let error: String = read_bounded(&path, 3)
            .expect_err("oversized file")
            .to_string();
        assert!(error.contains("exceeds max 3 bytes"), "got: {error}");
    }

    #[test]
    fn load_schemas_rejects_too_many_files() {
        let tmp: tempfile::TempDir = tempfile::tempdir().expect("tempdir");
        for index in 0_usize..=MAX_SCHEMA_FILES {
            let path: PathBuf = tmp.path().join(format!("schema_{index:03}.json"));
            fs::write(&path, br#"{"title":"X","type":"object"}"#).expect("write");
        }
        let error: String = load_schemas(tmp.path())
            .expect_err("schema cap")
            .to_string();
        assert!(error.contains("too many JSON schemas"), "got: {error}");
    }
