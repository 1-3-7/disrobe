...
const DOCUMENT_FIGURE_BUDGET: [FigureBudget; 39] = [
    ...
    FigureBudget {
        ...
        figures: 2,
        digest: "6f6f34c2457bbeff",
    },
    FigureBudget {
        path: "docs/src/languages/shell.md",
        figures: 2,
        digest: "c91a9f343c75351d",
    },
    FigureBudget {
...
pub(crate) fn run(root: &Path) -> Result<()> {
    ...
    println!(
        "xtask regen: xtask/src/figures.rs checked {scanned} markdown file(s): {excluded} \
         generated-tree exclusion(s), {wrapped} marker-backed figure(s), {ignored} ignored \
         figure(s), {budgeted} budgeted figure(s), {unbudgeted_shapes} unbudgeted figure(s)"
    );
    Ok(())
}
