...
const RECOVERY_STRENGTH_TAGS: [&str; 3] = ["strong", "recompile", "self-reported"];
const RECOVERY_STRENGTH_NAMES: [&str; 3] = ["strong", "recompile-only", "coverage-self-reported"];
...
const RECOVERY_MONO_FONT: &str = "'JetBrains Mono', monospace";
const RECOVERY_VALUE_FILL: &str = "#F5F5F5";
...
pub(crate) fn run(root: &Path, check: bool) -> Result<()> {
    ...
    let status: ExitStatus = Command::new("node")
        .current_dir(root)
        .args(["xtask/graphgen/build.mjs", "--check"])
        .status()
        .wrap_err(
            "running chart byte comparison; install Node.js and graphgen dependencies first",
        )?;
    if !status.success() {
        bail!("chart regeneration comparison failed with {status}");
    }
    if check {
        println!(
...
fn recovery_label_slots(doc: &RecoveryDoc, tag_gutter: f64) -> Result<Vec<RecoveryLabelSlot>> {
    ...
    let percent_row_height: f64 = recovery_percent_row_height(doc);
...
fn recovery_percent_row_height(doc: &RecoveryDoc) -> f64 {
    let labels: Vec<String> = recovery_category_labels(doc);
    let widest: f64 = labels
        .iter()
        .map(|label: &String| recovery_sans_width(label))
        .fold(0.0_f64, f64::max);
    let gutter: f64 = (widest.ceil() + 18.0).clamp(150.0, 240.0);
    let lines: usize = labels
        .iter()
        .map(|label: &String| recovery_wrapped_lines(label, gutter - 32.0))
        .max()
        .unwrap_or(1);
    RECOVERY_PERCENT_ROW_HEIGHT.max((lines as f64).mul_add(14.0, 10.0))
}

fn recovery_word_span(text: &str, word: &str) -> Option<std::ops::Range<usize>> {
    let lowercase: String = text.to_ascii_lowercase();
    lowercase
        .match_indices(word)
        .find_map(|(start, matched): (usize, &str)| {
            let end: usize = start + matched.len();
            let boundary = |byte: u8| !byte.is_ascii_alphanumeric() && byte != b'_';
            (lowercase[..start].bytes().next_back().is_none_or(boundary)
                && lowercase[end..].bytes().next().is_none_or(boundary))
            .then_some(start..end)
        })
}

fn recovery_qualifier(label: &str) -> String {
    let mut qualifier: String = String::with_capacity(label.len());
    let mut rest: &str = label;
    while let Some((before, after_open)) = rest.split_once('(') {
        let Some((_, after_close)) = after_open.split_once(')') else {
            break;
        };
        qualifier.push_str(before);
        rest = after_close;
    }
    qualifier.push_str(rest);
    for word in ["full", "pinned"] {
        if let Some(span) = recovery_word_span(&qualifier, word) {
            let end: usize = qualifier.len() - qualifier[span.end..].trim_start().len();
            qualifier.replace_range(span.start..end, "");
        }
    }
    while let Some(span) = recovery_word_span(&qualifier, "module") {
        qualifier.replace_range(span, "mod");
    }
    if let Some(start) = qualifier.to_ascii_lowercase().find("body-lowering") {
        qualifier.replace_range(start..start + "body-lowering".len(), "bodies");
    }
    qualifier
        .split_whitespace()
        .collect::<Vec<&str>>()
...
fn recovery_category_labels(doc: &RecoveryDoc) -> Vec<String> {
    let prefixes: [(&str, &str); 10] = [
        ...
        ("dalvik", "dalvik"),
        ("ruby", "ruby"),
        ("react native hermes", "hermes"),
        ("secret recall", "planted apk"),
        ("frisk ioc category recall", ""),
    ];
    let mut labels: Vec<(String, String)> = Vec::new();
    for group in doc
        .groups
        .iter()
        .filter(|group: &&RecoveryGroup| group.kind == PERCENT_KIND)
    {
        let heading: String = group.heading.to_lowercase();
        let ecosystem: &str = prefixes
            .iter()
            .find_map(|&(prefix, short): &(&str, &str)| {
                heading.starts_with(prefix).then_some(short)
            })
            .unwrap_or_else(|| heading.split(' ').next().unwrap_or(""));
        for bar in &group.bars {
            let normalized: String = recovery_qualifier(&bar.label);
            let label: String = format!("{ecosystem} {normalized}").trim().to_owned();
            let hint: String = bar
                .label
                .split_once('(')
                .and_then(|(_, after_open): (&str, &str)| after_open.split_once(')'))
                .map(|(inner, _): (&str, &str)| inner)
                .and_then(|inner: &str| {
                    inner
                        .split(|c: char| c.is_whitespace() || c == ',')
                        .find(|word: &&str| {
                            !word.is_empty()
                                && !["the", "of", "all", "full", "set", "and"]
                                    .contains(&word.to_ascii_lowercase().as_str())
                        })
                })
                .unwrap_or("")
                .to_lowercase();
            labels.push((label, hint));
        }
    }
    ...
    for (label, hint) in &mut labels {
        if counts.get(label).copied().unwrap_or(0) > 1 && !hint.is_empty() {
            let candidate: String = format!("{label} {hint}");
    ...
    labels
        .into_iter()
        .map(|(label, _): (String, String)| label)
        .collect()
}

fn recovery_sans_width(text: &str) -> f64 {
    let units: f64 = text
        .chars()
        .map(|c: char| match c {
            ' ' | ':' | ';' => 0.3,
...
fn recovery_wrapped_lines(text: &str, width: f64) -> usize {
    let mut lines: usize = 1;
    let mut line: String = String::new();
    for word in text.split_whitespace() {
        let trial: String = if line.is_empty() {
            word.to_owned()
        } else {
            format!("{line} {word}")
        };
        if !line.is_empty() && recovery_sans_width(&trial) > width {
            lines += 1;
...
mod tests {
    ...
    fn render_test_label(slot: &RecoveryLabelSlot) -> String {
        format!(
            "<text x=\"{}\" y=\"{}\" font-size=\"11.5\" fill=\"{RECOVERY_VALUE_FILL}\" font-family=\"{}\" text-anchor=\"start\" font-weight=\"500\" id=\"{}\" dominant-baseline=\"central\" xml:space=\"preserve\">{}</text>",
            slot.x,
            slot.y,
            RECOVERY_MONO_FONT,
            slot.id,
    ...
    #[test]
    fn recovery_qualifiers_preserve_boundaries_and_parentheses() {
        for (label, expected) in [
            (" FULL pinned MODULE BODY-lowering (the set) ", "mod bodies"),
            (
                "full full pinned pinned module MODULE",
                "full pinned mod mod",
            ),
            (
                "fullness pinned_value modules",
                "fullness pinned_value modules",
            ),
            (
                "module (one) body-lowering (two) tail (unclosed",
                "mod bodies tail (unclosed",
            ),
            ("éFULL\tPINNED\nMODULE", "émod"),
            ("body-lowering BODY-LOWERING", "bodies BODY-LOWERING"),
        ] {
            assert_eq!(recovery_qualifier(label), expected, "{label}");
        }
    }

    #[test]
    fn recovery_wrapped_labels_keep_values_aligned_and_reject_stale_y() -> Result<()> {
        let mut doc: RecoveryDoc = test_recovery_doc();
        doc.groups[0].bars[0].label =
            "inter-procedural flow at depth greater than one, gcc -O2".to_owned();
        let slots: Vec<RecoveryLabelSlot> = recovery_label_slots(&doc, TEST_TAG_GUTTER)?;
        assert_eq!(slots[0].y, "172.00");
        assert_eq!(slots[1].y, "210.00");
        assert_eq!(slots[2].y, "288.00");
        let rendered: String = render_test_svg(&slots);
        let stale: String = rendered.replacen("y=\"172.00\"", "y=\"166.50\"", 1);
        assert!(slot_validation_error(&stale, &slots)?.contains("stale y"));
        Ok(())
    }
