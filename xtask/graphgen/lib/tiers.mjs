...
export const TIERS = [
  { strength: STRONG, label: "strong", tag: "strong", color: brand.themes.dark.text },
  {
    strength: RECOMPILE_ONLY,
    label: "recompile-only",
    tag: "recompile",
    color: brand.semantic.dark.detection,
  },
  {
    strength: SELF_REPORTED,
    label: "coverage-self-reported",
    tag: "self-reported",
    color: brand.semantic.dark.experimental,
  },
];

export const REPRODUCIBILITY = [
  { ci: true, tag: "CI", label: "CI gate on documented triggers" },
  { ci: false, tag: "local", label: "local measurement; see input prerequisites" },
];
...
export function tierFor(strength) {
  ...
  if (!tier) {
    throw new Error(
      `no chart tier is defined for grading strength "${strength}"; the defined tiers are ` +
...
function reproducibilityOf(entry) {
  ...
  const declared = LOCAL_DECLARATIONS.find(
    (phrase) => text.includes(phrase.toLowerCase()) || entry.label.toLowerCase().includes("(local"),
  );
...
function everyBar(doc) {
  ...
  for (const group of doc.groups) {
    for (const bar of group.bars) {
      ...
      const conditional =
        typeof bar.verified_by?.conditional === "string"
          ? bar.verified_by.conditional
          : "";
      const gate =
    ...
    if (strengths.size === 0) {
      throw new Error(
        `recovery.json bar "${entry.key}" carries no recorded grading strength: no ` +
          ...
          "descriptor with an oracle_strength of strong, recompile-only or " +
