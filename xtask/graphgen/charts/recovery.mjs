...
export function renderRecovery(doc) {
  ...
  const percentLabelLines = percentBars.map((bar) =>
    wrapSans(bar.label, 12, gridLeftA - 32),
  );
  const percentRowHeight = Math.max(
    PERCENT_ROW_HEIGHT,
    Math.max(...percentLabelLines.map((lines) => lines.length)) * 14 + 10,
  );
  const chartAh = 16 + percentBars.length * percentRowHeight;
  const chartA = renderChart(INNER, chartAh, {
    ...
    yAxis: {
      type: "category",
      inverse: true,
      data: percentLabelLines.map((lines) => lines.join("\n")),
      axisLine: { show: false },
      axisTick: { show: false },
      axisLabel: { color: C.muted, fontSize: 12, lineHeight: 14 },
    },
    series: [
  ...
  const statRows = stats.map((s) => {
    const valueWidth = Math.ceil(s.value.length * 15 * 0.6) + 12;
    return {
      ...s,
      valueWidth,
      lines: wrapSans(s.label, 12, colGap - valueWidth - 32),
    };
  });
  const statRowHeight =
    Math.max(...statRows.map((s) => s.lines.length)) * 14 + 24;
  statRows.forEach((s, i) => {
    const col = Math.floor(i / perCol);
    const row = i % perCol;
    const sx = LEFT + col * colGap;
    const sy = y + row * statRowHeight;
    ...
    s.lines.forEach((line, lineIndex) => {
      svg.text(sx + s.valueWidth, sy + lineIndex * 14, line, {
        size: 12,
        fill: C.muted,
      });
    });
    svg.text(sx + s.valueWidth, sy + s.lines.length * 14, s.tag, {
      size: TAG_SIZE,
      fill: C.faint,
      mono: true,
