...
import { barTiers } from "../lib/tiers.mjs";
...
test("recovery tags every bar with how it was graded", () => {
  ...
  for (const [, tag] of svg.matchAll(/id="disrobe-recovery-[a-z-]*tier-\d+"[^>]*>([^<]+)</g)) {
    assert.match(tag, /^(strong|recompile|self-reported) (CI|local)$/);
  }
});

test("recovery honors local scope recorded in gate conditions", () => {
  const doc = recoveryDoc();
  const group = doc.groups.find((g) => g.heading.startsWith("Native taint source-to-sink flow"));
  assert.ok(group);
  const categories = group.bars.filter((bar) => !bar.label.startsWith("aggregate"));
  assert.equal(categories.length, 8);
  for (const conditional of ["Local-only measurement.", "local-only measurement."]) {
    for (const bar of categories) bar.verified_by.conditional = conditional;
    const tiers = barTiers(doc);
    for (const bar of categories) {
      assert.equal(tiers.resolved.get(tiers.key(group.heading, bar.label)).ci, false, bar.label);
    }
  }
});
