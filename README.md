...
Read those two Python numbers together. A module counts as recovered only when every one of its code objects recompiles to equivalent bytecode, and a module typically holds dozens, so a small per-object miss rate compounds into a large per-module one. If the question is "will a whole readable module come back", the answer is the 54.5%, not the 96.6%. The gap between them is the center of the evaluation rather than a footnote, and the [whitepaper](docs/src/architecture/whitepaper.md) works through it.
