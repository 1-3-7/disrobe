...
| PyArmor | Recover: v6-v9-pro static unpack (default, super, no-wrap); BCC-compiled functions linked back to their source module, qualified name, class and arity as a JSON function-to-source map plus a reconstructed `.py` skeleton, recovered from the BCC dispatch table and the residual code objects; straight-line BCC function bodies reconstructed to runnable Python by reversing the runtime binary-op dispatch table and verifying each recovered body against CPython, with control-flow bodies degrading to the annotated skeleton; the carved native body is also lifted to pseudo-C via the in-house x86-64 decompiler. Detect-only: v3-v5 RSA-wrapped-key tier (runtime-key wall). |
