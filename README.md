...
| Static type recovery | Recovers integer width and signedness per stack slot from instruction semantics (subregister access, movsx/movzx, div/idiv, sar/shr, signed and unsigned compares) over a lattice with union-find, reporting unknown where the byte stream carries no sign signal rather than guessing; `native decompile` emits it as a `types.json` sidecar | `crates/disrobe-typerec` graded against an unstripped sibling's DWARF on an O0 corpus, with a mutation check asserting the grader rejects seeded-wrong widths and signs |
