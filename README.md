...
| Native packers, larger local samples | the same decoders over uncommitted vendor samples score lower on the whole-image measure, one nspack case reaching 50% `[local]` | whole-image byte comparison; the residual is the resource directory and the relocations each stub rebuilds at run time | `crates/disrobe-pass-native/tests/{petite_unpack,fsg_unpack,nspack_byte_recovery}.rs` |
