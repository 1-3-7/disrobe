...
|---|---|---|---|
| Python `.pyc`, full CPython 3.14 stdlib | <!-- m:py_stdlib_full_pct -->95.09%<!-- /m --> per-code-object (16880 / 18262, 571 modules) `[local]` | recompile to equivalent bytecode | `crates/disrobe-pass-py-decompile/tests/harness/py_arbitrary_measure.py` over the full Lib; pinned in `xtask/data/recovery.json` |
| Python `.pyc`, pinned 200-module corpus | <!-- m:py_stdlib_pinned_pct -->96.6%<!-- /m --> per-code-object (6053 / 6286), floor 90% `[CI]` | recompile to equivalent bytecode | `crates/disrobe-pass-py-decompile/tests/arbitrary_recompile_gate.rs` |
...
| WebAssembly, op-coverage | 133 / 133 corpus functions fully op-covered (100% of the 38 parseable modules; 2 modules skipped on wat-parse or signature-extraction failure) `[CI]` | output re-parses, every operator lowered | `crates/disrobe-pass-wasm-deob/tests/semantic_recovery_corpus.rs` |
