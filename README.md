...
| Containers, firmware | Recover | Of <!-- roster-breadth:containers-declared -->103<!-- /roster-breadth --> detected formats, <!-- roster-breadth:containers-exercised -->42<!-- /roster-breadth --> generic routes write member bytes from a committed input, one committed DMG input reaches detection only, and 59 have no committed input; LUKS1 is metadata-only through the generic route and graded separately through raw-volume-key routing | measured `[CI]` | [containers](docs/src/languages/containers.md) |
