...
| Surface | Coverage |
|---|---|
| Symbols and structure | DWARF/PDB/STABS across x86/ARM/RISC-V/MIPS/PowerPC/SPARC/eBPF. Rust + C++ + Swift + Itanium demangle; C++ RTTI/vtable and class-hierarchy recovery. Not a raw SSA decompiler: this is the unpack, symbol-recovery, disassembly, and chain-detect layer that produces unpacked, symbol-annotated input for Ghidra/IDA (`native export --format ghidra\|ida\|json`). |
...
Ghidra and IDA are native SSA decompilers; `disrobe` is the unpack, symbol-recovery, disassembly, and chain-detect layer that produces unpacked, symbol-annotated input for them (`native export --format ghidra|ida|json`), plus full bytecode-to-source across 20+ ecosystems.
