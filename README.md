...
| Native packers, local corpus | petite / fsg / nspack content recovery `[local]` (vendor fixtures not committed; nspack gate `#[ignore]`d, so no number reproduces from a clean checkout) | byte-identity / RVA-aligned percentage | `crates/disrobe-pass-native/tests/{petite_unpack,fsg_unpack,nspack_byte_recovery}.rs` |
