...
| Native VM protectors | Detect-only | CLI and auto detect only; the Rust `recover_detected` helper carves VMProtect and Themida protected sections | pass-gated | [unpack](docs/src/languages/native-unpack.md) |
...
| Containers, firmware | Recover | Of <!-- roster-breadth:containers-declared -->102<!-- /roster-breadth --> detected formats, <!-- roster-breadth:containers-exercised -->41<!-- /roster-breadth --> generic routes write member bytes from a committed input and 60 remain unverified; LUKS1 is metadata-only through the generic route and graded separately through raw-volume-key routing | measured `[CI]` | [containers](docs/src/languages/containers.md) |
...
| Python `.pyc`, pinned 200-module corpus | <!-- m:py_stdlib_pinned_pct -->96.53%<!-- /m --> per object, floor 96.51% `[CI]` | recompile-equivalence | `crates/disrobe-pass-py-decompile/tests/arbitrary_recompile_gate.rs` |
...
| <!-- evidence-pair:apk-jadx-cfr:dex -->Android DEX | 60 / 163 methods recompile | JADX 1.5.5: 281 / 303 methods recompile | JADX recovers 221 more clean methods; JADX has the higher clean rate | `cargo run --locked -p disrobe-bench-head-to-head -- --check --only apk-jadx-cfr`<!-- /evidence-pair --> |
...
**Native VM-protector devirtualization.** `recover_detected` emits bounded verbatim protected-section artifacts for VMProtect and Themida; the Themida route reports `.winlice` sections as WinLicense. Direct WinLicense, Enigma Protector, Armadillo, Obsidium, PE-Protector, PELock, and Yoda's Protector routes emit no recovered image. Yoda's Protector has an original-assisted comparison carve and an emulator report, but no single-input recovered-image route. The generic VM lifter is validated on Tigress-shape inputs and does not lift these commercial protectors back to source.
