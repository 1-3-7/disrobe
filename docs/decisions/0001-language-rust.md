...
1. **Rust** - compile-time memory safety without GC, strict isolation of any `unsafe` boundary code, strong workspace/crate model, mature `object`/`wasmparser`/`capstone`/`iced-x86`/`yaxpeax` ecosystem, deterministic by default.
...
Chosen option: **Rust**, because it is the only option that delivers compile-time memory safety on the hostile parsing surface *and* GC-free determinism *and* a first-class workspace model, simultaneously. The parsing surface avoids `unsafe`; the remaining unsafe code is isolated to audited boundary crates and shim layers such as C interop, WASM exports, archive/io helpers, build/install helpers, and native-loader interfaces.
