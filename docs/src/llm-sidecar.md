...
The [sidecar example](https://github.com/1-3-7/disrobe/blob/main/docs/demo/sidecar.json)
shows workspace initialization, recovery and recompilation of `add.py`, and the resulting
Pack-4 bundle. It includes the commands, generated-file hashes, and reconstruction briefs.
...
An input that never reaches Mir gets `applicable: false` and a `reason` naming the missing rung. A module that reaches Mir but produces no findings gets `applicable: true` with an empty array. Consumers can distinguish an unavailable analysis from a completed analysis with no findings.
