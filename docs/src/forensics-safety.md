...
- **Rust-first parsing surface.** Format decoders avoid `unsafe`; the remaining unsafe code is limited to audited boundary code such as C interop, WASM exports, archive/io shims, build/install helpers, and native-loader interfaces. Any panic or abort on adversarial input that is not a clean `Result::Err` is a bug.
