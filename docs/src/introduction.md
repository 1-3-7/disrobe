...
| Android (Dalvik) | <!-- m:dalvik_verifier_pct -->100%<!-- /m --> of the presentable classes in the committed dex corpus pass the JVM verifier (<!-- m:dalvik_verifier_count -->102 of 102<!-- /m -->). A further 53 of the corpus's 155 classes are link-skipped and never reach the verifier, because they reference supertypes the harness does not bundle, so those are ungraded rather than passing | `-Xverify:all` over assembled jar |
