...
|---|---|---|
| Python bytecode | <!-- m:py_stdlib_full_pct -->95.09%<!-- /m --> per-code-object equivalence on the full CPython 3.14 stdlib (<!-- m:py_stdlib_full_count -->17378 of 18276<!-- /m -->); <!-- m:py_stdlib_pinned_pct -->96.53%<!-- /m --> on the pinned 200-module corpus (<!-- m:py_stdlib_pinned_count -->6068 of 6286<!-- /m -->). Whole-module exact, where a module counts only if all of its code objects pass: 123 of <!-- m:py_stdlib_pinned_modules -->200<!-- /m --> modules on the pinned corpus | recompile on CPython 3.14.5, opcode diff |
...
| Containers | <!-- m:containers_formats -->102<!-- /m --> formats detected, including a bounded raw-volume-key LUKS1 route; <!-- roster-breadth:containers-exercised -->41<!-- /roster-breadth --> generic routes are driven to member bytes by a committed input | extraction over the committed corpus, pinned per format; tracked LUKS1 plaintext comparison |
