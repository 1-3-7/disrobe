...
Use Disrobe's recovery engines from Python to decompile bytecode, analyze native
code, deobfuscate scripts, and extract packaged files. The `disrobe` module uses
the same Rust implementations as the CLI and provides typed report objects.
It supports Python 3.9+ through PyO3's stable ABI.

Most analysis calls leave file I/O to the caller. The optional Flutter engine-symbol
cache reads and writes the directory passed as `cache_dir`. Build the bindings from
source; wheels are not published to PyPI.

The [WebAssembly example](https://github.com/1-3-7/disrobe/blob/main/docs/demo/python.json)
shows an addition function recovered as WAT. The [0.10.5 compatibility results](https://github.com/1-3-7/disrobe/blob/main/docs/demo/python-tests.json)
cover 60 API checks for typed results, error payloads, artifact round trips, and
CLI surface mappings.

## Install

Install Python 3.9+, the repository's Rust toolchain, and the target's native
compiler and linker. Windows MSVC builds require the Visual C++ build tools,
Windows SDK, and a Python installation that includes `python3.lib`. See the
[source build prerequisites](installation.md#build-from-source).
...
`.\.venv\Scripts\Activate.ps1` in Windows PowerShell. Then install the build tool
and compile the extension into that environment:
...
The `pyproject.toml` requires `maturin>=1.5,<2.0`, takes the version from the Rust
workspace, sets `module-name = "disrobe.disrobe"`, and points at
`crates/disrobe-python/Cargo.toml`. On
...
if none is found. A `py.typed` marker and a `.pyi` stub provide type information for the documented
report attributes.
...
Analysis functions generally return concrete report classes. Some functions return
text, bytes, collections, or tuples; the function table gives each return type.
The stub's `_Report` base describes the serialization methods shared by report
objects.
...
report: disrobe.PyDisasmReport = disrobe.py_disasm(pyc_bytes, pack="pack-2")
metadata_json: str = report.to_json()
...
These names are defined in the type stub, not exported by the runtime module.
Pass the documented string values to functions; do not import the aliases at runtime.
...
|---|---|---|
| `DisrobeError` | `Exception` | An underlying recovery or analysis operation fails |
...
Invalid Python argument types can also raise standard Python exceptions.
...
| | `native_match(a, b, *, stage=None, function=None, limit=None)` | `NativeMatch` |
...
| Flutter | `flutter_engine_symbols(input, engine_symbol_map=None, *, source=None, cache_dir=None, no_cache=False)` | `FlutterEngineSymbols` |
...
| | `jvm_jni_link(dex_bytes, native_libs)` | `JniLink` |
...
| | `wasm_lift(wasm_bytes, *, target)` | `WasmLift` |
...
| Containers | `byte_coverage(image_bytes)` | `ByteCoverage` |
| | `container_detect(container_bytes)` | `ContainerDetection` |
...
outputs to disk. `max_depth` must be 1-16. Other values in the unsigned-byte
range raise `DisrobeError`; values below 0 or above 255 raise `OverflowError`.
The registered pass tree covers pyarmor, pyinstaller, nuitka,
...
Wired families: `python-bytecode`/`pyc` (py-decompile), `jvm-class`/`class`/`java`/`kotlin`
...
recovered: disrobe.CanonicalSource = disrobe.decompile(
...
For a classfile report with typed attributes, use the dedicated parser:
...
class_bytes: bytes = Path("Hello.class").read_bytes()
parsed: disrobe.JvmClass = disrobe.jvm_parse_class(class_bytes)
method_count: int = parsed.method_count
...
Named passes can return different Python objects. Check a registry result before
using operations specific to its expected type.
...
def print_result(result: object, **context: object) -> None:
    print(result, context)


disrobe.register_pass("reverse", reverse_bytes)
disrobe.register_consumer("print", print_result)

try:
    output: object = disrobe.run_chain(["reverse", "reverse"], b"hello")
    if not isinstance(output, bytes):
        raise TypeError("the reverse chain must return bytes")

    restored: bytes = output
    disrobe.emit("print", restored, source="example")
finally:
    disrobe.unregister("reverse")
    disrobe.unregister("print")
```

### `Pass` and `OutputConsumer` protocols

These protocols describe callables in the stub. They are not exported as runtime
classes; register ordinary Python functions with the corresponding signatures.
...
data: bytes = Path("suspect.bin").read_bytes()

strings: disrobe.StringsReport = disrobe.strings_extract(data, min_len=6)
string_count: int = strings.string_count

iocs: disrobe.IocReport = disrobe.ioc_extract(data)
indicator_count: int = iocs.indicator_count

behavior: disrobe.BehaviorReport = disrobe.behavior_analyze(data)
category_count: int = behavior.category_count

ident: disrobe.IdentifyReport = disrobe.identify(data)
fmt: str | None = ident.format
finding_count: int = ident.finding_count

caps: disrobe.Capabilities = disrobe.capabilities(data)
match_count: int = caps.match_count

overlay: disrobe.OverlayReport = disrobe.extract_recursive(data, max_depth=4)
chunks_total: int | None = overlay.chunks_total
bytes_carved: int | None = overlay.bytes_carved

rule: disrobe.YaraReport = disrobe.yara_generate(data, name="suspect")
...
data: bytes = Path("binary.elf").read_bytes()

syms: disrobe.SymbolsReport = disrobe.native_symbols(data)
...
sig: disrobe.SigmakerReport = disrobe.native_sigmaker(data, at=0x1000)
ida_pattern: str | None = sig.ida_pattern

patched_bytes: bytes
patch_report: disrobe.PatchReport
...
dr: bytes = Path("module.dr").read_bytes()

functions: disrobe.FunctionList = disrobe.query_functions(dr)
fn_count: int = functions.count

callers: disrobe.QueryReport = disrobe.query_calls_to(dr, "malloc")
match_count: int = callers.match_count

complex_fns: disrobe.QueryReport = disrobe.query_complexity_over(dr, threshold=20)
graph: disrobe.CallGraph = disrobe.query_call_graph(dr)
...
report: disrobe.EnvelopeReport = disrobe.envelope_verify(envelope)
...
report: disrobe.PyDecompileReport = disrobe.py_decompile(
    Path("module.pyc").read_bytes(), pack="pack-2"
)

agents_brief: str = disrobe.agents_md(report)
skill_brief: str = disrobe.skill_md(report)

prov: disrobe.Provenance = disrobe.provenance(report)
...
200-module corpus is <!-- m:py_stdlib_pinned_pct -->96.67%<!-- /m --> (<!-- m:py_stdlib_pinned_count -->6077 of 6286<!-- /m -->, regression threshold 96.67%). Legacy CPython
...
see the [comparison rules](./languages/python.md#measured-opcode-structure).
...
report: disrobe.PyDecompileReport = disrobe.py_decompile(
    Path("module.pyc").read_bytes(), roundtrip=True
)
...
status: disrobe.RoundtripStatus | None = report.roundtrip_status
...
Round-tripping (when `roundtrip=True`) invokes a matching host interpreter to
compile the recovered source and compare bytecode. It does not execute the
recovered program.
...
result: disrobe.PyDisasmReport = disrobe.py_disasm(Path("module.pyc").read_bytes())
...
deob: disrobe.PyDeobReport = disrobe.py_deob(obfuscated, cleanup=True)
peeled_source: str | None = deob.peeled_source
cleanup_source: str | None = deob.cleanup_source
layer_count: int = deob.layer_count

detection: disrobe.PyDeobDetection = disrobe.py_deob_detect(obfuscated)
match_count: int = detection.match_count

passes: list[disrobe.ObfuscatorPass] = disrobe.py_deob_list_passes()
first_id: str | None = passes[0].id if passes else None

if first_id is not None:
    per_pass: disrobe.PyDeobDetection = disrobe.py_deob_detect_pass(
        obfuscated, first_id
    )
    print(per_pass.match_count)
```
...
detection: disrobe.PyarmorDetection = disrobe.pyarmor_detect(
    Path("wrapped.py").read_text(encoding="utf-8")
)
...
the <!-- m:pyarmor_frac -->72 / 72<!-- /m --> manifest-named v8/v9 default-trial wrappers.
That structural check does not compare source, emitted `.pyc` bytes, semantic or
execution behavior, or external-tool output.
...
classification: disrobe.PyarmorClassification = disrobe.pyarmor_classify(
    Path("wrapped.py").read_text(encoding="utf-8"), payload
)
...
image: bytes = Path("app.exe").read_bytes()

archive: disrobe.PyInstallerArchive = disrobe.pyinstaller_extract(image)
...
archive_json: str = archive.to_json()
print(archive_json)
```

Use an entry name from the archive report to recover its bytes:
...
image: bytes = Path("app.exe").read_bytes()
entry_name: str = "main"
main_payload: bytes = disrobe.pyinstaller_entry_bytes(image, entry_name)
print(len(main_payload))
```
...
det: disrobe.NuitkaDetection = disrobe.nuitka_detect(image)
...
extraction: disrobe.NuitkaExtraction = disrobe.nuitka_extract(image)
...
bundle: bytes = Path("index.android.bundle").read_bytes()

disasm_result: disrobe.HermesDisassembly = disrobe.hermes_disasm(bundle)
...
lift: disrobe.HermesLift = disrobe.hermes_lift(bundle)
function_surface_count: int = lift.function_surface_count

info: disrobe.HermesInfo = disrobe.hermes_info(bundle)
...
data: bytes = Path("universal.dylib").read_bytes()

macho: disrobe.MachoReport = disrobe.macho_dump(data)
kind: str | None = macho.kind
fat_entry_count: int = macho.fat_entry_count
slice_count: int = macho.slice_count

swift: disrobe.SwiftReport = disrobe.swift_analyze(data)
...
cls: disrobe.JvmClass = disrobe.jvm_parse_class(Path("Hello.class").read_bytes())
...
dex: disrobe.DexFileReport = disrobe.jvm_parse_dex(Path("classes.dex").read_bytes())
class_count: int = dex.class_count
dex_method_count: int = dex.method_count

decompiled: disrobe.JvmDecompiledClass = disrobe.jvm_decompile_class(
    Path("Hello.class").read_bytes()
)
...
decompiled_dex: disrobe.JvmDecompiledDex = disrobe.jvm_decompile_dex(
    Path("classes.dex").read_bytes()
)
...
detections: disrobe.DetectionList = disrobe.jvm_detect(Path("obf.class").read_bytes())
detection_count: int = detections.count

backends: disrobe.JvmBackends = disrobe.jvm_backends()
jvm_count: int = backends.jvm_count
android_count: int = backends.android_count

apk: disrobe.ApkResources = disrobe.apk_resources(Path("app.apk").read_bytes())
...
pe_bytes: bytes = Path("Sample.dll").read_bytes()

pe: disrobe.DotnetPe = disrobe.dotnet_parse_pe(pe_bytes)
...
metadata: disrobe.DotnetMetadata = disrobe.dotnet_parse_metadata(pe_bytes)
...
detection: disrobe.DotnetDetection = disrobe.dotnet_detect(pe_bytes)
primary: str | None = detection.primary
match_count: int = detection.match_count

analysis: disrobe.DotnetAnalysis = disrobe.dotnet_analyze(pe_bytes)
pe_bitness: str | None = analysis.pe_bitness
is_native_aot: bool = analysis.native_aot
primary_protector: str | None = analysis.primary_protector
opcode_spec_coverage_pct: int | None = analysis.opcode_spec_coverage_pct

decompilation: disrobe.DotnetDecompilation = disrobe.dotnet_decompile(pe_bytes)
...
decoders: disrobe.DotnetDecoders = disrobe.dotnet_recover_decoders(pe_bytes)
pure_decoders_found: int | None = decoders.pure_decoders_found
constants_recovered: int = decoders.constants_recovered

image_bytes: bytes = Path("HelloAppAot.exe").read_bytes()

native_aot: disrobe.DotnetNativeAot = disrobe.dotnet_native_aot(image_bytes)
...
backend_list: disrobe.BackendList = disrobe.dotnet_backends()
...
wasm_bytes: bytes = Path("module.wasm").read_bytes()

analysis: disrobe.WasmAnalysis = disrobe.wasm_analyze(wasm_bytes)
...
detection: disrobe.WasmDetection = disrobe.wasm_detect(wasm_bytes)
...
unminified: disrobe.JsUnminify = disrobe.js_unminify(source)
recovered_source: str | None = unminified.source

bundle_source: str = Path("bundle.js").read_text(encoding="utf-8")
unbundled: disrobe.JsUnbundle = disrobe.js_unbundle(bundle_source)
module_count: int = unbundled.module_count
bundler: str | None = unbundled.bundler

unbundled_hinted: disrobe.JsUnbundle = disrobe.js_unbundle(
    bundle_source, bundler="webpack5"
)
```
...
bytecode: bytes = Path("chunk.luac").read_bytes()

det: disrobe.LuaDetection = disrobe.lua_detect(bytecode)
lua_format: str | None = det.format

decompiled: disrobe.LuaDecompilation = disrobe.lua_decompile(bytecode)
...
deob: disrobe.LuaDeobfuscation = disrobe.lua_deobfuscate(
    Path("obf.lua").read_text(encoding="utf-8"), authorize=True
)
...
go_bytes: bytes = Path("binary").read_bytes()

analysis: disrobe.GoAnalysis = disrobe.go_analyze(go_bytes)
...
symbols: disrobe.GoSymbols = disrobe.go_symbols(go_bytes)
...
pclntab: disrobe.GoPclntab = disrobe.go_pclntab(go_bytes)
...
ruby_bytes: bytes = Path("hello.rb.enc").read_bytes()

det: disrobe.RubyDetection = disrobe.ruby_detect(ruby_bytes, source_path="hello.rb.enc")
flavor: str | None = det.flavor

analysis: disrobe.RubyAnalysis = disrobe.ruby_decompile(
    ruby_bytes, source_path="hello.rb.enc"
)
...
php_bytes: bytes = Path("obfuscated.php").read_bytes()

det: disrobe.PhpDetection = disrobe.php_detect(php_bytes)
...
scan: disrobe.PhpScan = disrobe.php_scan(php_bytes)
hit_count: int = scan.hit_count
family_count: int = scan.family_count

decoded: disrobe.PhpDecode = disrobe.php_decode(php_bytes, max_depth=10)
...
batch_script: str = Path("dropper.bat").read_text(encoding="utf-8")
batch_result: disrobe.BatchDeobReport = disrobe.batch_deobfuscate(
    batch_script, args=["/run"]
)
...
ps_script: str = Path("obf.ps1").read_text(encoding="utf-8")
ps_det: disrobe.PowershellDetection = disrobe.powershell_detect(ps_script)
obfuscator: str | None = ps_det.obfuscator
ps_confidence: float | None = ps_det.confidence
marker_count: int = ps_det.marker_count

ps_deob: disrobe.PowershellDeobfuscation = disrobe.powershell_deobfuscate(ps_script)
...
container_bytes: bytes = Path("archive.zip").read_bytes()

det: disrobe.ContainerDetection = disrobe.container_detect(container_bytes)
detected: bool = det.detected
kind: str | None = det.kind
is_zip_family: bool = det.is_zip_family

members: disrobe.ContainerMembers = disrobe.container_members(container_bytes)
fmt: str | None = members.format
size: int | None = members.size
listing: disrobe.ContainerListing | None = members.listing
...
pkl: bytes = Path("model.pkl").read_bytes()

listing: str = disrobe.pickle_disasm(pkl)

decompilation: disrobe.PickleDecompilation = disrobe.pickle_decompile(pkl)
pkl_source: str | None = decompilation.source

safety: disrobe.PickleSafety = disrobe.pickle_safety(pkl)
...
trace: disrobe.PickleTrace = disrobe.pickle_trace(pkl)
...
polyglot: disrobe.PicklePolyglot = disrobe.pickle_polyglot(pkl)
is_pickle: bool = polyglot.is_pickle
is_polyglot: bool = polyglot.is_polyglot
kind_count: int = polyglot.kind_count

ml_report: disrobe.PickleMlReport = disrobe.pickle_ml_detect(
    Path("model.pt").read_bytes()
)
...
instr: disrobe.Instruction = disrobe.Instruction(
    offset=0,
    mnemonic="mov",
    operands=["rax", "rbx"],
    bytes=b"\x48\x89\xd8",
...
flow: disrobe.InstructionFlow = instr.flow
text: str = instr.text()
```
...
sym: disrobe.Symbol = disrobe.Symbol(address=0x1000, name="entry", kind="function")
sym.name = "main"
sym.kind = "export"
```
...
co: disrobe.CodeObject = disrobe.CodeObject.from_dr(Path("module.dr").read_bytes())
...
instrs: list[disrobe.Instruction] = co.instructions
syms: list[disrobe.Symbol] = co.symbols
metadata: dict[str, str] = co.metadata
capabilities: list[str] = co.capabilities

new_sym: disrobe.Symbol = disrobe.Symbol(
    address=0x2000, name="renamed_fn", kind="function"
)
...
Path("module_patched.dr").write_bytes(fresh_dr)
```
...
- `auto` returns the plan document without writing stage trees. `extract(data, out_dir)`
  writes container members, and `flutter_engine_symbols` can use an explicit cache directory.
- External backend tools (`jvm_backends`, `dotnet_backends`, `native_probe_backends`)
  are probed for availability but never executed.
- AS3, BEAM (beyond `disasm`/`parse`), and the freezer family beyond
