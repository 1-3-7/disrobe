...
Pass `--sha256` and `--date` to include those values in the rule's metadata. Both fields are omitted by default; the analysis core does not read the system clock.
