...
`detect` also supports JSON, NDJSON and SARIF. Its structured output includes each matching detector, confidence score and signature markers. SARIF records use note severity and identify the input file; a format detection alone does not establish a vulnerability.

## The standard recovery loop

```sh
disrobe auto input.bin --out recovered/ --capture-stages
