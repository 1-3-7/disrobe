...
`disrobe serve` exposes byte classification and envelope operations over HTTP and gRPC,
classification and error lookup over LSP, and pass-specific analysis, recovery, and workspace
tools over MCP. Requests carry input bytes inline.

The [protocol example](https://github.com/1-3-7/disrobe/blob/main/docs/demo/services.json)
records MCP initialization and an indicator query, LSP classification, and HTTP/gRPC health
and classification requests. All three classification transports agree on the Wasm fixture's
byte count, route, and BLAKE3 hash.
