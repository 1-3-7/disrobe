...
| `disrobe catalog [ecosystem]` | List the supported obfuscator, packer, protector, freezer, and bundler registry by ecosystem. The live binary reports 169 families across 15 ecosystems; filter with `python`, `js`, `jvm`, `dotnet`, `native`, `go`, `wasm`, `ruby`, `lua`, `php`, `beam`, `as3`, `mobile`, `swift`, or `shell`. `--json` emits `{ family_count, ecosystem_count, ecosystems[] }`. |
