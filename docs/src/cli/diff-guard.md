...
disrobe guard check some/path --root extra/protected/subtree
```

Decides whether a path about to be written or edited is inside a protected ground-truth subtree. It denies writes to `out/**/stages`, `out/**/final`, and any `.disrobe-stage-lock`-marked path, and allows writes elsewhere. `--root` adds extra protected subtrees.
