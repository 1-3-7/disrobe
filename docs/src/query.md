...
The report names the detected format, the match count, and the per-rule evidence. The [MCP server](./integrations/mcp.md) also offers a behavior summary, which uses a separate byte-analysis API; its results are not interchangeable with this IR rule report.
