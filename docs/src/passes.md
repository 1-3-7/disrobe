...
| Native and containers | `binfmt.container`, `go.classify`, `native.image-classify`, `native.ne-structure`, `native.packer-unpack`, `nativelang.classify`, `webview.carve` |

The standard CLI enables automatic webview recovery: Electron and Tauri use `webview.carve`, while Wails uses `go.classify` with Go support enabled. `disrobe webview` also provides direct frontend extraction. Build features can change the chain registry; `disrobe passes` lists the routes in the binary you are running.
...
Many diagnostics carry a `DR-<DOMAIN>-<NNNN>` code. The curated error registry provides explanations for registered codes:

```sh
disrobe explain DR-PYARM-0007
