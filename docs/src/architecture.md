...
| `disrobe-binfmt` | Container, archive, filesystem, firmware, and encrypted-volume layer (<!-- m:containers_formats -->102<!-- /m --> formats detected; 101 generic extraction routes plus bounded raw-volume-key LUKS1, with <!-- roster-breadth:containers-exercised -->41<!-- /roster-breadth --> generic routes reached by a committed input, plus recursive carving) with shared path-safety and decompression-quota machinery. |
