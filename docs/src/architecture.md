...
| `disrobe-binfmt` | Container, archive, filesystem, and firmware layer (<!-- m:containers_formats -->101<!-- /m --> formats detected, each with an in-tree extractor and <!-- roster-breadth:containers-exercised -->41<!-- /roster-breadth --> of them reached by a committed input, plus recursive carving) with shared path-safety and decompression-quota machinery. |
