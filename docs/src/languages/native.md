...
`disrobe` includes native decompilation for x86-64, AArch64, ARM32, and MIPS32, alongside symbol recovery, disassembly, identification, patching, fingerprinting, and diffing. It can export recovered information to Ghidra and IDA, and drive Ghidra headlessly. Architecture-specific output and limits are described in the [decompile guide](./native-decompile.md).
...
- Themida and VMProtect support covers detection and section carving. See [native unpacking](./native-unpack.md) for the available outputs and recovery limits.
