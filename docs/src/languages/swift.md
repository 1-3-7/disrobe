...
- FairPlay-encrypted regions (App Store DRM) are reported detect-only via `LC_ENCRYPTION_INFO`. Without the decryption key, the pass cannot recover class metadata from those encrypted regions.
