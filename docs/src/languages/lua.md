...
- When a MoonSec v3 constant-pool key is available only at runtime, `disrobe` returns `fully_recovered: false` with a `runtime keys` residual marker.
- The MoonSec `emulate_perm_builder` tests use a purpose-built bootstrap whose permutation is derived at runtime. These tests cover the table-builder algorithm; the corpus contains no captured MoonSec dump to establish end-to-end recovery.

### Prometheus Vmify refusals

`Vmify` recovery refuses unsupported shapes instead of substituting placeholder functions for missing bodies. A refusal names its cause, and where the cause is a specific expression it names the byte offset of that expression in the text the layer was reading. The execution comparison below covers the committed Lua 5.1 Vmify fixtures.
