...
`disrobe` peels PHP eval chains, evaluates supported decode loops and literal-key ciphers, and extracts Phar members. It also identifies ionCube, SourceGuardian, and Zend Guard envelopes, recovers statically available legacy layers, and reports when loader-resident keys keep a payload sealed. These operations run locally without uploading the input.
...
Every recovery carries an explicit stage in its manifest: `EvalChainPeeled`, `OpArrayDecompiled`, `StructuralOnly`, or `PlainSource`, so a caller can tell a fully peeled payload from an envelope parse.
