...
A separate and much smaller population is graded by the real JVM rather than self-reported. Of 83,577 non-stub candidate bodies, a deterministic 100-permille sample takes 8,414, and 2,998 of those can be re-hosted into an isolated carrier. Of the ones presented, <!-- m:dalvik_body_attested_frac -->2985 of 2998<!-- /m --> pass `-Xverify:all`. The remaining 5,416 are excluded by harness limits and are ungraded rather than passing: 1,407 constructors, 1,730 invokespecial receivers, 2,137 unresolvable framework dependencies, and 142 others.
