...
- The Dalvik body figure measured on the gitignored real FOSS apks is a self-report, not verifier-attested. Across all three apks the lifter self-reports a lowered body for <!-- m:dalvik_body_frac -->82923 / 83943<!-- /m --> defined methods, <!-- m:dalvik_body_pct -->98.8%<!-- /m -->. That is the corpus total and not any single apk. The figure counts the lifter returning a body rather than a throw-stub, so it grades its own output. Those apks are gitignored, so neither figure below re-derives in CI.
