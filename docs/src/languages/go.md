...
- garble's hashed user identifiers are reported as a `name_recovery_wall` when the required build seed or name mapping is unavailable. The pass does not infer original names from the hashes. Surviving standard-library names in `pclntab` are recovered.
