...
The per-code-object figure is measured against an independent oracle, not the tool's own output: each recovered module is recompiled on CPython 3.14.5 and its code objects are diffed against the originals. The full stdlib measurement is **<!-- m:py_stdlib_full_pct -->95.09%<!-- /m -->** (<!-- m:py_stdlib_full_count -->17378 of 18276<!-- /m --> code objects across 571 modules). On the pinned 200-module corpus (6286 code objects) the rate is **<!-- m:py_stdlib_pinned_pct -->96.6%<!-- /m -->** (<!-- m:py_stdlib_pinned_count -->6072 of 6286<!-- /m -->), above a 90% floor a committed CI gate enforces (`arbitrary_recompile_gate.rs`). uncompyle6 stops near 3.8 and decompyle3 near 3.9; the ML-based decompilers self-flag benchmark contamination, and there is no model here to contaminate.
