...
Note the limit of that grading. Those fixtures are assembled by the test itself from the format specification, so the same reading of the specification produces both the bytes and the expected formulas. That catches a decoder which contradicts the specification, but not a misreading shared by both sides, and a wrong function-table index would rename every `CALL` and `EXEC` an analyst reads. No workbook produced by Excel and no independent decoder is in the loop yet, so treat recovered XLM formulas as a strong lead rather than a transcript.
