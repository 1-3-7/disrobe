...
| Images | ReadyToRun (R2R) and Native AOT detection; `dotnet native-aot` exposes the library's name, type, method-boundary, and managed-signature recovery for PE, ELF, and Mach-O images; single-file bundles extracted member by member |
...
pseudo-C bodies. Its measured coverage is x86_64 across PE, ELF, and Mach-O images.
The parser reads layout metadata from the image; recovery on aarch64, arm, and x86 remains
outside the measured population.
...
no redistributable product-produced sample for those families.
