...
Use the run reports to inspect recovered artifacts, locate the stage that stopped, and choose the next command. A refusal records the unsupported condition or missing prerequisite. Its reason determines whether another pass, tool, or input can help.
...
The report prints the input identity, overall verdict, tier histogram, and stage rows. A run on the repository's `corpus/webview/tauri/wvfix.exe` fixture produced this excerpt:
