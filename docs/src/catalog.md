...
A **wall** identifies a required input that is absent, such as a runtime key or network-fetched payload. Every wall is detect-only, and detect-only also covers families identified without static recovery. See [recovery limits](./introduction.md#recovery-limits) and [reading a result](./reading-a-result.md) for how to interpret these reports.
...
| **Ruby** | MRI/YARV 2.6-3.4 and mruby source recovery; the published MRI measure compares opcode-name multiset recall after recompilation. Ruby2Exe and Ocra freezers detected; JRuby and TruffleRuby AOT classified. |
...
| **Hermes / Flutter** | Hermes bytecode v60-v96 header parse, v62/v71/v74/v76/v83/v84/v89/v96 lift against builds of one sample by the real Hermes compiler; Flutter Dart kernel byte-exact body recovery and ARM64 AOT disasm, graded against a self-authored Dart 3.12.2 android-arm64 product-mode corpus and, measured locally, a real RustDesk 1.4.9 `libapp.so` fetched by pinned hash. |
...
The standard CLI also recovers supported Electron and Tauri layouts through `webview.carve` in `auto`; Wails uses `go.classify`. See [Webview desktop frontends](./languages/webview.md) for the output paths and package extraction workflow.
